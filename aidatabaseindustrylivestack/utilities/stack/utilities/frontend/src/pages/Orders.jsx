...
const REQUEST_STATUS_DISPLAY_LABELS = {
  pending: 'Submitted',
  confirmed: 'Confirmed',
  ...
  cancelled: 'Cancelled',
  returned: 'Returned',
  exception: 'Logistics Exception',
};

const ROUTE_STATUS_DISPLAY_LABELS = {
  pending: 'Request Received',
  confirmed: 'Confirmed',
  preparing: 'Request Received',
  processing: 'Prepared',
  picked: 'Assigned',
  packed: 'Packed',
  shipped: 'Dispatched',
...
const REQUIRED_DUALITY_SOURCE = 'UTILITY_SERVICE_REQUESTS_DV';

const CARRIER_DISPLAY_LABELS = {
  PilotFreight: 'Critical Infrastructure Courier',
};
...
function formatStatusLabel(value, labels = REQUEST_STATUS_DISPLAY_LABELS) {
  const key = normalizeStatusKey(value);
  return labels[key] || toTitleLabel(value);
}

function formatCarrierName(value) {
  return CARRIER_DISPLAY_LABELS[value] || value;
}

function buildUtilityServiceRequestDualitySql(duality, orderId) {
  const id = duality?.document?.serviceRequestId ?? orderId ?? ':service_request_id';
  return `SELECT DATA
FROM   utility_service_requests_dv
WHERE  JSON_VALUE(DATA, '$.serviceRequestId' RETURNING NUMBER) = ${id};`;
}

const SHIP_STATUS_STEPS = [
  { key: 'preparing', label: 'Request Received', icon: Package },
  { key: 'picked', label: 'Assigned', icon: Package },
  { key: 'packed', label: 'Packed', icon: Package },
  { key: 'shipped', label: 'Dispatched', icon: Truck },
  { key: 'in_transit', label: 'In Transit', icon: Truck },
  { key: 'out_for_delivery', label: 'Arriving', icon: Navigation },
...
const REQUEST_DETAIL_TABS = [
  ...
  { id: 'route', label: 'Logistics Route' },
];
...
const STATUS_OPTIONS = [
  ...
  { value: 'shipped', label: 'Dispatched / In Transit' },
...
/* ─── Utility Service Request Duality Panel ──────────────────────────────── */
function OrderDualityPanel({ orderId, onClose }) {
  const [view, setView] = useState('relational'); // 'relational' | 'json' | 'route'
  const { data: detail, loading: loadingDetail } = useData(() => api.serviceRequests.detail(orderId), [orderId]);
  const { data: duality, loading: loadingDuality, error: dualityError } = useData(() => api.serviceRequests.duality(orderId), [orderId]);
  ...
  const hasExactDualityProvenance = Boolean(
    duality?.document
      && duality?.source === 'UTILITY_SERVICE_REQUESTS_DV'
      && duality?.requiredSource === REQUIRED_DUALITY_SOURCE
      && duality?.executionMode === 'native-duality-view'
      && duality?.nativeDualityViewAvailable === true
  );
  const dualityUnavailable = dualityError?.code === 'DUALITY_VIEW_UNAVAILABLE'
    || dualityError?.status === 503;
  const unavailableSource = dualityError?.details?.requiredSource || REQUIRED_DUALITY_SOURCE;
  const dualitySql = buildUtilityServiceRequestDualitySql(duality, orderId);
        ...
        <div className="mx-4 mb-3 orders-detail-panel">
          ...
          <div className="orders-detail-tabbar">
            <div className="orders-detail-tabset">
              ...
              <span className="text-[10px] text-[var(--color-text-dim)] ml-3 hidden sm:inline">
                Same governed request data — relational rows, JSON duality document, and spatial logistics route.
              </span>
            </div>
            <JetButton
              label="Close"
              title="Close utility service request detail"
            ...
            <div className="p-4 space-y-3">
              {loadingDetail ? (
                <div className="flex items-center gap-2 text-sm text-[var(--color-text-dim)] py-4 justify-center">
                  <Loader2 size={14} className="animate-spin" /> Loading utility service request details…
                </div>
              ) : !order ? (
                <p className="text-sm text-[var(--color-text-dim)] text-center py-4">Utility service request not found</p>
              ) : (
                <>
                  <div className="flex items-center gap-4 text-xs text-[var(--color-text-dim)]">
                    <span className="text-[10px] px-2 py-0.5 rounded border font-mono" style={{ background: 'rgba(67,124,148,0.1)', borderColor: 'rgba(67,124,148,0.3)', color: 'var(--color-text)' }}>
                      SELECT * FROM utility_service_requests / utility_request_items
                    </span>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      { label: 'Requesting Service Point', value: order.REQUESTING_SERVICE_POINT_NAME ?? `${order.FIRST_NAME} ${order.LAST_NAME}` },
                      { label: 'Service Point Location', value: `${order.REQUESTING_SERVICE_POINT_CITY ?? order.CITY}, ${order.REQUESTING_SERVICE_POINT_REGION ?? order.STATE_PROVINCE}` },
                      { label: 'Request Priority Value', value: formatCurrency(order.REQUEST_VALUE ?? order.ORDER_TOTAL) },
                      { label: 'Logistics Cost', value: formatCurrency(order.LOGISTICS_COST ?? order.SHIPPING_COST) },
                    ].map(s => (
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)]">
                        <th className="text-left py-1.5 px-2">Line Item #</th>
                        <th className="text-left py-1.5 px-2">Service / Supply</th>
                        <th className="text-left py-1.5 px-2">Utility Operator Network / Partner</th>
                    ...
                    <tbody>
                      {items.map(item => (
                        <tr key={item.LINE_ITEM_ID ?? item.ITEM_ID} className="border-b border-[var(--color-border)]/20">
                          <td className="py-1.5 px-2 font-mono">{item.LINE_ITEM_ID ?? item.ITEM_ID}</td>
                          <td className="py-1.5 px-2 font-medium">{item.SERVICE_SUPPLY_NAME ?? item.PRODUCT_NAME}</td>
                          <td className="py-1.5 px-2 text-[var(--color-text-dim)]">{item.UTILITY_OPERATOR_OR_PARTNER ?? item.BRAND_NAME}</td>
                          <td className="py-1.5 px-2 text-[var(--color-text-dim)]">{item.UTILITY_CATEGORY ?? item.CATEGORY}</td>
                          <td className="py-1.5 px-2 text-right">{item.QUANTITY}</td>
                          <td className="py-1.5 px-2 text-right">{formatCurrency(item.UNIT_COST ?? item.UNIT_PRICE)}</td>
                          <td className="py-1.5 px-2 text-right font-medium">{formatCurrency(item.LINE_VALUE ?? item.LINE_TOTAL)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {view === 'json' && (
            <div className="p-4 space-y-3">
              {loadingDuality ? (
                <div className="flex items-center gap-2 text-sm text-[var(--color-text-dim)] py-4 justify-center">
                  <Loader2 size={14} className="animate-spin" /> Querying UTILITY_SERVICE_REQUESTS_DV duality view…
                </div>
              ) : dualityUnavailable ? (
                <div className="rounded-lg p-4" role="alert" style={{ background: 'rgba(199,70,52,0.06)', border: '1px solid rgba(199,70,52,0.25)' }}>
                  <p className="text-sm font-semibold tone-red">Native duality view unavailable</p>
                  <p className="text-xs text-[var(--color-text-dim)] mt-1">
                    Required source: <span className="font-mono text-[var(--color-text)]">{unavailableSource}</span>
                  </p>
                  <p className="text-xs text-[var(--color-text-dim)] mt-1">
                    No compatibility view is substituted, so this feature remains unavailable until the exact Utilities view can execute.
                  </p>
                </div>
              ) : dualityError ? (
                <p className="text-sm tone-red text-center py-4">{dualityError.message}</p>
              ) : hasExactDualityProvenance ? (
                <>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] px-2 py-0.5 rounded border font-mono" style={{ background: 'rgba(170,100,59,0.1)', borderColor: 'rgba(170,100,59,0.3)', color: 'var(--color-text)' }}>
                        SELECT DATA FROM utility_service_requests_dv
                      </span>
                      <span className="text-[10px] text-[var(--color-text-dim)]">
                        Exact native source: <span className="text-[var(--color-text)] font-mono">{duality.source}</span>
                      </span>
                    </div>
                    <JetButton
                  ...
                  <div className="rounded-lg p-3 text-xs leading-relaxed" style={{ background: 'rgba(170,100,59,0.06)', border: '1px dashed rgba(170,100,59,0.3)' }}>
                    ...
                    <span className="text-[var(--color-text-dim)]"> — This is the same utility service request data from the relational tab, exposed as a single JSON document with nested line items. Read and write through either interface with the same ACID transaction.</span>
                  </div>
                  ...
                  <div className="rounded-lg overflow-hidden" style={{ background: 'var(--color-surface-muted)', border: '1px solid rgba(170,100,59,0.25)' }}>
                    <div className="px-3 py-1.5 flex items-center justify-between" style={{ background: 'rgba(170,100,59,0.08)', borderBottom: '1px solid rgba(170,100,59,0.2)' }}>
                      ...
                      <span className="text-[10px] text-[var(--color-text-dim)] font-mono">
                        {duality.document.lineItems?.length ?? 0} nested line items
                      </span>
                    </div>
...
{JSON.stringify(duality.document, null, 2)}
                  ...
                  <div className="rounded-lg overflow-hidden" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
                    <div className="px-3 py-1.5" style={{ background: 'rgba(67,124,148,0.06)', borderBottom: '1px solid var(--color-border)' }}>
                      <span className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider">Executable SQL Example</span>
                    </div>
                    <pre className="p-3 text-[11px] font-mono text-[var(--color-text)] overflow-x-auto whitespace-pre">{dualitySql}</pre>
                  </div>
                </>
              ) : (
          ...
          {view === 'route' && (
            <div className="p-4 space-y-3">
              {loadingDetail ? (
                <div className="flex items-center gap-2 text-sm text-[var(--color-text-dim)] py-4 justify-center">
                  <Loader2 size={14} className="animate-spin" /> Loading logistics data…
                </div>
              ) : !order ? (
                <p className="text-sm text-[var(--color-text-dim)] text-center py-4">Utility service request not found</p>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-[var(--color-text-dim)]">No location data available for this utility service request</p>
                </div>
              ) : (() => {
                const from = [order.CENTER_LAT, order.CENTER_LON];
                const to = [order.CUST_LAT, order.CUST_LON];
                const arc = curvedPositions(from, to);
                const carrierName = formatCarrierName(shipment?.CARRIER_DISPLAY_NAME || shipment?.LOGISTICS_PARTNER_DISPLAY_NAME || shipment?.CARRIER);
                const carrierColor = CARRIER_COLORS[carrierName] || CARRIER_COLORS[shipment?.CARRIER] || '#4C825C';
                const currentStep = shipment?.LOGISTICS_ROUTE_STATUS || shipment?.SHIP_STATUS || order.ORDER_STATUS || 'preparing';
                ...
                const estHours = routeTimeHours || shipment?.ESTIMATED_HOURS || (distanceMiles ? Math.round(distanceMiles / 55 * 10) / 10 : null);
                ...
                return (
                  <>
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      ...
                      {(routeGeometry || isRouteData) && (
                        <span className="text-[10px] px-2 py-0.5 rounded border font-mono" style={{ background: 'rgba(67,124,148,0.08)', borderColor: 'rgba(67,124,148,0.25)', color: 'var(--color-text)' }}>
                          {routeGeometry ? 'SDO_GCDR.ELOC_ROUTE_GEOM' : 'SDO_GCDR.ELOC_ROUTE'}
                        </span>
                      )}
                      {routeGeometry && (
                        <JetButton
                      ...
                      {shipment && (
                        <span className="flex items-center gap-1 text-[var(--color-text-dim)]">
                          ...
                          <span style={{ color: carrierColor }} className="font-semibold">{carrierName}</span>
                          {(shipment.LOGISTICS_ROUTE_REFERENCE_ID || shipment.TRACKING_NUMBER) && (
                            <span className="font-mono opacity-60">Route {shipment.LOGISTICS_ROUTE_REFERENCE_ID || shipment.TRACKING_NUMBER}</span>
                          )}
                        </span>
                      )}
                    </div>
                      ...
                      >
                        ...
                        <CircleMarker center={from} radius={8} fillColor="#437C94" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            ...
                            <div className="route-map-tooltip__content">
                              <span className="route-map-tooltip__label">Field Operations Site</span>
                              <span>{order.FIELD_LOGISTICS_SITE_NAME || order.CENTER_NAME || 'Field Operations Site'}</span>
                            </div>
                          </Tooltip>
                        </CircleMarker>
                        ...
                        <CircleMarker center={to} radius={8} fillColor="#4C825C" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            className="route-map-tooltip">
                            <div className="route-map-tooltip__content">
                              <span className="route-map-tooltip__label">Service Point</span>
                              <span>{order.REQUESTING_SERVICE_POINT_NAME ?? `${order.FIRST_NAME} ${order.LAST_NAME}`} - {order.REQUESTING_SERVICE_POINT_CITY ?? order.CITY}, {order.REQUESTING_SERVICE_POINT_REGION ?? order.STATE_PROVINCE}</span>
                            </div>
                          </Tooltip>
                        </CircleMarker>
                      </MapContainer>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {[
                        ...
                        { label: 'Estimated Transit', value: estHours ? `${estHours} hrs` : '-', color: '#AA643B' },
                        { label: 'Logistics Cost', value: shipment?.LOGISTICS_COST || shipment?.SHIP_COST ? formatCurrency(shipment.LOGISTICS_COST ?? shipment.SHIP_COST) : '-', color: '#437C94' },
                        {
                          label: 'Route Status',
                          value: shipment?.LOGISTICS_ROUTE_STATUS_DISPLAY_NAME
                            ?? order.LOGISTICS_MOVEMENT_DISPLAY_NAME
                            ?? order.REQUEST_STATUS_DISPLAY_NAME
                            ?? formatStatusLabel(currentStep, ROUTE_STATUS_DISPLAY_LABELS),
                          color: carrierColor,
                        },
                      ].map(c => (
                    ...
                    {shipment && (
                      <div className="rounded-lg p-3" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
                        <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Request Progress</p>
                    ...
                    <div className="rounded-lg overflow-hidden" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
                      ...
                      <p className="px-3 pt-3 text-xs leading-relaxed text-[var(--color-text-dim)]">
                        Oracle Spatial calculates the distance between the fulfillment logistics site and the requesting service point using governed SDO_GEOMETRY coordinates.
                      </p>
...
-- SDO_GCDR.ELOC_DRIVE_TIME_POLYGON for isochrone zones` : `-- Distance between field operations site and service point
SELECT ROUND(SDO_GEOM.SDO_DISTANCE(
         l.location,               -- logistics site SDO_GEOMETRY point
         c.location,               -- service point SDO_GEOMETRY point
         0.05, 'unit=MILE'), 1)    AS distance_miles
FROM   field_logistics_sites_v l, service_points_v c
WHERE  l.field_logistics_site_id = ${order.FIELD_LOGISTICS_SITE_ID || order.FULFILLMENT_CENTER_ID || ':field_logistics_site_id'}
AND    c.service_point_id = ${order.REQUESTING_SERVICE_POINT_ID || order.CUSTOMER_ID || ':service_point_id'};
...
export default function Orders() {
  ...
  const { data: orders, loading } = useData(
    () => api.serviceRequests.list({ status, page, limit: 20 }),
    [status, page, userKey]
  );
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <RegisterOraclePanel title="Utility Service Requests">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-[var(--color-text)] leading-relaxed">
              Utility service requests are stored in a classic <span className="tone-ocean font-mono">relational schema</span> - ACID transactions, foreign keys, and referential integrity.
              ...
              without ETL or duplication. Select any utility service request row and toggle between{' '}
          ...
          {/* UTILITY_SERVICE_REQUESTS_DV definition */}
          <div>
            <p className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider mb-2">
              UTILITY_SERVICE_REQUESTS_DV - Service Requests + Line Items as JSON
            </p>
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              REST-style JSON access to utility service requests with nested line items. The utilities-named duality view exposes governed request data through operator-focused field names.
            </p>
            <SqlBlock code={`SELECT DATA
FROM   utility_service_requests_dv
WHERE  JSON_VALUE(DATA, '$.serviceRequestId' RETURNING NUMBER) = :service_request_id;`} />
          </div>

          {/* UTILITY_SERVICES_V example */}
          <div>
            <p className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider mb-2">
              UTILITY_SERVICES_V - Service and Supply Catalog
            </p>
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              Utilities-facing relational access to utility services, supplies, utility categories, operational value, and utility partners.
            </p>
            <SqlBlock code={`SELECT utility_service_id,
       service_name,
       utility_category,
       service_value,
       utility_operator_or_partner
FROM   utility_services_v
...
SELECT r.service_request_id, r.requesting_service_point_name,
       r.request_value, i.service_supply_id,
       i.quantity, i.unit_cost
FROM   utility_service_requests r
JOIN   utility_request_items i
...
SELECT DATA FROM utility_service_requests_dv
WHERE  JSON_VALUE(DATA, '$.serviceRequestId' RETURNING NUMBER) = :id;
-- Returns utilities JSON fields such as serviceRequestId, requestStatus, and lineItems`} />
          </div>

          {/* Visual diagram */}
          <div>
            <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">One Data Set, Two Faces</p>
            <div className="rounded-lg p-3 space-y-2" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
              ...
              <div className="flex gap-2">
                <div className="flex-1 rounded p-2 text-[9px] text-center" style={{ background: '#437C9415', border: '1px solid #437C9440', color: 'var(--color-text)' }}>
                  ...
                  <div>SELECT *</div>
                  <div>FROM utility_service_requests</div>
          ...
          <div className="rounded-lg p-3 space-y-2" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
            ...
            <DiagramBox label="utility_service_requests + utility_request_items" sub="Energy & Utilities-named views over compatible relational rows" color="#437C94" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕ CREATE JSON RELATIONAL DUALITY VIEW</div>
            <DiagramBox label="UTILITY_SERVICE_REQUESTS_DV" sub="JSON document: {serviceRequestId, requestStatus, lineItems: [...]} - WITH UPDATE" color="#AA643B" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕</div>
            <DiagramBox label="services + inventory" sub="Utility-service catalog + field-logistics stock levels" color="#437C94" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕ CREATE JSON RELATIONAL DUALITY VIEW</div>
            <DiagramBox label="asset_capacity_v" sub="Utilities-facing supply and capacity view for field operations sites" color="#AA643B" />
          </div>
          ...
          <div className="rounded-lg p-3 text-center" style={{ background: 'rgba(170,100,59,0.06)', border: '1px dashed rgba(170,100,59,0.3)' }}>
            <p className="text-[10px] text-[var(--color-text-dim)] leading-relaxed">
              ...
              The governed service request rows can be queried through
              <span className="font-mono text-[var(--color-text)]">utility_service_requests</span> and{' '}
              <span className="font-mono text-[var(--color-text)]">utility_request_items</span>, then exposed as nested JSON documents through{' '}
              <span className="font-mono text-[var(--color-text)]">UTILITY_SERVICE_REQUESTS_DV</span>.
          ...
          <div>
            <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
              Virtual Private Database (VPD) on Service Requests
            </p>
            <p className="text-sm text-[var(--color-text)] leading-relaxed mb-2">
              A second <span className="tone-red font-mono">DBMS_RLS</span> policy on the internal compatibility table restricts fulfillment managers
              to utility service requests routed through their regional centers. The policy function queries{' '}
              <code className="text-xs tone-teal mx-1">fulfillment_center_id</code> against the user's assigned region.
              Admins and analysts see all 3,000 service requests; a regional manager sees only their subset.
            </p>
          </div>
          <div className="flex flex-wrap gap-1.5">
          ...
          <SqlBlock code={`-- VPD policy function for utility service requests
...
BEGIN
    ...
    -- Fulfillment mgr: service requests for their region's centers
  ...
  statement_types => 'SELECT');`} />
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <span className="oj-fwk-icon oj-fwk-icon-tree-document tone-ocean" aria-hidden="true" /> Utility Service Requests
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Select a utility service request to compare the relational view, JSON duality document, and logistics route generated from the same governed utility data.
        </p>
      </div>
          ...
          >
            ...
            <div className="flex items-center gap-2 flex-wrap flex-1">
              ...
              <span className="text-[var(--color-text-dim)] text-xs">
                {isFM
                  ? `Filtered to ${currentUser.REGION} - ${(orders || []).length} service requests visible`
                  : `${(orders || []).length} service requests visible`
                }
              </span>
            </div>
      ...
      {/* Utility Service Requests Table */}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[11px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)] bg-[var(--color-surface)]/50">
                ...
                <th className="text-left py-3 px-4">Requesting Service Point</th>
                <th className="text-left py-3 px-4">Service Point Location</th>
                <th className="text-center py-3 px-4">Request Status</th>
                <th className="text-right py-3 px-4">Line Items</th>
                <th className="text-right py-3 px-4">Request Priority Value</th>
                <th className="text-center py-3 px-4">Related Operational Signal</th>
                <th className="text-left py-3 px-4">Field Operations Site</th>
                <th className="text-left py-3 px-4">Created At</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={9} className="py-8 text-center text-[var(--color-text-dim)]">Loading utility service requests...</td></tr>
              ) : (orders || []).length === 0 ? (
                <tr><td colSpan={9} className="py-8 text-center text-[var(--color-text-dim)]">No utility service requests found</td></tr>
              ) : (
                (orders || []).map(o => {
                  const requestId = o.SERVICE_REQUEST_ID ?? o.ORDER_ID;
                  const requestStatus = o.REQUEST_STATUS ?? o.ORDER_STATUS;
                  const servicePointName = o.REQUESTING_SERVICE_POINT_NAME ?? o.CUSTOMER_NAME;
                  const servicePointCity = o.REQUESTING_SERVICE_POINT_CITY ?? o.CUSTOMER_CITY;
                  const servicePointRegion = o.REQUESTING_SERVICE_POINT_REGION ?? o.CUSTOMER_STATE;
                  const signalInfluenced = o.SIGNAL_INFLUENCED_FLAG ?? o.SOCIAL_DRIVEN;
                  const signalLabel = o.RELATED_SIGNAL_LABEL ?? (signalInfluenced ? 'Related operational signal' : 'No related signal');
                  const logisticsSiteName = o.FIELD_LOGISTICS_SITE_NAME ?? o.FULFILLMENT_CENTER;
                  const statusLabel = o.REQUEST_STATUS_DISPLAY_NAME ?? formatStatusLabel(requestStatus);
                  ...
                  return [
                    <tr key={requestId}
                      ...
                      <td className="py-3 px-4">{servicePointName}</td>
                      <td className="py-3 px-4 text-[var(--color-text-dim)]">{servicePointCity}, {servicePointRegion}</td>
                      <td className="py-3 px-4 text-center">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${STATUS_COLORS[requestStatus] || ''}`}>
                          {statusLabel}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-right">{o.ITEM_COUNT}</td>
                      <td className="py-3 px-4 text-right font-medium">{formatCurrency(o.REQUEST_VALUE ?? o.ORDER_TOTAL)}</td>
                      <td className="py-3 px-4 text-center">
                        {signalInfluenced ? (
                          <span className="tone-rose text-xs font-semibold">{signalLabel}</span>
                        ) : (
                          <span className="text-[var(--color-text-dim)] text-xs">{signalLabel}</span>
                        )}
                      </td>
                      <td className="py-3 px-4 text-xs text-[var(--color-text-dim)]">{logisticsSiteName || '-'}</td>
