...
const CONNECTION_COLORS = {
  reported_outage:       '#312D2A',
  root_caused_by:      '#C74634',
  used_demand_response: '#4C825C',
  opened_repair_action:   '#437C94',
  assigned_field_crew:          '#796087',
  located_at:         '#AA643B',
  has_reliability_gap:        '#A36472',
  followed_by:         '#4F7D7B',
  assigned_to:         '#5F7D4F',
  escalated_to:        '#AA643B',
  repeat_outage_after:    '#C74634',
  meter_indicates:       '#437C94',
  uses_asset:         '#F0CC71',
  shares_field_crew:     '#796087',
  case_signal:         '#C74634',
};

const EDGE_CATEGORY_ORDER = ['Operational Events', 'Field Coordination', 'Compliance & Risk'];

// ── Platform colors (Redwood) ───────────────────────────────────────────────
const PLATFORM_COLORS = {
  service_point:    '#C74634',
  outage_event:  '#437C94',
  condition:  '#AA643B',
  demand_response_event: '#4C825C',
  repair_action:  '#4F7D7B',
  field_crew:   '#796087',
  substation:   '#312D2A',
  reliability_gap:   '#A36472',
  asset:     '#F0CC71',
  meter_event: '#437C94',
};
...
function getNodeMetadata(node) {
  ...
  const operationalLabel = getMetric(node, 'OPERATIONS_LABEL', 'operations_label') || displayName || nodeId;
  const description = getMetric(node, 'DESCRIPTION', 'description') || '';

  return { nodeId, nodeType, displayName, operationalLabel, description };
}
...
function ForceGraph({ data, depth, height = 520, onNodeClick }) {
  ...
  useEffect(() => {
    ...
    node
      .on('mouseover', function(event, d) {
        const { nodeId, nodeType, displayName, operationalLabel, description } = getNodeMetadata(d);
        ...
        const openGaps = formatMetricValue(getMetric(d, 'OPEN_RELIABILITY_GAP_COUNT', 'open_reliability_gap_count'));
        ...
        tip.html(`
          <div style="font-size:11px;min-width:180px;line-height:1.7;color:#161513">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;border-bottom:1px solid rgba(49,45,42,0.12);padding-bottom:6px">
              ...
              <strong style="color:#161513">${escapeTooltipValue(operationalLabel)}</strong>
            ...
            <div style="display:grid;grid-template-columns:auto auto;gap:2px 12px">
              ...
              <span style="color:#6F757E">Open reliability gaps</span><span>${openGaps}</span>
...
const QUERY_COLORS = {
  reliability_gap_paths:    { color: '#C74634', icon: Network },
  repeat_outage_chain: { color: '#AA643B', icon: TrendingUp },
  shared_field_crew:  { color: '#796087', icon: Users },
  case_map:          { color: '#4F7D7B', icon: Table2 },
  restoration_hubs:         { color: '#4C825C', icon: Star },
};
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
            Run real SQL/PGQ queries against the <span className="tone-sienna font-mono">SERVICE_RESTORATION_NETWORK</span> property graph
          </p>
        </div>
        {activeQuery && (
...
export default function InfluencerGraph() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Oracle panel */}
      <RegisterOraclePanel title="Operational Event Graph">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              ...
              Energy & Utilities operational events as first-class graph objects. Vertices represent synthetic service points,
              electric outages, gas leaks, water main breaks, wastewater overflows, pipeline anomalies, well production issues,
              refinery constraints, LNG logistics delays, emissions excursions, HSE incidents, root causes, field crews,
              inspections, work orders, assets, and compliance records. Edges encode relationships like
              <code className="text-xs tone-plum mx-1">reported_outage · root_caused_by · has_reliability_gap · assigned_to</code>
              with a numeric <span className="tone-sienna font-mono">strength</span> weight.
              Graph traversal finds multi-hop asset, compliance, resolution, and shared-field-crew patterns without any external graph database.
            </p>
          </div>
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label="Compliance & Risk Paths" color="pink" />
            <FeatureBadge label="CONNECT BY" color="blue" />
            <FeatureBadge label="Operational Event Risk" color="green" />
          </div>
          <SqlBlock code={`-- ISO SQL/PGQ: Service Point-to-reliability-gap traversal
SELECT service_point_key, service_point_label, outage_event_key,
       outage_event_label, gap_key, gap_label,
       gap_edge, strength
FROM GRAPH_TABLE(
  service_restoration_network
  MATCH
    (service_point IS utility_entity)
    -[e1 IS restoration_link]->
    (outage_event IS utility_entity)
    -[e2 IS restoration_link]->
    (gap IS utility_entity)
  WHERE service_point.entity_key = :service_point_key
    AND gap.entity_type = 'reliability_gap'
  COLUMNS (
    service_point.entity_key AS service_point_key,
    service_point.operations_label AS service_point_label,
    outage_event.entity_key AS outage_event_key,
    outage_event.operations_label AS outage_event_label,
...
CREATE OR REPLACE PROPERTY GRAPH service_restoration_network
  VERTEX TABLES (
    utility_graph_entities KEY (entity_id) LABEL utility_entity
      PROPERTIES (entity_id, entity_key, node_id,
        entity_type, node_type, display_name,
        operations_label, description, operations_domain, risk_score,
        volume_count, engagement_rate, city, region),
    restoration_cases KEY (case_id) LABEL restoration_case
      PROPERTIES (case_id, case_key, case_type,
        severity, status, risk_score)
  )
  EDGE TABLES (
    utility_graph_relationships KEY (relationship_id)
      SOURCE KEY (from_entity_id)
        REFERENCES utility_graph_entities (entity_id)
      DESTINATION KEY (to_entity_id)
        REFERENCES utility_graph_entities (entity_id)
      LABEL restoration_link
      PROPERTIES (relationship_type, strength,
        interaction_count, evidence_text),
    restoration_case_entities KEY (case_entity_id)
      SOURCE KEY (case_id)
        REFERENCES restoration_cases (case_id)
...
SELECT node_id, node_type, display_name,
       operations_label, description
FROM utility_graph_node_metadata
WHERE node_id IN ('CAUSE-FEEDER-FAULT', 'GAP-REPEAT-OUTAGE', 'SP-1007')
ORDER BY node_id;`} />
...
ORDER BY category, edge_type;`} />
          <SqlBlock code={`-- Energy & Utilities metric projection used by the graph API
SELECT node_id, node_type, display_name,
       operations_label, pathway_volume,
       service_point_count, outage_event_count, risk_score,
       open_reliability_gap_count, direct_connection_count
FROM utility_graph_entity_metrics
...
WHERE center_node_id = 'CAUSE-FEEDER-FAULT'
  AND min_graph_depth <= 3
ORDER BY risk_score DESC;`} />
          <div className="grid grid-cols-2 gap-1.5 mt-2">
            <DiagramBox label="utility entities" sub="Vertex table" color="#C74634" />
            <DiagramBox label="operational links" sub="Edge table" color="#AA643B" />
...
CREATE FUNCTION vpd_utility_graph_entities(
...
--   restoration_case_entities`} />
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <Network className="text-[var(--color-accent)]" /> Operational Event Graph
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Explore de-identified outages, gas leaks, water main breaks, wastewater events, pipeline anomalies, well production issues, refinery constraints, LNG delays, emissions excursions, HSE incidents, affected assets, crews, inspections, work orders, compliance records, and resolution milestones using <span className="tone-sienna">Oracle Property Graph SQL/PGQ</span>.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-3 lg:flex lg:h-full lg:min-h-0 lg:flex-col">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search outage, gas leak, water break, well issue, refinery constraint, asset, crew..."
          ...
          >
            <h3 className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 flex items-center gap-1">
              <Users size={11} /> Operational Event Graph Nodes {rawInfluencers?.length ? `(${rawInfluencers.length})` : ''}
            </h3>
            {loading ? (
              <p className="text-xs text-[var(--color-text-dim)] py-4 text-center">Loading…</p>
            ) : (rawInfluencers || []).map(inf => {
              const { nodeId, nodeType, displayName, operationalLabel } = getNodeMetadata(inf);
              ...
              return (
                <button key={inf.INFLUENCER_ID}
                  ...
                  <div className="mt-0.5 truncate text-[10px] text-[var(--color-text-dim)]">{operationalLabel}</div>
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          {/* Selected restoration entity metrics + stats bar */}
          {network && (
            <div className="glass-card p-3">
              {network.center && (
                (() => {
              ...
              <p className="mb-3 text-[11px] leading-relaxed text-[var(--color-text-dim)]">
                <span className="font-semibold text-[var(--color-text)]">Current pathway focus:</span> Electric outage, gas leak, water/wastewater, pipeline, well, refinery, LNG, emissions, HSE, field crew, inspection, work order, compliance, and resolution milestones connected within the selected graph depth.
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-7 gap-2">
                {[
                  ...
                  { label: 'Open Reliability Gaps', value: formatMetricValue(getMetric(network.center, 'OPEN_RELIABILITY_GAP_COUNT', 'open_reliability_gap_count')), color: '#A36472' },
          ...
          {loadingNet ? (
            ...
            <div className="glass-card p-14 text-center text-[var(--color-text-dim)]">
              <Network size={28} className="mx-auto mb-3 opacity-40" />
              Select an operational event graph node to explore its network
            </div>
          )}
