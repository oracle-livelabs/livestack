 ...
 *   CUSTOMER_SEGMENT_MODEL  — K-Means Clustering (service point RFM segmentation)
...
async function handleMlRouteError(res, label, err, fallbackFn) {
  if (isMissingMlAssetError(err)) {
    return res.status(503).json({
      ...
      error: `${label} requires the persisted Energy & Utilities OML lifecycle`,
...
function omlSurgeProbability(row) {
  return Number(row.OML_SURGE_PROBABILITY) || 0;
}

function hasDemandRiskSignal(row) {
  return String(row.OML_SURGE_PREDICTION || '').toUpperCase() === 'SURGE'
    || omlSurgeProbability(row) >= 45;
}
...
function buildInventoryIntelligencePayload(candidateRows, meta) {
  ...
  const surgeProducts = new Set(
    candidates
      .filter(hasDemandRiskSignal)
      .map(a => a.PRODUCT_ID)
      .filter(Boolean)
  ).size;
  const totalRevenueAtRisk = candidates.reduce((sum, a) => sum + (Number(a.REVENUE_AT_RISK) || 0), 0);
  const leaderByProduct = new Map();

  candidates.forEach(a => {
    const key = a.PRODUCT_ID || a.PRODUCT_NAME;
    if (!key) return;
    const previous = leaderByProduct.get(key);
    if (!previous || omlSurgeProbability(a) > omlSurgeProbability(previous)) {
      leaderByProduct.set(key, {
        ...a,
        CHART_LABEL: a.PRODUCT_NAME,
      });
    }
  });

  const surgeProbabilityLeaders = [...leaderByProduct.values()]
    .sort((a, b) => omlSurgeProbability(b) - omlSurgeProbability(a))
    .slice(0, 8);
  ...
  candidates.forEach(a => {
    ...
    if (hasDemandRiskSignal(a)) centerMap[a.CENTER_NAME].surges++;
  });

  return {
    alerts,
    surgeProbabilityLeaders,
    summary: {
...
async function fallbackCustomerSegments({ limit }) {
  ...
  return {
    ...
    meta: {
      model: 'Pilot Site RFM segmentation heuristic (fallback)',
...
//
// Scores service points using CUSTOMER_SEGMENT_MODEL (K-Means, 4 clusters).
...
//
// Scores the deployment-owned persisted PRODUCT_CLUSTER_MODEL. Requests never
// retrain or replace the accepted model lifecycle.
// ─────────────────────────────────────────────────────────────────────────────
router.get('/vector-clusters', async (req, res) => {
  try {
    const k = Math.min(Math.max(parseInt(req.query.k) || 5, 2), 15);
    // Score only the persisted bootstrap model.
    const result = await db.execute(`
      WITH clustered AS (
        SELECT
          pcv.PRODUCT_ID,
