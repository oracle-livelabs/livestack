...
// GET /api/social/vector-readiness — Oracle model/vector/index provenance plus
// a nonempty executable probe. Live acceptance captures the matching plan.
router.get('/vector-readiness', async (req, res) => {
  try {
    const [columns, indexes, probe] = await Promise.all([
      db.executeAsUser(`SELECT table_name, column_name, data_type
