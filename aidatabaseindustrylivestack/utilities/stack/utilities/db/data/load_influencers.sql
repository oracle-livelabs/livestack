 ...
 * 500 Energy & Utilities signal sources across community, operational, regulatory, and partner channels
 */

SET SERVEROUTPUT ON
SET DEFINE OFF
PROMPT Loading Energy & Utilities signal sources...

DECLARE
    TYPE t_str IS TABLE OF VARCHAR2(100);
    v_prefixes t_str := t_str(
        'grid','crew','lineman','outage','meter','substation','feeder','storm',
        'dispatch','field','reliability','vegetation','transformer','voltage',
        'water','gas','microgrid','battery','solar','ev','derms','critical',
        'customer','callcenter','restoration','mutualaid','safety','inspection',
        'pressure','leak','streetlight','ami','sensor','relay','protection',
        'switching','controlroom','distribution','transmission','resilience',
        'load','peak','demand','utility','power','energy','service','response',
        'planner','operator','watermain','wastewater','pumpstation','treatment',
        'pipeline','corrosion','odorization','regulator','compressor','storage',
        'well','lift','producedwater','production','lng','terminal','refinery',
        'hydrocracker','turnaround','emissions','hse','regulatory','collections'
    );
...
BEGIN
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Energy & Utilities signal sources loaded: ' || v_count);
END;
/
