/*
 * load_orders.sql
 * 3000 service tickets with grid asset line items, varied statuses, and community-signal attribution
 */

SET SERVEROUTPUT ON
PROMPT Loading service tickets...
...
BEGIN
    ...
    FOR i IN 1..3000 LOOP
        ...
        -- 30% of service tickets are associated with a customer/community signal
        ...
        -- Assign operational event center for non-pending service tickets
        ...
        -- Update total operational value
        ...
        IF v_status IN ('shipped', 'delivered') AND v_center_id IS NOT NULL THEN
            ...
            v_est_hours := CASE WHEN v_distance IS NOT NULL
                                ...
                                ELSE NULL END;
            v_shipped_at := SYSTIMESTAMP - NUMTODSINTERVAL(DBMS_RANDOM.VALUE(2, 10) * 24, 'HOUR');
            v_delivered_at := CASE
                WHEN v_status = 'delivered'
                THEN v_shipped_at + NUMTODSINTERVAL(DBMS_RANDOM.VALUE(2, 36), 'HOUR')
                ELSE NULL
            END;
            ...
            INSERT INTO shipments (
                ...
                CASE MOD(i, 4)
                    WHEN 0 THEN 'GridCourier'
                    WHEN 1 THEN 'MobileCrew'
                    WHEN 2 THEN 'UtilityFleet'
                    ELSE 'FieldRoute'
                END,
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Utility requests loaded: ' || v_count);
END;
/
