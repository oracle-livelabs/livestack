...
import OwnerFinancialValidation from './pages/OwnerFinancialValidation';
...
const CUSTOMER_STORAGE_KEY = 'hospitalityLivestack.customerName';
const CUSTOMER_ALIASES = ['Hospitality LiveStack', 'Hospitality Hotels'];
const REPLACEABLE_ATTRIBUTES = ['aria-label', 'title', 'placeholder', 'alt'];
const DOM_SKIP_TAGS = new Set(['SCRIPT', 'STYLE', 'NOSCRIPT']);

const NAV_ITEMS = [
  { id: 'welcome', label: 'Welcome', iconClass: 'oj-fwk-icon oj-fwk-icon-info', featureTags: ['Oracle AI Database 26ai', 'Hospitality Operations'] },
  ...
  { id: 'dashboard', label: 'Property Performance Command Center', iconClass: 'oj-fwk-icon oj-fwk-icon-grid', featureTags: ['In-Memory', 'Native JSON'] },
  { id: 'social', label: 'Guest & Demand Signal Intelligence', iconClass: 'oj-fwk-icon oj-fwk-icon-sortrelevancehigh', featureTags: ['Vector Search', 'VPD'] },
  { id: 'graph', label: 'Guest Experience Network Graph', iconClass: 'oj-fwk-icon oj-fwk-icon-node-expand', featureTags: ['Property Graph', 'SQL/PGQ'] },
  { id: 'fulfillment', label: 'Housekeeping & Maintenance Coverage Map', iconClass: 'oj-fwk-icon oj-fwk-icon-calendar-clock', featureTags: ['Oracle Spatial', 'VPD'] },
  { id: 'orders', label: 'Reservation & Folio Operations', iconClass: 'oj-fwk-icon oj-fwk-icon-tree-document', featureTags: ['JSON Duality', 'VPD'] },
  { id: 'oml', label: 'OML Occupancy, Revenue & Labor Analytics', iconClass: 'oj-fwk-icon oj-fwk-icon-view', featureTags: ['OML', 'Vector K-Means'] },
  { id: 'askdata', label: 'Ask Hospitality Data', iconClass: 'oj-fwk-icon oj-fwk-icon-magnifier', featureTags: ['Natural Language SQL', 'Live Schema'] },
  { id: 'agents', label: 'Hospitality Agent Console', iconClass: 'oj-fwk-icon oj-fwk-icon-users', featureTags: ['PL/SQL Tools', 'JSON Audit'] },
  { id: 'ownerfinancial', label: 'Owner Financial Validation Workbench', iconClass: 'oj-fwk-icon oj-fwk-icon-list', featureTags: ['PL/SQL Validation', 'JSON Audit'] },
];
...
const PAGES = {
  ...
  oml: OMLAnalytics,
  askdata: AskData,
  ownerfinancial: OwnerFinancialValidation,
};
...
function replaceCustomerTerms(value, profile) {
  ...
  let next = value.replace(/\bhospitality-demo\.example\b/gi, profile.customerDomain);
  for (const alias of CUSTOMER_ALIASES) {
    next = next.replace(new RegExp(`\\b${alias.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\b`, 'g'), profile.customerName);
  }
  next = next.replace(/\bHospitality\b/g, profile.customerShortName);
  return next;
}

function hasReplaceableCustomerTerms(value) {
  return typeof value === 'string' && (
    /\bhospitality-demo\.example\b/i.test(value) ||
    /\bHospitality(?: LiveStack| Hotels)?\b/.test(value)
  );
}
...
function OracleBrand({ profile }) {
  const title = replaceCustomerTerms(CUSTOMER_NAME, profile);
  return (
    <button
      type="button"
...
function HospitalityGroupNameTool({ customerName, savedAcrossSessions, onApply }) {
  ...
  return (
    <div className="guest-name-tool">
      <button
        type="button"
        className="guest-name-tool__toggle"
      ...
      {isOpen && (
        <form className="guest-name-tool__panel" onSubmit={submit}>
          <label className="guest-name-tool__label" htmlFor="guest-name-input">Executive demo account</label>
          <input
            id="guest-name-input"
            className="guest-name-tool__input"
            value={draft}
            placeholder={CUSTOMER_NAME}
            onChange={(event) => setDraft(event.target.value)}
          />
          <label className="guest-name-tool__check">
            <input
              type="checkbox"
              checked={persist}
          ...
          <div className="guest-name-tool__actions">
            <button type="submit" className="guest-name-tool__apply">Apply</button>
            <button
              type="button"
              className="guest-name-tool__reset"
              onClick={() => {
          ...
          <p className="guest-name-tool__hint">Optional: adapt the visible demo account while preserving the hospitality-ready story defaults.</p>
        </form>
      )}
    </div>
  );
          ...
          <div className="app-shell">
            <aside className="app-sidebar">
              ...
              <div className="app-sidebar-footer">
                <HospitalityGroupNameTool
                  customerName={customerName}
                  savedAcrossSessions={savedAcrossSessions}
                  onApply={applyCustomerName}
                />
                <div className="app-sidebar-note">
                  <p className="app-sidebar-note__label">Active hospitality dataset</p>
