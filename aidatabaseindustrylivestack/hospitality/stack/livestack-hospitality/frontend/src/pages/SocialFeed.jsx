...
function formatOperationsRiskSeverityScore(item, rank = 0) {
...
function riskSeverityColor(score) {
  if (score > 75) return '#A73529';
  if (score > 50) return '#8A4E2F';
  return '#7A736E';
}
...
function formatAiOperationsRiskScore(item, riskSeverityScore) {
  ...
  const sentimentOperationsRisk = Number.isFinite(sentiment) ? (1 - Math.max(-1, Math.min(1, sentiment))) * 50 : 50;
  const score = Math.round((Number.isFinite(base) ? base : 50) * 0.7 + sentimentOperationsRisk * 0.3);
...
function FeedMetadataStrip({ item, riskSeverityScore }) {
  const aiOperationsRiskScore = formatAiOperationsRiskScore(item, riskSeverityScore);
  const metrics = [
    ['Revenue Impact', formatExposureImpact(item.VIEWS_COUNT)],
    ['Escalated Issues', formatNumber(item.SHARES_COUNT)],
    ['Open Service Requests', formatNumber(item.COMMENTS_COUNT)],
    ['Reviewed Signals', formatNumber(item.LIKES_COUNT)],
  ];
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-3.5 gap-y-1 mt-2 pt-2 border-t border-[var(--color-border)]/30 text-[11px] text-[var(--color-text-dim)]">
      ...
      {aiOperationsRiskScore != null && (
        <span className="ml-auto whitespace-nowrap">
          AI Issue Score: <span className={riskScoreClass(aiOperationsRiskScore)}>
            {aiOperationsRiskScore}
          </span>
        </span>
      )}
    </div>
...
function PostCard({ post, rank = 0 }) {
  ...
  const riskSeverityScore = formatOperationsRiskSeverityScore(post, rank);
  return (
      ...
      <div className="flex items-start justify-between gap-2.5">
          ...
          {sourceName && (
            <p className="text-xs text-[var(--color-accent)] font-medium mb-0.5">
              ...
              <span className="text-[var(--color-text-dim)] font-normal ml-2">
                Revenue Impact: {formatExposureImpact(post.VIEWS_COUNT)} · AI Confidence {post.INFLUENCE_SCORE}
              </span>
            </p>
          )}
        ...
        {riskSeverityScore != null && (
          <div className="flex-shrink-0 text-center">
            ...
            <div className="text-[9px] text-[var(--color-text-dim)] uppercase">Issue Severity</div>
          </div>
        )}
      </div>
...
function simColor(score) {
  if (score >= 0.7) return '#3E6F4D';
  if (score >= 0.5) return '#8A4E2F';
...
function VectorSearch() {
  ...
  const EXAMPLE_QUERIES = [
    'housekeeping delays tied to arrival pressure',
    'OTA overbooking discrepancy and guest recovery risk',
    'mobile key failure service recovery',
    'group booking block and room-readiness risk',
    'F&B outlet complaint and comp recovery trend',
    'brand-standard alert for room cleanliness',
  ];
  ...
  return (
    <div className="glass-card p-5 border border-teal-soft social-vector-search-panel">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="social-vector-search-panel__title">Room Type & Revenue Center Intelligence Search</h3>
        <span className="social-vector-search-panel__chip text-[var(--color-text)] border border-teal-soft">
          Governed match · revenue-impact ranking
        </span>
      </div>
      ...
      <div className="jet-control-row mb-3">
        <JetInputText
          value={query}
          placeholder="Search room types, guest signals, channel issues, or operational scenarios..."
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-[var(--color-text-dim)]">
              {results.length} room types matched for "<span className="tone-teal">{meta?.query}</span>"
            </p>
                {meta && (
                  <span className="text-[10px] text-[var(--color-text-dim)]">
                    evidence-ranked hospitality match · governed room and revenue index
                  </span>
                )}
          </div>
          {results.length === 0 ? (
            <p className="text-sm text-[var(--color-text-dim)]">No room types matched the query vector.</p>
          ) : (
                ...
                >
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className="text-[11px] font-mono whitespace-nowrap">
                      <span className="text-[var(--color-text-dim)]">Revenue Impact</span>{' '}
...
export default function SocialFeed() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <RegisterOraclePanel title="Guest & Demand Signal Intelligence">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              ...
              <span className="tone-pine">187 pre-embedded room and revenue-center vectors</span> and returns the top matches via an <span className="tone-plum font-mono">ANN index</span>
              ...
              The signal feed below uses <span className="tone-red font-mono">severity scoring</span> across 5,000 guest, OTA channel, demand, and operations signals with{' '}
          ...
          <SqlBlock code={`-- Real-time vector semantic search for room types and revenue centers
-- Embeds user query at runtime, then finds nearest
-- room type or revenue center vectors via ANN index (cosine distance)
...
SELECT p.product_id, p.product_name, p.category,
       ...
       ROUND(1 - VECTOR_DISTANCE(
         ...
         VECTOR_EMBEDDING(ALL_MINILM_L12_V2
                          USING 'suite with city views' AS DATA),
...
FETCH APPROXIMATE FIRST 10 ROWS ONLY;`} />
          <div>
            ...
            <div className="space-y-1.5">
              <DiagramBox label="User Query" sub="'late checkout availability pressure'" color="#4F7D7B" />
              ...
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="VECTOR_DISTANCE(COSINE)" sub="Query vector vs 187 room type or revenue center embeddings" color="#8A4E2F" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="ANN Index Scan" sub="FETCH APPROXIMATE FIRST K ROWS · 95% accuracy" color="#796087" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="Ranked Room Types and Revenue Impact" sub="Similarity score · property · revenue impact · signal links" color="#3E6F4D" />
            </div>
            <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 mt-4">Embedding Tables</p>
            <div className="space-y-1.5">
              <DiagramBox label="product_embeddings" sub="187 room types and revenue centers · 384-dim VECTOR · COSINE ANN index" color="#8A4E2F" />
              <DiagramBox label="signal_embeddings" sub="5,000 signal bulletins · 384-dim VECTOR · COSINE ANN index" color="#8A4E2F" />
              <DiagramBox label="semantic_matches" sub="574 pre-computed bulletin-room type or revenue center matches · vector method" color="#796087" />
            </div>
          </div>
          <div>
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <TrendingUp className="text-[var(--color-accent)]" /> Hospitality LiveStack Guest & Demand Signal Monitor
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Guest reviews, OTA channel discrepancies, demand spikes, and service alerts triaged into room impact, service priority, and operational response.
        </p>
      </div>
      ...
      <div>
        <h3 className="text-lg font-semibold text-[var(--color-text)]">
          Brand Standards & Operational Risk Activity Feed
        </h3>
      </div>
      ...
      <div className="jet-control-row">
        ...
        <JetSelectSingle
          ...
          options={[
            ...
            { value: 'instagram', label: 'Brand Standards bulletin' },
            { value: 'tiktok', label: 'Service recovery alert' },
            { value: 'twitter', label: 'Operations Control/Brand QA notice' },
            { value: 'youtube', label: 'Demand signal feed' },
            { value: 'threads', label: 'Property operations guest experience' },
          ]}
        />
        <JetSelectSingle
          value={influencer}
          className="jet-inline-field"
          placeholder="Operational Risk Category"
          onValueChange={(next) => { setInfluencer(next); setPage(1); }}
          options={[
            { value: '', label: 'Operational Risk Category' },
            ...influencers.map((i) => ({
              value: i.HANDLE,
        ...
        <div className="flex items-center gap-1 ml-2">
          <JetInputText
            value={postQuery}
            placeholder="Search guest issues, OTA discrepancies, demand spikes, or service alerts..."
            className="jet-inline-field"
      ...
      {postSearchResults ? (
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-[var(--color-text-dim)]">
            ...
            <span>Operational Risk intelligence results for "<span className="tone-teal">{postSearchResults.query}</span>"</span>
          ...
          {postSearchResults.posts?.length === 0 ? (
            ...
            postSearchResults.posts.map((p, idx) => {
              const riskSeverityScore = formatOperationsRiskSeverityScore(p, idx);
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {sourceName && (
                        <p className="text-xs text-[var(--color-accent)] font-medium mb-0.5">
                          {sourceName}
                          <span className="text-[var(--color-text-dim)] font-normal ml-2">
                            Revenue Impact: {formatExposureImpact(p.VIEWS_COUNT)} · AI Confidence {p.INFLUENCE_SCORE}
                          </span>
                        </p>
                      )}
