...
const CONNECTION_COLORS = {
  shared_stay_context: '#A73529',
  shared_guest_contact: '#A36472',
  shared_guest_context: '#8A4E2F',
  uses_channel:        '#4F7D7B',
  transfers_to:      '#3E6F4D',
  folio_settlement_at: '#796087',
  payment_testing_at: '#8A4E2F',
  venue_velocity:    '#697778',
};

const CONNECTION_LABELS = {
  shared_stay_context: 'shared stay context',
  shared_guest_contact: 'shared guest contact',
  shared_guest_context: 'shared guest context',
  uses_channel: 'uses booking channel',
  transfers_to: 'linked reservation',
  folio_settlement_at: 'folio settlement at',
  payment_testing_at: 'payment issue at',
  venue_velocity: 'venue activity spike',
};
...
const PLATFORM_LABELS = {
  ...
  payments: 'folio payment',
  venue: 'venue outlet',
  network: 'OTA network',
  unknown: 'unknown channel',
};

function graphCodeLabel(value) {
  const raw = String(value || '');
  return CONNECTION_LABELS[raw] || PLATFORM_LABELS[raw] || raw.replace(/_/g, ' ');
}

function platformColor(p) {
  return PLATFORM_COLORS[(p || '').toLowerCase()] || '#A73529';
}
...
function ForceGraph({ data, depth, height = 520, onNodeClick }) {
  ...
  useEffect(() => {
      ...
      .on('mouseover', function(event, d) {
        ...
        tip.html(`
          <div style="font-size:11px;line-height:1.65;color:#161513">
            ...
            <div>Revenue Impact: <strong>${formatCurrency(d.amount || 0)}</strong></div>
    ...
    const node = g.selectAll('.node')
      ...
      .call(d3.drag()
        .on('start', (event, d) => { if (!event.active) simulation.alphaTarget(0.3).restart(); d.international = d.x; d.fy = d.y; })
        .on('drag',  (event, d) => { d.international = event.x; d.fy = event.y; })
        .on('end',   (event, d) => { if (!event.active) simulation.alphaTarget(0); d.international = null; d.fy = null; })
      );
    ...
    node.append('circle')
      .attr('r', d => d.radius)
      .attr('fill', d => d.type === 'center' ? '#A73529' : platformColor(d.PLATFORM))
        ...
        tip.html(`
          <div style="font-size:11px;min-width:180px;line-height:1.7;color:#161513">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;border-bottom:1px solid rgba(49,45,42,0.12);padding-bottom:6px">
              ...
              ${isVerified(d) ? '<span style="color:#3E6F4D;font-size:10px;font-weight:600">confirmed</span>' : ''}
            </div>
            <div style="display:grid;grid-template-columns:auto auto;gap:2px 12px">
              ...
              <span style="color:#6F757E">Revenue Impact</span><span>${formatCurrency(d.FOLLOWER_COUNT || 0)}</span>
              <span style="color:#6F757E">Links</span><span style="color:#161513;font-weight:600">${d.linkCount || 0}</span>
              <span style="color:#6F757E">Operational Risk</span><span>${score}</span>
              <span style="color:#6F757E">Operational Risk ratio</span><span>${eng}</span>
              <span style="color:#6F757E">Hop</span><span>${d.type === 'center' ? '0 (center)' : d.hopLevel}</span>
            </div>
            <div style="color:#A73529;font-size:10px;font-weight:600;margin-top:6px;text-align:center">Click to investigate network</div>
          </div>
        `)
        .style('opacity', 1)
        .style('left', (event.pageX + 14) + 'px')
...
function NodeDetailPanel({ node, network, onExplore, onClose }) {
  ...
  const canExplore = node.NICHE === 'guest_profile';
  ...
  return (
    <div className="glass-card p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          { label: 'Revenue Impact', value: formatCurrency(node.FOLLOWER_COUNT || 0), iconClass: 'oj-fwk-icon-tree-document' },
          { label: 'Operational Risk Score', value: score, iconClass: 'oj-fwk-icon-sortrelevancehigh' },
            ...
            {Object.entries(connTypes).map(([type, count]) => (
              <div key={type} className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="capitalize">{graphCodeLabel(type)}</span>
                </span>
...
const QUERY_COLORS = {
  fraud_ring_reach:       { color: '#A73529', iconClass: 'oj-fwk-icon-folderhierarchy' },
  shared_guest_context:  { color: '#8A4E2F', iconClass: 'oj-fwk-icon-users' },
  folio_settlement_flow:  { color: '#4F7D7B', iconClass: 'oj-fwk-icon-arrowtail-e' },
  cross_channel_takeover: { color: '#437C94', iconClass: 'oj-fwk-icon-sortrelevancehigh' },
  risk_hubs:              { color: '#3E6F4D', iconClass: 'oj-fwk-icon-grid' },
};
    ...
    <div className="glass-card p-5 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
            Run real SQL/PGQ queries against the <span className="tone-sienna font-mono">GUEST_EXPERIENCE_NETWORK</span> property graph
          </p>
        </div>
        {activeQuery && (
          <JetButton
      ...
      {!activeQuery && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
          {queries.map(q => {
            const qStyle = QUERY_COLORS[q.id] || { color: '#A73529', iconClass: 'oj-fwk-icon-folderhierarchy' };
            return (
      ...
      {activeQuery && (
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3 p-3 rounded-xl"
            style={{ background: `${(QUERY_COLORS[activeQuery.id] || {}).color || '#A73529'}10`, border: `1px solid ${(QUERY_COLORS[activeQuery.id] || {}).color || '#A73529'}30` }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: `${(QUERY_COLORS[activeQuery.id] || {}).color || '#A73529'}22` }}>
              <JetGlyph
                iconClass={(QUERY_COLORS[activeQuery.id] || {}).iconClass || 'oj-fwk-icon-folderhierarchy'}
                className="graph-query-active-glyph"
                style={{ color: (QUERY_COLORS[activeQuery.id] || {}).color || '#A73529' }}
              />
            </div>
          ...
          {/* Error */}
          {error && (
            <div className="p-3 rounded-lg text-sm tone-red" style={{ background: 'rgba(167, 53, 41,0.1)', border: '1px solid rgba(167, 53, 41,0.3)' }}>
...
export default function InfluencerGraph() {
  ...
  // Track which user the current guest issue entity list belongs to
  const [listUser, setListUser] = useState(null);

  // Guest issue entity list - refetch when user or search changes
  ...
  // Network for selected guest issue entity
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Oracle panel */}
      <RegisterOraclePanel title="Guest Experience Network">
        <div className="space-y-4">
          <div>
            <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">What's Happening</p>
            <p className="text-[var(--color-text)] leading-relaxed">
              Oracle's <span className="tone-sienna font-mono">Property Graph</span> engine (SQL/PGQ - ISO standard) treats guests,
              reservations, booking channels, venues, folio payments, and service requests as a first-class graph. Edges encode guest evidence like
              <code className="text-xs tone-plum mx-1">shared_stay_context · uses_channel · folio_settlement_at · payment_testing_at</code>
              with numeric strength, event count, and revenue-impact values.
            </p>
          </div>
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label="Guest Experience Operational Risk Scoring" color="pink" />
            <FeatureBadge label="Case Evidence" color="blue" />
            <FeatureBadge label="No External Graph DB" color="green" />
          </div>
          <SqlBlock code={`-- Run as LIVESTACK. SQL/PGQ guest evidence within two hops.
...
FROM GRAPH_TABLE (fraud_network
  ...
  WHERE seed.entity_key = 'GUEST-8841'
...
WHERE entity_type IN ('guest_profile','booking_channel','payment_card','venue_outlet')
ORDER BY risk_score DESC
FETCH FIRST 25 ROWS ONLY;`} />
          <SqlBlock code={`-- Verify the persisted graph and its relational source objects.
SELECT object_name, status
FROM user_objects
WHERE object_type = 'PROPERTY GRAPH'
  AND object_name = 'FRAUD_NETWORK';

SELECT table_name
FROM user_tables
WHERE table_name IN (
  'FRAUD_ENTITIES', 'FRAUD_RELATIONSHIPS',
  'FRAUD_CASES', 'FRAUD_CASE_ENTITIES'
)
ORDER BY table_name;`} />
          <div className="grid grid-cols-2 gap-1.5 mt-2">
            <DiagramBox label="fraud_entities" sub="Guests and payment nodes" color="#A73529" />
            <DiagramBox label="fraud_relationships" sub="Evidence edge table" color="#8A4E2F" />
            <DiagramBox label="fraud_cases" sub="Service recovery vertices" color="#A36472" />
            <DiagramBox label="fraud_case_entities" sub="Service case membership edges" color="#3E6F4D" />
          </div>
          <div>
            ...
            <div className="grid grid-cols-2 gap-1">
              {Object.entries(CONNECTION_COLORS).map(([type, color]) => (
                <div key={type} className="flex items-center gap-1.5 text-[10px]">
                  ...
                  <span className="capitalize text-[var(--color-text-dim)]">{graphCodeLabel(type)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label="Guest Experience Graph API" color="blue" />
          </div>
...
FROM GRAPH_TABLE (fraud_network
  ...
  WHERE seed.entity_key = 'GUEST-8841'
  COLUMNS (
    dst.entity_key AS entity_key,
    dst.entity_type AS entity_type,
    dst.risk_score AS risk_score
  )
);`} />
        </div>
      </RegisterOraclePanel>
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <JetGlyph iconClass="oj-fwk-icon-node-expand" className="text-[var(--color-accent)]" /> Guest Experience Network
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Investigate connected guests, reservations, payment channels, venues, and service requests behind the guest-experience signal.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-3">
          ...
          <div className="relative graph-search-field">
            ...
            <JetInputText
              ...
              ariaLabel="Search guest issue entities"
          ...
          <div className="glass-card p-3 max-h-[480px] overflow-y-auto">
            <h3 className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 flex items-center gap-1">
              <JetGlyph iconClass="oj-fwk-icon-users" /> Guest Profiles {rawInfluencers?.length ? `(${rawInfluencers.length})` : ''}
            </h3>
            {loading ? (
              ...
              <JetActionCard
                ...
                <div className="flex items-center gap-2 mt-0.5 text-[10px] text-[var(--color-text-dim)]">
                  <span>{formatCurrency(inf.FOLLOWER_COUNT)} revenue impact</span>
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          {network && (
            <div className="glass-card p-3">
              ...
              <div className="grid grid-cols-7 gap-2">
                {[
                  { label: 'Revenue Impact',    value: formatCurrency(network.center?.FOLLOWER_COUNT || 0) },
                  { label: 'Operational Risk',        value: network.center?.INFLUENCE_SCORE || 0 },
          ...
          {loadingNet ? (
            ...
            <div className="glass-card p-14 text-center text-[var(--color-text-dim)]">
              <JetGlyph iconClass="oj-fwk-icon-node-expand" className="graph-empty-glyph mx-auto mb-3 opacity-40" />
              Select a guest issue entity to investigate its network
            </div>
          )}
