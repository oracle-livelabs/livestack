...
export const api = {
  ...
  fulfillment: {
    ...
    guests: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return apiFetch(`/fulfillment/guests?${qs}`);
    },
  ...
  ml: {
    ...
    guestSegments: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return apiFetch(`/ml/guest-segments?${qs}`);
    },
  ...
  selectai: {
    ...
    health: (profile) => {
      ...
      return apiFetch(`/selectai/health${qs}`);
    },
  ...
  financialValidation: {
    summary: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return apiFetch(`/financial-validation/summary${qs ? `?${qs}` : ''}`);
    },
    exceptions: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return apiFetch(`/financial-validation/exceptions${qs ? `?${qs}` : ''}`);
    },
    exceptionDetail: (id) => apiFetch(`/financial-validation/exceptions/${id}`),
    charts: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return apiFetch(`/financial-validation/charts${qs ? `?${qs}` : ''}`);
    },
    run: (payload) => apiFetch('/financial-validation/run', {
      method: 'POST',
    ...
    aiJobs: (params = {}) => {
      const normalized = {
        ...params,
        ...(Array.isArray(params.jobIds) ? { jobIds: params.jobIds.join(',') } : {}),
      };
      const qs = new URLSearchParams(normalized).toString();
      return apiFetch(`/financial-validation/ai-jobs${qs ? `?${qs}` : ''}`);
    },
    takeAction: (id, payload) => apiFetch(`/financial-validation/exceptions/${id}/action`, {
      method: 'POST',
      body: JSON.stringify(payload),
    }),
    closeNote: (submissionId, actor) => apiFetch('/financial-validation/close-note', {
      method: 'POST',
      body: JSON.stringify({ submissionId, actor }),
    }),
    internals: () => apiFetch('/financial-validation/internals'),
  },
