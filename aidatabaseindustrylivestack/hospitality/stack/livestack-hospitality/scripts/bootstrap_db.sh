...
echo ">>> Hospitality LiveStack bootstrap starting inside db container..."
...
cp "${APP_DIR}/db/schema/06a_hospitality_app_context_admin.sql" /tmp/06a_hospitality_app_context_admin.sql
apply_schema_user /tmp/06a_hospitality_app_context_admin.sql
...
SQL
  ...
  LOCAL_MODEL_SOURCE="${APP_DIR}/db/data/onnx/${ONNX_MODEL_FILENAME:-all_MiniLM_L12_v2.onnx}"

  echo ">>> Ensuring ONNX model is available in DATA_PUMP_DIR..."
  mkdir -p "$MODEL_DIR"
  if [ ! -s "$MODEL_PATH" ]; then
    rm -f "$MODEL_TEMP"
    if [ -s "$LOCAL_MODEL_SOURCE" ]; then
      echo ">>> Using bundled ONNX model: $LOCAL_MODEL_SOURCE"
      cp "$LOCAL_MODEL_SOURCE" "$MODEL_TEMP"
    else
      curl -fL \
        --retry 5 \
        --retry-delay 2 \
...
echo ">>> Installing trusted Hospitality application context and VPD policies..."
...
SET SERVEROUTPUT ON
@/tmp/06_security_admin.sql
@/tmp/06a_hospitality_app_context_admin.sql
EXIT
SQL
...
END;
/
@${APP_DIR}/db/schema/06b_hospitality_vpd_policies.sql
BEGIN
  hospitality_security_pkg.clear_user_context;
END;
/
EXIT
SQL
...
@${APP_DIR}/db/schema/11_hospitality_views.sql
@${APP_DIR}/db/schema/13_owner_financial_validation.sql
@${APP_DIR}/db/schema/09_comments.sql
@${APP_DIR}/db/data/load_fraud_graph.sql
@${APP_DIR}/db/data/seed_fulfillment_zones.sql
@${APP_DIR}/db/data/backfill_signal_source_names.sql
@${APP_DIR}/db/data/backfill_signal_language.sql
@${APP_DIR}/db/data/backfill_social_post_criticality.sql
@${APP_DIR}/db/data/refresh_demo_dates.sql
@${APP_DIR}/db/data/backfill_spatial_locations.sql
@${APP_DIR}/db/data/load_owner_financial_validation.sql
@${APP_DIR}/db/schema/12_oml_models.sql
@/tmp/08_agents_functions.sql
@${APP_DIR}/db/schema/06c_hospitality_security_cleanup.sql
BEGIN
  hospitality_security_pkg.clear_user_context;
END;
/
