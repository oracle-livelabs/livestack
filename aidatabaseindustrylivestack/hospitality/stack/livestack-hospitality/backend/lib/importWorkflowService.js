...
const { validateDemoFulfillmentCapacityRows } = require('./demoCapacityProfile');
...
function buildTemplateReadme() {
  return [
    ...
    '- Derived columns such as guests.location, fulfillment_centers.location, order_items.line_total, fulfillment_zones, and vector embedding tables are rebuilt by the importer and therefore are not included as editable CSV inputs.',
    '- Domain labels use the original schema names for compatibility: brands are properties, products are room types and revenue centers, influencers are signal sources, and social_posts are channel or demand bulletins.',
...
function validateBundledDemoCapacityProfile(parsed) {
  const rows = parsed?.dataset?.tables?.fulfillment_centers?.rows || [];
  const result = validateDemoFulfillmentCapacityRows(rows);

  if (!result.valid) {
    throw new ImportError('Bundled demo dataset has invalid Harborstone room capacity data.', 500, {
...
function getBundledDemoDataset(version = IMPORT_VERSION) {
  ...
  if (!cachedBundledDemoDataset || cachedBundledDemoDataset.archive?.signature !== archive.signature) {
    ...
    validateBundledDemoCapacityProfile(parsed);
...
async function clearOwnerFinancialDemoData(connection, warnings) {
  try {
    await execSql(connection, 'BEGIN owner_fin_validation_pkg.clear_demo_data; END;');
  } catch (error) {
    if (/ORA-(04043|06550)|PLS-00201/.test(error.message || '')) {
      warnings.push('Owner financial validation objects are not installed yet; the import continued without Scene 10 refresh.');
      return false;
    }
    throw error;
  }
...
async function rebuildOwnerFinancialDemoData(connection, enabled) {
  if (!enabled) return 0;
  await execSql(connection, 'BEGIN owner_fin_validation_pkg.refresh_demo_data(p_force => 0); END;');
  const countResult = await execSql(connection, 'SELECT COUNT(*) AS submission_count FROM hotel_financial_submissions');
  return Number(countResult.rows?.[0]?.SUBMISSION_COUNT || 0);
}
...
function buildFallbackDemandRegions(dataset) {
  const guests = dataset.tables.guests.rows;
  const orders = dataset.tables.orders.rows;
  const guestsById = buildSourceRowMap(guests, 'guest_id');
  const groups = new Map();

  for (const guest of guests) {
    if (!Number.isFinite(Number(guest.latitude)) || !Number.isFinite(Number(guest.longitude))) continue;

    const city = String(guest.city || '').trim();
    const state = String(guest.state_province || '').trim();
    const country = String(guest.country || 'US').trim();
    ...
    const group = groups.get(key) || {
      regionName: label,
      regionType: 'metro',
      minLat: Number(guest.latitude),
      maxLat: Number(guest.latitude),
      minLon: Number(guest.longitude),
      maxLon: Number(guest.longitude),
      guestCount: 0,
    ...
    group.guestCount += 1;
    group.lifetimeValueTotal += Number(guest.lifetime_value) || 0;
    group.minLat = Math.min(group.minLat, Number(guest.latitude));
    group.maxLat = Math.max(group.maxLat, Number(guest.latitude));
    group.minLon = Math.min(group.minLon, Number(guest.longitude));
    group.maxLon = Math.max(group.maxLon, Number(guest.longitude));
    groups.set(key, group);
  }

  for (const order of orders) {
    const guest = guestsById.get(normalizeSourceId(order.guest_id));
    if (!guest) continue;
    const city = String(guest.city || '').trim();
    const state = String(guest.state_province || '').trim();
    const country = String(guest.country || 'US').trim();
  ...
  return [...groups.values()]
    .map((group) => {
      ...
      const avgLifetimeValue = group.guestCount ? group.lifetimeValueTotal / group.guestCount : 0;
      ...
      return {
        ...
        population: Math.max(group.guestCount * 10000, group.guestCount),
        avgIncome: roundTo(Math.max(45000, avgLifetimeValue * 8 || 55000), 2),
        socialDensity: roundTo((group.socialOrderCount / Math.max(group.guestCount, 1)) * 100, 2) || 0,
  ...
  for (const row of rows) {
    await execSql(connection, `
      INSERT INTO shipments (
        ...
        distance_km, estimated_hours, ship_cost, routed_at, completed_at, created_at
      ) VALUES (
        :orderId, :centerId, :carrier, :trackingNumber, :shipStatus,
        :distanceKm, :estimatedHours, :shipCost, :routedAt, :completedAt, :createdAt
      )
    `, row);
    inserted += 1;
  }
...
function buildFallbackShipments(dataset, idMaps) {
  const orders = dataset.tables.orders.rows;
  const guestsById = buildSourceRowMap(dataset.tables.guests.rows, 'guest_id');
  const centersById = buildSourceRowMap(dataset.tables.fulfillment_centers.rows, 'center_id');
  const carriers = ['SecureCourier', 'PropertyLink', 'airport-transferTrack', 'GuestRoute'];
  const shipStatusMap = {
    confirmed: 'preparing',
    processing: 'packed',
    routed: 'in_transit',
  ...
  for (const order of orders) {
    ...
    const shipLat = Number.isFinite(Number(order.shipping_lat)) ? Number(order.shipping_lat) : Number(guest?.latitude);
    const shipLon = Number.isFinite(Number(order.shipping_lon)) ? Number(order.shipping_lon) : Number(guest?.longitude);
    ...
    const routedAt = createdAt ? new Date(createdAt.getTime() + (6 * 60 * 60 * 1000)) : null;
    const completedAt = orderStatus === 'completed' && routedAt && estimatedHours != null
      ? new Date(routedAt.getTime() + (estimatedHours * 60 * 60 * 1000))
      : null;
...
async function applyOptionalFallbacks(connection, dataset, idMaps, warnings, progress) {
  ...
  if (!dataset.tables.demand_regions.provided) {
    ...
    if (!generatedDemandRegions.length) warnings.push('No fallback demand_regions could be generated because guest geospatial data was missing.');
  }
...
async function regenerateVectorArtifacts(connection) {
  ...
  `);
  summary.signal_embeddings = postEmbeddings.rowsAffected || 0;
...
async function executeImportPlan({ dataset, dryRun = false, progress = null, refreshDemoDates = false }) {
  ...
  try {
    ...
    // Pooled sessions can retain the demo package context. Dataset replacement
    // is an administrator workflow, so reset the same Oracle session explicitly
    // before touching VPD-protected Scene 10 objects.
    await execSql(
      connection,
    ...
    const financialValidationEnabled = await clearOwnerFinancialDemoData(connection, warnings);
    ...
    if (progress) progress({ status: 'running', progress: 84, message: 'Rebuilding owner financial validation submissions...' });
    const financialSubmissions = await rebuildOwnerFinancialDemoData(connection, financialValidationEnabled);
    ...
    if (financialValidationEnabled) {
      summary.generated.owner_financial_submissions = financialSubmissions;
    }
...
async function inferCurrentDatasetState() {
  ...
  try {
    connection = await db.getConnection();
    await db.setUserContext(connection, 'admin_ava');
...
async function generateTemplateArchive({ version = IMPORT_VERSION }) {
  ...
  return {
    buffer: zip.toBuffer(),
    fileName: `hospitality-import-template-${version}.zip`,
...
module.exports = {
  ...
  _private: {
    ...
    parseArchiveDataset,
    validateBundledDemoCapacityProfile,
  },
};
