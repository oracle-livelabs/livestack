...
const TABLES = [
  ...
  },
  {
    ...
    columns: [
      ...
      enumColumn('center_type', ['Full-Service Hotel', 'Convention Hotel', 'Select-Service Hotel', 'Resort Property', 'Extended-Stay Hotel', 'Partner Property']),
  ...
  },
  {
    ...
    columns: [
      ...
      enumColumn('guest_tier', ['new', 'standard', 'preferred', 'vip']),
      numberColumn('lifetime_value'),
      timestampColumn('created_at'),
    ],
    uniqueKeys: [['guest_id'], ['email']],
  },
  {
    name: 'influencers',
    required: true,
  ...
  },
  {
    ...
    columns: [
      idColumn('order_id'),
      refIdColumn('guest_id'),
      enumColumn('order_status', ['pending', 'confirmed', 'processing', 'routed', 'completed', 'cancelled', 'returned']),
  ...
  },
  {
    ...
    columns: [
      ...
      enumColumn('ship_status', ['preparing', 'picked', 'packed', 'routed', 'in_transit', 'out_for_delivery', 'completed', 'exception']),
      numberColumn('distance_km'),
      numberColumn('estimated_hours'),
      numberColumn('ship_cost'),
      timestampColumn('routed_at'),
...
const TEMPLATE_NOTES = [
  ...
  'Derived columns such as guests.location, fulfillment_centers.location, order_items.line_total, fulfillment_zones, and vector embedding tables are regenerated by the importer.',
  ...
  'Scene 10 owner financial submissions, validation runs, exceptions, and rules are deterministic derived demo data and are regenerated after an import.',
  'Domain labels keep the original schema names for compatibility: brands are properties, products are room types and revenue centers, influencers are signal sources, and social_posts are channel or demand bulletins.',
