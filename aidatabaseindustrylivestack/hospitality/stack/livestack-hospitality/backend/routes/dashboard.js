...
// Supports: ?limit=10 &search=<room type or revenue center/property text> &brand=<exact property name>
  ...
  try {
    ...
    const result = await db.executeAsUser(`
      ...
      GROUP BY p.product_id, p.product_name, p.category, p.unit_price,
               b.brand_name, b.social_tier
      ORDER BY criticality_score DESC NULLS LAST, total_views DESC
      FETCH FIRST :limit ROWS ONLY
    `, binds, req.demoUser);

    res.json(result.rows);
  } catch (err) {
    console.error('Watched room types and revenue centers error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/social-velocity', async (req, res) => {
  try {
    ...
    const result = await db.executeAsUser(`
      ...
      FROM social_posts
      WHERE posted_at >= (SELECT MAX(posted_at) FROM social_posts)
...
router.get('/inmemory', async (req, res) => {
  try {
    ...
    let imStats = {};
    try {
      const im = await db.executeAsUser(`
