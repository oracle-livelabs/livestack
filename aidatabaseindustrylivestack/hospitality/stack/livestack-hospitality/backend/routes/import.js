...
function notConfigured(res) {
  return res.status(501).json({
    ...
    hint: 'Attbanquet-deposit `app.locals.importWorkflowService` or provide `backend/lib/importWorkflowService.js`',
  });
}
...
router.get('/template', async (req, res) => {
  ...
  try {
    ...
    const fileName = archive.fileName || `hospitality-import-template-${version}.zip`;
    const contentType = archive.contentType || 'application/zip';
    res.setHeader('Content-Type', contentType);
    res.setHeader('Content-Disposition', `attbanquet-depositment; filename="${fileName}"`);
