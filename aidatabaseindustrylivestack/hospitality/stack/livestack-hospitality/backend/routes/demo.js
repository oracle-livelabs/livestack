...
router.get('/start', async (req, res) => {
  ...
  try {
    ...
    const counts = {};
    const tables = [
      'brands', 'products', 'influencers', 'guests', 'social_posts',
    ...
    // ── 2. BRANDS (5-10%) ─────────────────────────────────────────────────
    if (aborted) return;
    send({ step: 'brands', status: 'running', message: 'Loading 50 properties...', progress: 5 });

    if (counts.brands > 0) {
      send({ step: 'brands', status: 'skipped', message: `${counts.brands} properties already loaded`, progress: 10, count: counts.brands });
    } else {
      // Inline property inserts — same data as load_all_data.sql
      const brandInserts = [
        "('Grand Harbor Hotel','harborstone-grand-harbor','Urban Hotel','Houston',29.7604,-95.3698,1998,245000000,'premium')",
        "('Canyon Reserve Resort','canyon-reserve-resort','Luxury Resort','Newark',40.7357,-74.1724,2004,186000000,'premium')",
        "('Lakeside Chicago Hotel','harborstone-lakeside-inn','Lifestyle Hotel','Chicago',41.8781,-87.6298,1987,132000000,'premium')",
        "('Summit Resort','harborstone-summit-lodge','Resort Operations','Detroit',42.3314,-83.0458,1992,221000000,'luxury')",
        "('City Center Grand Hotel','st-regis-city-center','Luxury Hotel','Columbus',39.9612,-82.9988,2001,98000000,'standard')",
        "('Vineyard Resort','harborstone-vineyard-resort','Resort Operations','Des Moines',41.5868,-93.625,1979,154000000,'premium')",
        "('Shoreline Beach Club','harborstone-beach-club','Beach Resort','Reno',39.5296,-119.8138,2016,91000000,'premium')",
        "('Landmark Convention Center','harborstone-convention-center','Events Venue','Boston',42.3601,-71.0589,2011,43000000,'standard')",
        "('Grand Garden Hotel','harborstone-garden-hotel','Urban Hotel','Cleveland',41.4993,-81.6944,1968,275000000,'luxury')",
        "('Springs Hotel Portland','springs-hotel-portland','Sustainable Hospitality','Portland',45.5152,-122.6784,2018,39000000,'emerging')",
        "('Airport Hotel Charlotte','harborstone-airport-hotel','Airport Hotel','Charlotte',35.2271,-80.8431,2007,76000000,'standard')",
        "('Dining and Events Hotel Tulsa','dining-events-hotel-tulsa','Food and Beverage','Tulsa',36.154,-95.9928,1996,117000000,'premium')",
        "('Mountain Spa Resort','northern-guestexp','Spa and Wellness','Minneapolis',44.9778,-93.265,2005,52000000,'standard')",
        "('Harborstone Gulf Coast Resort and Spa','harborstone-gulf-coast-resort','Revenue Operations','Baton Rouge',30.4515,-91.1871,1974,203000000,'premium')",
        "('Midwest Convention Hotel','harborstone-midwest-convention-hotel','Group Sales','Indianapolis',39.7684,-86.1581,1989,88000000,'standard')",
        "('Pacific Center Hotel','pacific-center-hotel','Folio Payments','Los Angeles',34.0522,-118.2437,1994,143000000,'premium')",
        "('Quality Services Hotel','quality-services-hotel','Guest Experience','San Jose',37.3382,-121.8863,2012,69000000,'standard')",
        "('Service Recovery Hotel','service-recovery-hotel','Service Recovery','Memphis',35.1495,-90.049,2009,58000000,'standard')",
        "('Harborstone Waterfront Milwaukee','harborstone-waterfront-milwaukee','Association and Government Group','Milwaukee',43.0389,-87.9065,2003,74000000,'standard')",
        "('Wellness Retreat San Diego','wellness-retreat-san-diego','Wellness Hospitality','San Diego',32.7157,-117.1611,2015,46000000,'emerging')",
        "('Digital Guest Services Hotel','digital-guest-services-hotel','Digital Guest Services','Phoenix',33.4484,-112.074,2019,34000000,'emerging')",
        "('Sustainability Lodge','sustainability-lodge','Sustainable Hospitality','Seattle',47.6062,-122.3321,2021,21000000,'emerging')",
        "('Harborstone Operations Analytics Center','harborstone-operations-analytics','Operations Analytics','Denver',39.7392,-104.9903,2014,18000000,'emerging')",
        "('Extended Stay Medical District','harborstone-medical-district-hotel','Group Billing','Philadelphia',39.9526,-75.1652,1999,112000000,'premium')",
        "('CleanStay Cincinnati Hotel','harborstone-cleanstay-hotel','Select-Service Hotel','Cincinnati',39.1031,-84.512,2008,65000000,'standard')",
        "('Harborstone Bridge Revenue Hotel','harborstone-bridge-hotel','Revenue Management','Atlanta',33.749,-84.388,1991,126000000,'premium')",
        "('Harborstone Continuity Guest Desk','harborstone-continuity-desk','Guest Experience Operations Risk','Dallas',32.7767,-96.797,2013,47000000,'standard')",
        "('Harborstone Direct Booking Center','harborstone-direct-booking-desk','Digital Hospitality','Raleigh',35.7796,-78.6382,2006,82000000,'standard')",
        "('Harborstone Portside Events Hotel','harborstone-portside-hotel','Event Revenue','Savannah',32.0809,-81.0912,1985,157000000,'premium')",
        "('Extended Stay Kansas City','harborstone-extended-stay','Extended Stay','Kansas City',39.0997,-94.5786,1997,93000000,'standard')",
        "('Harborstone U.S. Property Portfolio','harborstone-portfolio','Property Portfolio','St. Louis',38.627,-90.1994,1982,138000000,'premium')",
        "('Tech District Hotel','tech-district-hotel','Luxury Hospitality','Akron',41.0814,-81.519,2002,71000000,'standard')",
        "('GreenStay Pittsburgh Hotel','harborstone-sustainability-hotel','Sustainability','Pittsburgh',40.4406,-79.9959,1978,99000000,'standard')",
        "('Harborstone Guest Protection Desk','harborstone-guest-protection','Guest Protection','Tampa',27.9506,-82.4572,1995,61000000,'standard')",
        "('City Center Grand VIP Planning Events','harborstone-signature-events','Events Operations','Baltimore',39.2904,-76.6122,2000,55000000,'standard')",
        "('Harborstone Coastal Resort Wilmington','harborstone-coastal-resort-wilmington','Regional Hospitality','Wilmington',34.2257,-77.9447,1993,104000000,'premium')",
        "('Harborstone Assurance Services Desk','harborstone-assurance-services','Guest Protection','Nashville',36.1627,-86.7816,2006,57000000,'standard')",
        "('Harborstone Partner Direct Desk','harborstone-partner-direct','Partner Hospitality','San Antonio',29.4241,-98.4936,1990,118000000,'premium')",
        "('Harborstone Apex Resort Louisville','harborstone-apex-resort','Resort Operations','Louisville',38.2527,-85.7585,2004,67000000,'standard')",
        "('Harborstone Group Sales Strategy Center','harborstone-group-sales','Long-Stay Hospitality','Omaha',41.2565,-95.9345,1988,149000000,'premium')",
        "('Harborstone Continuity New Orleans Hotel','harborstone-continuity-desk','Operational Resilience','New Orleans',29.9511,-90.0715,2001,59000000,'standard')",
        "('Harborstone Channel Watch Austin','harborstone-channel-watch','Channel Intelligence','Austin',30.2672,-97.7431,2020,26000000,'emerging')",
        "('Harborstone Brand Standards Office','harborstone-brand-standards-office','Brand Standards','Washington',38.9072,-77.0369,2017,31000000,'emerging')",
        "('Harborstone Brand Standards Desk','harborstone-brand-standards','Brand Standards','Washington',38.9072,-77.0369,2010,44000000,'standard')",
        "('Harborstone Demand Signals Lab','harborstone-demand-signals','Demand Data','Long Beach',33.7701,-118.1937,2018,29000000,'emerging')",
        "('Harborstone Property Operations Las Vegas','harborstone-property-operations','Property Operations','Las Vegas',36.1699,-115.1398,2012,51000000,'standard')",
        "('Harborstone U.S. Property Portfolio Desk','harborstone-portfolio-insights','Property Portfolio Signals','Cleveland',41.4993,-81.6944,2016,37000000,'emerging')",
        "('Harborstone Data Connect Center','harborstone-data-connect','Data Services','Salt Lake City',40.7608,-111.891,2009,48000000,'standard')",
        "('Harborstone Events Exchange Miami','harborstone-events-exchange','Revenue Management','Miami',25.7617,-80.1918,2015,63000000,'standard')",
        "('Harborstone NorthStar Group Sales','harborstone-northstar-lodge','Group Sales','Fargo',46.8772,-96.7898,2008,54000000,'standard')",
      ];
      ...
      send({ step: 'brands', status: 'done', message: `${brandCount} properties loaded`, progress: 10, count: brandCount });
    }

    // ── 3. PRODUCTS (10-18%) ──────────────────────────────────────────────
    if (aborted) return;
    send({ step: 'products', status: 'running', message: 'Checking room types and revenue centers...', progress: 10 });

    if (counts.products > 0) {
      send({ step: 'products', status: 'skipped', message: `${counts.products} room types and revenue centers already loaded`, progress: 18, count: counts.products });
    } else {
      // Room Types and Revenue Centers are loaded via PL/SQL in load_products.sql — run inline
      // For the API we trigger the same pattern: use the load script via execute
      send({ step: 'products', status: 'error', message: 'Room Types and Revenue Centers require SQL*Plus load (run load_products.sql). Skipping.', progress: 18, count: 0 });
    }
    ...
    // ── 5. BUYERS (25-33%) ────────────────────────────────────────────────
    if (aborted) return;
    send({ step: 'guests', status: 'running', message: 'Checking guests...', progress: 25 });

    const guestCount = counts.guests || await tableCount('guests');
    if (guestCount > 0) {
      send({ step: 'guests', status: 'skipped', message: `${guestCount} guests already loaded`, progress: 33, count: guestCount });
    } else {
      send({ step: 'guests', status: 'error', message: 'Guests require SQL*Plus load (run load_guests.sql). Skipping.', progress: 33, count: 0 });
    }
    ...
    if (postCount > 0) {
      ...
      send({ step: 'social_posts', status: 'error', message: 'Channel signals require SQL*Plus load (run load_social_posts.sql). Skipping.', progress: 42, count: 0 });
    }
    ...
    if (edgeCount > 0) {
      send({ step: 'graph', status: 'skipped', message: `${edgeCount} edges, ${linkCount} property links already loaded`, progress: 56, count: edgeCount });
    } else {
    ...
    if (aborted) return;
    send({ step: 'spatial_centers', status: 'running', message: 'Populating property hotel locations...', progress: 56 });
    ...
    try {
      ...
      if (needsUpdate > 0) {
        ...
        `);
        send({ step: 'spatial_centers', status: 'done', message: `${needsUpdate} hotel locations populated`, progress: 62, count: needsUpdate });
      } else {
        const centerCount = await tableCount('fulfillment_centers');
        send({ step: 'spatial_centers', status: 'skipped', message: `${centerCount} hotels already have locations`, progress: 62, count: centerCount });
      }
    } catch (err) {
    ...
    try {
      ...
      if (prodEmbedCount >= productTotal && productTotal > 0) {
        ...
        send({ step: 'product_embeddings', status: 'skipped', message: 'No room types and revenue centers to embed', progress: 87, count: 0 });
      } else {
    ...
    try {
      ...
      if (matchCount > 0) {
        ...
        // Find elevated or critical risk signals that have embeddings, match to top-3 room types and revenue centers
