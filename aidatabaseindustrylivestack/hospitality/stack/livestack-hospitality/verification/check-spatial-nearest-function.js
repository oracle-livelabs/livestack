...
async function findSample() {
  const result = await db.executeAsUser(`
    SELECT c.guest_id, i.product_id
    FROM guests c
    JOIN inventory i
  ...
  assert(result.rows.length > 0, 'No guest/room-type sample with available service inventory found');
  return result.rows[0];
}

async function fetchExpectedRows(guestId, productId, maxResults) {
  const result = await db.executeAsUser(`
    WITH candidates AS (
      SELECT fc.center_id,
             fc.center_name,
  ...
  `, { guestId, productId, maxResults }, VERIFY_USER);

  return result.rows;
}

async function fetchFunctionRows(guestId, productId, maxResults) {
  const result = await db.executeCursorAsUser(`
    BEGIN
      :nearestCenters := find_nearest_centers(:guestId, :productId, :maxResults);
    END;
  `, {
    ...
    productId,
    maxResults
  }, 'nearestCenters', VERIFY_USER, { maxRows: maxResults });
...
async function main() {
  ...
  const guestId = sample.GUEST_ID;
  const productId = sample.PRODUCT_ID;

  const expectedRows = await fetchExpectedRows(guestId, productId, maxResults);
  const functionRows = await fetchFunctionRows(guestId, productId, maxResults);

  assertShape(functionRows);
  assertMatchesExpected(functionRows, expectedRows);

  console.log(
    `Spatial nearest function check passed for guest ${guestId}, room type ${productId}: ${functionRows.map((row) => row.CENTER_ID).join(', ')}.`
  );
}
