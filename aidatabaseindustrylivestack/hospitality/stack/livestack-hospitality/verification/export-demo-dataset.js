...
async function exportTable(rootDir, table) {
  const selectList = table.columns.map((column) => `  ${sqlExpressionForColumn(column)}`).join(',\n');
  const sql = `SELECT\n${selectList}\nFROM ${table.name}\nORDER BY ${table.pk}`;
  // The canonical bundle must be exported through an authorized VPD session.
  // A raw schema-owner connection is fail-closed and silently exports empty
  // protected tables.
  const result = await db.executeAsUser(sql, {}, 'admin_ava');
...
async function main() {
  ...
  try {
    ...
    for (const table of TABLES) {
      counts[table.name] = await exportTable(outputRoot, table);
