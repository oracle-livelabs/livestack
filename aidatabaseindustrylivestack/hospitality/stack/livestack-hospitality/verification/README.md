...
- **Fixture layout**
  - `verification/fixtures/required` mirrors the mandatory tables routed with every bundle (`brands`, `products`, `fulfillment_centers`, `guests`, `influencers`, `social_posts`, `post_product_mentions`, `inventory`, `orders`, `order_items`). Each CSV uses the exact header order emitted by `backend/lib/importCatalog.js`.
...
- **Checklist**
  ...
  - **Foreign keys:** `orders.guest_id`, `order_items.product_id`, `inventory.center_id`, and other reference columns must resolve to source IDs in the uploaded CSVs; broken references are flagged pre-import.
...
- **Derived and regenerated data**
  - Spatial point columns (`fulfillment_centers.location`, `guests.location`) are recalculated from the latitude/longitude pairs immediately after the base load.
