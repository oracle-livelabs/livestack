...
async function findSampleOrder() {
  ...
  assert(result.rows.length > 0, 'No sample order with items, guest location, and active assigned center found');
...
async function fetchRecommendations(orderId) {
  ...
  `, {
    ...
    orderId,
    strategy: 'balanced'
  }, 'recommendations', VERIFY_USER, { maxRows: 10 });
