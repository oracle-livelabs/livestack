/*
 * load_orders.sql
 * 3000 guest orders with line items, varied statuses, and signal attribution
 */
...
DECLARE
    ...
    v_statuses t_str := t_str('pending','confirmed','processing','routed','completed','completed','completed','cancelled');
BEGIN
    SELECT MIN(guest_id), MAX(guest_id) INTO v_min_cust, v_max_cust FROM guests;
    ...
    FOR i IN 1..3000 LOOP
        ...
        -- 30% of orders are signal-driven by guest, channel, demand, or operations signals
        ...
        -- Assign property hotel for non-pending service records
        ...
        INSERT INTO orders (
            guest_id, order_status, order_total, shipping_cost,
            fulfillment_center_id, social_source_id, demand_score,
            created_at
        ) VALUES (
            v_cust_id, v_status, 0,
            CASE
                WHEN DBMS_RANDOM.VALUE < 0.3 THEN 0          -- waived service fee
        ...
        -- Create shipment for routed/completed orders
        IF v_status IN ('routed', 'completed') AND v_center_id IS NOT NULL THEN
            ...
            BEGIN
                SELECT ROUND(SDO_GEOM.SDO_DISTANCE(
                  ...
                  INTO v_distance
                  FROM guests c, fulfillment_centers fc
                 WHERE c.guest_id = v_cust_id
            ...
            INSERT INTO shipments (
                ...
                CASE MOD(i, 4)
                    WHEN 0 THEN 'RoomReady'
                    WHEN 1 THEN 'CrewLink'
                    WHEN 2 THEN 'MaintTrack'
                    ELSE 'guestRoute'
                END,
                'HSP' || LPAD(v_order_id, 12, '0'),
                CASE v_status WHEN 'completed' THEN 'completed' ELSE 'in_transit' END,
                ...
                CASE v_status
                    WHEN 'completed' THEN SYSTIMESTAMP - NUMTODSINTERVAL(DBMS_RANDOM.VALUE(0, 2) * 24, 'HOUR')
                    ELSE NULL
                END
            );
        END IF;
