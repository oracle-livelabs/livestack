/*
 * 03_graph.sql
 * Property Graph for signal-source networks and property propagation
...
SELECT v2.display_name, v2.influence_score, v2.follower_count
FROM GRAPH_TABLE ( influencer_network
    MATCH (v1 IS influencer) -[e IS connects_to]-> (v2 IS influencer)
    WHERE v1.display_name = 'Brand Standards Intelligence Stream - Operations Control 01'
    COLUMNS (
        v2.display_name,
        v2.influence_score,
        v2.follower_count
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE a.display_name = 'Brand QA Monitoring Feed 01'
      AND b.display_name = 'Guest Exposure Engine - Property 01'
    COLUMNS (
        a.display_name AS source,
        b.display_name AS target,
        LISTAGG(e.connection_type, ' → ') AS path_types
    )
);

-- Property propagation: which signal sources amplify a property through the network
SELECT v1.display_name AS promoter,
       v2.display_name AS reached,
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE b.brand_name = 'Grand Harbor Hotel'
    COLUMNS (
