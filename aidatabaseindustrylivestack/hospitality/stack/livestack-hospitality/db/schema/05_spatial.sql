...
-- Property hotels: location geometry
ALTER TABLE fulfillment_centers ADD (
    location SDO_GEOMETRY
);

-- Guest LOCATION column is defined on the guests table in 01_tables.sql
...
BEGIN
    ...
    IF v_idx_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE INDEX idx_cust_spatial ON guests(location) INDEXTYPE IS MDSYS.SPATIAL_INDEX_V2';
    END IF;
END;
/
...
CREATE TABLE demand_regions (
    ...
    social_density    NUMBER(8,2),      -- brand standards/demand signals per 1000 population
...
-- Find the N nearest property hotels to a guest
-- that have the requested hospitality product available
-- ============================================================
        ...
        SELECT center_id,
               ...
               CASE
                   ...
                   WHEN coverage_status = 'full' THEN
                       'Full service coverage with ' || ROUND(distance_km, 1) || ' km guest distance.'
