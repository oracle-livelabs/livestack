...
echo ">>> Higher Education Student Success bootstrap starting inside db container..."
...
# VECTOR_EMBEDDING-dependent functions in 04_vector.sql must compile only
# after the model is registered and has passed a dimension probe.
...
END;
/
SELECT VECTOR_DIMENSION_COUNT(VECTOR_EMBEDDING(ALL_MINILM_L12_V2 USING 'Higher Education bootstrap model probe' AS DATA)) FROM dual;
EXIT SUCCESS
SQL
...
@${APP_DIR}/db/schema/10_highered_views.sql
