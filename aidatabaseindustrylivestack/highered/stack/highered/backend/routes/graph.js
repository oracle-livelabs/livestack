/**
 * Graph API — support network queries using Oracle Property Graph / SQL/PGQ
 */
...
// GET /api/graph/influencers — top support advocates with optional handle/name search
...
router.get('/network/:id', async (req, res) => {
  try {
    ...
    if (!centerRes.rows.length) return res.status(404).json({ error: 'Success advocate not found' });
...
// GET /api/graph/propagation/:brandSlug — academic program propagation network
...
const EXAMPLE_QUERIES = {
  influence_reach: {
    name: 'Support Reach (N-Hop Traversal)',
    description: 'Find all success advocates reachable within N hops from a starting advocate using SQL/PGQ GRAPH_TABLE pattern matching.',
    params: [
      { key: 'handle', label: 'Starting Advocate', default: '@veteransuccess_sf' },
      { key: 'hops',   label: 'Max Hops (1-3)',  default: 2, type: 'number' },
    ],
    buildSql: (p) => ({
      sql: `SELECT handle AS advocate_handle,
       influence_score AS support_score,
       follower_count AS audience_size,
       platform,
       niche AS support_focus
...
FETCH FIRST 25 ROWS ONLY`,
      binds: { handle: p.handle || '@veteransuccess_sf' },
      display: `-- SQL/PGQ: Find support advocates within ${p.hops || 2} hops
SELECT handle AS advocate_handle,
       influence_score AS support_score,
       follower_count AS audience_size,
       platform, niche AS support_focus
FROM GRAPH_TABLE ( influencer_network
    MATCH (v1 IS influencer)
          -[e IS connects_to]->{1,${p.hops || 2}}
          (v2 IS influencer)
    WHERE v1.handle = '${p.handle || '@veteransuccess_sf'}'
    COLUMNS (
...
FETCH FIRST 25 ROWS ONLY;`,
  ...
  mutual_connections: {
    name: 'Shared Support Connections (Triangle Pattern)',
    description: 'Find success advocates who connect two people or programs through shared support relationships. Uses SQL/PGQ multi-edge MATCH to find shared network nodes.',
    params: [
      { key: 'from_handle', label: 'From Advocate', default: '@innovationonline_bos' },
      { key: 'to_handle',   label: 'To Advocate',   default: '@commuterconnect_chi' },
    ],
    buildSql: (p) => ({
      sql: `SELECT mutual_handle AS shared_advocate,
       mutual_platform AS shared_channel,
       mutual_followers AS shared_audience,
       mutual_score AS support_score,
       e1_type, e2_type,
...
FETCH FIRST 20 ROWS ONLY`,
      binds: { from_handle: p.from_handle || '@innovationonline_bos', to_handle: p.to_handle || '@commuterconnect_chi' },
      display: `-- SQL/PGQ: Triangle pattern — shared support connections
SELECT mutual_handle AS shared_advocate,
       mutual_platform AS shared_channel,
       mutual_followers AS shared_audience,
       mutual_score AS support_score,
       e1_type, e2_type,
...
FROM GRAPH_TABLE ( influencer_network
    MATCH (a IS influencer)
          ...
          <-[e2 IS connects_to]-
          (b IS influencer)
    WHERE a.handle = '${p.from_handle || '@innovationonline_bos'}'
      AND b.handle = '${p.to_handle || '@commuterconnect_chi'}'
    COLUMNS (
        m.handle AS mutual_handle,
        m.platform AS mutual_platform,
        m.follower_count AS mutual_followers,
...
FETCH FIRST 20 ROWS ONLY;`,
  ...
  brand_propagation: {
    name: 'Program Signal Propagation Network',
    description: 'Trace how an academic program or advancement signal spreads through the success advocate network. Uses multi-edge pattern matching across program and advocate relationships.',
    params: [
      { key: 'brand_name', label: 'Academic Program Name', default: 'Northstar Advising Network' },
    ],
    buildSql: (p) => ({
      sql: `SELECT promoter AS source_advocate,
       reached AS reached_advocate,
       relationship_type,
...
FETCH FIRST 30 ROWS ONLY`,
      binds: { brand_name: p.brand_name || 'Northstar Advising Network' },
      display: `-- SQL/PGQ: Program signal propagation through network
SELECT promoter AS source_advocate,
       reached AS reached_advocate,
       relationship_type,
       connection_type, strength
FROM GRAPH_TABLE ( influencer_network
    MATCH (b IS brand)
          ...
          -[e2 IS connects_to]->
          (v2 IS influencer)
    WHERE b.brand_name = '${p.brand_name || 'Northstar Advising Network'}'
    COLUMNS (
        v1.handle AS promoter,
        v2.handle AS reached,
        e1.relationship_type,
...
FETCH FIRST 30 ROWS ONLY;`,
  ...
  cross_platform: {
    name: 'Cross-Channel Bridge Advocates',
    description: 'Identify success advocates who bridge different engagement channels and can coordinate outreach across student communities. Uses GRAPH_TABLE edge traversal with cross-platform filtering.',
    params: [
      { key: 'min_platforms', label: 'Min Channels Connected', default: 2, type: 'number' },
    ],
    buildSql: (p) => ({
      sql: `SELECT src_handle AS advocate_handle,
       src_platform AS primary_channel,
       src_score AS support_score,
       src_followers AS audience_size,
...
ORDER BY channels_reached DESC, support_score DESC
FETCH FIRST 20 ROWS ONLY`,
      binds: { min_platforms: parseInt(p.min_platforms) || 2 },
      display: `-- SQL/PGQ: Cross-channel bridge detection
SELECT src_handle AS advocate_handle,
       src_platform AS primary_channel,
       src_score AS support_score,
       src_followers AS audience_size,
...
FETCH FIRST 20 ROWS ONLY;`,
  ...
  community_hubs: {
    ...
    description: 'Find the most connected success advocates by graph degree — community hubs that maximize student support reach. Uses GRAPH_TABLE edge traversal with aggregation for degree centrality.',
    params: [
...
ORDER BY degree DESC, support_score DESC
FETCH FIRST 20 ROWS ONLY`,
