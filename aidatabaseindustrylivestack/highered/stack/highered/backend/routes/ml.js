 ...
 *   DEMAND_SURGE_MODEL      — Random Forest Classification (student-service demand surge detection)
 *   CUSTOMER_SEGMENT_MODEL  — K-Means Clustering (student engagement segmentation)
 *   REVENUE_PREDICT_MODEL   — GLM Regression (student request value prediction)
 *   PRODUCT_CLUSTER_MODEL   — K-Means Clustering (student-service grouping by behavior)
 *
...
const SOCIAL_DATA_ANCHOR_SQL = '(SELECT MAX(CAST(POSTED_AT AS DATE)) FROM SOCIAL_POSTS)';
const ORDER_DATA_ANCHOR_SQL = '(SELECT MAX(CAST(CREATED_AT AS DATE)) FROM ORDERS)';
const FORECAST_DATA_ANCHOR_SQL = '(SELECT MIN(FORECAST_DATE) FROM DEMAND_FORECASTS)';

function buildDemandProductFeaturesCte(lookbackHoursExpr = null) {
  const socialWhere = lookbackHoursExpr
    ? `WHERE CAST(sp.POSTED_AT AS DATE) >= ${SOCIAL_DATA_ANCHOR_SQL} - ${lookbackHoursExpr} / 24`
    : '';
  const salesWhere = lookbackHoursExpr
    ? `WHERE CAST(o.CREATED_AT AS DATE) >= ${ORDER_DATA_ANCHOR_SQL} - ${lookbackHoursExpr} / 24`
    : '';
...
async function fallbackCustomerSegments({ limit }) {
  const result = await db.execute(`
    WITH customer_metrics AS (
      ...
      LEFT JOIN (
        SELECT o.CUSTOMER_ID,
               ROUND(${ORDER_DATA_ANCHOR_SQL} - CAST(MAX(o.CREATED_AT) AS DATE)) AS RECENCY_DAYS,
    ...
    segmented AS (
      SELECT s.*,
             ...
             CASE
               WHEN s.RECENCY_SCORE = 4 AND s.FREQUENCY_SCORE >= 3 AND s.MONETARY_SCORE >= 3 THEN 'Retention Champion'
               WHEN s.RECENCY_SCORE >= 3 AND s.FREQUENCY_SCORE >= 3 THEN 'Persisting'
               WHEN s.RECENCY_SCORE = 4 AND s.FREQUENCY_SCORE <= 2 THEN 'Newly Engaged'
               WHEN s.RECENCY_SCORE <= 2 AND s.MONETARY_SCORE = 4 THEN 'At Risk'
               WHEN s.RECENCY_SCORE = 1 AND s.FREQUENCY_SCORE <= 2 THEN 'Disconnected'
               WHEN s.MONETARY_SCORE = 4 AND s.RECENCY_SCORE >= 2 THEN 'High Support Value'
               WHEN s.RECENCY_SCORE >= 3 AND s.MONETARY_SCORE <= 2 THEN 'Momentum Building'
               ELSE 'Needs Nudge'
             END AS SEGMENT,
             CASE
  ...
  return {
    ...
    meta: {
      model: 'Student engagement segmentation heuristic (fallback)',
...
async function fallbackRevenueForecast({ lookbackDays, forecastDays }) {
  const histResult = await db.execute(`
    WITH daily_rev AS (
      ...
      FROM ORDERS
	      WHERE CAST(CREATED_AT AS DATE) >= ${ORDER_DATA_ANCHOR_SQL} - :lookback
  ...
  const [surgeCount, customers, modelCount, regr] = await Promise.all([
    ...
    db.execute(`
      ...
      FROM (
        ...
        FROM ORDERS
	        WHERE CAST(CREATED_AT AS DATE) >= ${ORDER_DATA_ANCHOR_SQL} - 30
...
async function fallbackInventoryIntelligence() {
      ...
      FROM (
        SELECT df.*,
               ROW_NUMBER() OVER (
                 ...
                 ORDER BY
	                   ABS(df.FORECAST_DATE - ${FORECAST_DATA_ANCHOR_SQL}),
  ...
  const critical = alerts.filter(a => a.STOCK_STATUS === 'CRITICAL' || a.STOCK_STATUS === 'NO_CAPACITY').length;
  ...
  alerts.forEach(a => {
    ...
    if (a.STOCK_STATUS === 'CRITICAL' || a.STOCK_STATUS === 'NO_CAPACITY') centerMap[a.CENTER_NAME].critical++;
...
router.get('/demand-forecast', async (req, res) => {
  try {
    ...
    const result = await db.execute(`
      WITH product_features AS (
        ...
        LEFT JOIN (
            ...
            FROM POST_PRODUCT_MENTIONS ppm
            JOIN SOCIAL_POSTS sp ON ppm.POST_ID = sp.POST_ID
	            WHERE CAST(sp.POSTED_AT AS DATE) >= ${SOCIAL_DATA_ANCHOR_SQL} - :lookback / 24
            GROUP BY ppm.PRODUCT_ID
        ) eng ON p.PRODUCT_ID = eng.PRODUCT_ID
        LEFT JOIN (
            SELECT oi.PRODUCT_ID,
            ...
            FROM ORDER_ITEMS oi
            JOIN ORDERS o ON o.ORDER_ID = oi.ORDER_ID
	            WHERE CAST(o.CREATED_AT AS DATE) >= ${ORDER_DATA_ANCHOR_SQL} - :lookback / 24
            GROUP BY oi.PRODUCT_ID
        ) sales ON p.PRODUCT_ID = sales.PRODUCT_ID
        WHERE p.IS_ACTIVE = 1
      )
    ...
    res.json({
      ...
      meta: {
        lookback_hours: lookbackHours,
        model: 'DEMAND_SURGE_MODEL (Random Forest)',
  ...
  try {
      ...
      SELECT
        ...
        CASE
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) = 4
           ...
           AND NTILE(4) OVER (ORDER BY cm.monetary DESC) >= 3 THEN 'Retention Champion'
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) >= 3
           AND NTILE(4) OVER (ORDER BY cm.frequency DESC) >= 3 THEN 'Persisting'
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) = 4
           AND NTILE(4) OVER (ORDER BY cm.frequency DESC) <= 2 THEN 'Newly Engaged'
          ...
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) = 1
           AND NTILE(4) OVER (ORDER BY cm.frequency DESC) <= 2 THEN 'Disconnected'
          WHEN NTILE(4) OVER (ORDER BY cm.monetary DESC) = 4
           AND NTILE(4) OVER (ORDER BY cm.recency_days ASC) >= 2 THEN 'High Support Value'
          WHEN NTILE(4) OVER (ORDER BY cm.recency_days ASC) >= 3
           AND NTILE(4) OVER (ORDER BY cm.monetary DESC) <= 2 THEN 'Momentum Building'
          ELSE 'Needs Nudge'
        END AS segment,
    ...
    res.json({
      ...
      meta: {
        ...
        clusters: 4,
        segment_labels: segmentSummary.length,
      },
    });
  } catch (err) {
    const limit = Math.min(parseInt(req.query.limit) || 200, 1000);
  ...
  try {
    ...
    const histResult = await db.execute(`
      ...
      glm_scored AS (
        SELECT
          target_revenue,
          PREDICTION(REVENUE_PREDICT_MODEL USING
            ...
            social_signal AS social_signal,
            fulfillment_center_id AS fulfillment_center_id,
            request_age_days AS request_age_days
      ...
      glm_stats AS (
        SELECT
          ROUND(AVG(predicted_revenue), 2) AS avg_glm_predicted,
          ROUND(CORR(target_revenue, predicted_revenue), 4) AS glm_correlation
        FROM glm_scored
      )
      SELECT
        TO_CHAR(d.day_bucket, 'YYYY-MM-DD')                     AS day,
    ...
    const [surgeCount, customers, modelCount, regr] = await Promise.all([
      ...
      db.execute(`
        SELECT COUNT(*) AS cnt FROM (
          ...
          LEFT JOIN (
              ...
              FROM POST_PRODUCT_MENTIONS ppm
              JOIN SOCIAL_POSTS sp ON ppm.POST_ID = sp.POST_ID
	              WHERE CAST(sp.POSTED_AT AS DATE) >= ${SOCIAL_DATA_ANCHOR_SQL} - 30
              GROUP BY ppm.PRODUCT_ID
          ) eng ON p.PRODUCT_ID = eng.PRODUCT_ID
          LEFT JOIN (
      ...
      db.execute(`SELECT /*+ NO_PARALLEL */
        ...
        FROM (
          SELECT SUM(order_total) AS day_rev,
                 ROW_NUMBER() OVER (ORDER BY TRUNC(CAST(created_at AS DATE))) AS rn
	          FROM orders WHERE CAST(created_at AS DATE) >= ${ORDER_DATA_ANCHOR_SQL} - 30
          GROUP BY TRUNC(CAST(created_at AS DATE))
        )`),
    ]);
...
// (stored in demand_forecasts) with live service capacity levels to identify
// student services at risk of exceeding capacity due to signal-driven demand surges.
// ─────────────────────────────────────────────────────────────────────────────
