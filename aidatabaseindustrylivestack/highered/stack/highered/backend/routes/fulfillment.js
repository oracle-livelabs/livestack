  ...
  try {
	    const result = await db.executeAsUser(`
	      WITH forecast_anchor AS (
	        SELECT MIN(forecast_date) AS anchor_date
      ...
      JOIN brands b ON p.brand_id = b.brand_id
	      ...
	      LEFT JOIN demand_forecasts df ON p.product_id = df.product_id
	          AND df.forecast_date = (SELECT anchor_date FROM forecast_anchor)
	      ...
	      SELECT r.region_id,
             ...
             (SELECT ROUND(AVG(df.predicted_demand), 0)
	              ...
	              WHERE UPPER(df.region) = UPPER(r.region_name)
	                AND df.forecast_date BETWEEN (SELECT anchor_date FROM forecast_anchor)
	                                         AND (SELECT anchor_date FROM forecast_anchor) + 7
