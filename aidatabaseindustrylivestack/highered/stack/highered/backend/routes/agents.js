...
const STATIC_TEAMS = [
  {
    TEAM_NAME: 'STUDENT_SIGNAL_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed student/community signal analysis over live higher education coordination data.',
  },
  {
    TEAM_NAME: 'CAMPUS_SERVICE_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed campus services analysis using capacity and routing context.',
  },
  {
    TEAM_NAME: 'STUDENT_SUCCESS_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed student-success operations analysis using student requests and service-value context.',
  },
];

// Keep legacy SQL-routing keys private to the orchestration layer.  The UI and
// API contract speak only in the Higher Education story vocabulary.
const PUBLIC_TEAM_NAMES = {
  SOCIAL_TREND_TEAM: 'STUDENT_SIGNAL_TEAM',
  FULFILLMENT_TEAM: 'CAMPUS_SERVICE_TEAM',
  COMMERCE_TEAM: 'STUDENT_SUCCESS_TEAM',
};

function publicTeamName(team) {
  return PUBLIC_TEAM_NAMES[team] || team;
}

function publicToolName(tool, intent = '') {
  const publicSqlTool = intent === 'fulfillment'
    ? 'CAMPUS_SERVICE_SQL_TOOL'
    : 'STUDENT_SUCCESS_SQL_TOOL';
  return String(tool || '')
    .replace(/COMMERCE_SQL_TOOL/g, publicSqlTool)
    .replace(/detect_trending_products\(\)/g, 'STUDENT_SIGNAL_SQL_TOOL');
}
...
async function buildAgentContext(teamName) {
  if (teamName === 'SOCIAL_TREND_TEAM') {
    ...
    return {
      instructions: 'Focus on student services, success advocates, urgent signals, and concrete metrics.',
      context: {
        team: teamName,
  ...
  if (teamName === 'FULFILLMENT_TEAM') {
    ...
    return {
      instructions: 'Focus on capacity risk, routing, and practical campus-service actions.',
      context: {
        team: teamName,
  ...
  return {
    instructions: 'Focus on student requests, service value, student-signal attribution, and operational trends.',
    context: {
      team: teamName,
...
function fallbackAgentSummary(teamName, context) {
  if (teamName === 'SOCIAL_TREND_TEAM') {
    ...
    if (!products.length) {
      return context.trend_summary || 'No high-demand student services found in the current window.';
    }
    return products
      .slice(0, 3)
      .map((product) => {
  ...
  if (teamName === 'FULFILLMENT_TEAM') {
    ...
    if (!alerts.length) {
      return 'No current low-capacity student service alerts were found.';
    }
    return alerts
      .slice(0, 3)
      .map((item) =>
        `${item.PRODUCT_NAME} at ${item.CENTER_NAME}, ${item.CITY}: ${item.QUANTITY_ON_HAND} available slots vs capacity threshold ${item.REORDER_POINT} [${item.STOCK_STATUS}]`
      )
      .join(' | ');
  }
  ...
  return `Last 30 days: ${totalOrders.toLocaleString()} student requests, $${totalRevenue.toLocaleString()} resource impact, ${socialOrders.toLocaleString()} student-signal-driven student requests, $${socialRevenue.toLocaleString()} student-signal service value.`;
}
...
// Runs the SOCIAL_TREND_TEAM to identify urgent student service demand.
...
router.post('/detect-trends', async (req, res) => {
  ...
  try {
    ...
    const trendText = trendResult.rows[0]?.RESULT || 'No high-demand student services found';

    // 2. Get top trending student services for per-service action logging
    ...
    try {
      agentAnalysis = await Promise.race([
        askAgent('SOCIAL_TREND_TEAM',
          `Identify the top trending student services and higher education advocates from the last ${hours} hours ` +
          `using the detect trending student services tool with minimum urgency score ${threshold}`),
    ...
    for (const p of products) {
      ...
      await logAction('trend_detection_agent', 'detect_trends', 'product', p.PRODUCT_ID, {
        ...
        window_hours:  hours,
        reason: `${p.PEAK_MOMENTUM} student service with ${p.MENTION_COUNT} student/community signal mentions and urgency ${p.AVG_VIRALITY}`,
      }, confidence);
      loggedActions.push({ student_: p.PRODUCT_NAME, urgency: p.AVG_VIRALITY });
    }
    ...
    await logAction('trend_detection_agent', 'trend_analysis_complete', 'social_posts', null, {
      ...
      student_service_found:  products.length,
    ...
    res.json({
      message:      `Trend detection complete - ${products.length} urgent student services identified in last ${hours}h`,
      trending:     trendText,
...
// Full orchestration: trend detection → inventory check → student-success operations attribution.
  ...
  try {
    ...
    try {
      trendAnalysis = await Promise.race([
        askAgent('SOCIAL_TREND_TEAM',
          'What student services are trending right now based on student and community signal activity in the last 48 hours'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
    ...
    for (const p of topProducts) {
      ...
      allActions.push({ phase: 'trends', student_: p.PRODUCT_NAME });
    }

    await logEvent('trend_detected', 'master_orchestrator', {
      phase: 'trend_detection', student_service_found: topProducts.length,
    });
    ...
    try {
      fulfillmentAnalysis = await Promise.race([
        askAgent('FULFILLMENT_TEAM',
          'Which trending student services have critically low capacity and need immediate intervention'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
    ...
    for (const inv of criticalInventory) {
      await logAction('inventory_agent', 'inventory_alert', 'inventory', inv.PRODUCT_ID, {
        ...
        strategy:           `Pre-position capacity at ${inv.CENTER_NAME} - trending student service with ${inv.STOCK_STATUS} capacity`,
        reason: `${inv.STOCK_STATUS} capacity (${inv.QUANTITY_ON_HAND} available units) for trending student service at ${inv.CENTER_NAME}`,
      }, inv.STOCK_STATUS === 'no_capacity' ? 0.98 : 0.92);
    ...
    // ── PHASE 3: Student Success Operations Attribution ────────────────────────────────────────
    ...
    try {
      commerceAnalysis = await Promise.race([
        askAgent('COMMERCE_TEAM',
          'Summarize student-signal-driven student requests and resource impact from the last 7 days'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
      logOptionalAgentWarning('Student-success operations agent skipped', e);
    }
    ...
    await logAction('master_orchestrator', 'commerce_attribution', 'orders', null, {
      ...
      reason: `${socialPct}% of student requests ($${(commerce.SOCIAL_REVENUE || 0).toLocaleString()}) attributed to student and community signals in last 7 days`,
    }, 0.93);
    ...
    res.json({
      message: `Full cycle complete - ${topProducts.length} trends · ${criticalInventory.length} capacity alerts · ${socialPct}% student-signal-driven student requests`,
      phases: {
        trends: {
          student_service_found: topProducts.length,
        ...
        commerce: {
          total_student_: commerce.TOTAL_ORDERS,
          signal_student_: commerce.SOCIAL_ORDERS,
          signal_service_value: commerce.SOCIAL_REVENUE,
          social_pct:     `${socialPct}%`,
...
router.post('/trends', async (req, res) => {
  try {
    ...
    const q = question || 'What student services are trending right now based on student and community signal activity';
...
router.post('/fulfillment', async (req, res) => {
  try {
    const { question } = req.body;
    const q = question || 'Which trending student services have critically low capacity';
...
router.post('/commerce', async (req, res) => {
  try {
    const { question } = req.body;
    const q = question || 'How many student requests were opened in the last 24 hours and what is the total resource impact';
    const response = await askAgent('COMMERCE_TEAM', q);
    res.json({ team: 'COMMERCE_TEAM', question: q, response });
  } catch (err) {
    console.error('Student-success operations agent error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/chat', async (req, res) => {
  ...
  const inventoryStrong = ['capacity', 'campus services', 'inventory', 'reorder', 'replenish', 'out of capacity'];
  const commerceStrong = ['service value', 'student request', 'request total', 'order total'];

  // Weak signals — worth 1 point each (ambiguous, could relate to multiple intents)
  const trendWeak = ['trend', 'signal', 'student signal', 'post', 'engagement', 'views', 'likes', 'shares', 'sentiment'];
  const inventoryWeak = ['stock', 'route', 'routing', 'center', 'supply', 'logistics', 'completion', 'nearest', 'distance'];
  const commerceWeak = ['request', 'student', 'value', 'category', 'program', 'service', 'total'];
  ...
  try {
    if (intent === 'trends') {
      ...
      fallbackResult = trendRes.rows[0]?.RESULT || 'No high-demand student services found';
      ...
      // Check for higher education advocate-specific questions
      ...
      const productPatterns = [
        /["']([^"']+)["']/,                                                           // quoted: "AI Tutoring Session"
        /(?:inventory|stock|check)\s+(?:for|of|on)\s+(?:the\s+)?(.+?)(?:\s+across|\s+at|\s+in|\s*\??\s*$)/i,
        /(?:ship|deliver|send|route)\s+(?:the\s+)?(.+?)(?:\s+to\s+|\s+for\s+)/i,     // "ship same-day campus center slot to..."
      ...
      if (productName && (cityMatch || emailMatch)) {
        // Spatial routing - find best campus services center
        let customerEmail = emailMatch ? emailMatch[0] : null;

        // If user gave a city name, look up a synthetic student email in that city
        ...
        if (customerEmail) {
          try {
            ...
            fallbackResult = routeRes.rows[0]?.RESULT || 'No service route found';
            toolsUsed.push({ tool: 'find_best_fulfillment()', params: { synthetic_student: customerEmail, student_: productName }, status: 'success' });
      ...
      fallbackResult = `Last 30 days: ${(c.TOTAL_ORDERS || 0).toLocaleString()} student support requests, $${(c.TOTAL_REVENUE || 0).toLocaleString()} resource impact. ` +
        `${socialPct}% signal-driven ($${(c.SOCIAL_REVENUE || 0).toLocaleString()}). ` +
        `Avg request: $${c.AVG_ORDER_VALUE || 0}. ${(c.UNIQUE_STUDENTS || 0).toLocaleString()} unique students.`;
      fallbackData = [c];
  ...
  const toolHistory = toolsUsed.slice(0, 5).map((entry) => ({
    TOOL_NAME: publicToolName(entry.tool, intent),
  ...
  res.json({
    ...
    data: fallbackData,
    toolsUsed: toolsUsed.map((entry) => ({
      ...entry,
      team: entry.team ? publicTeamName(entry.team) : entry.team,
      tool: publicToolName(entry.tool, intent),
    })),
    toolHistory,
    elapsed,
  });
