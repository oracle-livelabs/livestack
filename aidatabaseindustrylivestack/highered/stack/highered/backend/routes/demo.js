...
router.get('/start', async (req, res) => {
  ...
  try {
    ...
    // ── 2. BRANDS (5-10%) ─────────────────────────────────────────────────
    if (aborted) return;
    send({ step: 'brands', status: 'running', message: 'Loading 12 academic programs...', progress: 5 });

    if (counts.brands > 0) {
      send({ step: 'brands', status: 'skipped', message: `${counts.brands} academic programs already loaded`, progress: 10, count: counts.brands });
    } else {
      // Inline academic program inserts — same data as load_all_data.sql
      const brandInserts = [
        "('Northstar Advising Network','northstar','Academic Advising','New York',40.7128,-74.006,2012,325000000,'premium')",
        "('Riverbend Enrollment Services','riverbend','Enrollment Management','Chicago',41.8781,-87.6298,2008,210000000,'premium')",
        "('Summit Career Pathways','summit','Career Readiness','Dallas',32.7767,-96.797,2015,185000000,'premium')",
        "('ClearPath Wellness Center','clearpath','Student Wellness','Seattle',47.6062,-122.3321,2019,76000000,'standard')",
        "('Pioneer Academic Support','pioneer','Learning Support','Denver',39.7392,-104.9903,2016,98000000,'standard')",
        "('Coastal Access Office','coastal','Student Services','Atlanta',33.749,-84.388,2018,124000000,'standard')",
        "('Lakeside Financial Aid','lakeside','Financial Aid','Phoenix',33.4484,-112.074,2014,260000000,'premium')",
        "('Horizon First-Year Experience','horizon','First-Year Experience','Boston',42.3601,-71.0589,2020,54000000,'emerging')",
        "('Liberty Transfer Center','liberty','Transfer Success','Miami',25.7617,-80.1918,2011,148000000,'standard')",
        "('Innovation Online Campus','innovation','Online Learning','San Francisco',37.7749,-122.4194,2017,132000000,'premium')",
        "('Desert Workforce Institute','desert','Workforce Programs','Houston',29.7604,-95.3698,2013,175000000,'premium')",
        "('Blue Ridge Graduate College','blueridge','Graduate Studies','Nashville',36.1627,-86.7816,2018,84000000,'standard')",
      ];
      ...
      send({ step: 'brands', status: 'done', message: `${brandCount} academic programs loaded`, progress: 10, count: brandCount });
    }
    ...
    try {
      ...
      if (zoneCount > 0) {
        ...
        const zoneTiers = [
          { type: 'express',   appeals: 80000,  hrs: 8  },
          { type: 'overnight', appeals: 160000, hrs: 16 },
          { type: 'standard',  appeals: 250000, hrs: 24 },
          { type: 'economy',   appeals: 500000, hrs: 72 },
        ];
        ...
        for (const tier of zoneTiers) {
          ...
          `, { zoneType: tier.type, maxHrs: tier.hrs, appeals: tier.appeals });
