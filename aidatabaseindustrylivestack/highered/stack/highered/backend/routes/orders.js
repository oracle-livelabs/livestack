...
// GET /api/orders/:id/duality - same student request from JSON Duality View
router.get('/:id/duality', async (req, res) => {
  try {
    ...
    res.json({
      source: 'STUDENT_REQUESTS_DV',
      viewDefinition: 'CREATE JSON RELATIONAL DUALITY VIEW student_requests_dv AS SELECT JSON {...} FROM student_service_requests_v r WITH UPDATE',
      sql: `SELECT DATA FROM student_requests_dv WHERE JSON_VALUE(DATA, '$._id' RETURNING NUMBER) = ${orderId}`,
      document: doc
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
