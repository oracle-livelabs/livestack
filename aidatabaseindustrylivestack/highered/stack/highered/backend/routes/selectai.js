...
function logTrace(trace, status, extra = {}) {
  console.info(JSON.stringify({
    event: 'ask_seer_highered_data_request',
...
router.get('/schema-objects', async (_req, res) => {
  const objects = getHigherEdSchemaObjectMetadata();
  res.json({
    objects,
    domains: groupHigherEdSchemaObjectMetadata(objects),
    meta: {
...
router.get('/health', async (req, res) => {
  ...
  const result = await checkAskHigherEdDataHealth({ demoUser: req.demoUser || 'admin_jess', profile });
