...
// Declaration evidence only. The app has no licensed/population-safe proof
// for V$IM_SEGMENTS, so it never estimates compression or claims population.
async function sendInMemoryDeclarationEvidence(req, res) {
  try {
    ...
    `, {}, req.demoUser);
    return res.status(result.rows.length ? 200 : 503).json({
      ready: result.rows.length > 0,
      evidenceMode: 'DECLARATION_ONLY',
      populationStatus: 'NOT_PROVEN',
      runtimePopulationClaimed: false,
      declaredSegmentCount: result.rows.length,
      segments: result.rows,
      message: result.rows.length ? 'Declaration only; runtime population not claimed.' : 'In-Memory declarations are unavailable.',
    });
  } catch (err) {
    console.error('In-Memory declaration evidence error:', err);
    return res.status(503).json({ ready: false, evidenceMode: 'DECLARATION_ONLY', populationStatus: 'NOT_PROVEN', runtimePopulationClaimed: false });
  }
}
