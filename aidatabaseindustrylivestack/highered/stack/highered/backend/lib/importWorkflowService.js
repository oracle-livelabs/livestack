...
function buildTemplateReadme() {
  return [
    '# Higher Education Student Success Import Template',
    '',
...
function cloneImportValue(value) {
  ...
  if (Array.isArray(value)) {
    return value.map(cloneImportValue);
  }
  return value;
}

function cloneParsedDataset(parsed) {
  const clonedTables = {};
...
function collectDemoDateValues(dataset, { anchorOnly = false } = {}) {
  const values = [];
  const anchorColumns = new Map([
    ['social_posts', new Set(['posted_at', 'processed_at', 'created_at'])],
    ['orders', new Set(['created_at', 'updated_at'])],
    ['shipments', new Set(['shipped_at', 'delivered_at', 'created_at'])],
    ['inventory', new Set(['last_restock_date', 'updated_at'])],
    ['demand_forecasts', new Set(['created_at'])],
    ['influencer_connections', new Set(['first_seen', 'last_interaction'])],
    ['brand_influencer_links', new Set(['first_mention', 'last_mention'])],
  ]);

  for (const tableData of Object.values(dataset.tables)) {
    const tableAnchorColumns = anchorColumns.get(tableData.table.name);
    for (const column of tableData.table.columns) {
      if (!isTemporalColumn(column)) continue;
      if (anchorOnly && !tableAnchorColumns?.has(column.name)) continue;
...
function rebaseDemoDatasetDates(parsed, targetNow = new Date()) {
  const cloned = cloneParsedDataset(parsed);
  const anchorValues = collectDemoDateValues(cloned.dataset, { anchorOnly: true });
  const fallbackValues = anchorValues.length ? anchorValues : collectDemoDateValues(cloned.dataset);
  const anchorDate = fallbackValues.reduce((latest, value) => (
  ...
  const deltaMs = targetNow.getTime() - anchorDate.getTime();
  const forecastDateValues = cloned.dataset.tables.demand_forecasts?.rows
    .map((row) => row.forecast_date)
    .filter((value) => value instanceof Date && Number.isFinite(value.getTime())) || [];
  const firstForecastDate = forecastDateValues.reduce((earliest, value) => (
    !earliest || value.getTime() < earliest.getTime() ? value : earliest
  ), null);
  const targetForecastDate = new Date(Date.UTC(
    targetNow.getUTCFullYear(),
    targetNow.getUTCMonth(),
    targetNow.getUTCDate()
  ));
  const forecastDateDeltaMs = firstForecastDate
    ? targetForecastDate.getTime() - firstForecastDate.getTime()
    : deltaMs;

  for (const tableData of Object.values(cloned.dataset.tables)) {
    for (const column of tableData.table.columns) {
      if (!isTemporalColumn(column)) continue;
      for (const row of tableData.rows) {
        const value = row[column.name];
        if (value instanceof Date && Number.isFinite(value.getTime())) {
          const columnDeltaMs = tableData.table.name === 'demand_forecasts' && column.name === 'forecast_date'
            ? forecastDateDeltaMs
            : deltaMs;
          row[column.name] = new Date(value.getTime() + columnDeltaMs);
        }
      }
    }
  }

  cloned.dataset.dateRebase = {
    anchorFrom: anchorDate.toISOString(),
    anchorTo: targetNow.toISOString(),
    deltaMs,
    forecastDateDeltaMs,
  };
...
async function rebuildFulfillmentZones(connection) {
  ...
  const tiers = [
    { zoneType: 'express', maxHrs: 8, appeals: 80000 },
    { zoneType: 'overnight', maxHrs: 16, appeals: 160000 },
    { zoneType: 'standard', maxHrs: 24, appeals: 250000 },
    { zoneType: 'economy', maxHrs: 72, appeals: 500000 },
  ];
  ...
  for (const tier of tiers) {
    const result = await execSql(connection, `
      ...
      SELECT center_id, :zoneType, :maxHrs,
             SDO_GEOM.SDO_BUFFER(location, :appeals, 1, 'unit=METER')
...
function buildFallbackShipments(dataset, idMaps) {
  ...
  const carriers = ['Advising', 'Success Team', 'Campus Ops', 'Community Partner'];
  const shipStatusMap = {
    confirmed: 'preparing',
    processing: 'packed',
    shipped: 'in_transit',
...
async function hydrateOmlModels(connection) {
  await execSql(connection, 'BEGIN hydrate_oml_models; END;');
  const result = await execSql(connection, `
    SELECT COUNT(*) AS model_count
    FROM user_mining_models
    WHERE model_name IN (
...
async function executeImportPlan({ dataset, dryRun = false, progress = null }) {
  ...
  try {
    ...
    try {
      if (progress) progress({ status: 'running', progress: 96, message: 'Training OML models...' });
      await execSql(connection, 'SAVEPOINT import_oml_models');
      const modelCount = await hydrateOmlModels(connection);
      summary.generated = {
        ...summary.generated,
        oml_models: modelCount,
      };
      await connection.commit();
    } catch (err) {
      try {
        await execSql(connection, 'ROLLBACK TO import_oml_models');
      } catch (_) {
        try { await connection.rollback(); } catch (_) {}
      }
      warnings.push(`OML model training was skipped after import: ${err.message}`);
    }
...
async function startDatasetJob({ parsed, kind, lockMessage, queuedMessage, startMessage, completeMessage, datasetSource, initiatingActor }) {
  ...
  setImmediate(async () => {
    try {
      ...
      await generationStore.completeGeneration({
        ...admission,
        jobId: job.jobId,
      ...
      try {
        await generationStore.failGeneration({ ...admission, jobId: job.jobId, message: err.message || 'Import failed.' });
      } catch (lifecycleError) {
        console.error('Could not finalize failed Higher Education dataset generation:', lifecycleError.message || lifecycleError);
      }
      updateJob(job.jobId, {
        status: 'failed',
        progress: 100,
...
async function generateTemplateArchive({ version = IMPORT_VERSION }) {
  ...
  return {
    buffer: zip.toBuffer(),
    fileName: `highered-student-success-import-template-${version}.zip`,
...
async function validateDemoRestore({ version = IMPORT_VERSION }) {
  ...
  return runDatasetValidation({
    parsed: rebaseDemoDatasetDates(demoDataset.parsed),
    fileOnly: false,
