...
- **Fixture layout**
  - `verification/fixtures/required` mirrors the mandatory tables shipped with every bundle (`brands (academic programs)`, `products (student services)`, `fulfillment_centers (campus service centers)`, `customers (students)`, `influencers (success advocates)`, `social_posts (student/community signals)`, `post_product_mentions`, `inventory`, `orders (student requests)`, `order_items`). Each CSV uses the exact header order emitted by `backend/lib/importCatalog.js`.
...
- **Checklist**
  ...
  - **Header validation:** Renaming `brand_slug` to `brands (academic programs)lug` or reordering columns triggers the column expectation error embedded in `backend/lib/importCatalog.js`.
  - **Foreign keys:** `orders (student requests).customer_id`, `order_items.product_id`, `inventory.center_id`, and other reference columns must resolve to source IDs in the uploaded CSVs; broken references are flagged pre-import.
...
- **Derived and regenerated data**
  - Spatial point columns (`fulfillment_centers (campus service centers).location`, `customers (students).location`) are recalculated from the latitude/longitude pairs immediately after the base load.
