...
const CAPABILITY_GROUPS = [
  {
    title: 'Student Success Operations Core',
    accent: '#437C94',
    icon: GraduationCap,
    summary: 'Academic programs, students, support cases, engagement events, and advancement touches remain the governed system of record.',
    detail: 'This is the operational core: ACID transactions, retention impact, campus-service routing, and longitudinal student history.',
  },
  {
    title: 'Student Need Signals',
    accent: '#A36472',
    icon: TrendingUp,
    summary: 'Student intent, advisor alerts, financial-aid friction, and alumni engagement signals connect need to institutional action.',
    detail: 'Signal posts and service mentions let the demo track how student and community activity affects enrollment, persistence, access, advancement, and intervention decisions.',
  },
  {
    title: 'Student Support Graph',
    accent: '#796087',
    icon: Network,
    summary: 'Advisor, program, service, alumni, and student-support links provide graph traversal and relationship analysis.',
    detail: 'The graph layer explains referral paths, collaboration networks, partnership strength, stewardship ties, and connected student-success exposure.',
  },
  {
    title: 'Campus Service Coverage',
    accent: '#AA643B',
    icon: MapPin,
    summary: 'Campus service sites, coverage zones, routes, and demand regions all live as Oracle Spatial geometry.',
    detail: 'The campus service map uses spatial proximity, buffered zones, and regional demand overlays to plan access and service routing.',
  },
  {
    title: 'Student Request Documents',
    accent: '#AA643B',
    icon: FileJson,
    summary: 'The same request data can be exposed as nested JSON documents without duplicating the underlying rows.',
    detail: 'Duality views expose student-service requests as document-style payloads for API, service, and case workflows.',
  },
  {
    title: 'ML, Vector, and AI Agents',
    accent: '#4C825C',
    icon: BrainCircuit,
    summary: 'Forecasts, vector search, advancement intelligence, and agent workflows run against the same governed Oracle data foundation.',
    detail: 'This is where semantic retrieval, enrollment-risk scoring, intervention prediction, advancement outreach, and action logging converge.',
  },
];
...
function StatusGrid({ status, projected = false }) {
  const cards = [
    { label: 'Student Services', value: status?.products ?? 0, accent: '#437C94' },
    { label: 'Student Signals', value: status?.social_posts ?? 0, accent: '#A36472' },
    { label: 'Student Requests', value: status?.orders ?? 0, accent: '#4C825C' },
...
const BEST_STATUS_STORAGE_KEY = 'higheredLiveFootprintBestStatus';
...
function restoreMessageForJob(job) {
  ...
  if (progress >= 92 || /vector artifacts|embedding|semantic/i.test(baseMessage)) {
    return `${baseMessage} Student-service and signal vectors are rebuilt during the final step and appear when the restore finishes.`;
  }
  return baseMessage;
}
...
export default function DataModel() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('highered-live-footprint-refresh', handleFootprintRefresh);
    return () => {
      window.clearInterval(refreshTimer);
      window.removeEventListener('focus', handleFocus);
      window.removeEventListener('highered-live-footprint-refresh', handleFootprintRefresh);
    };
  }, [refreshStatus]);
    ...
    try {
      const startRes = await fetch('/api/import/restore-demo', {
        ...
        headers: {
          ...
          // The restore endpoint intentionally requires an explicit confirmation
          // because it replaces the active dataset.
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              Start here to load or restore the Seer Higher Education dataset before exploring the fall-term enrollment and retention journey. The action prepares the governed Oracle AI Database 26ai foundation used by the command center, student intent signals, support graph, campus service coverage, advancement engagement, analytics, and AI agents.
            </p>
          </div>
          <div>
            <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Why It Matters</p>
            <p className="text-[var(--color-text)] leading-relaxed">
              The same foundation supports student-service search, signal attribution, support-network analysis, spatial routing, document projections, forecasting, interventions, advancement outreach, and agent actions without splitting evidence across separate data stores. After the dataset is ready, each downstream page is working from the same governed Oracle data model.
            </p>
          </div>
...
-- relational tables        -> programs, services, students, cases, case lines, alumni engagement
-- json / duality views     -> request and capacity documents for student-support workflows
-- property graph           -> advisor, advocate, alumni, program, and service links
-- spatial geometry         -> campus service sites, service zones, demand regions
-- vector embeddings        -> service vectors, signal vectors, service-signal matches
...
-- agent audit trail        -> agent_actions, event_stream`}
      ...
      <div>
        ...
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Prepare the governed Seer Higher Education dataset before you move into the fall-term enrollment, retention, and advancement scenario.
        </p>
      </div>
      ...
      <div className="glass-card p-5" style={{ borderLeft: '3px solid var(--color-accent)' }}>
        <p className="text-base text-[var(--color-text)] leading-7">
          Start here to load the Seer Higher Education demo dataset. This action prepares academic programs, student services, synthetic students, support cases, advising and financial-aid signals, alumni engagement touches, campus service geography, vector embeddings, ML outputs, and agent audit history. Once the load completes, the live footprint confirms that the database is ready for the term-level enrollment and retention demo journey.
        </p>
      </div>
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-[var(--color-text-dim)] mt-1 max-w-2xl">
              Load or restore the bundled higher education dataset, then verify the live record counts that power every use case in the demo.
            </p>
          </div>
          <JetButton
      ...
      <div className="glass-card p-6">
        ...
        <p className="text-sm text-[var(--color-text-dim)] leading-6 mb-4">
          The restore prepares the operational, analytical, spatial, graph, vector, advancement, and agent data domains used by the enrollment and retention story and the rest of the demo.
        </p>
