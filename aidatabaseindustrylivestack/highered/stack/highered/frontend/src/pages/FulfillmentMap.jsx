...
const LAYER_DEFS = [
 { key: 'customers', label: 'Student Support Tiers', color: '#4C825C' },
 { key: 'centers', label: 'Campus Service Sites', color: '#437C94' },
 { key: 'routes', label: 'Campus Service Routes', color: '#796087' },
 { key: 'zones', label: 'Service Zones', color: '#AA643B' },
 { key: 'h3', label: 'H3 Density Grid', color: '#C74634' },
 { key: 'demandRegions', label: 'Student Demand Regions', color: '#AA643B' },
];
...
function CoverageMetricCard({ iconClass, label, value, detail, accent = 'ocean', state }) {
 return (
 <section className={`coverage-metric-card coverage-metric-card--${accent}`} aria-label={`${label}: ${value}`}>
 <div className="coverage-metric-card__heading">
 <span className="coverage-metric-card__icon"><JetGlyph iconClass={iconClass} /></span>
 <span className="coverage-metric-card__label">{label}</span>
 {state ? <span className="coverage-metric-card__state">{state}</span> : null}
 </div>
 <p className="coverage-metric-card__value">{value}</p>
 <p className="coverage-metric-card__detail">{detail}</p>
 </section>
 );
}
...
function centerColor(type) {
 ...
 if (type === 'service hub') return '#4C825C';
 return '#AA643B';
}
...
function FulfillmentMapView({ centers, shipments, customers, zonesData, demandRegions, layers, setLayer }) {
 ...
 {/* -- LAYER: Student Demand Regions (SDO_GEOMETRY polygons, colored by demand_index) -- */}
 ...
 <span style={{ color: '#697778' }}>Aid Need Index</span>
 ...
 <span style={{ color: style.color, textTransform: 'capitalize' }}>{z.ZONE_TYPE}</span>
	 {' '}zone - &lt;={z.RADIUS_KM} km - {z.MAX_DELIVERY_HRS}h service response
 </div>
 </Tooltip>
 </Circle>
 );
 ...
 <strong>{cell.count}</strong> students<br />
 ...
 {/* -- LAYER: Campus Service Routes (polylines colored by carrier) -- */}
 ...
 positions={[[s.CENTER_LAT, s.CENTER_LON], [s.CUSTOMER_LAT, s.CUSTOMER_LON]]}
	 color={CARRIER_COLORS[serviceChannelLabel(s.CARRIER)] || '#6F757E'}
 weight={2}
 opacity={0.65}
 />
 );
 ...
 {/* -- LAYER: Student Risk Tiers (small dots colored by tier) -- */}
 ...
 Support value proxy: {formatCurrency(c.LIFETIME_VALUE)}
 </div>
 </Tooltip>
 </CircleMarker>
 );
 ...
 {/* -- LAYER: Campus Service Sites (large markers with popups) -- */}
 ...
 <div><span style={{ color: '#697778' }}>Pending: </span>{c.PENDING_SHIPMENTS} service routes</div>
 ...
 <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-brand-ocean inline-block" /> Regional Hub</span>
 <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-brand-pine inline-block" /> Service Center</span>
 <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-brand-sienna inline-block" /> Access Point</span>
 </div>
 )}
 {layers.customers && (
 ...
 {layers.routes && (
 <div className="flex items-center gap-3">
 <span className="flex items-center gap-1"><span className="w-4 h-0.5 bg-brand-plum inline-block" /> Advising</span>
 <span className="flex items-center gap-1"><span className="w-4 h-0.5 bg-brand-sienna inline-block" /> Success Team</span>
 <span className="flex items-center gap-1"><span className="w-4 h-0.5 bg-brand-ocean inline-block" /> Campus Ops</span>
 <span className="flex items-center gap-1"><span className="w-4 h-0.5 bg-brand-red inline-block" /> Community Partner</span>
 </div>
 )}
 ...
 {layers.h3 && (
 <div><span className="tone-sienna">H3 res-4</span> - {h3Cells.length} hexagons - {customers?.length ?? 0} students</div>
 )}
 {layers.zones && (
...
export default function FulfillmentMap() {
 ...
 <RegisterOraclePanel title="Campus Service Coverage">
 <div className="space-y-4">
 <div>
 <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">What's Happening</p>
 <p className="text-[var(--color-text)] leading-relaxed">
 Every campus service site, service zone, student location, and demand region is stored as an{' '}
 ...
 all campus service sites by proximity in a single SQL - no external routing API.
 ...
 The H3 layer bins student density client-side via{' '}
 ...
 <FeatureBadge label="demand_forecasts" color="red" />
	 <FeatureBadge label="student support tier" color="purple" />
 </div>
 <SqlBlock code={`-- Nearest campus service site with capacity
SELECT site.campus_service_site_name, site.city,
	 ROUND(SDO_GEOM.SDO_DISTANCE(
	 student.location, site.location, 0.005, 'unit=KM'), 2) AS dist_km,
	 cap.capacity_units
	FROM highered_students_v student
	CROSS JOIN campus_service_sites_v site
	JOIN student_service_capacity_v cap
	 ON cap.campus_service_site_id = site.campus_service_site_id
	 AND cap.service_id = :service_id
	WHERE student.student_id = :student_id
	 AND site.is_active = 1
	 AND cap.capacity_units > cap.reserved_units
ORDER BY dist_km
...
ORDER BY r.demand_index DESC;`} />
 ...
 Oracle <span className="tone-red font-mono">DBMS_RLS</span> applies a row-level security policy
	 to <code className="text-xs tone-teal mx-1">CAMPUS_SERVICE_SITES_V</code>. When a user is set via{' '}
 <span className="tone-sienna font-mono">sc_security_ctx.set_user_context()</span>, Oracle
 transparently appends a WHERE clause - campus service managers see only their regional centers,
...
-- The VPD policy function (transparent to app SQL):
	-- vpd_campus_service_region() returns:
		-- campus_service_mgr -> 'state_province IN (''California'')'
		-- admin/analyst -> NULL (no filter, sees all rows)
		-- viewer -> 'is_active = 1'

	-- Policy attached to CAMPUS_SERVICE_SITES_V:
	DBMS_RLS.ADD_POLICY(
	 object_name => 'CAMPUS_SERVICE_SITES_V',
	 policy_name => 'VPD_CAMPUS_SERVICE_REGION',
	 function_schema => USER,
	 policy_function => 'VPD_CAMPUS_SERVICE_REGION',
 ...
 <DiagramBox label="Campus Service Sites" sub="SDO_GEOMETRY points - R-Tree index" color="#437C94" />
 <div className="text-center text-[var(--color-text-dim)] text-[9px]">down</div>
 <DiagramBox label="Service Zones" sub="SDO_BUFFER circular polygons - 3 tiers" color="#AA643B" />
 <div className="text-center text-[var(--color-text-dim)] text-[9px]">down</div>
 <DiagramBox label="Student Risk Tiers" sub="new - standard - preferred - vip" color="#796087" />
 <div className="text-center text-[var(--color-text-dim)] text-[9px]">down</div>
 <DiagramBox label="H3 Density Grid" sub="Uber H3 res-4 - demand heatmap" color="#AA643B" />
 <div className="text-center text-[var(--color-text-dim)] text-[9px]">down</div>
 <DiagramBox label="Student Demand Regions" sub="SDO_GEOMETRY polygons - demand_index 0-100 - forecast join" color="#C74634" />
 </div>
 ...
 <JetGlyph iconClass="oj-fwk-icon-calendar-clock" className="fulfillment-page-glyph tone-teal" /> Campus Service Coverage
 </h2>
 <p className="text-sm text-[var(--color-text-dim)] mt-1">
 <span className="tone-teal">Six spatial layers</span> - sites, zones, routes, student support tiers, H3 density &amp; demand regions - all toggle-able
 </p>
 </div>

 {/* -- Operational snapshot -- */}
 <section className="coverage-metrics" aria-label="Campus service coverage operational snapshot">
 <div className="coverage-metrics__grid">
 <CoverageMetricCard
 iconClass="oj-fwk-icon-grid"
 label="Active service sites"
 value={(centers || []).length}
 detail="Sites available for student-service routing"
 accent="ocean"
 />
 <CoverageMetricCard
 ...
 detail="Capacity units across visible service sites"
 accent="pine"
 />
 <CoverageMetricCard
 iconClass="oj-fwk-icon-tree-document"
 label="Active service routes"
 value={formatNumber(pendingShipments)}
 detail="Requests currently awaiting or in service"
 accent="sienna"
 />
 <CoverageMetricCard
 ...
 value={(alerts || []).length}
 detail="Demand signals requiring service-manager review"
 accent="red"
 state={(alerts || []).length > 0 ? 'Review' : 'Clear'}
 />
 </div>
 </section>
 ...
 {/* -- Campus Service Sites Table -- */}
 <div className="glass-card p-5">
 <h3 className="text-sm font-semibold mb-3">Campus Service Sites</h3>
 ...
 <JetGlyph iconClass="oj-fwk-icon-message-warning" className="tone-red" /> Capacity Alerts - Student Demand Surge
 </h3>
 ...
 <span className={a.STOCK_STATUS === 'no_capacity' || a.STOCK_STATUS === 'critical' ? 'font-bold text-[var(--color-text)]' : 'text-[var(--color-text)]'}>
