...
export default function AdminEntry({
 ...
 const { replaceInstitutionLabels } = useInstitution();
 ...
 const headerContent = isOverlay ? 'Use Your Own Student Success Data' : 'Dataset Control';
 const headerDescription = isOverlay
 ? replaceInstitutionLabels('The Seer Higher Education synthetic dataset stays active by default. Upload your own v1 ZIP, validate it, or preview a restore before executing any replacement.')
 : 'Start on the seeded higher education demo dataset, or load your own v1 ZIP before entering the experience.';
 const quickActionLabel = isOverlay ? 'Jump To Upload' : 'Use Your Own Student Success Data';
 ...
 <h2 className="text-lg font-bold">1. Download student success template</h2>
 <p className="text-sm text-[var(--color-text-dim)]">
 Get the canonical package with required and optional CSV templates mapped to synthetic students, services, requests, signals, and campus service sites.
 </p>
 <button
 ...
 Download Student Success Template
 </button>
 </section>

 <section className="glass-card p-5 space-y-4">
 <h2 className="text-lg font-bold">2. Select completed student dataset ZIP</h2>
 <label
 htmlFor="import-zip"
 ...
 Select a `.zip` containing `manifest.json` and table CSV files. No real student PII is required for this demo.
 </p>
 </label>
 <input
 id="import-zip"
 ...
 <h2 className="text-lg font-bold mb-4">3. Validate or restore demo data</h2>
 <div className="flex flex-wrap gap-3">
 <button
 className="btn-ghost inline-flex items-center gap-2"
 ...
 Preview the higher education demo dataset import to see row counts and warnings, then execute the restore to overwrite the current data.
 </p>
 <div className="flex flex-wrap gap-3">
 <button
 className="btn-ghost inline-flex items-center gap-2"
