...
import { FileText, Filter, ChevronRight, ChevronDown, Loader2, Check, MapPin, Package, Truck, Navigation, Shield, Eye } from 'lucide-react';
...
const CARRIER_LABELS = {
	 ...
	 UPS: 'Success Team',
	 USPS: 'Campus Ops',
	 DHL: 'Community Partner',
};

function serviceChannelLabel(carrier) {
	 return CARRIER_LABELS[carrier] || carrier || 'Service Team';
}

const SHIP_STATUS_STEPS = [
 { key: 'preparing', label: 'Preparing', icon: Package },
 { key: 'picked', label: 'Assigned', icon: Package },
 { key: 'packed', label: 'Prepared', icon: Package },
 { key: 'shipped', label: 'Dispatched', icon: Truck },
 { key: 'in_transit', label: 'In Progress', icon: Truck },
 { key: 'out_for_delivery', label: 'Ready', icon: Navigation },
 { key: 'delivered', label: 'Complete', icon: Check },
];

const ORDER_DETAIL_TABS = [
 { id: 'relational', label: 'Relational' },
 { id: 'json', label: 'JSON Duality View' },
 { id: 'route', label: 'Service Route' },
];
...
function OrderDualityPanel({ orderId, onClose }) {
 ...
 <p className="text-sm text-[var(--color-text-dim)] text-center py-4">Student request not found</p>
 ) : (
 <>
 ...
 <span className="text-[10px] px-2 py-0.5 rounded border font-mono" style={{ background: 'rgba(67,124,148,0.1)', borderColor: 'rgba(67,124,148,0.3)', color: 'var(--color-text)' }}>
	 SELECT * FROM student_service_requests_v / student_request_lines_v
 </span>
 ...
 {[
 { label: 'Student', value: `${order.FIRST_NAME} ${order.LAST_NAME}` },
 { label: 'Location', value: `${order.CITY}, ${order.STATE_PROVINCE}` },
 { label: 'Support Value', value: formatCurrency(order.ORDER_TOTAL) },
 { label: 'Routing Cost', value: formatCurrency(order.SHIPPING_COST) },
 ].map(s => (
 ...
 <th className="text-left py-1.5 px-2">Student Service</th>
 <th className="text-left py-1.5 px-2">Academic Program</th>
 <th className="text-left py-1.5 px-2">Category</th>
 <th className="text-right py-1.5 px-2">Qty</th>
 <th className="text-right py-1.5 px-2">Resource Value</th>
 <th className="text-right py-1.5 px-2">Line Impact</th>
 </tr>
 </thead>
 <tbody>
 {items.map(item => (
 ...
 {loadingDuality ? (
 <div className="flex items-center gap-2 text-sm text-[var(--color-text-dim)] py-4 justify-center">
	 <Loader2 size={14} className="animate-spin" /> Querying STUDENT_REQUESTS_DV duality view...
 </div>
 ) : dualityError ? (
 ...
 <JetButton
 label={copied ? 'Copied' : 'Copy request document'}
 chroming="outlined"
 onAction={copyJson}
 />
 </div>
 ...
 <span className="text-[var(--color-text-dim)]"> - This is the exact same request data from the relational tab, but accessed through
	 Oracle's <span className="text-[var(--color-text)] font-mono">STUDENT_REQUESTS_DV</span> duality view. One governed request model stores normalized rows; the duality view exposes them as a
...
{JSON.stringify(duality.document, null, 2)}
 ...
 {loadingDetail ? (
 <div className="flex items-center gap-2 text-sm text-[var(--color-text-dim)] py-4 justify-center">
 <Loader2 size={14} className="animate-spin" /> Loading service route data...
 </div>
 ) : !order ? (
 <p className="text-sm text-[var(--color-text-dim)] text-center py-4">Student request not found</p>
 ...
 <p className="text-sm text-[var(--color-text-dim)]">No location data available for this request</p>
 </div>
 ) : (() => {
 const from = [order.CENTER_LAT, order.CENTER_LON];
 const to = [order.CUST_LAT, order.CUST_LON];
 const arc = curvedPositions(from, to);
	 const serviceChannel = serviceChannelLabel(shipment?.CARRIER);
 ...
 {/* Campus Service Site center marker */}
 ...
 <span>{order.CENTER_NAME || 'Campus Service Site'}</span>
 </div>
 </Tooltip>
 </CircleMarker>
 ...
 className="route-map-tooltip">
 <div className="route-map-tooltip__content">
 <span className="route-map-tooltip__label">Student</span>
 ...
 <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Campus Service Progress</p>
...
-- SDO_GCDR.ELOC_DRIVE_TIME_POLYGON for isochrone zones` : `-- Distance between campus service site and student
SELECT ROUND(SDO_GEOM.SDO_DISTANCE(
 ...
 0.05, 'unit=MILE'), 1) AS distance_miles
	FROM campus_service_sites_v site, highered_students_v student
	WHERE site.campus_service_site_id = ${order.FULFILLMENT_CENTER_ID || ':center_id'}
	AND student.student_id = ${order.CUSTOMER_ID || ':student_id'};
...
export default function Orders() {
 ...
 <RegisterOraclePanel title="Student Requests & Cases">
 ...
 Student service requests are stored in a classic <span className="tone-ocean font-mono">relational schema</span> - ACID transactions, foreign keys, referential integrity.
 ...
 without ETL or duplication. Click any request row and toggle between{' '}
 ...
 <FeatureBadge label="Zero ETL / Zero Sync" color="purple" />
	 ...
	 {/* STUDENT_REQUESTS_DV definition */}
 <div>
 <p className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider mb-2">
	 STUDENT_REQUESTS_DV - Student Requests & Cases + Line Items as JSON
 </p>
 <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
	 REST-style JSON access to student service requests with nested service items. Read-write: inserts through the view update both <span className="font-mono text-[var(--color-text)]">student_service_requests_v</span> and{' '}
	 <span className="font-mono text-[var(--color-text)]">student_request_lines_v</span> atomically.
 </p>
	 <SqlBlock code={`CREATE JSON RELATIONAL DUALITY VIEW student_requests_dv AS
SELECT JSON {
	 '_id': r.request_id,
	 'studentId': r.student_id,
	 'status': r.request_status,
	 'serviceValue': r.service_value_proxy,
	 'items': [
	 SELECT JSON {
	 'lineId': line.request_line_id,
	 'serviceId': line.service_id,
	 'quantity': line.requested_quantity,
	 'serviceValue': line.service_value_proxy }
	 FROM student_request_lines_v line WITH UPDATE
	 WHERE line.request_id = r.request_id ] }
	FROM student_service_requests_v r WITH UPDATE;`} />
 </div>

	 {/* STUDENT_SERVICE_CAPACITY_DV definition */}
 <div>
 <p className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider mb-2">
	 STUDENT_SERVICE_CAPACITY_DV - Student Services + Capacity as JSON
 </p>
 <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
 Student Services with nested capacity across all campus service sites. One document, two tables.
 </p>
	 <SqlBlock code={`CREATE JSON RELATIONAL DUALITY VIEW student_service_capacity_dv AS
SELECT JSON {
	 '_id': svc.service_id,
	 'serviceCode': svc.service_code,
	 'serviceName': svc.service_name,
	 'category': svc.service_category,
	 'serviceValue': svc.service_value_proxy,
	 'capacity': [
	 SELECT JSON {
	 'siteId': cap.campus_service_site_id,
	 'capacityUnits': cap.capacity_units,
	 'reservedUnits': cap.reserved_units }
	 FROM student_service_capacity_v cap WITH UPDATE
	 WHERE cap.service_id = svc.service_id ] }
	FROM student_services_v svc WITH UPDATE;`} />
 </div>
 ...
 <SqlBlock code={`-- Relational: traditional row-by-row access
	SELECT r.request_id, s.first_name || ' ' || s.last_name AS student_name,
	 r.service_value_proxy, line.service_id,
	 line.requested_quantity, line.service_value_proxy
	FROM student_service_requests_v r
	JOIN highered_students_v s ON s.student_id = r.student_id
	JOIN student_request_lines_v line ON line.request_id = r.request_id
...
-- Duality: same data as a single JSON document
 ...
 <div className="text-[8px] mt-1 text-[var(--color-text)]">document REST API</div>
	 ...
	 <div className="text-center text-[9px] text-[var(--color-text)] mt-1">OK Same ACID transaction - No sync - No ETL</div>
 </div>
 </div>
 ...
 <div className="text-[9px] text-center text-[var(--color-text)] font-bold mb-1">Duality View Architecture</div>
	 <DiagramBox label="student requests + request lines" sub="Normalized relational tables - ACID, FK constraints, indexes" color="#437C94" />
	 <div className="text-center text-[10px] text-[var(--color-text)]">&lt;-&gt; CREATE JSON RELATIONAL DUALITY VIEW</div>
	 <DiagramBox label="STUDENT_REQUESTS_DV" sub="JSON document: {_id, status, items: [...]} - WITH UPDATE" color="#AA643B" />
	 <div className="text-center text-[10px] text-[var(--color-text)]">&lt;-&gt;</div>
	 <DiagramBox label="student services + capacity" sub="Student service catalog + campus service capacity levels" color="#437C94" />
	 <div className="text-center text-[10px] text-[var(--color-text)]">&lt;-&gt; CREATE JSON RELATIONAL DUALITY VIEW</div>
	 <DiagramBox label="STUDENT_SERVICE_CAPACITY_DV" sub="JSON document: {serviceCode, serviceName, capacity: [...]} - WITH UPDATE" color="#AA643B" />
 </div>
 ...
 <strong className="text-[var(--color-text)]">How it works:</strong>{' '}
	 The same <span className="font-mono text-[var(--color-text)]">student request</span> + <span className="font-mono text-[var(--color-text)]">request line</span> rows you see in relational
	 queries are exposed as nested JSON documents through <span className="font-mono text-[var(--color-text)]">STUDENT_REQUESTS_DV</span>.
 ...
 {/* VPD on Student Requests */}
 <div>
 <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
 Virtual Private Database (VPD) on Student Requests
 </p>
 ...
 A second <span className="tone-red font-mono">DBMS_RLS</span> policy on the{' '}
		 <code className="text-xs tone-teal mx-1">STUDENT_SERVICE_REQUESTS_V</code> view restricts campus service managers
	 to requests routed through their regional service sites. The policy function queries{' '}
	 <code className="text-xs tone-teal mx-1">campus_service_site_id</code> against the user's assigned region.
 Admins and analysts see all 3,000 student service requests; a regional manager sees only their subset.
 </p>
 </div>
 <div className="flex flex-wrap gap-1.5">
 <FeatureBadge label="DBMS_RLS (Student Requests)" color="red" />
 ...
 <FeatureBadge label="Cascading VPD" color="orange" />
	 ...
	 <SqlBlock code={`-- VPD policy function for student requests
	CREATE OR REPLACE FUNCTION vpd_student_requests_region (
...
BEGIN
 ...
 IF v_role IN ('admin','analyst') THEN RETURN NULL; END IF;
	 ...
	 -- Campus Service Site manager: requests for their region's service sites
	 IF v_role = 'campus_service_mgr' AND v_region IS NOT NULL THEN
	 RETURN 'campus_service_site_id IN '
	 || '(SELECT campus_service_site_id FROM campus_service_sites_v'
...
-- Applied via:
	DBMS_RLS.ADD_POLICY('STUDENT_SERVICE_REQUESTS_V','VPD_STUDENT_REQUESTS_REGION',
	 policy_function => 'VPD_STUDENT_REQUESTS_REGION',
 ...
 <h2 className="text-2xl font-bold flex items-center gap-2">
	 <FileText className="tone-ocean" /> Student Requests &amp; Cases
 </h2>
 <p className="text-sm text-[var(--color-text-dim)] mt-1">
 Click any request to toggle between <span className="font-semibold text-[var(--color-text)]">Relational</span> and{' '}
 ...
 {isFM
 ? `Filtered to ${currentUser.REGION} - ${(orders || []).length} requests visible`
 : `${(orders || []).length} requests visible`
 }
 </span>
 </div>
 ...
 <th className="text-right py-3 px-4">Support Value</th>
 <th className="text-center py-3 px-4">Student Signal</th>
 <th className="text-left py-3 px-4">Campus Service Site</th>
 <th className="text-left py-3 px-4">Created</th>
 </tr>
 </thead>
 <tbody>
 {loading ? (
 <tr><td colSpan={9} className="py-8 text-center text-[var(--color-text-dim)]">Loading requests...</td></tr>
 ) : (orders || []).length === 0 ? (
 <tr><td colSpan={9} className="py-8 text-center text-[var(--color-text-dim)]">No requests found</td></tr>
 ) : (
 (orders || []).map(o => {
 ...
 <td className="py-3 px-4 text-center">
 {o.SOCIAL_DRIVEN ? (
 <span className="tone-rose text-xs font-semibold">Student Signal</span>
 ) : (
