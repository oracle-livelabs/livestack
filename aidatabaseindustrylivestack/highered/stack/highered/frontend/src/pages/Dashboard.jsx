...
function ProductDetailModal({ productId, onClose }) {
 ...
 <span> - </span>
 <span className="font-medium text-[var(--color-text)]">Resource value {formatCurrency(product.UNIT_PRICE)}</span>
 </div>
 </div>
 ) : (
 ...
 <p className="text-[10px] text-[var(--color-text-dim)]">Available Slots</p>
 </div>
 ...
 <p className="text-[10px] text-[var(--color-text-dim)]">Student Signals</p>
 </div>
 </div>
 ...
 <MapPin size={12} /> Capacity by Campus Service Site
 </h4>
 ...
 <th className="text-right py-2 px-2">Available Slots</th>
 ...
 {isLow && <span className="ml-1 text-[9px] uppercase tracking-wide">Low</span>}
 </td>
 </tr>
 );
 })}
 ...
 <div>
 <h4 className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 flex items-center gap-1.5">
 <MessageSquare size={12} /> Recent Student Signals
 </h4>
 ...
 <p className="text-sm text-[var(--color-text-dim)] text-center py-4">No detailed data available for this student service.</p>
 )}
 </div>
 )}
 ...
 <span className="text-[#AA643B] font-semibold">Student Service + Capacity as JSON Document</span>
 <span className="text-[var(--color-text-dim)]"> - The same student service and capacity data from the Details tab, exposed as a single nested JSON document.
 The duality view joins <span className="text-[#437C94] font-mono">products</span> (student services) and <span className="text-[#437C94] font-mono">inventory</span> (capacity) tables
...
function TrendingTable({ products, onSelect, selectedId }) {
 ...
 <th className="text-left py-2 px-3">Student Service</th>
 <th className="text-left py-2 px-3">Academic Program</th>
 <th className="text-right py-2 px-3">Signal Mentions</th>
 ...
 {p.PEAK_MOMENTUM === 'mega_viral' ? 'Critical' : p.PEAK_MOMENTUM === 'viral' ? 'High' : p.PEAK_MOMENTUM?.replace('_', ' ')}
 </span>
 </td>
...
const MOMENTUM_LABELS = { '': 'All', mega_viral: 'Critical', viral: 'High', rising: 'Rising' };
...
export default function Dashboard() {
 ...
 <RegisterOraclePanel title="Student Success Command Center">
 ...
 <SqlBlock code={`-- One query. Five student-success workloads. Zero ETL.
SELECT
 COUNT(r.request_id) AS student_requests_total,
 SUM(r.service_value_proxy) AS service_value_total,
 COUNT(sig.signal_id) FILTER (
 WHERE sig.momentum_flag = 'viral') AS urgent_signal_count,
 COUNT(aa.action_id) AS agent_actions,
 COUNT(route.service_route_id) FILTER (
 WHERE route.route_status = 'in_transit') AS active_service_routes
FROM student_service_requests_v r,
 student_signal_posts_v sig,
 agent_actions aa,
 student_service_routes_v route;`} />
 <SqlBlock code={`-- Student service search: Oracle UPPER() case-insensitive LIKE
SELECT svc.service_name, svc.academic_program,
 COUNT(DISTINCT m.post_id) AS signal_match_count,
 ROUND(AVG(sig.urgency_score), 2) AS avg_urgency
FROM student_services_v svc
JOIN post_product_mentions m ON svc.service_id = m.product_id
JOIN student_signal_posts_v sig ON m.post_id = sig.signal_id
WHERE sig.posted_at >= SYSTIMESTAMP - INTERVAL '7' DAY
 AND (UPPER(svc.service_name) LIKE UPPER(:search)
 OR UPPER(svc.academic_program) LIKE UPPER(:search))
GROUP BY svc.service_id, svc.service_name, svc.academic_program
ORDER BY avg_urgency DESC;`} />
 <div>
 <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Student Success Architecture</p>
 ...
 <DiagramBox label="Relational" sub="requests students" color="#437C94" />
 ...
 <div className="flex items-center justify-between">
 <div>
 <h2 className="text-2xl font-bold">Student Success Command Center</h2>
 <p className="text-sm text-[var(--color-text-dim)] mt-1">
 Real-time higher education student-success intelligence powered by Oracle 26ai Converged Database
 </p>
 </div>
 ...
 <StatCard iconClass="oj-fwk-icon-tree-document" label="Student Requests" value={formatNumber(s.ORDERS_TOTAL)} subValue={`${formatNumber(s.ORDERS_30D)} last 30d`} color="#437C94" />
 <StatCard iconClass="oj-fwk-icon-view" label="Student Support Value" value={formatCurrency(s.REVENUE_TOTAL)} subValue={`${formatCurrency(s.REVENUE_30D)} last 30d`} color="#4C825C" />
 ...
 <StatCard iconClass="oj-fwk-icon-sortrelevancehigh" label="High-Demand Services" value={formatNumber(s.TRENDING_PRODUCTS)} subValue={`${formatNumber(s.POSTS_TOTAL)} total signals`} color="#AA643B" />
 <StatCard iconClass="oj-fwk-icon-users" label="Agent Actions" value={formatNumber(s.AGENT_ACTIONS_TOTAL)} subValue={`${formatNumber(s.SHIPMENTS_IN_TRANSIT)} active service routes`} color="#796087" />
 </div>
 ...
 <Activity size={28} className="mx-auto text-[var(--color-text-dim)] opacity-40" />
	 <p className="text-sm text-[var(--color-text-dim)]">No student signals during this time period</p>
 ...
 />
	 <Area type="monotone" dataKey="TOTAL_LIKES" stroke="#AA643B" fill="url(#gradLikes)" strokeWidth={2} name="Engagement" />
 <Area type="monotone" dataKey="VIRAL_COUNT" stroke="#C74634" fill="url(#gradViral)" strokeWidth={2} name="Urgent Signals" />
 </AreaChart>
 </ResponsiveContainer>
 )}
 </div>

	 {/* Student Support Value by Category */}
 ...
 <span className="text-[10px] uppercase tracking-wider text-[var(--color-text-dim)] font-normal hidden sm:inline">
	 - Student-Signal Momentum (7 days)
 </span>
 </h3>
 ...
 placeholder="Search student services or programs..."
 ...
 <X size={12} />
	 ...
	 {/* Program filter chips (populated from trending results) */}
 ...
 {loadingTrending ? (
 <p className="text-sm text-[var(--color-text-dim)]">Loading high-demand student services...</p>
 ) : (
 <TrendingTable
 products={trending}
 ...
 <p className="text-sm text-[var(--color-text-dim)] text-center py-6">No student services match your search.</p>
 )}

 <p className="text-[10px] text-[var(--color-text-dim)] mt-3">
 Click any row to view capacity and signal mention details
 </p>
 </div>
 ...
 {[
 { label: 'Relational', desc: 'Requests & Capacity', color: '#437C94' },
	 { label: 'JSON', desc: 'Signal Payloads', color: '#4C825C' },
 { label: 'Graph', desc: 'Support Network', color: '#AA643B' },
 { label: 'Vector', desc: 'Semantic Matching', color: '#796087' },
 { label: 'Spatial', desc: 'Service Routing', color: '#4F7D7B' },
