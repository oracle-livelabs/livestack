...
const AGENT_COLORS = {
 ...
 campus_services_agent: '#4C825C',
 master_orchestrator: '#796087',
 chat_agent: '#4F7D7B',
};

const TEAM_INFO = {
 STUDENT_SIGNAL_TEAM: { label: 'Signal Agent', color: '#AA643B', iconClass: 'oj-fwk-icon-sortrelevancehigh', desc: 'Student and community signal analysis' },
 CAMPUS_SERVICE_TEAM: { label: 'Campus Services Agent', color: '#437C94', iconClass: 'oj-fwk-icon-tree-document', desc: 'Capacity and campus-service routing' },
 STUDENT_SUCCESS_TEAM: { label: 'Student Success Operations Agent', color: '#4C825C', iconClass: 'oj-fwk-icon-grid', desc: 'Student requests and service value' },
};

const EXAMPLE_QUESTIONS = [
 { text: 'Which student services are creating the highest fall census pressure?', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'trends' },
 { text: 'Show resource impact by student service category', iconClass: 'oj-fwk-icon-grid', team: 'student-success' },
 { text: 'Which enrollment and retention services have low capacity?', iconClass: 'oj-fwk-icon-tree-document', team: 'campus-services' },
 { text: 'What percentage of student requests are student-signal-driven?', iconClass: 'oj-fwk-icon-info', team: 'student-success' },
 { text: 'Find urgent FAFSA verification and emergency aid signals in the last 24 hours', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'trends' },
 { text: 'Check capacity for Student Emergency Fund', iconClass: 'oj-fwk-icon-tree-document', team: 'campus-services' },
 { text: 'Find nearest campus service site with same-day advising slots for a student in Miami', iconClass: 'oj-fwk-icon-arrowtail-e', team: 'campus-services' },
];
...
// -- Campus Services Route Map (rendered inside chat messages) ---------------------
function FulfillmentRouteMap({ routeData }) {
 const { customer, centers, product: serviceName } = routeData;
 ...
 <span className="text-xs font-bold tone-ocean">Campus Services Route - {serviceName}</span>
 ...
 <strong>Student</strong><br />{customer.city}, {customer.state}
 </div>
 </MapTooltip>
 </CircleMarker>
 </MapContainer>
...
function ChatAgent({ onActionLogged }) {
 ...
 Ask questions - auto-routed to <span className="font-semibold text-[var(--color-text)]">Signal</span>, <span className="font-semibold text-[var(--color-text)]">Campus Services</span>, or <span className="font-semibold text-[var(--color-text)]">Student Success Operations</span> agents
 </p>
 </div>
 </div>
 {messages.length > 0 && (
 ...
 <p className="text-sm text-[var(--color-text-dim)]">Ask me anything about your higher education student success data</p>
...
const PROFILE_COLOR_MAP = {
 HE_COHERE_PROFILE: '#AA643B',
 HE_LLAMA_PROFILE: '#796087',
 HE_VISION_PROFILE: '#4F7D7B',
 HE_GROK42_PROFILE: '#C74634',
 HE_EMBED_PROFILE: '#437C94',
};
...
const FALLBACK_PROFILE_INFO = {
 ...
 label: 'llama3.2',
 short: 'llama3.2',
 color: PROFILE_COLOR_MAP.HE_LLAMA_PROFILE,
 type: 'Ollama + Oracle SQL',
 },
};

export default function AgentConsole() {
 const [activeProfile, setActiveProfile] = useState('HE_LLAMA_PROFILE');
 ...
 <RegisterOraclePanel title="Student Success Agent Console">
 ...
 and Oracle AI Database 26ai executing SQL and PL/SQL tools against live data. Three specialist teams (<code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #AA643B' }}>STUDENT_SIGNAL_TEAM</code>,&nbsp;
 <code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #437C94' }}>CAMPUS_SERVICE_TEAM</code>, <code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #4C825C' }}>STUDENT_SUCCESS_TEAM</code>) route work across
 signal analysis, campus services, and student-success operations tasks. Oracle stores the source data, runs the queries, and records decisions in
...
-- Example flow:
-- 1. Classify the request as STUDENT_SIGNAL_TEAM
-- 2. Call detect_trending_products(p_hours=>24)
-- 3. Join capacity and campus service routing data in Oracle
...
VALUES ('trend_detection_agent','capacity_flag','student_service',
 ...
 {[
 { team: 'STUDENT_SIGNAL_TEAM', agent: 'TREND_AGENT', tools: ['TREND_SQL_TOOL', 'DETECT_TRENDS_TOOL', 'INFLUENCER_NETWORK_TOOL', 'LOG_DECISION_TOOL'], color: '#A36472' },
 { team: 'CAMPUS_SERVICE_TEAM', agent: 'FULFILLMENT_AGENT', tools: ['COMMERCE_SQL_TOOL', 'CHECK_INVENTORY_TOOL', 'FULFILLMENT_ROUTE_TOOL', 'LOG_DECISION_TOOL'], color: '#AA643B' },
 { team: 'STUDENT_SUCCESS_TEAM', agent: 'COMMERCE_AGENT', tools: ['COMMERCE_SQL_TOOL', 'LOG_DECISION_TOOL'], color: '#4C825C' },
 ].map(t => (
 ...
 <DiagramBox label="Urgent Student Signal Detected" sub="social_posts - urgency_score >= 75" color="#AA643B" />
 <div className="text-center text-[var(--color-text-dim)]">down</div>
 <DiagramBox label="STUDENT_SIGNAL_TEAM" sub="Ollama reasoning + tool routing" color="#796087" />
 <div className="text-center text-[var(--color-text-dim)]">down calls PL/SQL tool</div>
 <DiagramBox label="detect_trending_products()" sub="Vector match - Graph centrality" color="#A36472" />
 <div className="text-center text-[var(--color-text-dim)]">down</div>
 <DiagramBox label="CAMPUS_SERVICE_TEAM" sub="Capacity check - intervention logic" color="#437C94" />
 <div className="text-center text-[var(--color-text-dim)]">down</div>
 <DiagramBox label="STUDENT_SUCCESS_TEAM" sub="Pricing - promotions - routing" color="#4C825C" />
 <div className="text-center text-[var(--color-text-dim)]">down</div>
 ...
 <JetGlyph iconClass="oj-fwk-icon-users" className="agent-console-page-glyph tone-plum" /> Student Success Agent Console
 </h2>
