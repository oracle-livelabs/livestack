...
import { formatNumber, getPlatformColor, platformLabel } from '../utils/format';
...
const PLATFORM_COLORS = {
	 ...
	 tiktok: '#4F7D7B',
		 ...
		 threads: '#796087',
		 twitch: '#796087',
		 student_community: '#A36472',
		 short_form_student_video: '#4F7D7B',
		 campus_video: '#C74634',
		 public_student_threads: '#437C94',
		 peer_network_threads: '#796087',
};

function platformColor(p) {
	 return PLATFORM_COLORS[(p || '').toLowerCase()] || getPlatformColor(p);
}
...
function ForceGraph({ data, depth, height = 520, onNodeClick }) {
 ...
 <div style="display:grid;grid-template-columns:auto auto;gap:2px 12px">
	 <span style="color:#6F757E">Platform</span><span>${platformLabel(d.PLATFORM)}</span>
 <span style="color:#6F757E">Niche</span><span>${d.NICHE || ' - '}</span>
 <span style="color:#6F757E">City</span><span>${d.CITY || ' - '}</span>
 <span style="color:#6F757E">Audience</span><span>${formatNumber(d.FOLLOWER_COUNT || 0)}</span>
 ...
 <span style="color:#6F757E">Engagement</span><span>${eng}</span>
 <span style="color:#6F757E">Hop</span><span>${d.type === 'center' ? '0 (center)' : d.hopLevel}</span>
 </div>
	 <div style="color:#C74634;font-size:10px;font-weight:600;margin-top:6px;text-align:center">Click to explore support network -></div>
 </div>
 `)
 .style('opacity', 1)
 .style('left', (event.pageX + 14) + 'px')
...
function NodeDetailPanel({ node, network, onExplore, onClose }) {
 ...
 <span className="tone-pine text-xs">OK</span>
 )}
 </p>
 ...
 <div className="flex flex-wrap gap-1.5">
	 <span className={`platform-badge platform-${(node.PLATFORM || '').toLowerCase()}`}>{platformLabel(node.PLATFORM)}</span>
 ...
 {[
 { label: 'Audience', value: formatNumber(node.FOLLOWER_COUNT || 0), icon: Users },
 { label: 'Support Score', value: score, icon: Star },
 ...
 {/* Academic program relationships (from network.brands for center node) */}
 {node.type === 'center' && network?.brands?.length > 0 && (
 <div>
 <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Academic Program Relationships</p>
 <div className="space-y-1">
 ...
 <Network size={13} /> Explore Support Network
...
function GraphQueryExplorer() {
 ...
 Run real SQL/PGQ queries against the student support property graph
 </p>
 </div>
 {activeQuery && (
...
export default function InfluencerGraph() {
 ...
 // Track which user the current advocate list belongs to
 const [listUser, setListUser] = useState(null);

 // Advocate list - refetch when user or search changes (VPD filtering)
 const { data: rawInfluencers, loading } = useData(
 () => api.graph.influencers({ limit: 50, ...(search ? { search } : {}) }),
 [search, currentUser?.USERNAME]
 );

 // When the advocate list loads, stamp which user it belongs to
 useEffect(() => {
 ...
 // Auto-select first advocate ONLY when the list is fresh for the current user
 useEffect(() => {
 ...
 // Close detail panel when a new advocate is selected from the list
 ...
 {/* Oracle panel */}
 <RegisterOraclePanel title="Advisor, Program & Support Network">
 <div className="space-y-4">
 <div>
 ...
 advisor, program, alumni, and student-advocate relationships as a first-class graph object. Edges encode relationships like
 <code className="text-xs tone-plum mx-1">follows - collaborates - reshared - inspired_by</code>
 with a numeric <span className="tone-sienna font-mono">strength</span> weight.
 Graph traversal finds multi-hop support and outreach paths (up to 5 hops) without any external graph database.
 </p>
 </div>
 <div className="flex flex-wrap gap-1.5">
 ...
 <FeatureBadge label="Support Network Scoring" color="pink" />
 <FeatureBadge label="CONNECT BY" color="blue" />
 <FeatureBadge label="Program Attribution" color="green" />
 </div>
	 <SqlBlock code={`-- ISO SQL/PGQ: 5-hop support path
	SELECT reached.advocate_id, reached.handle,
	 reached.audience_size, reached.advocate_score
	FROM GRAPH_TABLE(
	 student_success_network
	 MATCH
	 (seed IS success_advocate {handle: :handle})
	 -[e IS connection]->{1,5} -- 1 to 5 hops
	 (reached IS success_advocate)
	 COLUMNS (
	 reached.advocate_id,
	 reached.handle,
	 reached.audience_size,
	 reached.advocate_score
	 )
	)
	ORDER BY advocate_score DESC
	FETCH FIRST 50 ROWS ONLY;`} />
	 <SqlBlock code={`-- Create the property graph over higher education semantic views
	CREATE PROPERTY GRAPH student_success_network
	 VERTEX TABLES (
	 success_advocates_v KEY (advocate_id) LABEL success_advocate
	 PROPERTIES (advocate_id, handle, display_name,
	 signal_channel, audience_size, engagement_rate,
	 advocate_score, advocate_focus, city, region, is_verified),
	 academic_programs_v KEY (program_id) LABEL academic_program
	 PROPERTIES (program_id, academic_program,
	 program_category, program_tier),
	 student_services_v KEY (service_id) LABEL student_service
	 PROPERTIES (service_id, service_name,
	 service_category, service_value_proxy),
	 student_signal_posts_v KEY (signal_id) LABEL student_signal
	 PROPERTIES (signal_id, signal_channel, posted_at,
	 urgency_score, momentum_flag)
	 )
	 EDGE TABLES (
	 influencer_connections KEY (connection_id)
	 SOURCE KEY (from_influencer)
	 REFERENCES success_advocates_v (advocate_id)
	 ...
	 REFERENCES academic_programs_v (program_id)
	 LABEL supports_program
	 PROPERTIES (relationship_type, post_count,
	 avg_engagement, service_value_attributed),
	 post_product_mentions KEY (mention_id)
	 SOURCE KEY (post_id)
	 REFERENCES student_signal_posts_v (signal_id)
	 DESTINATION KEY (product_id)
	 REFERENCES student_services_v (service_id)
	 LABEL mentions_service
	 PROPERTIES (confidence_score, mention_type)
	 );`} />
 <div className="grid grid-cols-2 gap-1.5 mt-2">
 <DiagramBox label="success advocates" sub="Vertex table" color="#C74634" />
 <DiagramBox label="advocate connections" sub="Edge table" color="#AA643B" />
 <DiagramBox label="program relationships" sub="Program edges" color="#A36472" />
 <DiagramBox label="service mentions" sub="Attribution" color="#4C825C" />
 </div>
 <div>
 ...
 <FeatureBadge label="Region Filtering" color="blue" />
	 ...
	 <SqlBlock code={`-- VPD policy function (applied to graph source views)
	CREATE FUNCTION vpd_graph_success_advocates(
...
-- Applied via DBMS_RLS.ADD_POLICY to:
	-- success_advocates_v, student_signal_posts_v,
	-- advocate connections, program support links,
	-- student service mentions`} />
 </div>
 </RegisterOraclePanel>
 ...
 <Network className="text-[var(--color-accent)]" /> Advisor, Program & Support Network
 </h2>
 <p className="text-sm text-[var(--color-text-dim)] mt-1">
 Explore advisor, student, alumni, and community-advocate relationships via <span className="tone-sienna">Oracle Property Graph (SQL/PGQ)</span> - hover vertices/edges for details, click to explore
 </p>
 </div>
 ...
 <Users size={11} /> Success Advocates {rawInfluencers?.length ? `(${rawInfluencers.length})` : ''}
 </h3>
 {loading ? (
 ...
 <span className="font-medium truncate">{inf.HANDLE}</span>
	 <span className={`platform-badge platform-${(inf.PLATFORM || '').toLowerCase()} !text-[9px] !py-0`}>{platformLabel(inf.PLATFORM)}</span>
 </div>
 <div className="flex items-center gap-2 mt-0.5 text-[10px] text-[var(--color-text-dim)]">
 <span>{formatNumber(inf.FOLLOWER_COUNT)} audience</span>
 <span className="text-[var(--color-accent)]">Score {inf.INFLUENCE_SCORE}</span>
 ...
 {/* Selected advocate metrics + stats bar */}
 {network && (
 <div className="glass-card p-3">
 {network.center && (
 ...
 <span className="tone-pine text-xs">OK</span>
	 ...
	 <span className={`platform-badge platform-${(network.center.PLATFORM || '').toLowerCase()} !text-[9px] !py-0 ml-1`}>{platformLabel(network.center.PLATFORM)}</span>
 {network.center.NICHE && <span className="text-[10px] text-[var(--color-text-dim)] font-normal ml-1">{network.center.NICHE}</span>}
 </p>
 <p className="text-[10px] text-[var(--color-text-dim)]">Center node - highest support score in network</p>
 </div>
 </div>
 )}
 <div className="grid grid-cols-7 gap-2">
 {[
 { label: 'Audience', value: formatNumber(network.center?.FOLLOWER_COUNT || 0) },
 { label: 'Support Score', value: network.center?.INFLUENCE_SCORE || 0 },
 ...
 <div className="glass-card p-14 text-center text-[var(--color-text-dim)]">
 <Network size={28} className="mx-auto mb-3 opacity-40" />
 Select a success advocate to explore their support network
 </div>
 )}
 ...
 Academic Program Relationships - {network.center?.HANDLE}
 </h4>
 ...
 ${formatNumber(b.REVENUE_ATTRIBUTED)} impact
 </span>
 )}
 </div>
 ))}
