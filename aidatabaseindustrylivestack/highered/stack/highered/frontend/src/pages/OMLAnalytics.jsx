...
const SEGMENT_COLORS = {
 'Retention Champion': '#AA643B',
 Persisting: '#4C825C',
 'Newly Engaged': '#4F7D7B',
 'At Risk': '#C74634',
 Disconnected: '#7A736E',
 'High Support Value': '#796087',
 'Momentum Building': '#437C94',
 'Needs Nudge': '#6F757E',
};
...
const TABS = [
 { key: 'demand', label: 'Student Demand Predictions', buttonLabel: 'Demand Surge', iconClass: 'oj-fwk-icon-sortrelevancehigh', color: '#AA643B' },
 { key: 'rfm', label: 'Student Engagement Segmentation', buttonLabel: 'Engagement Segments', iconClass: 'oj-fwk-icon-users', color: '#C74634' },
 { key: 'forecast', label: 'Student Service Demand and Value Forecast - Linear Regression', buttonLabel: 'Service Value Forecast', iconClass: 'oj-fwk-icon-view', color: '#4C825C' },
...
function MomentumBadge({ flag }) {
 const label = flag === 'mega_viral' ? 'Critical' : flag === 'viral' ? 'High' : flag?.replace('_', ' ') || ' - ';
 return (
...
function DemandOraclePanel() {
 ...
 <code className="text-xs tone-sienna">DBMS_DATA_MINING.CREATE_MODEL</code> on 12 student-signal engagement
 and service-value features. Oracle scores every student service <em>inline</em> at query time using{' '}
...
ORDER BY surge_probability DESC;`} />
 ...
 <div className="text-center text-[10px] text-[var(--color-text)]">down CREATE_MODEL</div>
 <DiagramBox label="DEMAND_SURGE_MODEL (Random Forest)" sub="ALGO_RANDOM_FOREST - PREP_AUTO" color="#C74634" />
 <div className="text-center text-[10px] text-[var(--color-text)]">down PREDICTION()</div>
 <DiagramBox label="Real-Time Scoring in SQL" sub="PREDICTION_PROBABILITY('SURGE' USING *)" color="#437C94" />
 <div className="text-center text-[10px] text-[var(--color-text)]">down result</div>
...
function RFMOraclePanel() {
 ...
 <div>
 <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
	 Student Engagement Segmentation - K-Means Clustering
 </p>
 <p className="text-sm text-[var(--color-text)] leading-relaxed">
 A <span className="tone-plum font-mono">K-Means</span> model (4 clusters) trained via{' '}
	 <code className="text-xs tone-plum">DBMS_DATA_MINING.CREATE_MODEL</code> on 6 retention and engagement features.
 Each synthetic student is assigned to a cluster using{' '}
 ...
 <code className="text-xs tone-plum">CLUSTER_PROBABILITY()</code> confidence.
	 Retention quartile labels (Retention Champion, Persisting, At Risk, etc.) are layered on top via NTILE(4) window functions.
 </p>
 </div>
 <div className="flex flex-wrap gap-1.5">
...
FROM student_metrics cm
 ...
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down CREATE_MODEL</div>
	 <DiagramBox label="STUDENT_SUCCESS_SEGMENT_MODEL (K-Means)" sub="ALGO_KMEANS - 4 clusters - PREP_AUTO" color="#796087" />
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down CLUSTER_ID()</div>
 <DiagramBox label="Cluster Assignment + Probability" sub="each synthetic student -> nearest centroid" color="#437C94" />
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down NTILE(4)</div>
	 <DiagramBox label="Retention Segment Labels + Persistence Risk" sub="Retention Champion - Persisting - At Risk - Needs Nudge" color="#4C825C" />
 </div>
 </div>
 );
}
...
function ForecastOraclePanel() {
 ...
 <div>
 <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
	 Student Support Value Model - GLM Regression + OLS Trend
 </p>
 <p className="text-sm text-[var(--color-text)] leading-relaxed">
 Two complementary Oracle ML techniques:{' '}
	 <code className="text-xs tone-pine">SERVICE_VALUE_MODEL</code> (Generalized Linear Model)
 trained via <code className="text-xs tone-pine">DBMS_DATA_MINING</code> predicts per-request service value
 from synthetic student and student service features. The time-series trend uses{' '}
...
FROM daily_value CROSS JOIN params CROSS JOIN glm_stats;`} />
 ...
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down CREATE_MODEL</div>
	 <DiagramBox label="SERVICE_VALUE_MODEL (GLM)" sub="ALGO_GENERALIZED_LINEAR_MODEL - PREP_AUTO" color="#C74634" />
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down PREDICTION()</div>
 <DiagramBox label="Per-Request Service Value Prediction" sub="GLM scores each student request inline in SQL" color="#437C94" />
...
function ClustersOraclePanel() {
 ...
 <div>
 <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
	 Student Service Cluster Model - K-Means Clustering
 </p>
 <p className="text-sm text-[var(--color-text)] leading-relaxed">
 A <span className="tone-teal font-mono">K-Means</span> model trained via{' '}
 <code className="text-xs tone-teal">DBMS_DATA_MINING.CREATE_MODEL</code> on 8 student service behavioral
 features (value, utilization, engagement, sentiment). Student Services are assigned using{' '}
 ...
 scores new student services automatically.
 </p>
 </div>
 <div className="flex flex-wrap gap-1.5">
...
-- avg_virality`} />
 ...
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down CREATE_MODEL</div>
	 <DiagramBox label="STUDENT_SERVICE_CLUSTER_MODEL (K-Means)" sub="ALGO_KMEANS - configurable clusters - PREP_AUTO" color="#AA643B" />
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down CLUSTER_ID()</div>
 <DiagramBox label="Cluster Assignment + Probability" sub="trained centroids - proper distance calculation" color="#796087" />
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down JOIN</div>
 <DiagramBox label="Student Service Details + Cluster Stats" sub="size - top category - avg probability" color="#4C825C" />
 </div>
 </div>
 );
}
...
function CapacityOraclePanel() {
 ...
 <div>
 <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
 DEMAND_SURGE_MODEL + Capacity - Campus Services Risk Intelligence
 </p>
 <p className="text-sm text-[var(--color-text)] leading-relaxed">
 Joins <span className="tone-plum font-mono">DEMAND_SURGE_MODEL</span> (Random Forest) predictions with
 live capacity levels across all campus service centers. Oracle scores each student service in real-time using{' '}
 <code className="text-xs tone-plum">PREDICTION_PROBABILITY()</code>, then compares predicted demand
 against available capacity to identify access risk - student services where student-signal-driven demand will exceed capacity.
 ...
 <FeatureBadge label="capacity at campus service centers" color="cyan" />
...
ORDER BY oml_surge_probability DESC;`} />
 ...
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down scores stored in</div>
 <DiagramBox label="demand_forecasts (daily OML predictions)" sub="predicted_demand - social_factor - confidence band" color="#A36472" />
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down JOIN</div>
 <DiagramBox label="capacity at campus service centers" sub="available slots - intervention threshold - service centers" color="#437C94" />
 <div className="text-center text-[10px] text-[var(--color-text-dim)]">down COMPARE</div>
 <DiagramBox label="Campus Services Risk: capacity_status + days_of_capacity + student_service_at_risk" sub="NO_CAPACITY - CRITICAL - AT_RISK - ADEQUATE" color="#C74634" />
 </div>
 </div>
 );
}
...
export default function OMLAnalytics() {
 ...
 <JetGlyph iconClass="oj-fwk-icon-view" className="oml-header-glyph tone-plum" /> Predictive Student Success Analytics
 </h2>
 ...
 <RegisterOraclePanel title="Predictive Student Success Analytics">
 ...
 {/* ==========================================
 Tab 1 - Student Demand Predictions
 ========================================== */}
 {activeTab === 'demand' && (
 <section
 id="oml-panel-demand"
 ...
 Student services scored by DEMAND_SURGE_MODEL - Oracle DBMS_DATA_MINING Random Forest
 </p>
 </div>
 <div className="flex items-center gap-2">
 <JetSelectSingle
 ...
 <p className="text-sm text-[var(--color-text-dim)] py-4 text-center">No student services with sufficient student-signal activity in this window.</p>
 ) : (
 <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
 {/* Bar chart - predicted demand */}
 <div className="lg:col-span-2">
 <p className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
 Top 10 - Predicted Student Requests (7-day horizon)
 </p>
 ...
 formatter={(v, n) => [formatNumber(v), n === 'PREDICTED_DEMAND' ? 'Predicted Student Requests' : n]}
 />
 ...
 <th className="text-left py-2 px-2">Student Service</th>
 ...
 <span><strong>Features:</strong> 12 - category, value, posts, sentiment, likes, shares, views, urgency, high_momentum_posts, rising_posts, units_requested, service_value</span>
 ...
 {/* ==========================================
 Tab 2 - Student Engagement Segmentation
 ========================================== */}
 {activeTab === 'rfm' && (
 <section
 id="oml-panel-rfm"
 ...
 <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
	 STUDENT_SUCCESS_SEGMENT_MODEL (K-Means, 4 clusters) via DBMS_DATA_MINING +{' '}
 <code className="tone-plum">NTILE(4)</code> RFM labeling - CLUSTER_ID() scoring
 </p>
 </div>

 {segLoading ? (
	 <p className="text-sm text-[var(--color-text-dim)] py-4 text-center">Scoring synthetic students via CLUSTER_ID(STUDENT_SUCCESS_SEGMENT_MODEL)...</p>
 ) : (
 ...
 labelStyle={{ color: 'var(--color-text)' }}
 cursor={false}
 formatter={(v, n, p) => [`${v} synthetic students`, p.payload.segment]}
 />
 </PieChart>
 </ResponsiveContainer>
 {/* Legend */}
 ...
 {/* Retention risk bar + segment table */}
 <div className="space-y-4">
 <div>
 <p className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Retention Risk Distribution</p>
 ...
 labelStyle={{ color: 'var(--color-text)' }}
 cursor={{ fill: 'rgba(49,45,42,0.08)' }}
 formatter={v => [`${v} synthetic students`, 'Count']}
 />
 </BarChart>
 </ResponsiveContainer>
 </div>
 ...
 <span>{s.count} synthetic students</span>
 <span className="tone-sienna">{formatCurrency(s.total_revenue)} impact</span>
 ...
 {/* Student table - filtered by selected segment */}
 <div>
 <div className="flex items-center justify-between mb-2">
 <p className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider">
 {selectedSegment ? `${selectedSegment} synthetic students` : 'Top synthetic students by RFM score'}
 </p>
 {selectedSegment && (
 <JetButton
 label="Clear"
 ...
 {filteredStudents.slice(0, 40).map((c, i) => (
 ...
 <span><strong>Segments:</strong> Retention Champion - Persisting - Newly Engaged - At Risk - Disconnected - High Support Value - Momentum Building - Needs Nudge</span>
 ...
 {/* ==========================================
 Tab 3 - Student Service Demand and Value Forecast
 ========================================== */}
 {activeTab === 'forecast' && (
 <section
 ...
 Student Service Demand and Value Forecast - Oracle Linear Regression
 </h3>
 <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
 ...
 {[
 { label: 'R2 (fit quality)', value: `${((model.r_squared || 0) * 100).toFixed(1)}%`, color: model.r_squared > 0.7 ? '#4C825C' : model.r_squared > 0.4 ? '#AA643B' : '#C74634' },
 { label: 'Daily Slope', value: `${model.daily_slope >= 0 ? '+' : ''}${formatCurrency(model.daily_slope)}/day`, color: model.daily_slope >= 0 ? '#4C825C' : '#C74634' },
 { label: 'Mean Daily Support Value', value: formatCurrency(model.mean_daily_revenue), color: '#C74634' },
 ...
 <span><strong>Oracle functions:</strong> {model.engine}</span>
	 <span><strong>R2:</strong> {(model.r_squared * 100).toFixed(1)}%
 ...
 <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
 Student Services clustered by semantic similarity using <code className="tone-teal">VECTOR_DISTANCE(COSINE)</code> on
 ...
 { label: 'Student Services Clustered', value: clusterData.total_products, color: '#C74634' },
 ...
 Cluster {cl.cluster_id}: {cl.size} student services - {cl.top_category}
 </div>
 </div>
 ))}
 </div>
 ...
 {cl.size} student services - Avg similarity: <span className="font-mono" style={{ color }}>{(cl.avg_similarity * 100).toFixed(1)}%</span>
 ...
 <span className="font-medium truncate block">
	 {p.is_centroid && <span style={{ color }} className="mr-1">Seed</span>}
	 {p.service_name || p.product_name}
 </span>
 <span className="text-[9px] text-[var(--color-text-dim)]">
	 {p.academic_program || p.brand_name} - {p.service_category || p.category} - {formatCurrency(p.service_value_proxy || p.unit_price)}
 </span>
 </div>
 ...
 +{cl.products.length - 12} more student services
 </div>
 )}
 </div>
 </div>
 ...
 <span><strong>K:</strong> {clusterData.k} clusters - {clusterData.total_products} student services</span>
 </div>
 </>
 )}
 </section>
 ...
 <p className="text-xs text-[var(--color-text-dim)] mt-0.5">
	 DEMAND_SURGE_MODEL predictions with live capacity - identifies campus services risk from student-signal-driven demand surges
 </p>
 </div>
 <JetButton
 ...
 <th className="text-right py-2 px-2">Service Value at Risk</th>
 </tr>
 </thead>
 <tbody>
 ...
 <span><strong>Data:</strong> demand_forecasts (daily OML predictions) + capacity at campus service centers</span>
 <span><strong>Engine:</strong> Oracle DBMS_DATA_MINING - student-signal demand surge to campus services risk assessment</span>
 </div>
 </>
 )}
 </section>
