...
import { SceneStoryPanel } from './components/HigherEdStory';
...
const CUSTOMER_STORAGE_KEY = 'higheredLivestack.customerName';
const CUSTOMER_ALIASES = [
  'Seer Higher Education',
  'Seer Higher Ed',
  'Seer State University',
  'Seer State',
];
...
const NAV_ITEMS = [
  ...
  { id: 'dashboard', label: 'Student Success Command Center', iconClass: 'oj-fwk-icon oj-fwk-icon-grid', featureTags: ['In-Memory', 'Native JSON'] },
  { id: 'social', label: 'Student Intent & Support Signals', iconClass: 'oj-fwk-icon oj-fwk-icon-sortrelevancehigh', featureTags: ['Vector Search', 'VPD'] },
  { id: 'graph', label: 'Advisor, Program & Support Network', iconClass: 'oj-fwk-icon oj-fwk-icon-node-expand', featureTags: ['Property Graph', 'SQL/PGQ'] },
  { id: 'fulfillment', label: 'Campus Service Coverage', iconClass: 'oj-fwk-icon oj-fwk-icon-calendar-clock', featureTags: ['Oracle Spatial', 'VPD'] },
  { id: 'orders', label: 'Student Requests & Cases', iconClass: 'oj-fwk-icon oj-fwk-icon-tree-document', featureTags: ['JSON Duality', 'VPD'] },
  { id: 'oml', label: 'Predictive Student Success Analytics', iconClass: 'oj-fwk-icon oj-fwk-icon-view', featureTags: ['OML', 'Vector K-Means'] },
  { id: 'askdata', label: 'Ask Seer Higher Ed Data', iconClass: 'oj-fwk-icon oj-fwk-icon-magnifier', featureTags: ['Natural Language SQL', 'Live Schema'] },
  { id: 'agents', label: 'Student Success Agent Console', iconClass: 'oj-fwk-icon oj-fwk-icon-users', featureTags: ['PL/SQL Tools', 'JSON Audit'] },
];
...
function replaceCustomerTerms(value, profile) {
  ...
  let next = value.replace(/\bseer-higher-ed\.example\b/gi, profile.customerDomain);
...
function hasReplaceableCustomerTerms(value) {
  return typeof value === 'string' && (
    /\bseer-higher-ed\.example\b/i.test(value) ||
    /\b(?:Seer Higher Education|Seer Higher Ed|Seer State University|Seer State|Seer)\b/.test(value)
  );
}
    ...
    <div className="customer-name-tool">
      ...
      {isOpen && (
        <form className="customer-name-tool__panel" onSubmit={submit}>
          <label className="customer-name-tool__label" htmlFor="customer-name-input">Replace demo institution</label>
          <input
          ...
          <p className="customer-name-tool__hint">Replaces Seer Higher Education, Seer Higher Ed, Seer State University, Seer State, and Seer labels on visible screens.</p>
        </form>
      )}
    </div>
  );
...
function AppShell({ activePage, setActivePage }) {
  ...
  const datasetLabel = useMemo(() => {
    if (!activeDataset) return `${customerProfile.customerName} synthetic student-success data loaded`;
          ...
          <div className="app-shell">
            ...
            <div className="app-main">
              <header className="app-topbar">
                ...
                <JetButton
                  label="Use Your Own Student Success Data"
                ...
                <div className="app-page-frame">
                  {activePage === 'welcome' ? (
                    ...
                    (() => {
                      ...
                      return <><SceneStoryPanel scene={activePage} /><PageComponent {...pageProps} /></>;
                    })()
                  )}
                </div>
              </main>
