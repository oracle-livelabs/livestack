...
/*
-- Find higher education advocates within 2 hops of an urgent signal post author
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE v1.handle = '@student_success'
    COLUMNS (
        v2.handle,
        v2.influence_score,
        v2.follower_count
...
-- Find shortest propagation path between two higher education advocates
SELECT *
FROM GRAPH_TABLE ( influencer_network
    MATCH SHORTEST (a IS influencer) -[e IS connects_to]->{1,5} (b IS influencer)
    WHERE a.handle = '@tutoring_mark'
...
-- Academic program propagation: which advocates propagate a academic program through the network
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE b.brand_name = 'Northstar Advising Network'
    COLUMNS (
        v1.handle, v2.handle,
        e1.relationship_type,
        e2.connection_type,
