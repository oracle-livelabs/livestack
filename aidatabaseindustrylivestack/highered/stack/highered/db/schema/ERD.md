...
## Higher education semantic mapping

The physical object names remain compatible with the portable LiveStack baseline and import contract. The higher education demo should describe them with these business meanings:

| Physical object | Demo meaning | Notes |
|---|---|---|
| `customers` | synthetic students | No real student PII is required. |
| `products` | student services and capacity items | Used by vector search and JSON duality examples. |
| `brands` | academic programs and service owners | Used for program/service ownership and graph links. |
| `orders` | student service requests | Used by transactions, request workflow, and duality views. |
| `fulfillment_centers` | campus service sites | Used by Oracle Spatial routing and capacity views. |
| `influencers` | success advocates and community signal sources | Used by Property Graph traversal. |
| `social_posts` | student and community signal posts | Used by momentum scoring and embeddings. |
| `shipments` | campus service route records | Used by service-route and response-time examples. |

Semantic views in `10_highered_views.sql` expose these meanings directly for demos and natural-language SQL.
