...
echo ">>> Healthcare bootstrap starting inside db container..."
...
@${APP_DIR}/db/schema/10_care_pathway_graph.sql
@${APP_DIR}/db/schema/11_healthcare_semantic_views.sql
EXIT
SQL
...
echo ">>> Installing private Healthcare context and fail-closed VPD policies..."
...
sqlplus -L -s "$ADMIN_CONNECT" \
  @"${APP_DIR}/db/schema/06a_healthcare_app_context_admin.sql" \
  "$APP_SCHEMA_USER_UPPER"
sqlplus -L -s "$APP_CONNECT" @/tmp/bootstrap_security_policies.sql
sqlplus -L -s "$ADMIN_CONNECT" @"${APP_DIR}/db/schema/16_healthcare_unified_audit_admin.sql" "$APP_SCHEMA_USER_UPPER"
...
BEGIN
  healthcare_security_pkg.set_actor_context('admin_jess');
END;
/
...
BEGIN
  healthcare_security_pkg.clear_actor_context;
END;
/
@${APP_DIR}/db/schema/10_care_pathway_graph.sql
@${APP_DIR}/db/data/load_care_pathway_graph.sql
@${APP_DIR}/db/schema/11_healthcare_semantic_views.sql
