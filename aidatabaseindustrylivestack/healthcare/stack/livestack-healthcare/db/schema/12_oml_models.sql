/*
 * 12_oml_models.sql
 * Healthcare OML lifecycle: four persisted, idempotently rebuilt models.
...
CREATE OR REPLACE PROCEDURE rebuild_healthcare_oml_models
AUTHID DEFINER
AS
...
END rebuild_healthcare_oml_models;
/

BEGIN
  rebuild_healthcare_oml_models;
END;
/
...
BEGIN
  ...
  IF v_count <> 4 THEN
    RAISE_APPLICATION_ERROR(-20071, 'Healthcare OML lifecycle did not create all four required models');
  END IF;
END;
/
