/*
 * 01_tables.sql
 * Core relational tables for Healthcare Signal Intelligence & Smart Fulfillment
...
CREATE TABLE influencers (
    ...
    platform          VARCHAR2(50) DEFAULT 'Supply Quality Notice'
                      CHECK (platform IN (
...
-- Durable singleton lease.  Dataset Restore workers must hold this Oracle
-- row; a process-local mutex is not authoritative across application restart.
...
INSERT INTO app_dataset_operation_lock (lock_id, updated_at) VALUES (1, SYSTIMESTAMP);
...
CREATE TABLE app_dataset_jobs (
    ...
    warnings_json    CLOB, errors_json CLOB, counts_json CLOB, summary_json CLOB,
    active_dataset_json CLOB, details_json CLOB, recovery_json CLOB,
    worker_id        VARCHAR2(100), lease_token VARCHAR2(100), lease_expires_at TIMESTAMP,
...
-- Journaled generation protocol.  A durable serialized copy of the prior
-- importable base data is captured before destructive work.  OML model DDL
-- can commit implicitly, so startup uses this journal to restore the prior
-- base generation and rebuild every required derived feature before serving.
