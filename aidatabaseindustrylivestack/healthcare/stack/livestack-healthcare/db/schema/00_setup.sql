 ...
 *   2. Connect as the application schema owner and run the schema scripts in order,
 *      including 11_healthcare_semantic_views.sql for healthcare-named query objects.
 *
 * SQLcl example:
...
GRANT EXECUTE ON SYS.DBMS_XPLAN TO &&APP_SCHEMA_USER;
GRANT SELECT ON SYS.V_$SQL TO &&APP_SCHEMA_USER;
GRANT SELECT ON SYS.V_$SQL_PLAN TO &&APP_SCHEMA_USER;
GRANT SELECT ON SYS.V_$SQL_PLAN_STATISTICS_ALL TO &&APP_SCHEMA_USER;
GRANT SELECT ON SYS.V_$SESSION TO &&APP_SCHEMA_USER;
...
-- Unified Auditing is installed and enabled by ADMIN through
-- 16_healthcare_unified_audit_admin.sql. The app schema receives no broad
...
SELECT 'Setup complete.' ||
       ' Connect as ' || UPPER('&&APP_SCHEMA_USER') ||
       ' and run the schema scripts in order, including 11_healthcare_semantic_views.sql.' AS next_step
FROM   dual;
