...
COMMENT ON TABLE brand_influencer_links IS
  'Many-to-many relationship between healthcare partners and signal sources. Tracks signal counts, source activity, and service-value attribution.';

COMMENT ON TABLE demand_regions IS
  'Geographic regions used for healthcare service demand, capacity pressure, and logistics planning. Each region has a boundary polygon, population, income, healthcare signal density, and operational demand index.';
...
COMMENT ON TABLE product_attributes IS
  'Extended care-service and supply attributes stored as JSON. Holds category-specific metadata like hazard class, purity, storage constraints, and specifications.';

COMMENT ON TABLE product_embeddings IS
  'Vector embeddings of care service and supply descriptions. 384-dim vectors from ALL_MINILM_L12_V2 ONNX model for semantic healthcare service matching.';

COMMENT ON TABLE semantic_matches IS
  'Cached results of vector similarity matching between healthcare signals and care services or supplies. Stores similarity scores and match methods.';


COMMENT ON TABLE order_items IS
  'Line items within a healthcare service request. Each links to a care service or supply with quantity and service value.';

COMMENT ON TABLE products IS
  'Healthcare services and supplies available for service requests. The inherited table name products is retained for compatibility. Use care_services_v for healthcare-facing questions about care services, supplies, categories, and service value.';
...
COMMENT ON COLUMN agent_actions.agent_name IS
  'Name of the AI agent that took the action. Healthcare-facing labels map inherited team names to Quality Signal Agent, Care Logistics Agent, and Care Service Request Agent.';
...
COMMENT ON COLUMN agent_actions.entity_type IS
  'Target entity type such as care_service, healthcare_partner, signal_source, service_request, or signal. Some legacy rows may use product, manufacturer, or order.';
COMMENT ON COLUMN agent_actions.entity_id IS
  'FK to the target entity. In healthcare-facing usage, product_id means care_service_id, brand_id means partner_id, and influencer_id means signal_source_id.';
...
COMMENT ON COLUMN app_users.role IS
  'User role: admin, analyst, fulfillment_mgr, care_coordinator, or viewer';
...
COMMENT ON COLUMN brands.brand_category IS
  'Manufacturer category (e.g. Biologics Manufacturing, Diagnostics, Care Logistics). This is the manufacturer category, not the product category. Use products.category for product categories.';
...
COMMENT ON COLUMN brands.annual_revenue IS
  'Annual operating value or organization revenue in US dollars for the healthcare partner or network.';
...
COMMENT ON COLUMN brand_influencer_links.brand_id IS
  'FK to brands, used as healthcare provider networks, service partners, manufacturers, or logistics partners. JOIN brands ON brands.brand_id = brand_influencer_links.brand_id';
COMMENT ON COLUMN brand_influencer_links.influencer_id IS
  'FK to influencers, used as signal sources in this healthcare demo. JOIN influencers ON influencers.influencer_id = brand_influencer_links.influencer_id';
...
COMMENT ON COLUMN brand_influencer_links.revenue_attributed IS
  'Service value or operating value attributed to this healthcare partner-source relationship.';
...
COMMENT ON COLUMN demand_forecasts.product_id IS
  'FK to products, used as healthcare operations in this demo. JOIN products ON products.product_id = demand_forecasts.product_id';
...
COMMENT ON COLUMN demand_regions.social_density IS
  'Healthcare signal density score 0-100 for quality, capacity, regulatory, or logistics signals in this region.';
COMMENT ON COLUMN demand_regions.demand_index IS
  'Canonical numeric Care Demand Index 0-100 combining population, care access, capacity pressure, and healthcare signal density. Use care_demand_regions_v for healthcare-friendly display labels.';
...
COMMENT ON COLUMN fulfillment_zones.zone_type IS
  'Canonical logistics zone key: express, overnight, standard, or economy. Use care_logistics_zones_v for healthcare-friendly display names such as Urgent Care Logistics and Routine Replenishment.';
...
COMMENT ON COLUMN influencers.niche IS
  'Signal niche: FDA, EMA, GxP, care logistics, port logistics, products, or manufacturer quality';
...
COMMENT ON COLUMN order_items.line_total IS
  'Service value in US dollars. Equals quantity multiplied by unit_price for the healthcare service-request line.';
...
COMMENT ON COLUMN post_product_mentions.product_id IS
  'FK to products, used as healthcare operations. JOIN products ON products.product_id = post_product_mentions.product_id';
...
COMMENT ON COLUMN product_attributes.product_id IS
  'FK to products, used as healthcare operations. JOIN products ON products.product_id = product_attributes.product_id';
...
COMMENT ON COLUMN product_embeddings.product_id IS
  'FK to products, used as healthcare operations. JOIN products ON products.product_id = product_embeddings.product_id';
...
COMMENT ON COLUMN semantic_matches.product_id IS
  'FK to products. The healthcare service matched to the signal.';
COMMENT ON COLUMN semantic_matches.similarity_score IS
  'Cosine similarity score 0.0 to 1.0 between healthcare signal embeddings and care service or supply embeddings.';
...
COMMENT ON COLUMN shipments.order_id IS
  'Compatibility FK to orders. For demo-facing SQL, query care_logistics_routes_v or join care_service_requests.service_request_id to shipments.order_id.';
...
COMMENT ON COLUMN shipments.carrier IS
  'Care logistics carrier or partner name, such as CryoLine, Specialty Care Courier, SafeTemp, or Clinical Express.';
...
COMMENT ON COLUMN shipments.ship_cost IS
  'Logistics cost in US dollars for the healthcare service request.';
...
COMMENT ON COLUMN social_posts.platform IS
  'Healthcare operations signal channel such as Supply Quality Notice, Compliance Signal, Cold Chain Bulletin, Regulatory Notice, or Capacity Alert.';
...
COMMENT ON COLUMN social_posts.detected_products IS
  'Comma-separated list of care services or supplies detected in this signal by NLP.';
...
-- ============================================================
-- HEALTHCARE SEMANTIC COMMENT OVERRIDES
-- These comments keep inherited physical names stable while improving
-- Ask Data, Select AI, and presenter-facing healthcare terminology.
-- ============================================================

COMMENT ON TABLE brand_influencer_links IS
  'Many-to-many relationship between healthcare partners and signal sources. Tracks signal counts, source activity, and service-value attribution while retaining the inherited table name.';

COMMENT ON TABLE demand_regions IS
  'Geographic regions used for healthcare service demand, capacity pressure, and logistics planning. Each region has a boundary polygon and operational demand index.';

COMMENT ON TABLE product_attributes IS
  'Extended care-service and supply attributes stored as JSON. Holds healthcare-specific metadata such as hazard class, purity, storage constraints, specifications, and handling requirements.';

COMMENT ON TABLE product_embeddings IS
  'Vector embeddings of care service and supply descriptions. 384-dim vectors from ALL_MINILM_L12_V2 ONNX model for semantic healthcare service matching.';

COMMENT ON TABLE semantic_matches IS
  'Cached vector, keyword, hybrid, or visual similarity matches between healthcare signals and care services or supplies.';

COMMENT ON TABLE order_items IS
  'Line items within a healthcare service request. Each links to a care service or supply with quantity and service value.';

COMMENT ON TABLE products IS
  'Healthcare care services, supplies, diagnostics, logistics offerings, and operational service bundles. The inherited table name products is retained for application compatibility; use care_services_v for healthcare-facing Ask Data questions.';

COMMENT ON COLUMN agent_actions.entity_type IS
  'Target entity type for the AI agent action, such as care_service, service_request, capacity_supply, signal_source, or signal. Some legacy rows may use product, order, inventory, shipment, or manufacturer.';

COMMENT ON COLUMN brands.brand_category IS
  'Healthcare partner category such as provider network, diagnostics, biologics manufacturing, quality advisory, or care logistics. The inherited column name is retained for compatibility.';

COMMENT ON COLUMN brands.annual_revenue IS
  'Annual operating value or organization revenue in US dollars for the healthcare partner or network.';

COMMENT ON COLUMN brand_influencer_links.brand_id IS
  'FK to brands, used as healthcare provider networks, service partners, manufacturers, or logistics partners in this demo.';

COMMENT ON COLUMN brand_influencer_links.influencer_id IS
  'FK to influencers, used as healthcare signal sources in this demo.';
...
COMMENT ON COLUMN demand_forecasts.product_id IS
  'FK to products, used as care services and supplies in this healthcare demo. JOIN products ON products.product_id = demand_forecasts.product_id or use care_services_v.';

COMMENT ON COLUMN influencers.niche IS
  'Healthcare signal-source niche such as FDA, EMA, GxP, care logistics, facility bulletin, capacity alert, product safety, or quality advisory.';

COMMENT ON COLUMN inventory.quantity_incoming IS
  'Units currently in transit or on order for the care logistics site.';

COMMENT ON COLUMN orders.shipping_lat IS
  'Care-site or logistics destination latitude for spatial routing.';

COMMENT ON COLUMN orders.shipping_lon IS
  'Care-site or logistics destination longitude for spatial routing.';

COMMENT ON COLUMN orders.updated_at IS
  'Timestamp of last healthcare service-request status change.';

COMMENT ON COLUMN order_items.line_total IS
  'Service value in US dollars. Equals quantity multiplied by unit_price.';

COMMENT ON COLUMN order_items.fulfilled_from IS
  'FK to fulfillment_centers. The care logistics site assigned to this service-request line item. NULL if not yet assigned.';

COMMENT ON COLUMN products.sku IS
  'Unique care service or supply code retained in the inherited SKU column.';

COMMENT ON COLUMN products.description IS
  'Detailed care service or supply description text. Used for vector embeddings and full-text search.';

COMMENT ON COLUMN products.is_active IS
  '1=active and available for service request, 0=discontinued or hidden.';

COMMENT ON COLUMN products.launch_date IS
  'Date the care service or supply first became available for requests.';

COMMENT ON COLUMN product_attributes.attributes IS
  'JSON document with category-specific care service or supply attributes such as hazard class, purity, storage, handling, and specifications.';

COMMENT ON COLUMN product_embeddings.embedding_text IS
  'Source text embedded for care service or supply matching, typically service name, description, and tags.';

COMMENT ON COLUMN semantic_matches.similarity_score IS
  'Cosine similarity score from 0.0 to 1.0 between healthcare signal embeddings and care service or supply embeddings.';

COMMENT ON TABLE shipments IS
  'Healthcare logistics movement records for service requests. Tracks carrier or partner, distance, cost, and movement status while retaining the inherited table name shipments.';

COMMENT ON COLUMN shipments.center_id IS
  'FK to fulfillment_centers. The care logistics site assigned to this service request.';

COMMENT ON COLUMN shipments.tracking_number IS
  'Carrier or logistics partner tracking number for the healthcare logistics movement.';

COMMENT ON COLUMN shipments.ship_status IS
  'Canonical logistics route status key. Healthcare display labels are exposed through care_logistics_route_status_lookup, such as Request Received, Assigned, Packed, Dispatched, In Transit, Arriving, Delivered, or Logistics Exception.';

COMMENT ON COLUMN shipments.ship_cost IS
  'Logistics cost in US dollars for the healthcare service request.';

COMMENT ON COLUMN shipments.shipped_at IS
  'Timestamp when the healthcare logistics route was dispatched to the carrier or partner.';

COMMENT ON COLUMN shipments.delivered_at IS
  'Timestamp when the healthcare logistics movement was completed. NULL if not yet completed.';
