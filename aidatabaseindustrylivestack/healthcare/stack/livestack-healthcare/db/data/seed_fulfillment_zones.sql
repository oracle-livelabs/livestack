...
SET SERVEROUTPUT ON
PROMPT Rebuilding Healthcare care logistics zones with Oracle Spatial...
...
BEGIN
    ...
    IF v_active_centers = 0 OR v_valid_centers <> v_active_centers THEN
        RAISE_APPLICATION_ERROR(
            -20086,
            'Care logistics zone generation requires valid WGS84 points for every active site'
        );
    END IF;
END;
/
...
BEGIN
    ...
    IF v_valid_zones <> v_expected_zones
       ...
       OR v_inactive_zones <> 0 THEN
        RAISE_APPLICATION_ERROR(
            -20087,
            'Every active care logistics site must have four valid Oracle Spatial service-zone tiers'
        );
    END IF;

    DBMS_OUTPUT.PUT_LINE('Oracle Spatial care logistics zones verified: ' || v_valid_zones);
END;
/
