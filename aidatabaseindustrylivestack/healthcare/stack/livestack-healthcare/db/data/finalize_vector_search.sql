...
BEGIN
    SAVEPOINT healthcare_vector_rebuild;
    ...
    IF evidence_is_ready THEN
        DBMS_OUTPUT.PUT_LINE(
            ...
            v_post_vectors || ' quality signals, ' ||
    ...
    DBMS_OUTPUT.PUT_LINE(
        ...
        v_product_vectors || ' products, ' ||
        v_total_rows || ' quality signals, ' ||
        v_semantic_matches || ' deterministic matches.'
    );
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK TO healthcare_vector_rebuild;
        RAISE;
END;
/

PROMPT Healthcare Oracle AI Vector Search artifacts are ready.
