/*
 * Hydrate Oracle Spatial point geometry after care logistics sites and care
 * sites exist. Nearest-site routing depends on these indexed points.
 */

SET SERVEROUTPUT ON
PROMPT Hydrating care logistics and care-site Oracle Spatial points...
...
BEGIN
    ...
    IF v_expected_centers = 0
       ...
       OR v_hydrated_centers <> v_expected_centers THEN
        RAISE_APPLICATION_ERROR(
            -20083,
            'Care logistics Spatial points must all be valid WGS84 points matching longitude/latitude'
        );
    END IF;
    ...
    IF v_invalid_customer_coordinates <> 0
       OR v_hydrated_customers <> v_expected_customers THEN
        RAISE_APPLICATION_ERROR(
            -20084,
            'Care-site Spatial points must all be valid WGS84 points matching longitude/latitude'
        );
    END IF;

    DBMS_OUTPUT.PUT_LINE('Care logistics Spatial points hydrated: ' || v_hydrated_centers);
    DBMS_OUTPUT.PUT_LINE('Geocoded care-site Spatial points hydrated: ' || v_hydrated_customers);
END;
/
