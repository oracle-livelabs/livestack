...
function buildTemplateReadme() {
  return [
    ...
    '- Domain labels use the original schema names for compatibility: brands are manufacturers, products are healthcare operations, influencers are signal sources, and social_posts are regulatory or supply bulletins.',
...
function buildFallbackShipments(dataset, idMaps) {
  ...
  const carriers = ['CryoLine', 'Specialty Care Courier', 'SafeTemp', 'Clinical Express'];
  const shipStatusMap = {
    confirmed: 'preparing',
    processing: 'packed',
    shipped: 'in_transit',
...
async function applyOptionalFallbacks(connection, dataset, idMaps, warnings, progress) {
  ...
  if (!dataset.tables.demand_regions.provided) {
    ...
    if (!generatedDemandRegions.length) warnings.push('No fallback demand_regions could be generated because care site geospatial data was missing.');
  }
...
async function inspectDateSensitiveOmlRefresh(connection) {
  const hookResult = await execSql(connection, `
    ...
    WHERE object_type = 'PROCEDURE'
      AND object_name = 'REBUILD_HEALTHCARE_OML_MODELS'
      AND status = 'VALID'
  `);
  if (!(hookResult.rows || []).length) {
    throw new ImportError(
      'Required OML rebuild procedure REBUILD_HEALTHCARE_OML_MODELS is missing or invalid.',
      500
    );
  }
  ...
  await execSql(connection, 'BEGIN REBUILD_HEALTHCARE_OML_MODELS; END;');
  ...
  if (
    ...
    throw new ImportError('Required Healthcare OML model rebuild did not produce all four exact models.', 500, { models });
  }
  ...
  return {
    ...
    models,
    rebuilt: models,
    rebuildHook: 'REBUILD_HEALTHCARE_OML_MODELS',
  };
}
...
async function captureRollbackDataset(connection) {
  ...
  return {
    format: 'healthcare-generation-rollback',
    version: IMPORT_VERSION,
...
function rehydrateRollbackDataset(snapshot) {
  if (!snapshot || snapshot.format !== 'healthcare-generation-rollback' || !snapshot.tables) {
...
function generationExpectedCounts(dataset) {
  ...
  return {
    ...
    spatialPoints,
    careServiceRequests: rows('orders').length,
...
async function proveRequiredGenerationFeatures(
  ...
  const graphObjectResult = await execSql(connection, `
    ...
    WHERE graph_name = 'CARE_PATHWAY_NETWORK'
  `);
  ...
  const graphCountResult = await execSql(connection, `
    SELECT
      (SELECT COUNT(*) FROM care_graph_entities) AS vertices,
      (SELECT COUNT(*) FROM care_graph_relationships) AS edges
    FROM dual
  `);
  ...
  const graphName = graphNames.length === 1 && graphNames[0] === 'CARE_PATHWAY_NETWORK'
  ...
  const dualityObjectResult = await execSql(connection, `
    SELECT object_name
    FROM user_objects
    WHERE object_name IN ('CARE_SERVICE_REQUESTS_DV', 'PRODUCTS_INVENTORY_DV')
      AND status = 'VALID'
  ...
  const dualityCountResult = await execSql(connection, `
    SELECT
      (SELECT COUNT(*) FROM care_service_requests_dv) AS request_rows,
...
async function generateTemplateArchive({ version = IMPORT_VERSION }) {
  ...
  return {
    buffer: zip.toBuffer(),
    fileName: `healthcare-import-template-${version}.zip`,
