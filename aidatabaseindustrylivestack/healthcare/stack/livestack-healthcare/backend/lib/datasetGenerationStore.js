...
function assertRequiredFeatureEvidence(evidence, { generationId = null } = {}) {
  ...
  const expectedRequests = Number(expected.careServiceRequests);
  ...
  if (
    evidence.oml.rebuildHook !== 'REBUILD_HEALTHCARE_OML_MODELS' ||
  ...
  if (
    String(evidence.graph.graph || '').toUpperCase() !== 'CARE_PATHWAY_NETWORK' ||
    Number(evidence.graph.vertices) <= 0 ||
    Number(evidence.graph.edges) <= 0 ||
    Number(evidence.graph.probeRows) <= 0
  ) {
    featureFailure('graph', 'exact CARE_PATHWAY_NETWORK with positive executed SQL/PGQ evidence is required');
  }
  ...
  if (
    ...
    featureFailure('duality', 'both exact healthcare Duality Views and positive generation-bound row evidence are required');
  }
