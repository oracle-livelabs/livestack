  ...
  try {
    const result = await db.executeAsUser(`
      SELECT
        ...
        (SELECT COUNT(*) FROM care_graph_entities)     AS care_graph_entities,
        (SELECT COUNT(*) FROM care_graph_relationships) AS care_graph_relationships,
        (SELECT COUNT(*) FROM care_pathway_cases)      AS care_pathway_cases,
    ...
    res.json({
      ...
      care_graph_entities: row.CARE_GRAPH_ENTITIES,
      care_graph_relationships: row.CARE_GRAPH_RELATIONSHIPS,
      care_pathway_cases: row.CARE_PATHWAY_CASES,
