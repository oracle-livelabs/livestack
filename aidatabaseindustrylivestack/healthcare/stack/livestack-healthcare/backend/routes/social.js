...
// GET /api/social/vector-readiness — source provenance and an executable,
// nonempty probe. The live gate additionally captures DBMS_XPLAN evidence.
router.get('/vector-readiness', async (req, res) => {
  try {
    const [columns, indexes, probe] = await Promise.all([
      db.executeAsUser(`SELECT table_name, column_name, data_type FROM user_tab_columns
        WHERE (table_name, column_name) IN (('PRODUCT_EMBEDDINGS', 'EMBEDDING'), ('POST_EMBEDDINGS', 'EMBEDDING'))`, {}, req.demoUser),
      db.executeAsUser(`SELECT index_name, table_name, status, index_type FROM user_indexes
        WHERE index_name IN ('IDX_PRODUCT_VEC', 'IDX_POST_VEC') ORDER BY index_name`, {}, req.demoUser),
      db.executeAsUser(`SELECT COUNT(*) AS result_count FROM product_embeddings
        WHERE embedding IS NOT NULL`, {}, req.demoUser),
    ]);
    const resultCount = Number(probe.rows?.[0]?.RESULT_COUNT || 0);
    if (columns.rows.length !== 2 || indexes.rows.length !== 2 || resultCount < 1) {
      return res.status(503).json({ status: 'NOT_READY', ready: false, source: 'ORACLE_METADATA_AND_EXECUTION',
        vectorColumns: columns.rows, vectorIndexes: indexes.rows,
        representativeExecution: { executed: true, resultCount }, fallback_full_scan: null });
    }
    res.json({ status: 'READY', ready: true, source: 'ORACLE_METADATA_AND_EXECUTION',
      model: { modelName: 'ALL_MINILM_L12_V2', miningFunction: 'EMBEDDING', algorithm: 'ONNX' },
      vectorColumns: columns.rows, vectorIndexes: indexes.rows,
      representativeExecution: { executed: true, resultCount }, planEvidence: 'LIVE_DBMS_XPLAN_REQUIRED', fallback_full_scan: false });
  } catch (err) {
    res.status(503).json({ status: 'UNAVAILABLE', ready: false, error: err.message, source: 'ORACLE_METADATA_AND_EXECUTION' });
  }
});
