...
const CARE_SERVICE_REQUEST_DV_SQL =
  `SELECT DATA FROM care_service_requests_dv WHERE JSON_VALUE(DATA, '$.serviceRequestId' RETURNING NUMBER) = :id`;

const LEGACY_CARE_SERVICE_REQUEST_DV_SQL =
  `SELECT DATA FROM care_service_requests_dv WHERE JSON_VALUE(DATA, '$._id' RETURNING NUMBER) = :id`;

const REQUIRED_CARE_SERVICE_REQUEST_DV = 'CARE_SERVICE_REQUESTS_DV';
...
function mapCareServiceRequestDocument(doc) {
  if (!doc) return doc;
  return {
    serviceRequestId: doc.serviceRequestId ?? doc._id,
    requestingCareSiteId: doc.requestingCareSiteId,
...
function sendDualityUnavailable(res) {
  const details = {
    ...
    source: REQUIRED_CARE_SERVICE_REQUEST_DV,
    requiredSource: REQUIRED_CARE_SERVICE_REQUEST_DV,
    sourceMode: 'duality-view',
    executionMode: 'unavailable',
    nativeDualityViewAvailable: false,
  };
  return res.status(503).json({
    error: 'Required healthcare JSON Relational Duality View is unavailable.',
  ...
  try {
    ...
    const result = await db.executeAsUser(`
      SELECT csr.service_request_id AS order_id,
             ...
             csr.urgency_score,
             csr.created_at,
             csr.requesting_care_site_name AS customer_name,
             csr.requesting_care_site_name,
             csr.requesting_care_site_id,
             csr.requesting_care_site_city AS customer_city,
             csr.requesting_care_site_city,
             csr.requesting_care_site_region AS customer_state,
             csr.requesting_care_site_region,
             csr.care_logistics_site_name AS fulfillment_center,
             csr.care_logistics_site_name,
             csr.care_logistics_site_id,
             (SELECT COUNT(*) FROM care_request_items cri WHERE cri.service_request_id = csr.service_request_id) AS item_count,
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const order = await db.executeAsUser(`
      ...
      FROM orders o
      JOIN care_service_requests csr ON csr.service_request_id = o.order_id
    ...
    const items = await db.executeAsUser(`
      SELECT oi.*,
             ...
             p.category, p.category AS care_category,
             b.brand_name, b.brand_name AS provider_network_or_partner
      FROM order_items oi
    ...
    const shipment = await db.executeAsUser(`
      SELECT s.*,
             ...
             CASE s.carrier
               WHEN 'TrialFreight' THEN 'Specialty Care Courier'
               ELSE s.carrier
    ...
    if (order.rows.length === 0) {
      return res.status(404).json({ error: 'Care service request not found' });
    }
...
// GET /api/orders/:id/duality — same care service request from JSON Duality View
router.get('/:id/duality', async (req, res) => {
  try {
    const orderId = parseInt(req.params.id);

    let sql = CARE_SERVICE_REQUEST_DV_SQL;
    let result;

    try {
      result = await db.executeAsUser(sql, { id: orderId }, req.demoUser);
    } catch (err) {
    ...
    if (result.rows.length === 0) {
      // The exact healthcare view may expose Oracle's required _id key on an
      // older generation. This remains the same required native view and does
      // not substitute another schema object.
      sql = LEGACY_CARE_SERVICE_REQUEST_DV_SQL;
      try {
        result = await db.executeAsUser(sql, { id: orderId }, req.demoUser);
      } catch (err) {
        if (isDualityViewUnavailableError(err)) return sendDualityUnavailable(res);
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Care service request not found in duality view' });
    }

    const rawDoc = unwrapDualityDocument(result.rows[0].DATA);
    const doc = mapCareServiceRequestDocument(rawDoc);
    ...
    res.json({
      ...
      sql: sql.replace(':id', String(orderId)),
      document: doc,
    });
  } catch (err) {
    res.status(500).json({
