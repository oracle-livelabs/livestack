/**
 * Graph API - Healthcare care-pathway property graph queries.
 *
 * The route names keep the original frontend contract for compatibility, but
 * the returned data now comes from CARE_PATHWAY_NETWORK tables rather than the
...
async function fetchEdgeMetadata(demoUser) {
  const result = await db.executeAsUser(`
    ...
    ORDER  BY CASE category
                WHEN 'Clinical Events' THEN 1
                WHEN 'Care Coordination' THEN 2
  ...
  const result = await db.executeAsUser(`
    ...
    ORDER  BY risk_score DESC NULLS LAST,
              CASE finding_type
                ...
                WHEN 'care_gap' THEN 2
                WHEN 'care_gap_pathway' THEN 3
                WHEN 'high_risk_pathway' THEN 4
                WHEN 'shared_provider' THEN 5
...
async function fetchConnections(nodeIds, limit, demoUser) {
  ...
  const result = await db.executeAsUser(`
    SELECT r.relationship_id,
           ...
           f.clinical_label   AS from_clinical_label,
           f.description      AS from_description,
           f.entity_type      AS from_type,
           f.pathway_volume   AS from_pathway_volume,
           f.patient_count    AS from_patient_count,
           f.encounter_count  AS from_encounter_count,
           f.risk_score       AS from_risk_score,
           f.open_care_gap_count AS from_open_care_gap_count,
           f.direct_connection_count AS from_direct_connection_count,
           f.clinical_domain  AS from_domain,
           f.city             AS from_city,
           ...
           t.clinical_label   AS to_clinical_label,
           t.description      AS to_description,
           t.entity_type      AS to_type,
           t.pathway_volume   AS to_pathway_volume,
           t.patient_count    AS to_patient_count,
           t.encounter_count  AS to_encounter_count,
           t.risk_score       AS to_risk_score,
           t.open_care_gap_count AS to_open_care_gap_count,
           t.direct_connection_count AS to_direct_connection_count,
           t.clinical_domain  AS to_domain,
           t.city             AS to_city,
           t.region           AS to_region,
           t.is_verified      AS to_verified,
           t.engagement_rate  AS to_engagement
    FROM   care_graph_relationships r
    JOIN   care_graph_entity_metrics f ON r.from_entity_id = f.entity_id
    JOIN   care_graph_entity_metrics t ON r.to_entity_id   = t.entity_id
    LEFT JOIN care_graph_edge_metadata m ON m.edge_type = r.relationship_type
...
function nodeFromEdge(edge, side) {
  ...
  const node = {
    ...
    CLINICAL_LABEL:  from ? edge.FROM_CLINICAL_LABEL : edge.TO_CLINICAL_LABEL,
    ...
    PATIENT_COUNT:   from ? edge.FROM_PATIENT_COUNT : edge.TO_PATIENT_COUNT,
    ENCOUNTER_COUNT: from ? edge.FROM_ENCOUNTER_COUNT : edge.TO_ENCOUNTER_COUNT,
    RISK_SCORE:      from ? edge.FROM_RISK_SCORE : edge.TO_RISK_SCORE,
    OPEN_CARE_GAP_COUNT: from ? edge.FROM_OPEN_CARE_GAP_COUNT : edge.TO_OPEN_CARE_GAP_COUNT,
...
function withMetricAliases(row) {
  ...
  const patientCount = row.PATIENT_COUNT ?? row.patient_count;
  const encounterCount = row.ENCOUNTER_COUNT ?? row.encounter_count;
  const openCareGapCount = row.OPEN_CARE_GAP_COUNT ?? row.open_care_gap_count;
  ...
  const clinicalLabel = row.CLINICAL_LABEL ?? row.clinical_label;
  const description = row.DESCRIPTION ?? row.description;
...
// GET /api/graph/influencers - compatibility endpoint for care graph vertices.
router.get('/influencers', async (req, res) => {
  try {
    ...
    if (niche) {
      where += ' AND clinical_domain = :niche';
      binds.niche = niche;
    }
    if (search) {
      where += ` AND (
        ...
        OR UPPER(clinical_label) LIKE UPPER(:search)
        OR UPPER(description) LIKE UPPER(:search)
        OR UPPER(clinical_domain) LIKE UPPER(:search)
    ...
    console.error('Care graph vertices error:', err);
    res.status(500).json({ error: err.message });
  }
});

// GET /api/graph/network/:id - ego network for one care graph vertex.
router.get('/network/:id', async (req, res) => {
  try {
    ...
    if (!centerRow) {
      return res.status(404).json({ error: 'Care graph entity not found' });
    }
    ...
    const [casesRes, edgeMetadata, findings] = await Promise.all([
      db.executeAsUser(`
        SELECT ce.case_entity_id AS link_id,
               ...
               c.summary
        FROM care_case_entities ce
        JOIN care_pathway_cases c ON ce.case_id = c.case_id
    ...
    console.error('Care graph network error:', err);
    res.status(500).json({ error: err.message });
  }
});

// GET /api/graph/edge-metadata - healthcare-friendly labels for canonical edge types.
router.get('/edge-metadata', async (req, res) => {
  try {
    res.json(await fetchEdgeMetadata(req.demoUser));
  } catch (err) {
    console.error('Care graph edge metadata error:', err);
    res.status(500).json({ error: err.message });
  }
});
  ...
  try {
    const result = await db.executeAsUser(`
      SELECT c.case_key,
             ...
             e.entity_type,
             e.clinical_domain
      FROM care_pathway_cases c
      JOIN care_case_entities ce ON c.case_id = ce.case_id
      JOIN care_graph_entities e ON ce.entity_id = e.entity_id
    ...
    console.error('Care case map error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
const EXAMPLE_QUERIES = {
  care_gap_paths: {
    name: 'Care Gap Pathways',
    description: 'Trace a patient through encounters to care gaps such as missed follow-up, medication reconciliation, or overdue monitoring.',
    params: [
      { key: 'patient_key', label: 'Patient Key', default: 'PAT-1001' },
    ],
    buildSql: (p) => ({
      sql: `SELECT patient_key, patient_name, patient_label,
       encounter_key, encounter_name, encounter_label,
       gap_key, gap_name, gap_label, gap_edge, strength
FROM GRAPH_TABLE ( care_pathway_network
    MATCH (p IS care_entity) -[e1 IS clinical_link]-> (enc IS care_entity)
          -[e2 IS clinical_link]-> (gap IS care_entity)
    WHERE p.entity_key = :patient_key
      AND p.entity_type = 'patient'
      AND gap.entity_type = 'care_gap'
    COLUMNS (
        p.entity_key AS patient_key,
        p.display_name AS patient_name,
        p.clinical_label AS patient_label,
        enc.entity_key AS encounter_key,
        enc.display_name AS encounter_name,
        enc.clinical_label AS encounter_label,
...
ORDER BY strength DESC`,
      binds: { patient_key: p.patient_key || 'PAT-1001' },
      display: `-- SQL/PGQ: Patient-to-care-gap traversal
SELECT patient_key, patient_name, patient_label,
       encounter_key, encounter_label,
       gap_key, gap_label, gap_edge, strength
FROM GRAPH_TABLE ( care_pathway_network
    MATCH (p IS care_entity)
          -[e1 IS clinical_link]->
          (enc IS care_entity)
          -[e2 IS clinical_link]->
          (gap IS care_entity)
    WHERE p.entity_key = '${p.patient_key || 'PAT-1001'}'
      AND p.entity_type = 'patient'
      AND gap.entity_type = 'care_gap'
    COLUMNS (
        p.entity_key AS patient_key,
...
ORDER BY strength DESC;`,
  ...
  readmission_chain: {
    name: 'Readmission Risk Chain',
    description: 'Follow the readmission-risk path from an index encounter to a gap and a comparable ED return.',
    params: [
      { key: 'patient_key', label: 'Patient Key', default: 'PAT-1001' },
    ],
    buildSql: (p) => ({
      sql: `SELECT patient_key, patient_label, index_encounter,
       risk_gap, risk_gap_label, related_return,
       first_edge, risk_edge, return_edge, return_strength
FROM GRAPH_TABLE ( care_pathway_network
    MATCH (p IS care_entity) -[e1 IS clinical_link]-> (enc IS care_entity)
          -[e2 IS clinical_link]-> (gap IS care_entity)
          -[e3 IS clinical_link]-> (return_enc IS care_entity)
    WHERE p.entity_key = :patient_key
      AND gap.entity_key = 'GAP-READMIT-RISK'
    COLUMNS (
        p.entity_key AS patient_key,
        p.clinical_label AS patient_label,
        enc.display_name AS index_encounter,
        gap.display_name AS risk_gap,
        gap.clinical_label AS risk_gap_label,
      ...
      binds: { patient_key: p.patient_key || 'PAT-1001' },
      display: `-- SQL/PGQ: Readmission-risk chain
SELECT patient_key, patient_label, index_encounter,
...
FROM GRAPH_TABLE ( care_pathway_network
  ...
  shared_care_team: {
    name: 'Shared Care-Team Cluster',
    description: 'Find other patients connected to the same provider or care coordination team.',
    params: [
      { key: 'patient_key', label: 'Patient Key', default: 'PAT-1002' },
    ],
    buildSql: (p) => ({
      sql: `SELECT patient_key, patient_label, shared_owner,
       shared_owner_label, related_patient_key,
       related_patient, related_patient_label, owner_strength, related_strength
FROM GRAPH_TABLE ( care_pathway_network
    MATCH (p1 IS care_entity) -[e1 IS clinical_link]-> (owner IS care_entity)
          <-[e2 IS clinical_link]- (p2 IS care_entity)
    WHERE p1.entity_key = :patient_key
      AND p1.entity_type = 'patient'
      AND p2.entity_type = 'patient'
      AND owner.entity_type = 'provider'
      AND p1.entity_key <> p2.entity_key
    COLUMNS (
        p1.entity_key AS patient_key,
        p1.clinical_label AS patient_label,
        owner.display_name AS shared_owner,
        owner.clinical_label AS shared_owner_label,
        p2.entity_key AS related_patient_key,
        p2.display_name AS related_patient,
        p2.clinical_label AS related_patient_label,
        e1.strength AS owner_strength,
        e2.strength AS related_strength
    )
)
ORDER BY related_strength DESC`,
      binds: { patient_key: p.patient_key || 'PAT-1002' },
      display: `-- SQL/PGQ: Shared care-team cluster
SELECT patient_key, patient_label, shared_owner,
       shared_owner_label, related_patient_key,
       related_patient_label, owner_strength, related_strength
FROM GRAPH_TABLE ( care_pathway_network
    MATCH (p1 IS care_entity)
          -[e1 IS clinical_link]->
          (owner IS care_entity)
          <-[e2 IS clinical_link]-
          (p2 IS care_entity)
    WHERE p1.entity_key = '${p.patient_key || 'PAT-1002'}'
      AND p1.entity_type = 'patient'
      AND p2.entity_type = 'patient'
      AND owner.entity_type = 'provider'
      AND p1.entity_key <> p2.entity_key
...
ORDER BY related_strength DESC;`,
  ...
  case_map: {
    ...
    description: 'Show all graph vertices involved in a care-pathway risk case and their evidence scores.',
    params: [
      { key: 'case_key', label: 'Case Key', default: 'CASE-SEPSIS-READMIT' },
    ],
    buildSql: (p) => ({
      sql: `SELECT case_key, case_type, severity,
       entity_key, entity_type, display_name, clinical_label,
       role, evidence_score
FROM GRAPH_TABLE ( care_pathway_network
    MATCH (c IS care_case) -[e IS case_involves]-> (entity IS care_entity)
    ...
    COLUMNS (
        ...
        entity.clinical_label AS clinical_label,
        e.role AS role,
        e.evidence_score AS evidence_score
    )
)
ORDER BY evidence_score DESC`,
      binds: { case_key: p.case_key || 'CASE-SEPSIS-READMIT' },
      display: `-- SQL/PGQ: Case evidence map
SELECT case_key, case_type, severity,
       entity_key, entity_type, display_name, clinical_label,
       role, evidence_score
FROM GRAPH_TABLE ( care_pathway_network
    MATCH (c IS care_case)
          -[e IS case_involves]->
          (entity IS care_entity)
    WHERE c.case_key = '${p.case_key || 'CASE-SEPSIS-READMIT'}'
    COLUMNS (
        c.case_key AS case_key,
        c.case_type AS case_type,
        c.severity AS severity,
...
ORDER BY evidence_score DESC;`,
  ...
  care_hubs: {
    name: 'Care Hub Detection',
    description: 'Find high-degree care entities that concentrate pathway risk across patients and cases.',
    params: [
...
SELECT entity_key, display_name, clinical_label,
       ...
       ROUND(AVG(strength), 3) AS avg_strength
FROM GRAPH_TABLE ( care_pathway_network
    MATCH (src IS care_entity)
          -[e IS clinical_link]->
          (dest IS care_entity)${p.entity_type ? `\n    WHERE src.entity_type = '${p.entity_type}'` : ''}
    COLUMNS (
        src.entity_key AS entity_key,
        src.display_name AS display_name,
        src.clinical_label AS clinical_label,
...
GROUP BY entity_key, display_name, clinical_label,
         entity_type, clinical_domain
ORDER BY direct_connection_count DESC, avg_strength DESC
FETCH FIRST 20 ROWS ONLY;`,
      };
...
// GET /api/graph/readiness — validates Oracle property-graph catalog and a
// nonempty SQL/PGQ probe before the graph UI calls traversal endpoints.
router.get('/readiness', async (req, res) => {
  try {
    const [catalog, probe] = await Promise.all([
      db.executeAsUser(`SELECT graph_name FROM user_property_graphs WHERE graph_name = 'CARE_PATHWAY_NETWORK'`, {}, req.demoUser),
      db.executeAsUser(`SELECT COUNT(*) AS probe_row_count FROM GRAPH_TABLE (care_pathway_network
        MATCH (a IS care_entity) -[e IS clinical_link]-> (b IS care_entity)
        COLUMNS (a.entity_id AS source_id, b.entity_id AS target_id))`, {}, req.demoUser),
    ]);
    const probeRowCount = Number(probe.rows?.[0]?.PROBE_ROW_COUNT || 0);
    const ready = catalog.rows.length === 1 && probeRowCount > 0;
    res.status(ready ? 200 : 503).json({ status: ready ? 'ACTIVE' : 'NOT_READY', available: ready,
      sourceObject: 'CARE_PATHWAY_NETWORK', executionSource: 'SQL_PGQ_GRAPH_TABLE',
      metadataSource: 'USER_PROPERTY_GRAPHS', probeRowCount });
  } catch (err) {
    res.status(503).json({ status: 'UNAVAILABLE', available: false, sourceObject: 'CARE_PATHWAY_NETWORK', error: err.message });
  }
});
...
router.post('/run-example', async (req, res) => {
  try {
    ...
    console.error('Care graph example query error:', err);
