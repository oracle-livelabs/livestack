 ...
 *   CUSTOMER_SEGMENT_MODEL  — K-Means Clustering (care site RFM segmentation)
...
async function handleMlRouteError(res, label, err, fallbackFn) {
  if (isMissingMlAssetError(err)) {
    return res.status(503).json({
      ...
      error: `${label} requires the persisted Healthcare OML lifecycle`,
...
const EXPECTED_OML_MODELS = [
  'DEMAND_SURGE_MODEL', 'CUSTOMER_SEGMENT_MODEL', 'REVENUE_PREDICT_MODEL', 'PRODUCT_CLUSTER_MODEL',
];

// Read-only lifecycle evidence. Model construction remains a deployment task;
// runtime scoring must fail closed when a required persisted model is absent.
router.get('/models/status', async (req, res) => {
  try {
    ...
    const models = EXPECTED_OML_MODELS.map((modelName) => ({ modelName, active: active.has(modelName) }));
    const ready = models.every((model) => model.active);
    res.status(ready ? 200 : 503).json({ ready, source: 'USER_MINING_MODELS', expectedModels: EXPECTED_OML_MODELS,
      activeCount: models.filter((model) => model.active).length, models });
  } catch (err) {
    res.status(503).json({ ready: false, source: 'USER_MINING_MODELS', expectedModels: EXPECTED_OML_MODELS, error: err.message });
  }
});

router.get('/persistence/status', async (req, res) => {
  try {
    const result = await db.executeAsUser(`SELECT COUNT(*) AS active_count FROM user_mining_models
      WHERE model_name IN ('DEMAND_SURGE_MODEL', 'CUSTOMER_SEGMENT_MODEL', 'REVENUE_PREDICT_MODEL', 'PRODUCT_CLUSTER_MODEL')`, {}, req.demoUser);
    const activeCount = Number(result.rows?.[0]?.ACTIVE_COUNT || 0);
    res.status(activeCount === EXPECTED_OML_MODELS.length ? 200 : 503).json({ persistent: activeCount === EXPECTED_OML_MODELS.length,
      modelLifecycle: { ready: activeCount === EXPECTED_OML_MODELS.length }, activeCount,
      source: 'USER_MINING_MODELS', fallbackAllowed: false });
  } catch (err) {
    res.status(503).json({ persistent: false, modelLifecycle: { ready: false }, source: 'USER_MINING_MODELS', error: err.message });
  }
});
...
function demandSupplyGap(row) {
  return (Number(row.PREDICTED_DEMAND) || 0) - (Number(row.QUANTITY_ON_HAND) || 0);
}

function buildInventoryIntelligencePayload(candidateRows, meta) {
  const candidates = candidateRows || [];
  const alerts = candidates.slice(0, 100);
  const critical = candidates.filter(a => a.STOCK_STATUS === 'CRITICAL' || a.STOCK_STATUS === 'OUT_OF_STOCK').length;
  const atRisk = candidates.filter(a => demandSupplyGap(a) > 0).length;
  const surgeProducts = new Set(
    candidates
      .filter(a => a.OML_SURGE_PREDICTION === 'SURGE')
      .map(a => a.PRODUCT_ID)
      .filter(Boolean)
  ).size;
  const totalRevenueAtRisk = candidates.reduce((sum, a) => sum + (Number(a.REVENUE_AT_RISK) || 0), 0);
  ...
  return {
    ...
    summary: {
      ...
      displayed_alerts: alerts.length,
    ...
    centerSummary: Object.values(centerMap).sort((a, b) => b.critical - a.critical || b.alerts - a.alerts),
    meta: {
      ...meta,
      monitored_rows: candidates.length,
      displayed_rows: alerts.length,
    },
  };
}
...
async function fallbackInventoryIntelligence() {
  const result = await db.execute(`
    ...
    today_forecast AS (
      SELECT PRODUCT_ID,
             -- Regional forecasts are collapsed to one product-level pressure signal
             -- so each inventory site is counted once in summaries.
             MAX(PREDICTED_DEMAND) AS PREDICTED_DEMAND,
             MAX(SOCIAL_FACTOR) AS SOCIAL_FACTOR,
             MIN(CONFIDENCE_LOW) AS CONFIDENCE_LOW,
             MAX(CONFIDENCE_HIGH) AS CONFIDENCE_HIGH,
    ...
    inventory_candidates AS (
      SELECT
        ...
        NVL(tf.SOCIAL_FACTOR, 1.0) AS SOCIAL_FACTOR,
        NVL(tf.CONFIDENCE_LOW, 0) AS CONFIDENCE_LOW,
        NVL(tf.CONFIDENCE_HIGH, 0) AS CONFIDENCE_HIGH,
        tf.MODEL_VERSION,
        CASE
        ...
        CASE
          ...
          WHEN i.QUANTITY_ON_HAND < NVL(tf.PREDICTED_DEMAND, 0) THEN 'AT_RISK'
          WHEN i.QUANTITY_ON_HAND < i.REORDER_POINT THEN 'LOW'
          ELSE 'ADEQUATE'
        END AS STOCK_STATUS,
        CASE
          WHEN NVL(tf.PREDICTED_DEMAND, 0) > 0 THEN ROUND(i.QUANTITY_ON_HAND / (tf.PREDICTED_DEMAND / 7), 1)
          ELSE NULL
        END AS DAYS_OF_SUPPLY,
        CASE
          WHEN i.QUANTITY_ON_HAND < NVL(tf.PREDICTED_DEMAND, 0)
            THEN ROUND((NVL(tf.PREDICTED_DEMAND, 0) - i.QUANTITY_ON_HAND) * p.UNIT_PRICE, 2)
          ELSE 0
      ...
      LEFT JOIN today_forecast tf ON p.PRODUCT_ID = tf.PRODUCT_ID
  ...
  return buildInventoryIntelligencePayload(result.rows, {
...
//
// Scores care sites using CUSTOMER_SEGMENT_MODEL (K-Means, 4 clusters).
...
router.get('/vector-clusters', async (req, res) => {
  try {
    ...
    // Score only the persisted bootstrap model. A request must never retrain it.
    const result = await db.execute(`
      WITH clustered AS (
        SELECT
          pcv.PRODUCT_ID,
...
router.get('/inventory-intelligence', async (req, res) => {
  try {
    // Get OML-scored inventory candidates. The response summarizes the full
    // candidate set, while returning only the highest-priority rows for display.
      ...
      inventory_candidates AS (
        SELECT
          ...
          CASE
            ...
            ELSE 'ADEQUATE'
          END AS stock_status,
          CASE WHEN NVL(tf.predicted_demand, 0) > 0
            THEN ROUND(i.quantity_on_hand / (tf.predicted_demand / 7), 1)
            ELSE NULL
          END AS days_of_supply,
          CASE WHEN i.quantity_on_hand < NVL(tf.predicted_demand, 0)
            THEN ROUND((NVL(tf.predicted_demand, 0) - i.quantity_on_hand) * p.unit_price, 2)
            ELSE 0
          END AS revenue_at_risk
        FROM inventory i
        ...
        JOIN fulfillment_centers fc ON i.center_id = fc.center_id
        LEFT JOIN today_forecast tf ON p.product_id = tf.product_id
        LEFT JOIN signal_features eng ON p.PRODUCT_ID = eng.PRODUCT_ID
        LEFT JOIN sales_features sales ON p.PRODUCT_ID = sales.PRODUCT_ID
        WHERE fc.is_active = 1
      )
      SELECT *
      FROM inventory_candidates
    ...
    res.json(buildInventoryIntelligencePayload(alertsResult.rows, {
