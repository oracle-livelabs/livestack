/**
 * Fulfillment API — Spatial routing and care logistics site management
 */
...
// VPD-aware KPI rollup for the Care Logistics Map.
router.get('/kpis', async (req, res) => {
  try {
    const result = await db.executeAsUser(`
      ...
      FROM care_logistics_kpis_v
    `, {}, req.demoUser);

    const row = result.rows?.[0] || {};
    res.json({
      active_care_logistics_site_count: metricValue(row, 'active_care_logistics_site_count'),
  ...
  try {
    const result = await db.executeAsUser(`
      SELECT
             ...
             recommended_action,
             last_updated_at,
             care_logistics_site_id AS center_id,
             care_logistics_site_name AS center_name,
    ...
    if (hasCustomerId && hasProductId) {
      ...
      result = await db.executeAsUser(`
        ...
        SELECT center_id, center_name, city, state_province,
               ...
               latitude, longitude, quantity_on_hand,
               distance_km,
...
// Executes the indexed-candidate/exact-ranking strategy, then reads the actual
// cursor plan on the same governed connection for live API and UI evidence.
router.get('/spatial-readiness', async (req, res) => {
  try {
      ...
      const probe = await execute(`
        ...
        indexed_candidates AS (
          SELECT /*+ GATHER_PLAN_STATISTICS LEADING(origin) USE_NL(center) INDEX(center idx_fc_spatial) */
                 /* HEALTHCARE_SPATIAL_NN_API_PROOF */
                 center.center_id,
                 center.location AS center_location,
                 origin.location AS origin_location
          FROM origin
    ...
    res.status(evidence.ready ? 200 : 503).json(evidence);
  } catch (err) {
    res.status(503).json({
      status: 'UNAVAILABLE',
      ready: false,
...
// Returns care site lat/lon + tier for the Trial Site Tier spatial layer
      ...
      ZONE_RADII.forEach(z => {
        virtualZones.push({
          ...
          ZONE_TYPE_DISPLAY_NAME:
            z.type === 'express' ? 'Urgent Care Logistics' :
            z.type === 'overnight' ? 'Next-Day Care Logistics' :
            z.type === 'standard' ? 'Standard Care Route' :
...
// Returns demand_regions polygons with forecast summary, colored by Care Demand Index.
  ...
  try {
    const result = await db.execute(`
      SELECT r.region_id,
             ...
             r.demand_index,
             r.demand_index AS care_demand_index,
             CASE
                 WHEN r.demand_index >= 85 THEN 'Critical Care Demand'
                 WHEN r.demand_index >= 70 THEN 'High Care Demand'
                 WHEN r.demand_index >= 55 THEN 'Moderate Care Demand'
                 WHEN r.demand_index >= 40 THEN 'Lower Care Demand'
