...
const SIGNAL_CARD_ACTIONS = [
  ...
  { label: 'Open care pathway graph', page: 'graph' },
...
const SIGNAL_CONCERN_LABELS = {
  ...
  'Cold Chain Bulletin': 'cold-chain excursion risks',
...
const SIGNAL_NEXT_STEPS = {
  ...
  'Cold Chain Bulletin': 'Check logistics impact',
...
function SignalCardActions() {
  return (
    <div className="signal-card-actions" aria-label="Healthcare operations actions">
    ...
    <section className="glass-card border border-teal-soft signal-summary-panel" aria-labelledby="signal-summary-title">
      <div className="signal-summary-panel__header">
        <div>
          ...
          <p className="signal-summary-panel__subtitle">What the current signal view indicates for healthcare operations.</p>
        </div>
...
function VectorSearch() {
  ...
  const EXAMPLE_QUERIES = [
    'oncology infusion slot capacity',
    'biologics cold-chain excursion risk',
    'cell therapy cryogenic shipper availability',
    'diagnostic kit supply alternative',
    'GxP deviation for biologics lot',
    'ED capacity escalation signals',
  ];
    ...
    <div className="glass-card p-5 border border-teal-soft social-vector-search-panel">
      <div className="mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <h3 className="social-vector-search-panel__title">Semantic Care Signal Search</h3>
          <span className="social-vector-search-panel__chip text-[var(--color-text)] border border-teal-soft font-mono">
            VECTOR_EMBEDDING · COSINE · ANN
          </span>
        </div>
        <p className="text-xs text-[var(--color-text-dim)] mt-2 leading-relaxed">
          Use natural language to find related care services, supplies, capacity risks, quality notices, and compliance signals across the Seer Health Network.
        </p>
      </div>
      ...
      <div className="jet-control-row mb-3">
        <JetInputText
          value={query}
          placeholder="Search for a care service, supply constraint, capacity risk, or quality notice…"
...
export default function SocialFeed() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <RegisterOraclePanel title="Quality and Capacity Signals">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              ...
              The signal feed below uses <span className="tone-red font-mono">momentum scoring</span> across 5,000 regulatory, partner, and capacity bulletins with{' '}
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label="Care-Service Embeddings" color="orange" />
            <FeatureBadge label="Signal Embeddings" color="orange" />
          </div>
          <SqlBlock code={`-- Real-time vector semantic search for care services and supplies
...
FETCH APPROXIMATE FIRST 10 ROWS ONLY;`} />
          <div>
            ...
            <div className="space-y-1.5">
              <DiagramBox label="User Query" sub="'care coordination kit for oncology trial'" color="#4F7D7B" />
              ...
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="VECTOR_DISTANCE(COSINE)" sub="Query vector vs 187 care-service embeddings" color="#AA643B" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="ANN Index Scan" sub="FETCH APPROXIMATE FIRST K ROWS · 95% accuracy" color="#796087" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="Ranked Services and Supplies" sub="Similarity score · partner · service value · signal mentions" color="#4C825C" />
            </div>
            <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 mt-4">Embedding Tables</p>
            <div className="space-y-1.5">
              <DiagramBox label="Care-service embeddings" sub="physical product_embeddings · 187 care services · 384-dim VECTOR · COSINE ANN index" color="#AA643B" />
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <TrendingUp className="text-[var(--color-accent)]" /> Quality & Capacity Signal Intelligence
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Oracle Vector Search with ONNX embeddings for semantic care-service matching, quality signal discovery, and operational momentum detection.
        </p>
      </div>

      <SceneStoryPanel scene="quality-signals" />
      ...
      <div className="jet-control-row">
        ...
        <JetSelectSingle
          ...
          options={[
            ...
            { value: 'Cold Chain Bulletin', label: 'Cold Chain Bulletin' },
        ...
        <JetSelectSingle
          value={influencer}
          className="jet-inline-field"
          placeholder="Provider / Partner"
          ariaLabel="Provider or partner filter"
          onValueChange={(next) => { setInfluencer(next); setPage(1); }}
          options={[
            { value: '', label: 'Provider / Partner' },
      ...
      <div className="mb-2">
        <h3 className="text-sm font-semibold text-[var(--color-text)]">
          Matched Quality &amp; Capacity Signals
        </h3>
        <p className="text-xs text-[var(--color-text-dim)] leading-snug">
          Ranked by semantic similarity, operational momentum, signal confidence, and network impact.
        </p>
      </div>
