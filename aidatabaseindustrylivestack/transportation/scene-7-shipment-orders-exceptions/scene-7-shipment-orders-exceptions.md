...
Oracle AI Database helps by keeping the same shipment order data available through relational tables and JSON Relational Duality Views. In this scene, the user can inspect a shipment order, review service legs, compare the JSON document shape, see cancellation reasons for cancelled orders, and connect the order back to route and terminal context.

Estimated Time: 10 minutes

<video controls width="100%">
  <source src="https://c4u04.objectstorage.us-ashburn-1.oci.customer-oci.com/p/EcTjWk2IuZPZeNnD_fYMcgUhdNDIDA6rt9gaFj_WZMiL7VvxPBNMY60837hu5hga/n/c4u04/b/livelabsfiles/o/livestack%2FVideos%2FTransportation%2FSeer_Transportation_Scene-6_with-CC.mp4" type="video/mp4">
...
3. Review the order list, status filters, and **Reason** column. Cancelled orders now show the operational reason, such as terminal capacity, missed pickup appointment, weather disruption, carrier no-show, or documentation issue.
4. Click order **49651** or another visible high-value shipment order. To discuss recoverable revenue, filter to **Cancelled** and review the reason patterns before opening a row.
...
The relational detail shows how the business sees the shipment: shipper, status, service legs, freight value, terminal, carrier, shipment progress, and cancellation reason when the shipment was cancelled.
...
2. Review the nested document shape for order header, service legs, shipper context, shipment details, and cancellation reason when present.
...
3. Compare the shipment route with the capacity and routing scene to show how order execution connects to spatial planning.
