...
const STATIC_TEAMS = [
  {
    ...
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed regulatory and market signal analysis over live financial services data.',
  },
  {
    TEAM_NAME: 'FULFILLMENT_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed branch service analysis using inventory and routing context.',
  },
  {
    TEAM_NAME: 'COMMERCE_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed client transaction analysis using orders and revenue context.',
  },
];
...
async function buildAgentContext(teamName) {
  if (teamName === 'SOCIAL_TREND_TEAM') {
    ...
    return {
      instructions: 'Focus on financial products, institutions, risk signals, risk severity, and concrete metrics.',
      context: {
        team: teamName,
  ...
  return {
    instructions: 'Focus on B2B financial orders, revenue, signal attribution, and business trends.',
    context: {
      team: teamName,
...
function fallbackAgentSummary(teamName, context) {
  if (teamName === 'SOCIAL_TREND_TEAM') {
    ...
    if (!products.length) {
      return context.trend_summary || 'No watched financial products found in the current window.';
    }
    return products
      .slice(0, 3)
      .map((product) => {
        const riskSeverity = product.CRITICALITY_SCORE == null ? 'n/a' : product.CRITICALITY_SCORE;
        return `${product.PRODUCT_NAME} (${product.BRAND_NAME}) avg risk severity ${riskSeverity}, ${product.RISK_EVENTS} risk events, ${product.EXPOSURE_COUNT} exposure, severity ${formatSeverityBand(product.SEVERITY_BAND)}`;
      })
      .join(' | ');
  }
...
// Runs the signal team to identify critical financial product signals.
  ...
  try {
    ...
    try {
      agentAnalysis = await Promise.race([
        askAgent('SOCIAL_TREND_TEAM',
          `Identify the top financial product signals and signal sources from the last ${hours} hours ` +
    ...
    for (const p of products) {
      ...
      await logAction('trend_detection_agent', 'detect_trends', 'product', p.PRODUCT_ID, {
        ...
        window_hours:  hours,
        reason: `${formatSeverityBand(p.SEVERITY_BAND)} financial product signal with ${p.RISK_EVENTS} compliance alerts and risk severity ${p.CRITICALITY_SCORE}`,
      }, confidence);
    ...
    res.json({
      message:      `Signal detection complete - ${products.length} critical financial product signals identified in last ${hours}h`,
  ...
  try {
    ...
    try {
      trendAnalysis = await Promise.race([
        askAgent('SOCIAL_TREND_TEAM',
          'What financial product signals are critical right now based on regulatory and market activity in the last 48 hours'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
    ...
    try {
      fulfillmentAnalysis = await Promise.race([
        askAgent('FULFILLMENT_TEAM',
          'Which critical financial products have low inventory and need immediate replenishment or allocation'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
    ...
    for (const inv of criticalInventory) {
      await logAction('inventory_agent', 'inventory_alert', 'inventory', inv.PRODUCT_ID, {
        ...
        strategy:           `Pre-position capacity at ${inv.CENTER_NAME} - critical financial product with ${inv.STOCK_STATUS} inventory`,
        reason: `${inv.STOCK_STATUS} capacity (${inv.QUANTITY_ON_HAND} units) for critical financial product at ${inv.CENTER_NAME}`,
    ...
    try {
      commerceAnalysis = await Promise.race([
        askAgent('COMMERCE_TEAM',
          'Summarize signal-driven client transactions and revenue attribution from the last 7 days'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 10000)),
      ]);
    } catch (e) {
    ...
    await logAction('master_orchestrator', 'commerce_attribution', 'orders', null, {
      ...
      reason: `${socialPct}% of orders ($${(commerce.SOCIAL_REVENUE || 0).toLocaleString()}) attributed to compliance and market signals in last 7 days`,
    }, 0.93);
...
router.post('/trends', async (req, res) => {
  try {
    ...
    const q = question || 'What financial product signals are critical right now based on regulatory and market activity';
...
router.post('/fulfillment', async (req, res) => {
  try {
    const { question } = req.body;
    const q = question || 'Which critical financial products have low inventory';
...
router.post('/chat', async (req, res) => {
  ...
  const trendStrong = ['risk signal', 'severity', 'escalating', 'critical signal', 'exposure', 'compliance alert', 'regulatory alert', 'trending', 'viral', 'virality', 'mega_viral', 'momentum', 'influencer', 'source', 'bulletin', 'regulatory', 'compliance', 'criticality', 'regulatory notice', 'rising'];
  ...
  const trendWeak = ['trend', 'social', 'post', 'engagement', 'views', 'likes', 'shares', 'sentiment', 'signal', 'finra', 'sec', 'fdic'];
  const inventoryWeak = ['capacity', 'ship', 'routing', 'center', 'service', 'operations', 'delivery', 'nearest', 'distance', 'site'];
  const commerceWeak = ['order', 'customer', 'client', 'price', 'category', 'brand', 'institution', 'product', 'financial product', 'total'];
  ...
  try {
    if (intent === 'trends') {
      ...
      fallbackResult = trendRes.rows[0]?.RESULT || 'No critical financial product signals found';
      toolsUsed.push({ tool: 'risk_signal_detector', params: { hours, minScore }, status: 'success' });
      ...
      // Extract financial product name from question
      const productPatterns = [
        /["']([^"']+)["']/,                                                           // quoted financial product name
      ...
      for (const pat of productPatterns) {
        ...
        if (m) {
          ...
          pn = pn.replace(/\s+(items?|products?|financial products?|financials?)\s*$/i, '').trim();
          // Skip if the extracted name looks like a non-financial product phrase.
      ...
      // Check if this is a routing question that mentions a client or city.
      const cityMatch = q.match(/(?:to|in|near)\s+(?:a\s+(?:customer|client)\s+in\s+)?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/);
      ...
      if (productName && (cityMatch || emailMatch)) {
        ...
        // If user gave a city name, look up a real client email in that city.
        if (!customerEmail && cityMatch) {
          ...
          try {
            ...
            if (custRes.rows.length > 0) {
              ...
              toolsUsed.push({ tool: 'client_lookup', params: { city: cityName }, status: 'success', email: customerEmail });
            }
          } catch (_) {}
        }
      ...
      // client transaction and revenue queries.
      ...
      fallbackResult = `Last 30 days: ${(c.TOTAL_ORDERS || 0).toLocaleString()} orders, $${(c.TOTAL_REVENUE || 0).toLocaleString()} revenue. ` +
        ...
        `Avg transaction: $${c.AVG_ORDER_VALUE || 0}. ${(c.UNIQUE_CUSTOMERS || 0).toLocaleString()} unique clients.`;
      fallbackData = [c];
