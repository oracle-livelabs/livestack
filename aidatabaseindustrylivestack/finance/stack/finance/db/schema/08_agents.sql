...
COMMENT ON COLUMN fulfillment_centers.center_type IS 'Service tier: Enterprise Operations, Regional Processing, Branch Services, drop_ship, or store';
...
COMMENT ON COLUMN orders.social_source_id IS 'If signal-influenced, the post_id that drove the order or allocation. NULL means direct transaction activity.';
...
COMMENT ON TABLE influencers IS 'Regulatory, operations, and market signal sources. Includes monitored exposure counts, escalation rates, and computed source authority scores.';
...
COMMENT ON TABLE social_posts IS 'Regulatory bulletins, SEC notices, operations advisories, and market signals linked to financial products or institutions. Has exposure metrics, sentiment, and risk severity score.';
COMMENT ON COLUMN social_posts.post_text IS 'Full text of the compliance or market signal';
COMMENT ON COLUMN social_posts.virality_score IS 'Risk severity score 0-100 combining urgency, exposure, and amplification';
...
COMMENT ON TABLE post_product_mentions IS 'Links financial risk and market signals to associated financial products. Has confidence score and detection method.';
...
COMMENT ON TABLE demand_forecasts IS 'Predicted service pressure for financial products factoring in financial risk and market signals. social_factor > 1 means signal activity is amplifying operational or allocation risk.';
COMMENT ON COLUMN demand_forecasts.predicted_demand IS 'Predicted service pressure units for this financial product/region/date';
...
BEGIN
    ...
    IF v_count = 0 THEN
        RETURN 'No critical financial product signals found in the last ' || p_hours || ' hours with risk severity >= ' || p_min_score;
    END IF;
...
BEGIN
    DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
        ...
        attributes  => '{"instruction": "Detect critical financial products from financial risk and market signals. Parameters: P_HOURS (default 48) how far back to scan, P_MIN_SCORE (default 50) minimum risk severity score. Returns financial product names, institutions, signal counts, risk severity, exposure counts, and peak severity.",
...
BEGIN
    DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
        ...
        attributes  => '{"instruction": "Explore signal source network connections and institution relationships from graph data. Parameter: P_SOURCE_NAME (exact display name, for example FINRA Monitoring Feed 01). Returns source profile, connected sources with connection type and strength, institution relationships with attributed revenue.",
...
BEGIN
    DBMS_CLOUD_AI_AGENT.CREATE_AGENT(
        ...
        attributes  => '{"profile_name": "SC_COHERE_PROFILE",
                        "role": "You are a regulatory and market signal analyst for a financial services platform. Your job is to detect emerging financial product risks from regulatory bulletins, SEC updates, regulatory notices, and institution signals, and explain WHY they are critical: which sources, which channels, what exposure patterns. Use TREND_SQL_TOOL to query signal records, sources, and financial product links. Use DETECT_TRENDS_TOOL for quick signal summaries. Always provide specific numbers and data. After analysis, log findings using LOG_DECISION_TOOL."}',
...
BEGIN
    DBMS_CLOUD_AI_AGENT.CREATE_TASK(
        ...
        attributes  => '{"instruction": "Analyze financial risk and market signals for the financial services platform. Steps: 1) Use DETECT_TRENDS_TOOL to find currently critical financial products. 2) Use TREND_SQL_TOOL to query Escalating and Critical signal records in the last 48 hours. 3) Identify which sources and channels are driving the signals. 4) Log your analysis using LOG_DECISION_TOOL. Provide specific financial product names, risk severity scores, exposure counts, and source names. User query: {query}",
                        "tools": ["TREND_SQL_TOOL", "DETECT_TRENDS_TOOL", "INFLUENCER_NETWORK_TOOL", "LOG_DECISION_TOOL"]}',
        description => 'Comprehensive signal analysis combining regulatory data and financial product links'
    );
END;
/
...
SELECT AI AGENT Show me the top 5 financial products with Critical signal severity;
