...
COMMENT ON TABLE app_users IS
  'Application users for the Seer Bank VPD row-level security demo. Each user has a role and optional region filter.';

COMMENT ON TABLE brand_influencer_links IS
  'Many-to-many relationship between institutions and signal sources. Tracks signal counts, escalation activity, and attributed revenue exposure.';

COMMENT ON TABLE demand_regions IS
  'Geographic regions used for financial product service-pressure forecasting. Each region has a boundary polygon, population, income, and signal density.';
...
COMMENT ON TABLE fulfillment_zones IS
  'SLA service zones around Seer Bank service centers. Each zone has a boundary polygon and maximum service-response time.';
...
COMMENT ON COLUMN brand_influencer_links.avg_engagement IS
  'Average escalation activity rate across signals for this institution-source pair';
...
COMMENT ON COLUMN orders.shipping_lat IS
  'Client destination latitude for spatial routing';
COMMENT ON COLUMN orders.shipping_lon IS
  'Client destination longitude for spatial routing';
...
COMMENT ON COLUMN shipments.ship_cost IS
  'Processing fee cost in US dollars charged to the client';
...
-- ============================================================
-- SEER BANK FINANCE SEMANTIC ALIASES
-- Metadata-only comments for Ask Data / Select AI. These do not rename tables.
-- ============================================================

COMMENT ON TABLE brands IS
  'Financial institutions, counterparties, business lines, and service organizations in the Seer Bank demo.';

COMMENT ON TABLE orders IS
  'Seer Bank client transactions and cases. Use this table for transaction value, status, urgency, signal-linked transactions, and service-center assignment.';

COMMENT ON TABLE social_posts IS
  'Regulatory, market, fraud, and operations risk signals for Seer Bank. Use virality_score as risk severity and momentum_flag as severity.';

COMMENT ON TABLE influencers IS
  'Risk signal sources for Seer Bank such as regulators, market feeds, fraud operations, and branch operations advisories.';

COMMENT ON TABLE inventory IS
  'Service capacity for Seer Bank financial products at service centers. quantity_on_hand means available processing capacity.';

COMMENT ON TABLE fulfillment_centers IS
  'Seer Bank service centers grouped by Enterprise Operations, Regional Processing, and Branch Services tiers for spatial routing and SLA coverage.';

COMMENT ON TABLE shipments IS
  'Service route and SLA events for Seer Bank transactions and cases. Use ship_status as service route status.';
