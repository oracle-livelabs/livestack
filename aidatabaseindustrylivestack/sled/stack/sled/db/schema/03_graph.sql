...
CREATE TABLE influencer_connections (
    ...
    service_region_code VARCHAR2(30)
                      CHECK (service_region_code IN ('FRONT_RANGE','WESTERN_SLOPE','SOUTHERN_COLORADO')),
...
/*
-- Find community partners within 2 hops of an urgent signal post author
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE v1.handle = '@public service_sarah'
    COLUMNS (
        v2.handle,
        v2.influence_score,
        v2.follower_count
...
-- Find shortest propagation path between two community partners
SELECT *
FROM GRAPH_TABLE ( influencer_network
    MATCH SHORTEST (a IS influencer) -[e IS connects_to]->{1,5} (b IS influencer)
    WHERE a.handle = '@digital intake_mark'
...
-- Public Service program propagation: which advocates propagate a public program through the network
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE b.brand_name = 'CivicBridge Network'
    COLUMNS (
        v1.handle, v2.handle,
        e1.relationship_type,
        e2.connection_type,
