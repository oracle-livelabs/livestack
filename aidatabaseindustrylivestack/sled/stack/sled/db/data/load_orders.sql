/*
 * load_orders.sql
 * 3000 service requests with service line items, varied statuses, and community-signal attribution
 */

SET SERVEROUTPUT ON
PROMPT Loading service requests...
...
BEGIN
    ...
    FOR i IN 1..3000 LOOP
        ...
        -- 30% of service requests are associated with a resident/community signal
        ...
        -- Assign service access center for non-pending service requests
        IF v_status != 'pending' THEN
            ...
            FROM (
                ...
                JOIN customers c ON c.customer_id = v_cust_id
                WHERE CASE
                          WHEN fc.longitude <= -106.2 THEN 'WESTERN_SLOPE'
                          WHEN fc.latitude < 39.2 THEN 'SOUTHERN_COLORADO'
                          ELSE 'FRONT_RANGE'
                      END = CASE
                          WHEN c.longitude <= -106.2 THEN 'WESTERN_SLOPE'
                          WHEN c.latitude < 39.2 THEN 'SOUTHERN_COLORADO'
                          ELSE 'FRONT_RANGE'
                      END
                  AND fc.is_active = 1
        ...
        -- Create public service routing/dispatch record for routed or completed requests
        IF v_status IN ('shipped', 'delivered') AND v_center_id IS NOT NULL THEN
            ...
            INSERT INTO shipments (
                ...
                CASE MOD(i, 4)
                    WHEN 0 THEN 'ServiceCourier'
                    WHEN 1 THEN 'MobileService'
                    WHEN 2 THEN 'Service CounterFleet'
                    ELSE 'FieldRoute'
                END,
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Service requests loaded: ' || v_count);
END;
/
