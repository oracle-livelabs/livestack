/**
 * Service access API - spatial routing and agency capacity management.
 */
...
const PUBLIC_FULFILLMENT_KEYS = {
  ...
  SHIP_COST: 'SERVICE_COST',
  SHIPPED_AT: 'STARTED_AT',
  DELIVERED_AT: 'COMPLETED_AT',
  CUSTOMER_ID: 'RESIDENT_ID',
  customerId: 'residentId',
  CUSTOMER_CITY: 'RESIDENT_CITY',
  CUSTOMER_LAT: 'RESIDENT_LAT',
  CUSTOMER_LON: 'RESIDENT_LON',
  CUSTOMER_TIER: 'RESIDENT_RISK_TIER',
  customer_tier: 'resident_risk_tier',
  LIFETIME_VALUE: 'SERVICE_VALUE_EXPOSURE',
  lifetime_value: 'service_value_exposure',
  ...
  social_factor: 'resident_signal_factor',
  SOCIAL_DENSITY: 'RESIDENT_SIGNAL_DENSITY',
  social_density: 'resident_signal_density',
  PEAK_SOCIAL_FACTOR: 'PEAK_SIGNAL_FACTOR',
  peak_social_factor: 'peak_signal_factor',
  FORECAST_PRODUCTS: 'FORECAST_SERVICES',
  forecast_products: 'forecast_services',
};

function sanitizeFulfillmentText(value) {
  if (typeof value !== 'string') return value;
  return value
    .replace(/\bproducts\b/gi, 'services')
    .replace(/\bproduct\b/gi, 'service')
    .replace(/\bcustomers\b/gi, 'residents')
    .replace(/\bcustomer\b/gi, 'resident')
    .replace(/\bshipments\b/gi, 'service requests')
    .replace(/\bshipment\b/gi, 'service request')
    .replace(/\bfulfillment\b/gi, 'resolution')
    .replace(/\bsocial\b/gi, 'resident signal')
    .replace(/\bout_of_stock\b/gi, 'no_capacity')
    .replace(/\bstock\b/gi, 'capacity');
}

function sanitizeFulfillmentPayload(value) {
  if (value instanceof Date) return value.toISOString();
  if (Array.isArray(value)) return value.map(sanitizeFulfillmentPayload);
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, entryValue]) => {
      const publicKey = PUBLIC_FULFILLMENT_KEYS[key] || key;
      ...
      return [publicKey, sanitizeFulfillmentPayload(entryValue)];
    }));
  }
  return sanitizeFulfillmentText(value);
}

router.use((req, res, next) => {
  const originalJson = res.json.bind(res);
  res.json = (payload) => originalJson(sanitizeFulfillmentPayload(payload));
  next();
});

// GET /api/fulfillment/centers
// VPD: SLED_APP_CTX filters fulfillment_centers by the authenticated region.
router.get('/centers', async (req, res) => {
  try {
    const result = await db.executeAsUser(`
      SELECT fc.center_id, fc.center_name, fc.center_type,
             fc.city, fc.state_province, fc.service_region_code, fc.postal_code,
...
router.get('/shipments', async (req, res) => {
  try {
    ...
    if (status) {
      const internalStatus = toInternalServiceTaskStatus(status);
      if (!internalStatus) {
        return res.status(400).json({ error: 'Unknown service task status filter' });
      }
      where += " AND s.ship_status = :status";
    ...
    const result = await db.executeAsUser(`
      SELECT s.shipment_id, s.order_id, s.carrier, s.tracking_number,
             ...
             fc.center_name, fc.city AS center_city, fc.service_region_code,
  ...
  try {
    const result = await db.executeAsUser(`
      SELECT p.product_id, p.product_name, p.category,
             b.brand_name,
             i.center_id, fc.center_name, fc.city, fc.service_region_code,
...
router.get('/zones', async (req, res) => {
  try {
    ...
    const centers = await db.executeAsUser(`
      SELECT center_id, center_name, center_type, service_region_code, latitude, longitude
    ...
    centers.rows.forEach(c => {
      ZONE_RADII.forEach(z => {
        virtualZones.push({
          ...
          SERVICE_REGION_CODE: c.SERVICE_REGION_CODE,
          LATITUDE:         c.LATITUDE,
