...
const { captureGenerationFeatureEvidence } = require('./sledFeatureEvidence');
const {
...
function parseDemoAnchorDate(value, label = 'demo anchor date') {
...
function cloneDataset(dataset) {
  return {
    ...dataset,
    tables: Object.fromEntries(Object.entries(dataset.tables || {}).map(([tableName, table]) => [
      tableName,
      {
        ...table,
...
function findDatasetDemoSeedAnchor(dataset) {
  return minDate(valuesForDateColumn(dataset, 'demand_forecasts', 'forecast_date'))
    || minDate(importDateColumns().flatMap(({ tableName, columnName }) => valuesForDateColumn(dataset, tableName, columnName)));
}

function buildDemoDateRefreshSqlPlan() {
  return importDateColumns().map(({ tableName, columnName, optional }) => ({
    tableName,
    columnName,
    optional,
    updateSql: `UPDATE ${tableName} SET ${columnName} = ${columnName} + :offsetDays WHERE ${columnName} IS NOT NULL`,
  }));
}

function reanchorDemoDates(dataset, { targetAnchor, anchorSource = 'default' } = {}) {
  const restoreAnchor = targetAnchor || parseDemoAnchorDate(process.env.DEMO_RESTORE_ANCHOR_DATE, 'DEMO_RESTORE_ANCHOR_DATE') || new Date();
  const originalSeedAnchor = findDatasetDemoSeedAnchor(dataset);
  if (!originalSeedAnchor) {
    return {
      dataset: cloneDataset(dataset),
      metadata: {
        anchorSource,
        anchorStrategy: 'no_date_columns_found',
  ...
  const offsetMs = restoreAnchor.getTime() - originalSeedAnchor.getTime();
  const offsetDays = offsetMs / MS_PER_DAY;
  const shifted = cloneDataset(dataset);
  const shiftedColumns = {};
  let shiftedValueCount = 0;

  for (const { tableName, columnName } of importDateColumns()) {
    const rows = shifted.tables?.[tableName]?.rows || [];
    for (const row of rows) {
      const value = row[columnName];
      if (value instanceof Date && !Number.isNaN(value.getTime())) {
        row[columnName] = new Date(value.getTime() + offsetMs);
        shiftedColumns[tableName] = shiftedColumns[tableName] || {};
        shiftedColumns[tableName][columnName] = (shiftedColumns[tableName][columnName] || 0) + 1;
        shiftedValueCount += 1;
      }
    }
  }
...
function buildDemoDateRefreshOptions({ body = {}, query = {}, headers = {} } = {}) {
  const requestedAnchor = body.demoAnchorDate
    || body.anchorDate
    || query.demoAnchorDate
    || query.anchorDate
    || headers['x-demo-anchor-date']
    || process.env.DEMO_RESTORE_ANCHOR_DATE;
  return {
    targetAnchor: parseDemoAnchorDate(requestedAnchor, 'demo restore anchor date') || new Date(),
    anchorSource: requestedAnchor ? 'request_or_env' : 'current_time_default',
  };
}

async function refreshDemoDatesInDatabase(connection, metadata) {
  if (!metadata || !Number.isFinite(metadata.offsetDays) || metadata.offsetDays === 0) {
    return { skipped: true, shiftedColumns: metadata?.shiftedColumns || {} };
  }
  const plan = buildDemoDateRefreshSqlPlan();
  const results = [];
  for (const step of plan) {
    try {
      const result = await execSql(connection, step.updateSql, { offsetDays: metadata.offsetDays });
      results.push({ ...step, rowsAffected: result.rowsAffected || 0 });
    } catch (err) {
      if (!step.optional) throw err;
      results.push({ ...step, skipped: true, reason: err.message });
    }
  }
...
function buildTemplateReadme() {
  return [
    '# State and Local Government Service Operations Import Template',
    '',
...
const COLORADO_DEMO_BOUNDS = Object.freeze({
  minLat: 36.99,
  maxLat: 41.00,
  minLon: -109.05,
  maxLon: -102.05,
...
function coloradoRegionalServiceArea(latitude, longitude) {
  if (longitude <= -107.2 && latitude <= 38.3) return 'Southwest Colorado Regional Service Area';
  if (longitude <= -107.2) return 'Western Slope Regional Service Area';
  if (longitude <= -106.0) return 'Central Mountains Regional Service Area';
  if (latitude >= 40.0 && longitude >= -104.2) return 'Northeast Plains Regional Service Area';
  if (latitude >= 40.0) return 'North Central Colorado Regional Service Area';
  if (latitude <= 37.9) return 'Southern Colorado Regional Service Area';
  if (latitude < 39.2) return 'Pikes Peak Regional Service Area';
  return 'Front Range Central Regional Service Area';
}

function fallbackColoradoServiceArea(customer) {
  const sourceLat = Number(customer.latitude);
  const sourceLon = Number(customer.longitude);
  const isInColorado = sourceLat >= COLORADO_DEMO_BOUNDS.minLat
    && sourceLat <= COLORADO_DEMO_BOUNDS.maxLat
    && sourceLon >= COLORADO_DEMO_BOUNDS.minLon
    && sourceLon <= COLORADO_DEMO_BOUNDS.maxLon;
  const latitude = Math.min(COLORADO_DEMO_BOUNDS.maxLat, Math.max(COLORADO_DEMO_BOUNDS.minLat, sourceLat));
  const longitude = Math.min(COLORADO_DEMO_BOUNDS.maxLon, Math.max(COLORADO_DEMO_BOUNDS.minLon, sourceLon));
  const regionName = isInColorado
    ? coloradoRegionalServiceArea(latitude, longitude)
    : 'Colorado Regional Service Area';
...
function buildFallbackDemandRegions(dataset) {
  ...
  for (const customer of customers) {
    ...
    const serviceArea = fallbackColoradoServiceArea(customer);

    const group = groups.get(serviceArea.key) || {
      regionName: serviceArea.regionName,
      regionType: 'region',
      minLat: serviceArea.latitude,
      maxLat: serviceArea.latitude,
      minLon: serviceArea.longitude,
      maxLon: serviceArea.longitude,
      customerCount: 0,
    ...
    group.minLat = Math.min(group.minLat, serviceArea.latitude);
    group.maxLat = Math.max(group.maxLat, serviceArea.latitude);
    group.minLon = Math.min(group.minLon, serviceArea.longitude);
    group.maxLon = Math.max(group.maxLon, serviceArea.longitude);
    groups.set(serviceArea.key, group);
  }
  ...
  return [...groups.values()]
    .map((group) => {
      ...
      const minLat = Math.max(COLORADO_DEMO_BOUNDS.minLat, group.minLat - latPadding);
      const maxLat = Math.min(COLORADO_DEMO_BOUNDS.maxLat, group.maxLat + latPadding);
      const minLon = Math.max(COLORADO_DEMO_BOUNDS.minLon, group.minLon - lonPadding);
      const maxLon = Math.min(COLORADO_DEMO_BOUNDS.maxLon, group.maxLon + lonPadding);
...
function buildFallbackDemandForecasts(dataset, demandRegionRows) {
  ...
  for (const mention of mentions) {
    ...
    metrics.totalUrgency += Number(post.virality_score) || 0;
  ...
  for (const metrics of metricsByProduct.values()) {
    const avgUrgency = metrics.socialPostCount ? metrics.totalUrgency / metrics.socialPostCount : 0;
    const baseDemand = Math.max(5, Math.round((metrics.orderedQuantity * 1.2) + (metrics.mentionCount * 2) + (avgUrgency / 8)));
    const socialFactor = roundTo(Math.min(3, 1 + (metrics.mentionCount / 10) + (avgUrgency / 100)), 2) || 1;
    ...
    for (const region of regions) {
      ...
      rows.push({
        ...
        explanation: JSON.stringify({
          ...
          avgUrgency: roundTo(avgUrgency, 2),
...
function buildFallbackShipments(dataset, idMaps) {
  ...
  const carriers = [
    'State Document Courier',
    'County Services Courier',
    'Secure Benefits Mail',
...
async function assertGlobalImportContext(connection) {
  const result = await execSql(connection, `
    SELECT SYS_CONTEXT('SLED_APP_CTX', 'ACCESS_SCOPE') AS access_scope,
           SYS_CONTEXT('SLED_APP_CTX', 'AUTHENTICATED') AS authenticated
    FROM dual
  `);
  const row = result.rows?.[0] || {};
  if (String(row.AUTHENTICATED || '').toUpperCase() !== 'Y'
      || String(row.ACCESS_SCOPE || '').toUpperCase() !== 'GLOBAL') {
    throw new ImportError('Global demo access is required for dataset import or restore.', 403);
  }
}
...
async function executeImportPlan({
  ...
  return db.withUserConnection(demoUser, async ({ connection }) => {
    try {
      await assertGlobalImportContext(connection);
      ...
      if (progress) progress({ status: 'running', progress: 84, message: 'Assigning canonical Colorado service regions...' });
      await execSql(connection, 'BEGIN refresh_sled_service_regions; END;');
      ...
      const summary = {
        ...summarizeCounts(insertedCounts, fallbackCounts, zonesCreated),
        demoDateRefresh,
      };
      ...
      try {
      if (progress) progress({ status: 'running', progress: 96, message: 'Refreshing Oracle DBMS_DATA_MINING models...' });
      summary.modelLifecycle = await refreshSledOmlModels({ connection, progress });
      } catch (err) {
        warnings.push(`Oracle DBMS_DATA_MINING model refresh was skipped after import: ${err.message}`);
      }

      try {
      if (progress) progress({ status: 'running', progress: 97, message: 'Persisting machine learning outputs...' });
      summary.mlPersistence = await refreshPersistentMlData({
        connection,
        ...
        warnings.push(`Persisting machine learning outputs was skipped after import: ${err.message}`);
      }

      if (demoDateRefresh) {
        try {
        const validation = await runDemoDateValidation(connection);
        summary.demoFreshnessGuard = summarizeDemoDateValidation(validation);
        if (!validation.passed) {
          throw new ImportError('Demo freshness validation failed after date refresh.', 500, summary.demoFreshnessGuard);
        }
        } catch (err) {
          if (err instanceof ImportError) throw err;
          warnings.push(`Demo freshness validation could not run after import: ${err.message}`);
        }
      }
...
async function inferCurrentDatasetState(demoUser = null) {
...
async function getActiveDataset({ demoUser = null } = {}) {
  let stored = await getStoredDatasetState({ demoUser });
  if (!stored) {
    stored = await saveDatasetState(
      await inferCurrentDatasetState(demoUser),
      { demoUser }
    );
  }
...
async function persistDatasetState(source, warnings, demoUser = null) {
  try {
    return await saveDatasetState(buildDatasetState(source), { demoUser });
  } catch (err) {
...
async function runDatasetValidation({ parsed, fileOnly = false, lockKind, lockMessage, demoUser = null, executeOptions = {} }) {
  if (!parsed.valid) {
...
async function startDatasetJob({ parsed, kind, lockMessage, queuedMessage, startMessage, completeMessage, datasetSource, demoUser, executeOptions = {} }) {
  ...
  let transitionToken;
  try {
    generation = await generationStore.admitGeneration({ actor: demoUser, jobId: job.jobId, operation: kind });
    transitionToken = beginDatasetServingTransition({ generationId: generation.generationId, jobId: job.jobId });
    associateDatasetServingTransition(transitionToken, { status: 'admitted' });
  } catch (error) {
    endOperation();
    throw error;
  }
  ...
  setImmediate(async () => {
    try {
      await generationStore.updateGeneration({ actor: demoUser, generationId: generation.generationId, status: 'applying', jobStatus: 'running', message: startMessage });
      ...
      const featureEvidence = await captureGenerationFeatureEvidence({ actor: demoUser, generationId: generation.generationId });
      result.summary.featureEvidence = featureEvidence;
      const requestedDataset = buildDatasetState(datasetSource);
      await generationStore.activateGeneration({
        actor: demoUser,
        generationId: generation.generationId,
        source: requestedDataset.source,
        label: requestedDataset.label,
        version: requestedDataset.version,
      });
      const activeDataset = await getStoredDatasetState({ demoUser });
      try {
        const telemetry = await recordDatasetRefresh({
          jobId: job.jobId,
          operation: kind,
          datasetSource,
          activeDataset,
        ...
        if (telemetry?.recorded) {
          result.summary.telemetry = { recorded: true, objectKey: telemetry.objectKey };
        }
      } catch (err) {
        warnings.push(`Dataset refresh telemetry was skipped: ${err.message}`);
      }
      ...
      try { await generationStore.failGeneration({ actor: demoUser, generationId: generation?.generationId, message: err.message }); } catch (lifecycleError) { console.error('SLED durable generation failure persistence error:', lifecycleError); }
      updateJob(job.jobId, {
        status: 'failed',
        progress: 100,
...
async function generateTemplateArchive({ version = IMPORT_VERSION }) {
  ...
  return {
    buffer: zip.toBuffer(),
    fileName: `sled-service-operations-import-template-${version}.zip`,
    contentType: 'application/zip',
  };
}

async function validateDataset({ req, body = {}, demoUser = null, version = IMPORT_VERSION }) {
...
async function validateDemoRestore({ body = {}, query = {}, headers = {}, demoUser = null, version = IMPORT_VERSION }) {
  const demoDataset = getBundledDemoDataset(version);
  const demoDateRefreshOptions = buildDemoDateRefreshOptions({ body, query, headers });
  const { dataset, metadata } = reanchorDemoDates(demoDataset.parsed.dataset, demoDateRefreshOptions);
  return runDatasetValidation({
    parsed: {
      ...demoDataset.parsed,
      dataset,
    ...
    demoUser,
    executeOptions: {
      demoDateRefresh: metadata,
    },
  });
}

async function startDemoRestore({ body = {}, query = {}, headers = {}, demoUser = null, version = IMPORT_VERSION }) {
  const demoDataset = getBundledDemoDataset(version);
  const demoDateRefreshOptions = buildDemoDateRefreshOptions({ body, query, headers });
  const { dataset, metadata } = reanchorDemoDates(demoDataset.parsed.dataset, demoDateRefreshOptions);
  return startDatasetJob({
