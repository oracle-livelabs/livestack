...
import { formatRegionLabel, resolveAccessScope, useUser } from '../context/UserContext';
...
export default function UserSwitcher() {
  ...
  const currentScope = resolveAccessScope(currentUser);
  const currentScopeDetail = currentScope === 'GLOBAL'
    ? 'Global VPD - all Colorado service regions'
    : currentScope === 'REGIONAL'
      ? `Regional VPD - ${formatRegionLabel(currentUser.REGION) || 'configuration required'}`
      : 'Restricted VPD - no operational rows';
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="text-[9px] text-[var(--color-text-dim)] flex items-center gap-0.5 truncate">
              {currentScope === 'REGIONAL' && <MapPin size={8} />}
              {currentScopeDetail}
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div className="max-h-[320px] overflow-y-auto">
            {users.map((user) => {
              ...
              const scopeDetail = scope === 'GLOBAL'
                ? 'Global VPD - all Colorado service regions'
                : scope === 'REGIONAL'
                  ? `Regional VPD - ${formatRegionLabel(user.REGION) || 'configuration required'}`
                  : 'Restricted VPD - no operational rows';
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5 min-w-0">
                      ...
                      <span className="text-[8px] text-[var(--color-text-dim)] opacity-80 truncate">
                        {scopeDetail}
                      </span>
                    </div>
                  </div>
          ...
          >
            <div>EXEC SLED_SECURITY_PKG.SET_USER_CONTEXT('{currentUser.USERNAME}');</div>
            <div>SYS_CONTEXT('SLED_APP_CTX','ACCESS_SCOPE') = {currentScope}</div>
          </div>
        </div>
      )}
    </div>
