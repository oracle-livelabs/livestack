...
const TABLES = [
  ...
  {
    name: 'return_policy_clauses',
    required: true,
    pk: 'policy_id',
    columns: [
      idColumn('policy_id'),
      stringColumn('clause_code', { required: true }),
      stringColumn('clause_title', { required: true }),
      stringColumn('category'),
      stringColumn('clause_text', { required: true }),
      enumColumn('severity', ['standard', 'manager_review', 'deny', 'vip_override']),
      dateColumn('effective_start'),
      dateColumn('effective_end'),
      timestampColumn('created_at'),
    ],
    uniqueKeys: [['policy_id'], ['clause_code']],
  },
  {
    ...
    columns: [
      ...
      refIdColumn('customer_id'),
      refIdColumn('product_id'),
      stringColumn('return_reason', { required: true }),
      stringColumn('damage_description'),
      enumColumn('return_channel', ['web', 'store', 'contact_center', 'marketplace', 'mobile']),
      numberColumn('return_value'),
      enumColumn('risk_rating', ['Low', 'Medium', 'High', 'Very High'], { required: true }),
      enumColumn('recommendation', ['Approve', 'Deny', 'Request Info', 'Review'], { required: true }),
      enumColumn('status', ['Needs Review', 'In Review', 'Approved Draft', 'Denied Draft', 'Needs Info Draft', 'Closed']),
      stringColumn('policy_clause'),
      numberColumn('confidence_score'),
  ...
  },
  {
    ...
    columns: [
      ...
      refIdColumn('return_id'),
      stringColumn('document_type', { required: true }),
      stringColumn('title', { required: true }),
      stringColumn('excerpt'),
      numberColumn('similarity_score'),
      stringColumn('source_uri'),
      timestampColumn('created_at'),
    ],
    foreignKeys: [
      { column: 'return_id', refTable: 'return_requests' },
    ],
    uniqueKeys: [['document_id']],
  },
  {
    name: 'return_events',
    required: false,
    ...
    columns: [
      idColumn('event_id'),
      refIdColumn('return_id'),
      stringColumn('event_type', { required: true }),
      stringColumn('event_note'),
      stringColumn('actor'),
      timestampColumn('created_at'),
    ],
    foreignKeys: [
  ...
  },
  {
    ...
    columns: [
      ...
      refIdColumn('return_id'),
      enumColumn('decision_type', ['Approve', 'Deny', 'Request Info', 'Review'], { required: true }),
      stringColumn('decision_summary'),
      numberColumn('confidence_score'),
...
const TEMPLATE_NOTES = [
  ...
  'inventory.csv, return_policy_clauses.csv, and return_requests.csv are required. return_documents.csv, return_events.csv, return_decisions.csv, and shipments.csv are optional.',
  'When optional graph, shipment, demand, or return-evidence files are omitted, the importer regenerates fallback data.',
...
function buildManifest() {
  return {
    ...
    format: 'sporting-goods-retail-import',
