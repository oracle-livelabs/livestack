...
} = require('../lib/retailRestoreConcurrencyBarrier');
...
router.get('/template', async (req, res) => {
  ...
  try {
    ...
    const fileName = archive.fileName || `sporting-goods-retail-import-template-${version}.zip`;
...
router.post('/validate', requireDatasetCommand, requireDemoIdentity, requireDemoAdmin, uploadSingleFile, async (req, res) => {
  const service = getImportService(req);
...
router.post('/upload', requireDatasetCommand, requireDemoIdentity, requireDemoAdmin, uploadSingleFile, async (req, res) => {
  const service = getImportService(req);
...
router.post('/restore-demo/validate', requireDatasetCommand, requireDemoIdentity, requireDemoAdmin, async (req, res) => {
  const service = getImportService(req);
...
router.post('/restore-demo', requireDatasetCommand, requireDemoIdentity, requireDemoAdmin, async (req, res) => {
  const service = getImportService(req);
  ...
  try {
    await waitForRetailRestoreBarrier(req);
