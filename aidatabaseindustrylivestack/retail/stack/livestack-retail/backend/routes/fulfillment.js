...
function requirePositiveSpatialPlanHash(proof) {
  ...
  if (!Number.isInteger(planHashValue) || planHashValue <= 0) {
    throw new FeaturePlanEvidenceError(
      'ORACLE_SPATIAL',
      'The current Spatial API proof has no positive plan hash.',
...
// VPD: RETAIL_APP_CTX filters fulfillment_centers by the trusted persona scope.
...
router.get('/nearest', async (req, res) => {
  try {
    ...
    const boundedResults = Number.parseInt(maxResults, 10);
    if (!Number.isInteger(boundedResults) || boundedResults < 1 || boundedResults > 25) {
      return res.status(400).json({
        category: 'INVALID_REQUEST', feature: 'ORACLE_SPATIAL', available: true,
        error: 'maxResults must be an integer from 1 through 25.',
      });
    }

    let response;
    if (customerId && productId) {
      const customerIdValue = Number.parseInt(customerId, 10);
      ...
      response = await db.withUserConnection(req.demoUser, async ({ connection, execute }) => {
        ...
        WHERE c.customer_id = :customerId
          ...
          AND SDO_NN(fc.location, c.location, 'sdo_num_res=25', 1) = 'TRUE'
        ORDER BY SDO_GEOM.SDO_DISTANCE(c.location, fc.location, 0.005, 'unit=KM'),
                 fc.center_id
        FETCH FIRST :maxResults ROWS ONLY
        /* RETAIL_SPATIAL_NEAREST_PROOF */
      `, { customerId: customerIdValue, productId: productIdValue, maxResults: boundedResults });
        const cursor = await capturePreviousCursor(
          connection,
          'ORACLE_SPATIAL'
        );
        const spatialIndexBindings = await readSpatialIndexBindings(execute);
        const spatialGeometryMetadata = await readSpatialGeometryMetadata(
          execute
        );
        const proof = requirePositiveSpatialPlanHash(classifySpatialPlan(
          { ...cursor, resultRowCount: result.rows.length },
          {
    ...
    if (err?.code === 'FEATURE_PLAN_UNAVAILABLE'
        || /ORA-132|ORA-298|ORA-00942|ORA-04063/i.test(String(err.message || ''))) {
      return res.status(503).json({
        category: 'FEATURE_UNAVAILABLE', feature: 'ORACLE_SPATIAL',
        available: false, error: 'Oracle Spatial evidence is unavailable.',
        details: err?.details || null,
      });
    }
    res.status(500).json({ category: 'DATABASE_ERROR', feature: 'ORACLE_SPATIAL', available: false, error: err.message });
  }
});

router.get('/spatial-readiness', async (req, res) => {
  try {
    const evidence = await db.withUserConnection(req.demoUser, async ({ connection, execute }) => {
      ...
      if (!sample.rows?.length) {
        const error = new Error(
          'No visible customer geometry can establish Spatial execution.'
        );
        error.code = 'FEATURE_PLAN_UNAVAILABLE';
        throw error;
      }
      const result = await execute(`
        SELECT /* RETAIL_SPATIAL_UI_PROOF */
               center_id,
               SDO_GEOM.SDO_DISTANCE(location, :customerLocation, 0.005, 'unit=KM') distance_km
        FROM fulfillment_centers
        WHERE location IS NOT NULL
          AND SDO_NN(location, :customerLocation2, 'sdo_num_res=5', 1) = 'TRUE'
        ORDER BY SDO_GEOM.SDO_DISTANCE(location, :customerLocation3, 0.005, 'unit=KM'),
                 center_id
        FETCH FIRST 5 ROWS ONLY
      `, {
        customerLocation: sample.rows[0].LOCATION,
        customerLocation2: sample.rows[0].LOCATION,
        customerLocation3: sample.rows[0].LOCATION,
      });
      const cursor = await capturePreviousCursor(
        connection,
        'ORACLE_SPATIAL'
    ...
    return res.json({
      ...
      feature: 'ORACLE_SPATIAL',
      scopedEmpty: false,
      candidateOperation: 'SDO_NN',
      rankingOperation: 'SDO_GEOM.SDO_DISTANCE',
      proof: evidence.proof,
      results: evidence.result.rows,
      resultCount: evidence.result?.rows?.length || 0,
    });
  } catch (err) {
    return res.status(503).json({
      category: 'FEATURE_UNAVAILABLE', feature: 'ORACLE_SPATIAL',
      available: false, error: 'Current Spatial execution evidence is unavailable.',
    });
  }
});
