...
router.get('/:id/duality', async (req, res) => {
  try {
    ...
    res.json({
      ...
      viewDefinition: 'CREATE JSON RELATIONAL DUALITY VIEW orders_dv AS SELECT JSON {...} FROM orders o',
      sql: `SELECT DATA FROM orders_dv WHERE JSON_VALUE(DATA, '$._id' RETURNING NUMBER) = ${orderId}`,
      document: doc
    });
  } catch (err) {
    if (/ORA-00942|ORA-04063|ORA-40654|ORA-408/i.test(String(err.message || ''))) {
      return res.status(503).json({
        category: 'FEATURE_UNAVAILABLE',
        code: 'DUALITY_VIEW_UNAVAILABLE',
        feature: 'JSON_RELATIONAL_DUALITY',
        available: false,
        error: 'The native ORDERS_DV duality view is unavailable.',
      });
    }
    res.status(500).json({
      ...
      feature: 'JSON_RELATIONAL_DUALITY',
      available: false,
      error: 'The order duality document could not be read.',
    });
  }
});
