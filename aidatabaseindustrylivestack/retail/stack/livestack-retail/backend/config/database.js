...
async function setSecurityContext(connection, username, options = {}) {
  const effectiveUsername = String(
    username || getRequestIdentity().username || DEFAULT_DEMO_USER
  ).trim();
  return connection.execute(
    `BEGIN
       retail_security_pkg.set_user_context(:username);
       DBMS_SESSION.SET_IDENTIFIER(SUBSTR(:username, 1, 64));
     END;`,
 ...
 * Sets a default admin demo user context so pooled connections cannot retain
 * stale VPD package state from prior requests.
 */
 ...
 * Calls retail_security_pkg.set_user_context(username) before running the query
...
async function withUserConnection(username, callback, options = {}) {
  ...
  try {
    ...
    const executeOnConnection = (sql, binds = {}, executeOptions = {}) => connection.execute(
      sql,
      binds,
      { outFormat: oracledb.OUT_FORMAT_OBJECT, autoCommit: false, ...executeOptions }
    );
...
async function resolveDemoIdentity(username) {
  ...
  try {
    ...
    const result = await connection.execute(`
      SELECT SYS_CONTEXT('RETAIL_APP_CTX', 'USERNAME') AS username,
             SYS_CONTEXT('RETAIL_APP_CTX', 'ROLE') AS role,
             SYS_CONTEXT('RETAIL_APP_CTX', 'REGION') AS region,
             SYS_CONTEXT('RETAIL_APP_CTX', 'ACCESS_SCOPE') AS access_scope,
             SYS_CONTEXT('RETAIL_APP_CTX', 'AUTHENTICATED') AS authenticated
      FROM dual
    `, {}, { outFormat: oracledb.OUT_FORMAT_OBJECT });
    const row = result.rows?.[0] || {};
    if (row.AUTHENTICATED !== 'Y' || !row.USERNAME || !row.ROLE || !row.ACCESS_SCOPE) {
      const error = new Error('Oracle did not establish an authenticated Retail identity');
