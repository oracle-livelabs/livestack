...
USING (
  SELECT 'DEMAND_SURGE_MODEL' logical_name, 'DEMAND_SURGE_MODEL' physical_name,
         (SELECT COUNT(*) FROM oml_demand_training_v) training_row_count FROM dual
  UNION ALL
  SELECT 'CUSTOMER_SEGMENT_MODEL', 'CUSTOMER_SEGMENT_MODEL',
         (SELECT COUNT(*) FROM oml_customer_rfm_v) FROM dual
  UNION ALL
  SELECT 'REVENUE_PREDICT_MODEL', 'REVENUE_PREDICT_MODEL',
         (SELECT COUNT(*) FROM oml_revenue_training_v) FROM dual
  UNION ALL
  SELECT 'PRODUCT_CLUSTER_MODEL', 'PRODUCT_CLUSTER_MODEL',
         (SELECT COUNT(*) FROM oml_product_cluster_v) FROM dual
) source
...
WHEN MATCHED THEN UPDATE SET
  ...
  target.training_fingerprint =
    RAWTOHEX(STANDARD_HASH(source.logical_name || ':' || source.training_row_count, 'SHA256')),
...
USING (
  SELECT logical_name, generation_id, training_fingerprint, training_row_count
...
WHEN NOT MATCHED THEN INSERT (
  generation_id, logical_name, training_fingerprint, training_row_count,
  status, created_at, updated_at
) VALUES (
  source.generation_id, source.logical_name, source.training_fingerprint,
  source.training_row_count, 'ACTIVE', SYSTIMESTAMP, SYSTIMESTAMP
);
COMMIT;
