 ...
 * major regions. The social_factor column simulates AI-detected demand
 * momentum from sporting-goods customer signals. The explanation column stores
 ...
 *   Creator and customer demand signals -> Vector semantic trends -> Time-series demand
 *
...
-- AllTerrain Hiking Boots across regions (high demand momentum from outdoor creators)
...
SELECT p.product_id, 'Bay Area (SF)', TRUNC(SYSDATE) + LEVEL,
  ...
  'social_aware_v2',
  '{"model":"social_aware_v2","drivers":["trail_creator_demand_spike","influencer_collaboration","weekend_effect"],"top_influencer":"@glowqueen","virality_score":87,"confidence":"high","trend":"accelerating"}'
FROM (SELECT product_id FROM products WHERE product_name = 'AllTerrain Hiking Boots' FETCH FIRST 1 ROW ONLY) p
CONNECT BY LEVEL <= 30;
...
SELECT p.product_id, 'New York Metro', TRUNC(SYSDATE) + LEVEL,
  ...
  'social_aware_v2',
  '{"model":"social_aware_v2","drivers":["outdoor_creator_post","outdoor_brand_ambassador","spring_trail_promo"],"top_influencer":"@nyc_trendsetter","virality_score":79,"confidence":"high","trend":"rising"}'
FROM (SELECT product_id FROM products WHERE product_name = 'AllTerrain Hiking Boots' FETCH FIRST 1 ROW ONLY) p
CONNECT BY LEVEL <= 30;
...
SELECT p.product_id, 'Los Angeles Basin', TRUNC(SYSDATE) + LEVEL,
  ...
  'social_aware_v2',
  '{"model":"social_aware_v2","drivers":["trail_community_mention","gear_review","spring_trail_season"],"top_influencer":"@labeauty_official","virality_score":74,"confidence":"medium","trend":"steady_growth"}'
FROM (SELECT product_id FROM products WHERE product_name = 'AllTerrain Hiking Boots' FETCH FIRST 1 ROW ONLY) p
CONNECT BY LEVEL <= 30;

-- Product 2 - sports tech and wearables (Seattle + Austin training communities)
INSERT INTO demand_forecasts (product_id, region, forecast_date, predicted_demand, confidence_low, confidence_high, social_factor, model_version, explanation)
SELECT p.product_id, 'Seattle Metro', TRUNC(SYSDATE) + LEVEL,
  ...
  'social_aware_v2',
  '{"model":"social_aware_v2","drivers":["sports_tech_reviewer","product_launch","training_community_cluster"],"top_influencer":"@techwithmark","virality_score":82,"confidence":"high","trend":"launch_spike"}'
...
SELECT p.product_id, 'Austin Metro', TRUNC(SYSDATE) + LEVEL,
  ...
  'social_aware_v2',
  '{"model":"social_aware_v2","drivers":["spring_event_effect","startup_community","running_club_buzz"],"top_influencer":"@austintech","virality_score":71,"confidence":"medium","trend":"rising"}'
FROM (SELECT product_id FROM products ORDER BY product_id OFFSET 1 ROW FETCH NEXT 1 ROW ONLY) p
CONNECT BY LEVEL <= 30;
...
-- Product 4 - outdoor care and trail prep (Miami + NYC)
INSERT INTO demand_forecasts (product_id, region, forecast_date, predicted_demand, confidence_low, confidence_high, social_factor, model_version, explanation)
SELECT p.product_id, 'Miami-South Florida', TRUNC(SYSDATE) + LEVEL,
  ...
  'social_aware_v2',
  '{"model":"social_aware_v2","drivers":["creator_field_test","trail_season_prep","waterproof_boot_feedback"],"top_influencer":"@miamiglam","virality_score":91,"confidence":"very_high","trend":"mega_viral"}'
...
SELECT p.product_id, 'New York Metro', TRUNC(SYSDATE) + LEVEL,
  ...
  'social_aware_v2',
  '{"model":"social_aware_v2","drivers":["weekend_hiking_event","gear_guide_feature","outdoor_boots_surge"],"top_influencer":"@nycbeautyeditor","virality_score":94,"confidence":"very_high","trend":"explosive_growth"}'
FROM (SELECT product_id FROM products ORDER BY product_id OFFSET 3 ROW FETCH NEXT 1 ROW ONLY) p
CONNECT BY LEVEL <= 30;
...
-- Additional cross-region forecast for AllTerrain Hiking Boots nationwide surge
INSERT INTO demand_forecasts (product_id, region, forecast_date, predicted_demand, confidence_low, confidence_high, social_factor, model_version, explanation)
