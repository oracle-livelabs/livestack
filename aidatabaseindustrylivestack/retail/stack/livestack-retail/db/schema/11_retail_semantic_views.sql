...
COMMENT ON TABLE return_policy_clauses IS
  'Sporting-goods service policy clauses used to ground recommendations, eligibility, exception handling, and review decisions.';

COMMENT ON TABLE return_requests IS
  'Sporting-goods post-purchase service cases scored by policy, customer history, product context, evidence, risk rating, recommendation, and status.';

COMMENT ON TABLE return_documents IS
  'Grounding evidence for service decisions, including policy clauses, product notes, image notes, warranty terms, marketplace context, and customer history snippets.';

COMMENT ON TABLE return_events IS
  'Timeline events for sporting-goods service cases, including customer contact, evidence review, agent analysis, and reviewer action.';

COMMENT ON TABLE return_decisions IS
  'Auditable service decisions and AI-assisted recommendations for sporting-goods post-purchase cases.';

COMMENT ON TABLE social_posts IS
  'Sporting-goods customer and creator signal events used for product demand, sentiment, momentum, and service-risk analysis.';
...
COMMENT ON TABLE retail_returns_workflow_v IS
  'Sporting-goods service workflow view for service exposure, policy evidence counts, customer value, risk rating, recommendation, and status.';

COMMENT ON TABLE retail_signal_product_v IS
  'Retail signal view that maps customer and creator signal events to the sporting-goods products they influence, including demand momentum and service-risk context.';

COMMENT ON TABLE retail_order_return_v IS
  'Retail order view with service context for Ask Retail Data and dashboard narration.';
...
COMMENT ON TABLE agent_actions IS
  'Auditable AI agent actions for sporting-goods demand, fulfillment, service, and commerce workflows, including confidence, status, entity context, and decision payload.';
