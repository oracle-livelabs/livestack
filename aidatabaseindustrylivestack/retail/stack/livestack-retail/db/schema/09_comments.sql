 ...
 *   2. Optional SELECT AI metadata for manually configured profiles that
 *      enable table and column comments.
 *
 * Run as: schema owner
...
COMMENT ON COLUMN brands.brand_category IS
  'Brand industry category (e.g. Outdoor, Footwear, Fitness, Sports Tech). This is the brand category, not the product category. Use products.category for product categories.';
...
COMMENT ON COLUMN products.subcategory IS
  'Product subcategory within the main category (e.g. Hiking under Footwear, Hydration under Outdoor)';
