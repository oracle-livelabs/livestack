          ...
          <div className="orders-detail-tabbar">
            <div className="orders-detail-tabset">
              ...
              <span className="text-[10px] text-[var(--color-text-dim)] ml-3 hidden sm:inline">
                One connected order record across operations, APIs, and fulfillment workflows.
              </span>
            </div>
            <JetButton
              label="Close"
          ...
          {view === 'json' && (
            <div className="p-4 space-y-3">
              {loadingDuality ? (
                ...
                <div className="text-sm tone-red text-center py-4" role="alert">
                ...
                <>
                  ...
                  <div className="rounded-lg p-3 text-xs leading-relaxed" style={{ background: 'rgba(170,100,59,0.06)', border: '1px dashed rgba(170,100,59,0.3)' }}>
                    ...
                    single read-only JSON document with nested items. Both interfaces observe the same committed ACID transaction.</span>
                  </div>
                        ...
                        <CircleMarker center={from} radius={8} fillColor="#437C94" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            ...
                            <div className="route-map-tooltip__content">
                              ...
                              <span>{order.CENTER_NAME || 'Fulfillment Center'}</span>
                            </div>
                          </Tooltip>
                        </CircleMarker>
...
export default function Orders() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <RegisterOraclePanel title="Unified Order Intelligence">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-[var(--color-text)] leading-relaxed">
              Orders are stored in a classic <span className="tone-ocean font-mono">relational schema</span> - ACID transactions, foreign keys, referential integrity.
              Oracle's <span className="font-mono text-[var(--color-text)]">JSON Duality Views</span> expose the <em>exact same data</em> as read-only JSON documents,
              ...
              the same underlying rows rendered two ways. Each endpoint is an independent read-only VPD session; no cross-request transaction is claimed.
            </p>
          </div>
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label="READ ONLY" color="green" />
            <FeatureBadge label="Nested JSON Projection" color="cyan" />
            <FeatureBadge label="ACID-consistent projection" color="blue" />
          ...
          <div>
            ...
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              Read-only JSON projection of orders with nested line items. Application mutations continue through explicitly authorized relational transaction APIs.
            </p>
...
FROM orders o;`} />
          ...
          <div>
            ...
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              Products with a nested brand object and inventory entries that include fulfillment-center detail. The projection reads products, brands, inventory, and fulfillment centers.
            </p>
...
-- Returns: {"_id":1, "status":"shipped", "items":[...]}`} />
          ...
          <div>
            ...
            <div className="rounded-lg p-3 space-y-2" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
              ...
              <div className="text-center text-[9px] text-[var(--color-text)] mt-1">✓ Same underlying rows · No sync · No ETL</div>
            </div>
          </div>
          ...
          <div className="rounded-lg p-3 space-y-2" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
            ...
            <DiagramBox label="products + inventory" sub="Product catalog + fulfillment center stock levels" color="#437C94" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕ CREATE JSON RELATIONAL DUALITY VIEW</div>
            <DiagramBox label="PRODUCTS_INVENTORY_DV" sub="Read-only JSON document: {sku, productName, inventory: [...]}" color="#AA643B" />
          </div>
          ...
          <div className="rounded-lg p-3 text-center" style={{ background: 'rgba(170,100,59,0.06)', border: '1px dashed rgba(170,100,59,0.3)' }}>
            <p className="text-[10px] text-[var(--color-text-dim)] leading-relaxed">
              ...
              The deployed views are read-only projections. Their API executes
              <span className="font-mono text-[var(--color-text)]"> SELECT DATA</span>; no document write is claimed.
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-text)] leading-relaxed mb-2">
              ...
              Admins and analysts have global read scope; a regional manager sees only orders whose center and customer remain visible in the active Oracle context.
            </p>
          </div>
          <div className="flex flex-wrap gap-1.5">
...
BEGIN
    ...
    IF v_scope = 'global' AND v_role IN ('admin','analyst') THEN
        RETURN NULL;
    ELSIF v_scope = 'regional' AND v_role = 'fulfillment_mgr' THEN
...
-- Applied via:
DBMS_RLS.ADD_POLICY(
  object_name => 'ORDERS', policy_name => 'VPD_RT_ORDERS',
  policy_function => 'VPD_RETAIL_OPERATIONAL',
  statement_types => 'SELECT,INSERT,UPDATE,DELETE',
  update_check => TRUE,
  policy_type => DBMS_RLS.CONTEXT_SENSITIVE);`} />
          <p className="text-xs text-[var(--color-text-dim)] leading-relaxed">
            A second DML-only policy calls <span className="font-mono">VPD_RETAIL_WRITE_ENTITLEMENT</span>. The order list, detail, and duality endpoints mounted here use read-only connections.
          </p>
        </div>
      </RegisterOraclePanel>
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <ShoppingCart className="tone-ocean" /> Unified Order Intelligence
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Manage customer orders, fulfillment activity, API integrations, and shipment visibility through one connected retail data platform.
        </p>
      </div>

      <RetailSceneStory scene="orders" />
              ...
              <span className="text-[var(--color-text-dim)] text-xs">
                {vpdAccess.scope === 'REGIONAL'
                  ...
                  : vpdAccess.scope === 'RESTRICTED'
                    ? `Restricted scope - ${(orders || []).length} protected orders visible`
