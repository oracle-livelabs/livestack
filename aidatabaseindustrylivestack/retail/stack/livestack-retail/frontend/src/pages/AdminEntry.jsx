...
export default function AdminEntry({
  ...
  useEffect(() => {
    ...
    setBusyAction(null);
  }, [currentUser?.USERNAME, activeDataset?.generationId, activeDataset?.updatedAt]);
  ...
  async function pollStatus(jobId) {
      ...
      try {
        ...
        if (TERMINAL_STATUSES.has(normalized)) {
          ...
          if (['completed', 'complete', 'success'].includes(normalized)) {
            window.dispatchEvent(new CustomEvent('retail-live-footprint-refresh'));
            onDatasetChanged?.();
          }
          return false;
        }
    ...
    if (await tick()) statusTimerRef.current = setInterval(tick, 2000);
  }
  ...
  const headerContent = isOverlay ? 'Use Your Own Retail Data' : 'Dataset Control';
  const headerDescription = isOverlay
    ? 'Download a retail import template, add product, order, inventory, customer signal, fulfillment, and returns data, then validate or restore the active dataset without changing the application shell or Oracle Database 26ai feature labels.'
    : 'Start on the seeded demo dataset, or load your own v1 ZIP before entering the experience.';
  const quickActionLabel = isOverlay ? 'Jump To Upload' : 'Load Your Own Retail Data';

  return (
    <>
    <RegisterOraclePanel title="Dataset Governance Oracle Internals">
      <div className="space-y-4">
        <p className="text-sm">
          ZIP validation parses and validates the candidate without activating it. Upload and Restore require both the explicit dataset-mutation command header and the validated administrator identity; they create a durable Oracle job and singleton lease before background work starts. The active generation changes only after required feature readiness succeeds.
        </p>
        <div className="flex flex-wrap gap-2">
          <FeatureBadge label="Oracle Application Context" color="red" />
          <FeatureBadge label="Durable Restore" color="blue" />
          <FeatureBadge label="Feature Readiness" color="green" />
          <FeatureBadge label="Singleton Lease" color="orange" />
          <FeatureBadge label="Atomic Activation" color="purple" />
        </div>
        <SqlBlock code={`-- Authoritative active dataset/readiness row
...
-- Durable operation and singleton ownership
SELECT j.job_id, j.operation, j.status, j.progress,
       JSON_VALUE(j.payload, '$.phase') AS phase,
       l.lease_token, l.status AS lease_status,
       l.heartbeat_at, l.lease_expires_at
FROM app_dataset_jobs j
LEFT JOIN app_dataset_operation_lock l
  ON l.owner_job_id = j.job_id
WHERE j.job_id = :job_id;`} />
        <p className="text-xs text-[var(--color-text-dim)] leading-relaxed">
          Dataset activation also commits a deterministic outbox record. Delivery can retry after commit without replaying the data mutation.
        </p>
        <p className="text-xs text-[var(--color-text-dim)]">
          Active dataset: {datasetLabel}. Job: {jobState?.jobId || 'none'} ·
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-3 text-sm text-[var(--color-text-dim)] max-w-3xl">
              Active retail dataset: {datasetLabel}.
            </p>
        ...
        <div className="mt-6 glass-card p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              disabled={isWorking || !restoreReady || !isAdmin}
            >
          ...
          {!isAdmin && (
            <p className="mt-3 text-xs tone-sienna" role="note">
              Dataset preview, validation, upload, and Restore are available only to the Administrator. Switch to the Admin persona to enable these controls.
            </p>
          )}
          {restorePreview && (
