...
import { assertPreparedJetAssets } from './jet-asset-contract.mjs';
...
const preparedAssets = await assertPreparedJetAssets(frontendDir);
if (preparedAssets.oracleJetVersion !== oracleJetVersion) {
  throw new Error(
    `Prepared Oracle JET version ${preparedAssets.oracleJetVersion} does not match installed version ${oracleJetVersion}.`,
  );
}

console.log(
  `Prepared and verified ${preparedAssets.requiredAssets.length} Oracle JET assets in ${path.relative(frontendDir, jetDir)} using @oracle/oraclejet ${oracleJetVersion}.`,
);
