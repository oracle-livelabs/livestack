...
**Predictive Quality and Supply Analytics** helps life sciences teams forecast release risk, trial-site readiness, supply impact, product cohorts, and cold-chain capacity so they can prioritize action before continuity or compliance is affected. The page keeps model-backed scoring close to governed regulated data instead of pushing sensitive analysis into a separate platform.

Quality, clinical operations, manufacturing, and supply teams need to prioritize what matters before a trial site, batch release, or controlled inventory position is affected. That becomes difficult when model inputs, operational records, quality signals, and business context sit in different systems.

**Oracle AI Database** helps address that challenge with Oracle Machine Learning for SQL. The app keeps model training, scoring, and analytics close to governed life sciences data, then exposes the results through business-friendly tabs.
...
In this scene, you will learn what life sciences decision the analytics workspace supports, what evidence the user should inspect, and what action the team may take next.
...
Perform the following set of steps to use the analytics workspace to show how model-backed risk scores can support regulated supply decisions.
...
Perform the following set of steps to review site readiness and show how the business can segment trial sites by behavior and operational risk.
...
This helps clinical operations understand which trial sites may need closer planning, supply monitoring, or support before enrollment or treatment schedules are affected.
...
Emphasize that the forecast is not separate from the operational data. It is scored against the same governed Oracle data foundation used by the other scenes.

## Task 4: Change Product Cohorts

![Product Cohorts tab with cluster control highlighted](images/product-cohorts.png)

Perform the following set of steps to use the Product Cohorts tab to explain how similar regulated products can be grouped for planning and risk review.
...
Perform the following set of steps to review cold-chain capacity and show how model-backed risk can turn into replenishment and allocation decisions.
...
This scene connects AI to operational action: model scores help users decide where capacity, replenishment, or quality follow-up may need attention.
