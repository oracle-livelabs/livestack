...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--reproducible",
        action="store_true",
        help="Normalize ZIP timestamps and file modes for byte-identical rebuilds",
    )
...
def is_default_excluded(relative_path: str, *, is_dir: bool) -> bool:
    ...
    if any(part in DEFAULT_DIRECTORY_NAMES for part in parts):
        return True
    if not is_dir and (
        basename.endswith(DEFAULT_SUFFIXES) or basename.endswith((".swp", ".swo", "~"))
    ):
        return True
    return False
...
def should_exclude(
    relative_path: str, *, is_dir: bool, args: argparse.Namespace
) -> bool:
    if matches_custom_exclude(relative_path, args.exclude):
        return True
    return not args.no_default_excludes and is_default_excluded(
        relative_path, is_dir=is_dir
    )


def archive_name(
    relative_path: Path, args: argparse.Namespace, source: Path
) -> str:
...
def collect_paths(
    source: Path, output: Path, args: argparse.Namespace
    ...
    if not source.is_dir():
        raise ValueError(
            f"Source does not exist or is not a regular file/directory: {source}"
        )
    ...
    for current, dirnames, filenames in os.walk(
        ...
        for dirname in dirnames:
            ...
            if path.is_symlink():
                ...
                entries.append(
                    (path, archive_name(relative, args, source) + "/", True)
                )
        dirnames[:] = kept_dirs
        ...
        for filename in filenames:
            ...
            if path.resolve() == output_resolved:
                ...
                entries.append(
                    (path, archive_name(relative, args, source), False)
                )
        ...
        if not args.no_default_excludes:
            dirty = [
                ...
                if is_default_excluded(
                    name.rstrip("/"), is_dir=name.endswith("/")
                )
            ]
            if dirty:
                raise ValueError(
                    "ZIP contains excluded metadata: " + ", ".join(dirty[:5])
                )
        return len(names)
...
def main() -> int:
    ...
    if args.root_name and (
        "/" in args.root_name
        or "\\" in args.root_name
        or args.root_name in {".", ".."}
    ):
        raise ValueError("--root-name must be a single safe directory name")

    entries, skipped = collect_paths(source, output, args)
    write_zip(entries, output, reproducible=args.reproducible)
    member_count = validate_zip(output, args)

    print(f"Created {output}")
    print(
        f"Archived {member_count} entries "
        f"({sum(not is_dir for _, _, is_dir in entries)} files)"
    )
    if skipped:
