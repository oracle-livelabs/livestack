/*
 * load_graph_data.sql
 * Manufacturing network account connections (3000+ edges) and product line network account links
 */

SET SERVEROUTPUT ON
PROMPT Loading supplier network graph edges...
...
BEGIN
    ...
    -- Generate ~3000 manufacturing network account connections
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Manufacturing network account connections loaded: ' || v_count);

    -- Generate product line network account links (~1500)
    v_count := 0;
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Product line network account links loaded: ' || v_count);
END;
/
