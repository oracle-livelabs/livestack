...
router.get('/schema-objects', async (_req, res) => {
  const objects = getMediaSchemaObjectMetadata();
  res.json({
    objects,
    domains: groupMediaSchemaObjectMetadata(objects),
    meta: {
...
router.get('/health', async (req, res) => {
  ...
  const result = await checkAskMediaDataHealth({ demoUser: req.demoUser || 'admin_jess', profile });
