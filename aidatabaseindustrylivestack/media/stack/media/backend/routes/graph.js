...
router.get('/influencers', async (req, res) => {
  try {
    ...
    return featureUnavailable(res, {
      feature: 'PROPERTY_GRAPH_SQL_PGQ',
      source: 'INFLUENCER_NETWORK',
      message: 'Creator graph base data is unavailable.',
    });
  }
});

// GET /api/graph/edge-metadata — relationship inventory used by graph clients
router.get('/edge-metadata', async (req, res) => {
  try {
    const [connections, brandRelationships] = await Promise.all([
      db.executeAsUser(`
        ...
        ORDER BY edge_count DESC, connection_type
      `, {}, req.demoUser),
      db.executeAsUser(`
        SELECT relationship_type,
               COUNT(*) AS edge_count,
               SUM(post_count) AS post_count,
               ROUND(AVG(avg_engagement), 3) AS avg_engagement,
               ROUND(SUM(revenue_attributed), 2) AS revenue_attributed
        FROM brand_influencer_links
        GROUP BY relationship_type
        ORDER BY edge_count DESC, relationship_type
      `, {}, req.demoUser),
    ]);

    return res.json({
      connectionTypes: connections.rows,
      brandRelationshipTypes: brandRelationships.rows,
      generationId: req.activeGenerationId,
    });
  } catch (err) {
    return featureUnavailable(res, {
      feature: 'PROPERTY_GRAPH_SQL_PGQ',
      source: 'INFLUENCER_CONNECTIONS + BRAND_INFLUENCER_LINKS',
      message: 'Creator graph relationship metadata is unavailable.',
    });
  }
});
...
router.get('/network/:id', async (req, res) => {
  try {
    ...
    return featureUnavailable(res, {
      feature: 'PROPERTY_GRAPH_SQL_PGQ',
      source: 'INFLUENCER_NETWORK',
      message: 'Creator graph network data is unavailable.',
    });
  }
});
...
FETCH FIRST 20 ROWS ONLY;`,
  ...
  brand_propagation: {
    name: 'Studio or Label Propagation Network',
    description: 'Trace how a studio or label signal spreads through the creator network — from content creators to their connections. Uses multi-edge pattern: brand ←[promotes]— influencer —[connects_to]→ reached.',
    params: [
      { key: 'brand_name', label: 'Studio or Label Name', default: 'Aurora Studios' },
    ],
    buildSql: (p) => ({
...
FETCH FIRST 30 ROWS ONLY`,
      binds: { brand_name: p.brand_name || 'Aurora Studios' },
...
FROM GRAPH_TABLE ( influencer_network
    MATCH (b IS brand)
          ...
          -[e2 IS connects_to]->
          (v2 IS influencer)
    WHERE b.brand_name = '${p.brand_name || 'Aurora Studios'}'
    COLUMNS (
        v1.handle AS promoter,
        v2.handle AS reached,
        e1.relationship_type,
...
async function readVisibleExampleDefaults(demoUser) {
  const result = await db.executeAsUser(`
    WITH reach_seed AS (
      SELECT influencer.handle,
             COUNT(*) connection_count
      FROM influencers influencer
      JOIN influencer_connections connection
        ON connection.from_influencer = influencer.influencer_id
      GROUP BY influencer.handle
      ORDER BY connection_count DESC, influencer.handle
      FETCH FIRST 1 ROW ONLY
    ),
    mutual_seed AS (
      SELECT source_a.handle from_handle,
             source_b.handle to_handle,
             COUNT(*) shared_connections
      FROM influencer_connections edge_a
      JOIN influencer_connections edge_b
        ON edge_b.to_influencer = edge_a.to_influencer
       AND edge_b.from_influencer > edge_a.from_influencer
      JOIN influencers source_a
        ON source_a.influencer_id = edge_a.from_influencer
      JOIN influencers source_b
        ON source_b.influencer_id = edge_b.from_influencer
      GROUP BY source_a.handle, source_b.handle
      ORDER BY shared_connections DESC, source_a.handle, source_b.handle
      FETCH FIRST 1 ROW ONLY
    ),
    brand_seed AS (
      SELECT brand.brand_name,
             COUNT(*) propagation_paths
      FROM brands brand
      JOIN brand_influencer_links link
        ON link.brand_id = brand.brand_id
      JOIN influencer_connections connection
        ON connection.from_influencer = link.influencer_id
      GROUP BY brand.brand_name
      ORDER BY propagation_paths DESC, brand.brand_name
      FETCH FIRST 1 ROW ONLY
    )
    SELECT (SELECT handle FROM reach_seed) reach_handle,
           (SELECT from_handle FROM mutual_seed) mutual_from_handle,
           (SELECT to_handle FROM mutual_seed) mutual_to_handle,
           (SELECT brand_name FROM brand_seed) propagation_brand_name
    FROM dual
  `, {}, demoUser);
  const row = result.rows?.[0] || {};
  return {
    handle: row.REACH_HANDLE,
    from_handle: row.MUTUAL_FROM_HANDLE,
    to_handle: row.MUTUAL_TO_HANDLE,
    brand_name: row.PROPAGATION_BRAND_NAME,
  };
}

// GET /api/graph/example-queries — list available queries with live defaults
router.get('/example-queries', async (req, res) => {
  try {
    const liveDefaults = await readVisibleExampleDefaults(req.demoUser);
    const queries = Object.entries(EXAMPLE_QUERIES).map(([id, q]) => ({
      ...
      params: q.params.map((param) => ({
        ...param,
        default: liveDefaults[param.key] ?? param.default,
      })),
    }));
    res.json(queries);
  } catch (err) {
    console.error('Graph example catalog error:', err);
    featureUnavailable(res, {
      feature: 'PROPERTY_GRAPH_SQL_PGQ',
      source: 'INFLUENCER_NETWORK',
      correlationId: req.headers['x-correlation-id'] || null,
      message: 'Native Oracle SQL/PGQ graph query catalog is unavailable.',
    });
  }
});
...
router.post('/run-example', async (req, res) => {
  try {
    ...
    for (const param of queryDef.params) {
      if ((resolvedParams[param.key] == null || resolvedParams[param.key] === '')
          && liveDefaults[param.key] != null) {
        resolvedParams[param.key] = liveDefaults[param.key];
      }
    }
    ...
    featureUnavailable(res, {
      ...
      source: 'INFLUENCER_NETWORK',
      correlationId: req.headers['x-correlation-id'] || null,
      message: 'Native Oracle SQL/PGQ graph execution is unavailable.',
      details: {
