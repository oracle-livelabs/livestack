...
async function executeSpatialWithPlan(demoUser, generationId, sql, binds) {
  return db.withUserConnection(demoUser, async ({ connection }) => {
    if (!generationId) throw new Error('Active Media generation is unavailable');
...
// GET /api/fulfillment/kpis — governed rights-capacity summary
router.get('/kpis', async (req, res) => {
  try {
    const result = await db.executeAsUser(`
      SELECT (SELECT COUNT(*) FROM fulfillment_centers WHERE is_active = 1) AS active_coverage_hubs,
             (SELECT NVL(SUM(capacity_units), 0) FROM fulfillment_centers WHERE is_active = 1) AS total_capacity_units,
             (SELECT NVL(SUM(quantity_on_hand), 0) FROM inventory) AS available_capacity_units,
             (SELECT NVL(SUM(quantity_reserved), 0) FROM inventory) AS reserved_capacity_units,
             (SELECT COUNT(*) FROM inventory WHERE quantity_on_hand <= reorder_point) AS capacity_alerts,
             (SELECT COUNT(*) FROM orders
               WHERE order_status IN ('pending', 'confirmed', 'processing')) AS active_campaign_requests
      FROM dual
    `, {}, req.demoUser);
    ...
    return featureUnavailable(res, {
      ...
      message: 'VPD-governed coverage KPI data is unavailable.',
    });
  }
});

// GET /api/fulfillment/centers
// VPD: MEDIA_APP_CTX filters fulfillment_centers by validated role/region.
router.get('/centers', async (req, res) => {
  try {
    ...
    return featureUnavailable(res, {
      feature: 'APPLICATION_CONTEXT_VPD',
      source: 'MEDIA_APP_CTX + DBMS_RLS',
      message: 'VPD-governed coverage data is unavailable.',
    });
  }
});
...
router.get('/nearest', async (req, res) => {
  try {
    ...
    const boundedResults = Number(maxResults);
    if (!Number.isInteger(boundedResults) || boundedResults < 1 || boundedResults > 25) {
      return res.status(400).json({ error: 'maxResults must be an integer from 1 to 25' });
    }
    if ((lat != null || lon != null)
        && (!Number.isFinite(Number(lat)) || Number(lat) < -90 || Number(lat) > 90
          || !Number.isFinite(Number(lon)) || Number(lon) < -180 || Number(lon) > 180)) {
      return res.status(400).json({ error: 'lat/lon must be finite WGS84 coordinates' });
    }
    if (customerId != null || productId != null) {
      if (!Number.isInteger(Number(customerId)) || Number(customerId) < 1
          || !Number.isInteger(Number(productId)) || Number(productId) < 1) {
        return res.status(400).json({
          error: 'customerId and productId must both be positive integers',
        });
      }
    }
    ...
    if (customerId && productId) {
      ...
      result = await executeSpatialWithPlan(req.demoUser, req.activeGenerationId, `
        SELECT /*+ GATHER_PLAN_STATISTICS */ /* MEDIA_SPATIAL_API_PROOF */
        ...
        WHERE c.customer_id = :customerId
          ...
          AND SDO_NN(
                fc.location,
                c.location,
                'sdo_num_res=' || TO_CHAR(GREATEST(:maxResults * 4, 12))
              ) = 'TRUE'
        ORDER BY SDO_GEOM.SDO_DISTANCE(c.location, fc.location, 0.005, 'unit=KM')
        FETCH FIRST :maxResults ROWS ONLY
      `, { customerId: parseInt(customerId), productId: parseInt(productId), maxResults: boundedResults });
      ...
      result = await executeSpatialWithPlan(req.demoUser, req.activeGenerationId, `
        ...
        ORDER BY SDO_GEOM.SDO_DISTANCE(
            SDO_GEOMETRY(2001, 4326, SDO_POINT_TYPE(:lon3, :lat3, NULL), NULL, NULL),
            fc.location, 0.005, 'unit=KM'
        )
        FETCH FIRST :maxResults ROWS ONLY
      `, { lat: parseFloat(lat), lon: parseFloat(lon),
           lat2: parseFloat(lat), lon2: parseFloat(lon),
           lat3: parseFloat(lat), lon3: parseFloat(lon),
           maxResults: boundedResults });
    } else {
      return res.status(400).json({ error: 'Provide customerId+productId or lat+lon' });
    }

    res.setHeader('X-Oracle-Feature', 'Oracle Spatial');
    const representativeResult = result.evidence?.representativeResult;
    if (!representativeResult?.centerId
        || representativeResult.distanceKm == null) {
      throw new Error(
        'Current-generation Spatial representative result is unavailable'
      );
    }
    res.setHeader('X-Oracle-Plan-Proof', 'IDX_FC_SPATIAL');
    res.json({
      generationId: req.activeGenerationId,
      ...
      representativeResult: {
        ...
        centerId: representativeResult.centerId,
        distanceKm: representativeResult.distanceKm,
      },
    });
  } catch (err) {
    featureUnavailable(res, {
      feature: 'ORACLE_SPATIAL',
      source: 'IDX_FC_SPATIAL',
      correlationId: req.headers['x-correlation-id'] || null,
      message: 'Native Oracle Spatial nearest-neighbor execution is unavailable.',
    });
  }
});
...
// Returns distribution zone polygons. If fulfillment_zones is empty, generates
