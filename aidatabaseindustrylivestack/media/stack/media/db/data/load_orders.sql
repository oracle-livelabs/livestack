/*
 * load_orders.sql
 * 3000 campaign orders with content line items, varied statuses, and community-signal attribution
 */

SET SERVEROUTPUT ON
PROMPT Loading campaign orders...
...
BEGIN
    ...
    FOR i IN 1..3000 LOOP
        ...
        -- 30% of campaign orders are associated with a audience/social signal
        ...
        -- Assign distribution hub for non-pending campaign orders
        ...
        -- Update total content revenue
        UPDATE orders SET order_total = v_total WHERE order_id = v_order_id;

        -- Create content routing/dispatch record for routed or completed requests
        IF v_status IN ('shipped', 'delivered') AND v_center_id IS NOT NULL THEN
            ...
            INSERT INTO shipments (
                ...
                CASE MOD(i, 4)
                    WHEN 0 THEN 'StreamOps'
                    WHEN 1 THEN 'MobileMedia'
                    WHEN 2 THEN 'StudioFleet'
                    ELSE 'TheatricalRoute'
                END,
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Campaign orders loaded: ' || v_count);
END;
/
