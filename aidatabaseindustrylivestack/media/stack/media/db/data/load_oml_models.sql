/*
 * load_oml_models.sql
 * Builds media demo Oracle Machine Learning feature views and persisted models.
 *
 * Run as the application schema owner after core media demo data is loaded.
 */
...
PROMPT =====================================================
PROMPT Building media Oracle Machine Learning models
PROMPT =====================================================
...
ranked AS (
    SELECT scored.*,
           NTILE(4) OVER (
             ORDER BY demand_signal_score DESC, avg_virality DESC, total_posts DESC, product_id
           ) AS demand_quartile
    FROM scored
)
SELECT product_id,
       ...
       units_sold,
       revenue,
       CASE WHEN demand_quartile = 1 THEN 'SURGE' ELSE 'STABLE' END AS surge_label
FROM ranked;
...
CREATE OR REPLACE PROCEDURE rebuild_media_oml_models
AS
    v_count NUMBER;

    PROCEDURE create_settings_table_if_missing(p_table_name IN VARCHAR2) IS
    BEGIN
        SELECT COUNT(*)
        INTO v_count
        FROM user_tables
        ...
        IF v_count = 0 THEN
            EXECUTE IMMEDIATE
              'CREATE TABLE ' || DBMS_ASSERT.SIMPLE_SQL_NAME(p_table_name) ||
...
BEGIN
    ...
    IF v_count = 4 THEN
        DBMS_OUTPUT.PUT_LINE(
          'All four immutable Media runtime models already exist; no model DDL executed.'
        );
        RETURN;
    ELSIF v_count <> 0 THEN
        RAISE_APPLICATION_ERROR(
          -20013,
          'Partial Media OML model inventory detected; refusing destructive repair.'
        );
    END IF;
    ...
    create_settings_table_if_missing('OML_DEMAND_SETTINGS');
    create_settings_table_if_missing('OML_CUSTOMER_SEGMENT_SETTINGS');
    create_settings_table_if_missing('OML_REVENUE_SETTINGS');
    create_settings_table_if_missing('OML_PRODUCT_CLUSTER_SETTINGS');

    EXECUTE IMMEDIATE 'DELETE FROM oml_demand_settings';
    EXECUTE IMMEDIATE 'DELETE FROM oml_customer_segment_settings';
    EXECUTE IMMEDIATE 'DELETE FROM oml_revenue_settings';
    EXECUTE IMMEDIATE 'DELETE FROM oml_product_cluster_settings';
    ...
    EXECUTE IMMEDIATE q'[INSERT INTO oml_demand_settings VALUES ('RFOR_NUM_TREES', '50')]';
...
BEGIN
    rebuild_media_oml_models;
END;
/

PROMPT Media OML model inventory:
...
PROMPT Media OML model build complete.
