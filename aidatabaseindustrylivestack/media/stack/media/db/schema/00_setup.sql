...
-- In-Memory Column Store observability (optional)
-- Lets /api/dashboard/inmemory report live V$IM_SEGMENTS sizes when the
-- database permits this grant. If unavailable, the app clearly labels
-- USER_SEGMENTS-based estimates with IS_ESTIMATED = true.
BEGIN
    EXECUTE IMMEDIATE 'GRANT SELECT ON SYS.V_$IM_SEGMENTS TO &&APP_SCHEMA_USER';
    DBMS_OUTPUT.PUT_LINE('Granted optional In-Memory telemetry grant on SYS.V_$IM_SEGMENTS.');
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Skipping optional In-Memory telemetry grant: ' || SQLERRM);
END;
/
