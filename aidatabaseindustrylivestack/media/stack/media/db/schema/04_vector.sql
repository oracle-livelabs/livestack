...
CREATE TABLE product_embeddings (
    ...
    embedding_model   VARCHAR2(100) DEFAULT 'all_MiniLM_L12_v2' NOT NULL,
...
CREATE TABLE semantic_matches (
    ...
    post_id           NUMBER NOT NULL REFERENCES social_posts(post_id),
    product_id        NUMBER NOT NULL REFERENCES products(product_id),
    -- similarity_score NOT NULL is enforced by the inline column constraint.
    similarity_score  NUMBER(6,5) NOT NULL, -- cosine similarity -1 to 1
    match_rank        NUMBER(4) NOT NULL,
    match_method      VARCHAR2(30) DEFAULT 'vector' NOT NULL
...
BEGIN
    ...
    SELECT p_post_id,
           pe.product_id,
           ROUND(
             1 - VECTOR_DISTANCE(v_post_vector, pe.embedding, COSINE),
             5
           ) AS sim_score,
           ROW_NUMBER() OVER (
    ...
    WHERE 1 - VECTOR_DISTANCE(v_post_vector, pe.embedding, COSINE) >= p_threshold
