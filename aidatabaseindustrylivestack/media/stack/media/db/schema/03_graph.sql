...
/*
-- Find media creators within 2 hops of an urgent signal post author
...
-- Find shortest propagation path between two media creators
SELECT *
FROM GRAPH_TABLE ( influencer_network
    MATCH SHORTEST (a IS influencer) -[e IS connects_to]->{1,5} (b IS influencer)
    WHERE a.handle = '@journeycaster_maya'
      AND b.handle = '@streamwatch_jen'
    COLUMNS (
        a.handle AS source,
        b.handle AS target,
...
-- Studio or label propagation: which creators propagate a studio or label through the network
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE b.brand_name = 'Aurora Studios'
    COLUMNS (
        v1.handle, v2.handle,
        e1.relationship_type,
        e2.connection_type,
