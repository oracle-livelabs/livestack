...
COMMENT ON TABLE app_users IS
  'Application users for the media VPD row-level security demo. Each user has a role and optional region filter.';

COMMENT ON TABLE brand_influencer_links IS
  'Many-to-many relationship between studios and labels and media creators. Tracks signal post counts, engagement, and attributed content revenue.';

COMMENT ON TABLE demand_regions IS
  'Geographic audience regions used for demand forecasting. Each region has a boundary polygon, population, income, and community signal density.';

COMMENT ON TABLE event_stream IS
  'Transactional event log using JSON payloads. Stores agent actions, campaign order events, and audience/social signal triggers for event-driven processing.';

COMMENT ON TABLE fulfillment_zones IS
  'Content routing zones around distribution hubs. Each zone has a boundary polygon and maximum routing time.';

COMMENT ON TABLE influencer_connections IS
  'Graph edges between media creators representing community connections. Used by SQL/PGQ graph queries for network analysis.';

COMMENT ON TABLE post_embeddings IS
  'Vector embeddings of audience/social signal text. 384-dim vectors from ALL_MINILM_L12_V2 ONNX model for semantic similarity search.';

COMMENT ON TABLE product_attributes IS
  'Extended content asset attributes stored as JSON. Holds category-specific metadata such as modality, acuity, duration, equipment, and service requirements.';

COMMENT ON TABLE product_embeddings IS
  'Vector embeddings of content asset descriptions. 384-dim vectors from ALL_MINILM_L12_V2 ONNX model for semantic service matching.';

COMMENT ON TABLE semantic_matches IS
  'Cached results of vector similarity matching between audience/social signal posts and content assets. Stores similarity scores and match methods.';
...
BEGIN
  ...
  INTO l_table_count
  FROM user_tables
  WHERE table_name = 'SOCIAL_POST_PAYLOADS';

  IF l_table_count > 0 THEN
    EXECUTE IMMEDIATE q'[COMMENT ON TABLE social_post_payloads IS 'Raw and enriched JSON payloads for audience/social signal posts. Stores the original platform API response and NLP enrichment data.']';
  END IF;
END;
/

COMMENT ON TABLE order_items IS
  'Requested services or supplies within a campaign order. Each links to a content asset with quantity and value proxy. Service value = quantity * unit_price.';

COMMENT ON TABLE products IS
  'Content assets, capacity slots, and production supply items. Each service belongs to a studio or label and has a category, value proxy, and tags. This is the ONLY table with content asset categories. Use products.category for category queries. Service value per category = SUM(order_items.line_total) grouped by products.category via JOIN order_items ON order_items.product_id = products.product_id.';
...
COMMENT ON COLUMN agent_actions.entity_type IS
  'Target entity type: content_asset, studio_label, creator, campaign_order, or signal_post';
...
COMMENT ON COLUMN app_users.role IS
  'User role: admin, analyst, regional operations manager, campaign planner, or viewer';
...
COMMENT ON COLUMN brands.brand_slug IS
  'URL-friendly unique identifier (e.g. aurora, streamwave, cinepulse)';
COMMENT ON COLUMN brands.brand_category IS
  'Studio or label category (e.g. Film Discovery, Theatrical Releases, Fan Communities). This is the program category, not the content asset category. Use products.category for service categories.';
COMMENT ON COLUMN brands.headquarters_city IS
  'City where the studio or label headquarters is located';
COMMENT ON COLUMN brands.headquarters_lat IS
  'Studio or label headquarters geographic latitude for spatial queries';
COMMENT ON COLUMN brands.headquarters_lon IS
  'Studio or label headquarters geographic longitude for spatial queries';
COMMENT ON COLUMN brands.founded_year IS
  'Year the studio or label was founded (4-digit year)';
COMMENT ON COLUMN brands.annual_revenue IS
  'Annual studio or label content revenue proxy in US dollars';
...
COMMENT ON COLUMN brand_influencer_links.brand_id IS
  'FK to brands/studios and labels. JOIN brands ON brands.brand_id = brand_influencer_links.brand_id';
COMMENT ON COLUMN brand_influencer_links.influencer_id IS
  'FK to influencers/media creators. JOIN influencers ON influencers.influencer_id = brand_influencer_links.influencer_id';
COMMENT ON COLUMN brand_influencer_links.relationship_type IS
  'Relationship type: organic, partner, referral, creator, or ambassador baseline value';
COMMENT ON COLUMN brand_influencer_links.post_count IS
  'Total number of signal posts this media creator has made about this studio or label';
COMMENT ON COLUMN brand_influencer_links.avg_engagement IS
  'Average engagement rate across signal posts for this studio or label/creator pair';
COMMENT ON COLUMN brand_influencer_links.revenue_attributed IS
  'Total content revenue in US dollars attributed to this creator for this studio or label';
COMMENT ON COLUMN brand_influencer_links.first_mention IS
  'Timestamp of the first signal post mentioning this studio or label by this creator';
COMMENT ON COLUMN brand_influencer_links.last_mention IS
  'Timestamp of the most recent signal post mentioning this studio or label by this creator';
...
COMMENT ON COLUMN customers.postal_code IS
  'Synthetic audience account postal/zip code';
...
COMMENT ON COLUMN demand_forecasts.product_id IS
  'FK to products/content assets. JOIN products ON products.product_id = demand_forecasts.product_id';
...
COMMENT ON COLUMN demand_forecasts.confidence_low IS
  'Lower bound of the confidence interval for predicted audience demand in units';
COMMENT ON COLUMN demand_forecasts.confidence_high IS
  'Upper bound of the confidence interval for predicted audience demand in units';
...
COMMENT ON COLUMN demand_forecasts.explanation IS
  'Human-readable explanation of content distribution factors driving this forecast';
...
COMMENT ON COLUMN demand_regions.population IS
  'Estimated population within the audience region';
COMMENT ON COLUMN demand_regions.avg_income IS
  'Average household income in US dollars within the audience region';
COMMENT ON COLUMN demand_regions.social_density IS
  'Audience/social signal density score 0-100 for this region';
COMMENT ON COLUMN demand_regions.demand_index IS
  'Composite audience demand index 0-100 combining population, income, and community signal density';
...
COMMENT ON COLUMN event_stream.event_type IS
  'Event type: campaign_order_created, signal_detected, capacity_triggered, agent_action, rights_alert';
...
COMMENT ON COLUMN fulfillment_zones.zone_type IS
  'Content routing zone type: same_day, next_day, standard, or express baseline value';
COMMENT ON COLUMN fulfillment_zones.max_delivery_hrs IS
  'Maximum content routing time in hours for this zone';
...
COMMENT ON COLUMN influencers.influencer_id IS
  'Unique media creator identifier (PK)';
COMMENT ON COLUMN influencers.display_name IS
  'Media and Entertainment creator display name or real name';
COMMENT ON COLUMN influencers.niche IS
  'Creator niche: Film Discovery, Theatrical Releases, Fan Communities, Kids and Family, Sports Media, Family Entertainment, Ad Inventory, Animation, Independent Film, Interactive Streaming, or Streaming and Live Entertainment';
COMMENT ON COLUMN influencers.city IS
  'City where the media creator is based';
...
COMMENT ON COLUMN influencer_connections.from_influencer IS
  'FK to influencers/media creators. Source node in the graph edge.';
COMMENT ON COLUMN influencer_connections.to_influencer IS
  'FK to influencers/media creators. Target node in the graph edge.';
...
COMMENT ON COLUMN influencer_connections.interaction_count IS
  'Total number of interactions between these two media creators';
...
COMMENT ON COLUMN influencer_connections.last_interaction IS
  'Timestamp of the most recent interaction between the two media creators';
...
COMMENT ON COLUMN inventory.quantity_incoming IS
  'Content asset capacity or supply units currently incoming';
...
COMMENT ON COLUMN orders.shipping_lat IS
  'Campaign order destination latitude for spatial routing';
COMMENT ON COLUMN orders.shipping_lon IS
  'Campaign order destination longitude for spatial routing';
COMMENT ON COLUMN orders.estimated_delivery IS
  'Estimated content completion or routing date';
COMMENT ON COLUMN orders.actual_delivery IS
  'Actual content completion or routing date (NULL if not yet completed)';
COMMENT ON COLUMN orders.updated_at IS
  'Timestamp of last campaign order status change';
...
COMMENT ON COLUMN order_items.line_total IS
  'Line total in US dollars. Equals quantity * unit_price. SUM for campaign order content revenue.';
COMMENT ON COLUMN order_items.fulfilled_from IS
  'FK to fulfillment_centers/distribution hubs. The center assigned to this line item. NULL if not yet assigned.';
...
COMMENT ON COLUMN post_product_mentions.product_id IS
  'FK to products/content assets. JOIN products ON products.product_id = post_product_mentions.product_id';
...
COMMENT ON COLUMN products.sku IS
  'Baseline SKU - unique content asset or supply code for capacity tracking';
COMMENT ON COLUMN products.description IS
  'Detailed content asset description text. Used for vector embeddings and full-text search.';
COMMENT ON COLUMN products.subcategory IS
  'Content asset subcategory within the main category (e.g. Co-op RPG under Streaming and Live Entertainment, Theatrical Releases under Live Entertainment)';
...
COMMENT ON COLUMN products.weight_kg IS
  'Content asset or supply weight in kilograms for routing cost calculation; virtual services use a near-zero value';
COMMENT ON COLUMN products.is_active IS
  '1=active and available for content intelligence, 0=inactive or hidden';
COMMENT ON COLUMN products.launch_date IS
  'Date the content asset was first available';
...
COMMENT ON COLUMN product_attributes.product_id IS
  'FK to products/content assets. JOIN products ON products.product_id = product_attributes.product_id';
COMMENT ON COLUMN product_attributes.attributes IS
  'JSON document with category-specific content asset attributes (duration, modality, equipment, acuity, requirements)';
...
COMMENT ON COLUMN product_embeddings.product_id IS
  'FK to products/content assets. JOIN products ON products.product_id = product_embeddings.product_id';
...
COMMENT ON COLUMN product_embeddings.embedding_text IS
  'Source text that was embedded (content asset name + description + tags)';
COMMENT ON COLUMN product_embeddings.embedding IS
  'VECTOR(384,FLOAT32) embedding from ALL_MINILM_L12_V2 for cosine similarity search';
...
COMMENT ON COLUMN semantic_matches.post_id IS
  'FK to social_posts. The audience/social signal post that was matched.';
COMMENT ON COLUMN semantic_matches.product_id IS
  'FK to products/content assets. The content asset matched to the signal post.';
COMMENT ON COLUMN semantic_matches.similarity_score IS
  'Cosine similarity score 0.0 to 1.0 between signal post and content asset embeddings';
...
COMMENT ON COLUMN shipments.order_id IS
  'FK to orders/campaign orders. JOIN orders ON orders.order_id = shipments.order_id';
COMMENT ON COLUMN shipments.center_id IS
  'FK to fulfillment_centers/distribution hubs. The center assigned to this campaign order.';
COMMENT ON COLUMN shipments.carrier IS
  'Content routing team or logistics partner name';
COMMENT ON COLUMN shipments.tracking_number IS
  'Routing reference number for content dispatch tracking';
...
COMMENT ON COLUMN shipments.ship_cost IS
  'Content routing cost proxy in US dollars';
COMMENT ON COLUMN shipments.shipped_at IS
  'Timestamp when the content route started';
COMMENT ON COLUMN shipments.delivered_at IS
  'Timestamp when the content route was completed (NULL if not yet completed)';
COMMENT ON COLUMN shipments.created_at IS
  'Timestamp when the content routing record was created';
...
COMMENT ON COLUMN social_posts.influencer_id IS
  'FK to influencers/media creators. JOIN influencers ON influencers.influencer_id = social_posts.influencer_id. NULL if not from a tracked creator.';
...
COMMENT ON COLUMN social_posts.detected_products IS
  'Comma-separated list of content asset names detected in this signal post by NLP';
...
BEGIN
  ...
  IF l_table_count > 0 THEN
    EXECUTE IMMEDIATE q'[COMMENT ON COLUMN social_post_payloads.payload_id IS 'Unique payload identifier (PK)']';
    EXECUTE IMMEDIATE q'[COMMENT ON COLUMN social_post_payloads.post_id IS 'FK to social_posts. JOIN social_posts ON social_posts.post_id = social_post_payloads.post_id']';
    EXECUTE IMMEDIATE q'[COMMENT ON COLUMN social_post_payloads.platform IS 'Source platform: instagram, tiktok, twitter, youtube, or threads']';
    EXECUTE IMMEDIATE q'[COMMENT ON COLUMN social_post_payloads.raw_payload IS 'Original JSON response from the platform or community signal API']';
    EXECUTE IMMEDIATE q'[COMMENT ON COLUMN social_post_payloads.enrichments IS 'JSON with NLP enrichment data: entities, topics, sentiment breakdown, and content asset mentions']';
    EXECUTE IMMEDIATE q'[COMMENT ON COLUMN social_post_payloads.created_at IS 'Timestamp when the payload was stored']';
  END IF;
END;
/
