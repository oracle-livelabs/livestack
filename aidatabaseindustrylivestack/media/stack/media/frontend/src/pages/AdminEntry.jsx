...
export default function AdminEntry({
  ...
  const readinessLabel = activeDataset?.readiness?.status || 'UNKNOWN';
  ...
  async function handleUpload() {
    if (!canMutate) {
      setUploadResult({ success: false, message: 'Select the active demo administrator to replace data.' });
      return;
    }
    if (!selectedFile) {
  ...
  async function handleRestoreDemo() {
    if (!canMutate) {
      setUploadResult({ success: false, message: 'Select the active demo administrator to restore data.' });
      return;
    }
  ...
  const headerEyebrow = isOverlay ? 'Media Dataset Tool' : 'Admin Entry';
  const headerContent = isOverlay ? 'Use Your Own Media Data' : 'Media Dataset Control';
  const headerDescription = isOverlay
    ? 'The seeded studio and streaming dataset stays active by default. Upload your own v1 ZIP with content assets, audience accounts, signal posts, campaign requests, distribution hubs, creator relationships, and demand forecasts, then validate it before replacing anything.'
    : 'Start on the seeded media demo dataset, or load your own v1 ZIP with studio, audience, campaign, distribution, creator, and forecast data before entering the experience.';
  const quickActionLabel = isOverlay ? 'Jump To Media Upload' : 'Load Your Own Media Data';
      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-1 text-xs uppercase tracking-[0.12em] text-[var(--color-text-dim)]">
              Required feature readiness: {readinessLabel}
            </p>
        ...
        <div ref={loadDataRef} className="grid md:grid-cols-2 gap-6">
          <section className="glass-card p-5 space-y-4">
            ...
            <p className="text-sm text-[var(--color-text-dim)]">
              Get the canonical media schema package with required and optional CSV templates for content assets, audience accounts, campaign requests, distribution hubs, audience signals, creator relationships, and demand forecasts.
            </p>
            <button
          ...
          <section className="glass-card p-5 space-y-4">
            ...
            >
              ...
              <p className="mt-2 text-xs text-[var(--color-text-dim)]">
                Select a `.zip` containing `manifest.json` and media table CSV files.
              </p>
            </label>
            <input
              id="import-zip"
        ...
        <div className="mt-6 glass-card p-5 space-y-4">
          ...
          <p className="text-xs text-[var(--color-text-dim)]">
            Preview the media demo dataset import to see row counts and warnings, then execute the restore to overwrite the current data.
          </p>
          <div className="flex flex-wrap gap-3">
            <button
              className="btn-ghost inline-flex items-center gap-2"
            ...
            <button
              ...
              disabled={isWorking || !restoreReady || !canMutate}
            >
