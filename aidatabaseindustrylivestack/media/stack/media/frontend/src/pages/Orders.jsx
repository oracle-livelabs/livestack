...
const SHIP_STATUS_STEPS = [
  { key: 'preparing', label: 'Planning', icon: Package },
  { key: 'picked', label: 'Rights Selected', icon: Package },
  { key: 'packed', label: 'Activation Packaged', icon: Package },
  { key: 'shipped', label: 'Launched', icon: Truck },
  { key: 'in_transit', label: 'In Flight', icon: Truck },
  { key: 'out_for_delivery', label: 'Market Ready', icon: Navigation },
  { key: 'delivered', label: 'Activated', icon: Check },
];

const ORDER_DETAIL_TABS = [
  { id: 'relational', label: 'Relational' },
  { id: 'json', label: 'JSON Duality View' },
  { id: 'route', label: 'Activation Route' },
];
...
const STATUS_OPTIONS = [
  ...
  { value: 'processing', label: 'Building Package' },
  { value: 'shipped', label: 'Launched' },
  { value: 'delivered', label: 'Activated' },
...
const CAMPAIGN_STATUS_LABELS = {
  pending: 'Pending',
  confirmed: 'Confirmed',
  processing: 'Building Package',
  shipped: 'Launched',
  in_transit: 'In Flight',
  out_for_delivery: 'Market Ready',
  delivered: 'Activated',
  cancelled: 'Cancelled',
  returned: 'Closed Review',
  preparing: 'Planning',
  picked: 'Rights Selected',
  packed: 'Activation Packaged',
};

function campaignStatusLabel(value) {
  const normalized = String(value || '').trim().toLowerCase();
  return CAMPAIGN_STATUS_LABELS[normalized] || String(value || '-').replace(/_/g, ' ');
}
...
function routeProviderColor(value) {
  return ROUTE_PROVIDER_COLORS[normalizeRouteProvider(value)] || '#4C825C';
}

function routeProviderLabel(value) {
  return ROUTE_PROVIDER_LABELS[normalizeRouteProvider(value)] || 'Activation Route';
}

/* ─── Campaign Order Duality Panel ─────────────────────────────────────────────── */
          ...
          <div className="orders-detail-tabbar">
            <div className="orders-detail-tabset">
              ...
              <span className="text-[10px] text-[var(--color-text-dim)] ml-3 hidden sm:inline">
                Same campaign - relational, JSON, and route views
              </span>
            </div>
            <JetButton
              label="Close"
          ...
          {view === 'relational' && (
            <div className="p-4 space-y-3">
              {loadingDetail ? (
                ...
                <p className="text-sm text-[var(--color-text-dim)] text-center py-4">Campaign request not found</p>
              ) : (
                <>
                  ...
                  {/* Campaign request summary */}
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      { label: 'Audience Account', value: `${order.FIRST_NAME} ${order.LAST_NAME}` },
                      { label: 'Location', value: `${order.CITY}, ${order.STATE_PROVINCE}` },
                      { label: 'Total', value: formatCurrency(order.ORDER_TOTAL) },
                      { label: 'Activation Cost', value: formatCurrency(order.SHIPPING_COST) },
                    ].map(s => (
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-[10px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)]">
                        ...
                        <th className="text-left py-1.5 px-2">Content Asset</th>
                        <th className="text-left py-1.5 px-2">Studio or Label</th>
          ...
          {view === 'json' && (
            <div className="p-4 space-y-3">
              {loadingDuality ? (
                ...
                <div className="text-sm tone-red text-center py-4" role="status">
                ...
                <>
                  ...
                  <div className="rounded-lg p-3 text-xs leading-relaxed" style={{ background: 'rgba(170,100,59,0.06)', border: '1px dashed rgba(170,100,59,0.3)' }}>
                    ...
                    <span className="text-[var(--color-text-dim)]"> - This is the exact same campaign request data from the relational tab, but accessed through
                    ...
                    single read-only JSON document with nested items from the same committed ACID data.</span>
                  </div>
...
{JSON.stringify(duality.document, null, 2)}
          ...
          {view === 'route' && (
            <div className="p-4 space-y-3">
              {loadingDetail ? (
                <div className="flex items-center gap-2 text-sm text-[var(--color-text-dim)] py-4 justify-center">
                  <Loader2 size={14} className="animate-spin" /> Loading activation route data...
                </div>
              ) : !order ? (
                <p className="text-sm text-[var(--color-text-dim)] text-center py-4">Campaign request not found</p>
                ...
                const carrierColor = routeProviderColor(shipment?.CARRIER);
                ...
                return (
                  <>
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      ...
                      {routeGeometry && (
                        <JetButton
                          label={showDrivingRoute ? 'Coverage Route' : 'Arc Only'}
                      ...
                      {shipment && (
                        <span className="flex items-center gap-1 text-[var(--color-text-dim)]">
                          ...
                          <span style={{ color: carrierColor }} className="font-semibold">{routeProviderLabel(shipment.CARRIER)}</span>
                      ...
                      >
                        ...
                        <CircleMarker center={from} radius={8} fillColor="#437C94" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            ...
                            <div className="route-map-tooltip__content">
                              <span className="route-map-tooltip__label">Coverage Hub</span>
                              <span>{order.CENTER_NAME || 'Coverage Hub'}</span>
                            </div>
                          </Tooltip>
                        </CircleMarker>
                        ...
                        <CircleMarker center={to} radius={8} fillColor="#4C825C" fillOpacity={0.9} color="#fff" weight={2}>
                          <Tooltip permanent direction="top" offset={[0, -10]}
                            className="route-map-tooltip">
                            <div className="route-map-tooltip__content">
                              <span className="route-map-tooltip__label">Audience Account</span>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {[
                        ...
                        { label: 'Est. Activation', value: estHours ? `${estHours} hrs` : '-', color: '#AA643B' },
                        { label: 'Activation Cost', value: shipment?.SHIP_COST ? formatCurrency(shipment.SHIP_COST) : '-', color: '#437C94' },
                        { label: 'Status', value: campaignStatusLabel(shipment?.SHIP_STATUS || order.ORDER_STATUS), color: carrierColor },
                      ].map(c => (
                    ...
                    {shipment && (
                      <div className="rounded-lg p-3" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
                        <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Activation Progress</p>
...
-- SDO_GCDR.ELOC_DRIVE_TIME_POLYGON for isochrone zones` : `-- Distance between distribution hub and audience account
...
-- This statement calculates exact distance; indexed SDO_NN proof is shown in
-- the Rights, Capacity & Live Event Coverage scene.`}</pre>
                    </div>
                  </>
                );
              })()}
...
export default function Orders() {
  ...
  const { data: orders, loading, error: ordersError } = useData(
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <RegisterOraclePanel title="Campaign & Rights Requests">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-[var(--color-text)] leading-relaxed">
              Campaign and rights requests are stored in a classic <span className="tone-ocean font-mono">relational schema</span> - ACID transactions, foreign keys, referential integrity.
              Oracle's <span className="font-mono text-[var(--color-text)]">JSON Duality Views</span> let the <em>exact same data</em> be read as JSON documents,
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label="Canonical read-only views" color="green" />
          ...
          <div>
            <p className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider mb-2">
              ORDERS_DV - Campaign &amp; Rights Requests + Line Items as JSON
            </p>
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              REST-style read access to campaign and rights requests with nested line items from the{' '}
              <span className="font-mono text-[var(--color-text)]">orders</span> and{' '}
              <span className="font-mono text-[var(--color-text)]">order_items</span> tables.
            </p>
...
FROM orders o;`} />
          ...
          <div>
            <p className="text-[10px] font-semibold text-[var(--color-text)] uppercase tracking-wider mb-2">
              PRODUCTS_INVENTORY_DV - Content Assets + Rights Capacity as JSON
            </p>
            <p className="text-xs text-[var(--color-text-dim)] mb-2 leading-relaxed">
              Content Assets with nested rights capacity across all distribution hubs. One document, two tables.
            </p>
...
-- Result: {"_id":1, "status":"launched", "items":[...]}`} />
          </div>
          ...
          <div className="rounded-lg p-3 space-y-2" style={{ background: 'var(--color-surface-muted)', border: '1px solid var(--color-border)' }}>
            ...
            <DiagramBox label="ORDERS_DV" sub="Read-only JSON document: {_id, status, items: [...]}" color="#AA643B" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕</div>
            <DiagramBox label="Content assets + rights capacity" sub="Content Asset catalog + distribution capacity levels" color="#437C94" />
            <div className="text-center text-[10px] text-[var(--color-text)]">↕ CREATE JSON RELATIONAL DUALITY VIEW</div>
            <DiagramBox label="PRODUCTS_INVENTORY_DV" sub="Read-only JSON document: {_id, productName, brand, inventory: [...]}" color="#AA643B" />
          </div>
          ...
          <div className="rounded-lg p-3 text-center" style={{ background: 'rgba(170,100,59,0.06)', border: '1px dashed rgba(170,100,59,0.3)' }}>
            <p className="text-[10px] text-[var(--color-text-dim)] leading-relaxed">
              ...
              Both interfaces read the same committed relational rows, so there is no copied document store or synchronization lag.
            </p>
          </div>

          {/* VPD on Campaign Requests */}
          <div>
            <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
              Virtual Private Database (VPD) on Campaign &amp; Rights Requests
            </p>
            <p className="text-sm text-[var(--color-text)] leading-relaxed mb-2">
              ...
              <code className="text-xs tone-teal mx-1">ORDERS</code> table restricts regional operations managers
              to requests routed through their assigned distribution hubs. The policy function queries the existing{' '}
              <code className="text-xs tone-teal mx-1">fulfillment_center_id</code> key against the user's assigned region.
              Admins and analysts see the global request set; a regional manager sees only their subset.
            </p>
          </div>
          <div className="flex flex-wrap gap-1.5">
            <FeatureBadge label="DBMS_RLS (Campaign Requests)" color="red" />
          ...
          <SqlBlock code={`-- Canonical VPD policy function for ORDERS
CREATE OR REPLACE FUNCTION vpd_media_rows (
    p_schema IN VARCHAR2, p_table IN VARCHAR2
) RETURN VARCHAR2 AS
    v_auth  VARCHAR2(1)  := SYS_CONTEXT('MEDIA_APP_CTX', 'AUTHENTICATED');
...
BEGIN
    ...
    END IF;
    RETURN '1 = 0'; -- restricted, missing, or invalid context
END;

-- Applied via:
DBMS_RLS.ADD_POLICY('ORDERS','VPD_MEDIA_SELECT',
  ...
  statement_types => 'SELECT');`} />
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <FileText className="tone-ocean" /> Campaign &amp; Rights Requests
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Click any request to toggle between <span className="font-semibold text-[var(--color-text)]">Relational</span>,{' '}
          <span className="font-semibold text-[var(--color-text)]">JSON Duality View</span>, and Activation Route views for the same governed campaign data.
        </p>
      </div>
          ...
          >
            ...
            <div className="flex items-center gap-2 flex-wrap flex-1">
              ...
              <span className="text-[var(--color-text-dim)] text-xs">
                {isFM
                  ? `Filtered to ${currentUser.REGION} - ${(orders || []).length} campaign requests visible`
      ...
      {/* Campaign & Rights Requests Table */}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[11px] text-[var(--color-text-dim)] uppercase tracking-wider border-b border-[var(--color-border)] bg-[var(--color-surface)]/50">
                ...
                <th className="text-left py-3 px-4">Audience Account</th>
                <th className="text-left py-3 px-4">Location</th>
                <th className="text-center py-3 px-4">Status</th>
                <th className="text-right py-3 px-4">Items</th>
                <th className="text-right py-3 px-4">Total</th>
                <th className="text-center py-3 px-4">Audience Signal</th>
                <th className="text-left py-3 px-4">Coverage Hub</th>
                    ...
                    <tr key={o.ORDER_ID}
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${STATUS_COLORS[o.ORDER_STATUS] || ''}`}>
                          {campaignStatusLabel(o.ORDER_STATUS)}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-4 text-center">
                        {o.SOCIAL_DRIVEN ? (
                          <span className="tone-rose text-xs font-semibold">Audience Signal</span>
                        ) : (
