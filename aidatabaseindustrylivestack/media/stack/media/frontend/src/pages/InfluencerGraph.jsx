        ...
        tip.html(`
          <div style="font-size:11px;min-width:180px;line-height:1.7;color:#161513">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;border-bottom:1px solid rgba(49,45,42,0.12);padding-bottom:6px">
              ...
              ${isVerified(d) ? '<span style="color:#4C825C;font-size:10px;font-weight:600">check verified</span>' : ''}
            </div>
            <div style="display:grid;grid-template-columns:auto auto;gap:2px 12px">
              ...
              <span style="color:#6F757E">Followers</span><span>${formatNumber(d.FOLLOWER_COUNT || 0)}</span>
  ...
  return (
    <div className="glass-card p-4 space-y-4">
          ...
          <div>
            <p className="font-semibold text-sm flex items-center gap-1">
              ...
              {(node.IS_VERIFIED === 'Y' || node.IS_VERIFIED === 1) && (
                <span className="tone-pine text-xs">check</span>
              )}
            </p>
      ...
      {node.type === 'center' && network?.brands?.length > 0 && (
        <div>
          <p className="text-[10px] font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">Studio or Label Relationships</p>
          <div className="space-y-1">
...
function GraphQueryExplorer() {
  ...
  const { beginRequest, isCurrent } = useGenerationRequestGuard(() => {
    setQueries([]);
    setActiveQuery(null);
    setParams({});
    setResult(null);
  ...
  useEffect(() => {
    const token = beginRequest('query-catalog');
    api.graph.exampleQueries().then(qs => {
      if (isCurrent(token)) setQueries(qs);
    }).catch(() => {});
  ...
  const runQuery = useCallback(async () => {
    ...
    const token = beginRequest('query-runner');
    setRunning(true);
    setError(null);
    setResult(null);
    try {
  ...
  }, [activeQuery, beginRequest, isCurrent, params]);
    ...
    <div className="glass-card p-5 space-y-4">
      ...
      {activeQuery && (
        <div className="space-y-4">
          ...
          {/* Error */}
          {error && (
            <div className="p-3 rounded-lg" role="alert" style={{ background: 'rgba(199,70,52,0.1)', border: '1px solid rgba(199,70,52,0.3)' }}>
              <p className="text-sm font-semibold tone-red">
                Oracle Property Graph SQL/PGQ is unavailable
              </p>
              <p className="text-xs text-[var(--color-text-dim)] mt-1">{error}</p>
            </div>
          )}
...
export default function InfluencerGraph() {
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Oracle panel */}
      <RegisterOraclePanel title="Creator Influence Network">
        <div className="space-y-4">
          <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label="Studio Attribution" color="green" />
          </div>
...
CREATE PROPERTY GRAPH influencer_network
  VERTEX TABLES (
    ...
    brands         KEY (brand_id)      LABEL studio
      PROPERTIES (brand_id, brand_name,
        brand_category, social_tier),
    products       KEY (product_id)    LABEL content_asset
    ...
    post_product_mentions   KEY (mention_id)
      ...
      PROPERTIES (confidence_score, mention_type)
          ...
          <div className="grid grid-cols-2 gap-1.5 mt-2">
            ...
            <DiagramBox label="influencer_connections" sub="Edge table" color="#AA643B" />
            <DiagramBox label="brand_influencer_links" sub="Studio attribution edges" color="#A36472" />
            <DiagramBox label="post_product_mentions" sub="Content attribution" color="#4C825C" />
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              ...
              {currentUser?.ROLE === 'fulfillment_mgr' ? (
                ...
                <span className="tone-ocean"> This persona is restricted by fail-closed VPD.</span>
              )}
            </p>
          </div>
          <div className="flex flex-wrap gap-1.5">
          ...
          <SqlBlock code={`-- Canonical VPD policy function (applied to graph tables)
CREATE FUNCTION vpd_media_rows(
...
--   post_product_mentions`} />
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <Network className="text-[var(--color-accent)]" /> Creator Influence Network
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Explore creator, studio, label, and fan-community propagation with <span className="tone-sienna">Oracle Property Graph (SQL/PGQ)</span>.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search creator, platform, or niche..."
          ...
          <div className="glass-card p-3 max-h-[480px] overflow-y-auto">
            <h3 className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2 flex items-center gap-1">
              <Users size={11} /> Creators {rawInfluencers?.length ? `(${rawInfluencers.length})` : ''}
            </h3>
            {loading ? (
              ...
              <div
                role="alert"
                data-testid="creator-graph-base-unavailable"
                className="rounded-lg border border-[var(--color-danger)]/40 bg-[var(--color-danger)]/5 p-3"
              >
                <p className="text-xs font-semibold tone-red">
                  Creator graph base data is unavailable
                </p>
              ...
              <p className="text-xs text-[var(--color-text-dim)] py-4 text-center">
                No creators available for this persona
              </p>
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          {loadingNet ? (
            ...
            <div
              role="alert"
              data-testid="creator-graph-network-unavailable"
              className="glass-card p-14 text-center"
            >
              <Network size={28} className="mx-auto mb-3 tone-red" />
              <p className="text-sm font-semibold tone-red">
                Creator graph network data is unavailable
              </p>
          ...
          {network?.brands?.length > 0 && !clickedNode && (
            <div className="glass-card p-4">
              <h4 className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">
                Studio, Label, and Content Line Relationships - {network.center?.HANDLE}
              </h4>
