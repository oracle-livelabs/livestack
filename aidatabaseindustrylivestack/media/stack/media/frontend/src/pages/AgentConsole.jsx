...
const AGENT_DISPLAY_LABELS = {
  trend_detection_agent: 'audience signal agent',
  inventory_agent: 'rights capacity agent',
  fulfillment_agent: 'coverage agent',
  master_orchestrator: 'media orchestrator',
  chat_agent: 'agent runtime',
};

const ACTION_DISPLAY_LABELS = {
  capacity_action: 'rights capacity action',
  trend_alert: 'audience signal action',
  revenue_action: 'content revenue action',
  chat_query: 'agent chat question',
};

const ENTITY_DISPLAY_LABELS = {
  product: 'content asset',
  order: 'campaign request',
  customer: 'audience account',
  trends: 'signal intent',
  distribution: 'coverage intent',
  revenue: 'revenue intent',
};

function displayIdentifier(value, labels = {}) {
  const key = String(value || '').trim().toLowerCase();
  return labels[key] || String(value || '').replace(/_/g, ' ');
}

function actionPreviewText(action, payload) {
  if (!payload) return '';
  if (action?.ACTION_TYPE === 'chat_query' && payload.question) {
    return `Asked: ${payload.question}`;
  }
  return payload.reason || payload.question || payload.product_name || payload.strategy || JSON.stringify(payload).slice(0, 120);
}

function buildOptimisticChatAction(result, question) {
  const createdAt = new Date().toISOString();
  const toolsCalled = Array.isArray(result?.toolsUsed) ? result.toolsUsed.length : 0;
  return {
    ACTION_ID: `chat-${createdAt}-${Math.random().toString(36).slice(2, 8)}`,
    AGENT_NAME: 'chat_agent',
    ACTION_TYPE: 'chat_query',
    ENTITY_TYPE: result?.intent || null,
    ENTITY_ID: null,
    DECISION_PAYLOAD: JSON.stringify({
      question,
      team: result?.team || 'agent runtime',
      profile: result?.profile || null,
      runtime_mode: result?.runtimeMode || null,
      agent_used: Boolean(result?.agentUsed),
      tools_called: toolsCalled,
      reason: `Chat query routed to ${result?.team || 'agent runtime'}${result?.intent ? ` (intent: ${result.intent})` : ''}`,
    }),
    CONFIDENCE: 0.9,
...
const TEAM_INFO = {
  MEDIA_SIGNAL_TEAM: { label: 'Audience Signal Agent', color: '#AA643B', iconClass: 'oj-fwk-icon-sortrelevancehigh', desc: 'Audience and community signal analysis' },
  MEDIA_DISTRIBUTION_TEAM: { label: 'Coverage & Capacity Agent', color: '#437C94', iconClass: 'oj-fwk-icon-tree-document', desc: 'Rights capacity, coverage readiness, and coverage-hub routing' },
  MEDIA_REVENUE_TEAM: { label: 'Content Revenue Agent', color: '#4C825C', iconClass: 'oj-fwk-icon-grid', desc: 'Campaign requests and content revenue' },
  SOCIAL_TREND_TEAM: { label: 'Audience Signal Agent', color: '#AA643B', iconClass: 'oj-fwk-icon-sortrelevancehigh', desc: 'Audience and community signal analysis' },
  FULFILLMENT_TEAM: { label: 'Coverage & Capacity Agent', color: '#437C94', iconClass: 'oj-fwk-icon-tree-document', desc: 'Rights capacity, coverage readiness, and coverage-hub routing' },
  COMMERCE_TEAM: { label: 'Content Revenue Agent', color: '#4C825C', iconClass: 'oj-fwk-icon-grid', desc: 'Campaign requests and content revenue' },
};

const EXAMPLE_QUESTIONS = [
  { text: 'Which content assets are seeing the highest demand right now?', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'trends' },
  { text: 'Show me content revenue by content category', iconClass: 'oj-fwk-icon-grid', team: 'revenue' },
  { text: 'Which launch event is at greatest churn or revenue risk this weekend?', iconClass: 'oj-fwk-icon-message-warning', team: 'revenue' },
  { text: 'Which content assets have low rights capacity?', iconClass: 'oj-fwk-icon-tree-document', team: 'coverage' },
  { text: 'What percentage of campaign requests are audience-signal-driven?', iconClass: 'oj-fwk-icon-info', team: 'revenue' },
  { text: 'Find urgent audience-signal posts in the last 24 hours', iconClass: 'oj-fwk-icon-sortrelevancehigh', team: 'trends' },
  { text: 'Check capacity for Midnight Harbor Premiere Window', iconClass: 'oj-fwk-icon-tree-document', team: 'coverage' },
  { text: 'Find nearest coverage hub with same-day studio slots for an audience account in Miami', iconClass: 'oj-fwk-icon-arrowtail-e', team: 'coverage' },
];
...
// ── Content coverage route map (rendered inside chat messages) ─────────────────────
function CoverageRouteMap({ routeData }) {
          ...
          <CircleMarker center={[customer.lat, customer.lon]} radius={7}
            ...
            <MapTooltip permanent direction="bottom" offset={[0, 8]}
              ...
              <div style={{ fontSize: 10 }}>
                <strong>Audience Account</strong><br />{customer.city}, {customer.state}
              </div>
            </MapTooltip>
          </CircleMarker>
        </MapContainer>
...
function ChatAgent({ activeProfile, onActionLogged }) {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      const result = await api.agents.chat(question, activeProfile);
      ...
      if (onActionLogged) {
        Promise.resolve(onActionLogged(result, question)).catch((error) => {
          console.error('Failed to refresh recent agent actions:', error);
        });
      }
    } catch (err) {
      setMessages(prev => [...prev, {
  ...
  }, [input, sending, activeProfile, onActionLogged]);
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--color-text-dim)]">
              Ask questions - auto-routed to the <span className="font-semibold text-[var(--color-text)]">Signal</span>, <span className="font-semibold text-[var(--color-text)]">Coverage &amp; Capacity Agent</span>, or <span className="font-semibold text-[var(--color-text)]">Revenue</span> agents
            </p>
          </div>
        </div>
        {messages.length > 0 && (
        ...
        {messages.length === 0 && (
          <div className="space-y-3 py-4">
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-[var(--color-text-dim)]">Ask me anything about your media content intelligence data</p>
              ...
              {msg.role === 'agent' && (
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {msg.runtimeMode && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded surface-ocean-soft tone-ocean">
                        {msg.runtimeMode === 'ollama' ? 'MODEL + ORACLE' : 'ORACLE TOOL PATH'}
                      </span>
                    )}
                  ...
                  {msg.data && msg.data.type === 'route' && (
                    <CoverageRouteMap routeData={msg.data} />
                  )}
...
export default function AgentConsole() {
  ...
  const { data: actions, refetch: refetchActions, setData: setActions } = useData(() => api.agents.actions({ limit: 3 }));
  const refreshActionsAfterChat = useCallback(async (result, question) => {
    const optimisticAction = buildOptimisticChatAction(result, question);
    setActions((previous) => [optimisticAction, ...(Array.isArray(previous) ? previous : [])].slice(0, 3));
    await refetchActions();
  }, [refetchActions, setActions]);
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              These agent workflows run through the application layer, with <span className="font-mono text-[var(--color-text)]">{activeProfileInfo.short}</span> handling reasoning
              and Oracle AI Database 26ai executing SQL and PL/SQL tools against live data. Three specialist teams (<code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #AA643B' }}>MEDIA_SIGNAL_TEAM</code>,&nbsp;
              <code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #437C94' }}>MEDIA_DISTRIBUTION_TEAM</code>, <code className="text-xs font-semibold" style={{ color: 'var(--color-text)', borderBottom: '1px solid #4C825C' }}>MEDIA_REVENUE_TEAM</code>) route work across
              signal analysis, coverage routing, and content operations tasks. Oracle stores the source data, runs the queries, and records decisions in
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <FeatureBadge label={activeProfileInfo.short} color="pink" />
...
-- Selected runtime profile provides reasoning; Oracle remains the data and execution layer

-- Example flow:
-- 1. Classify the request as MEDIA_SIGNAL_TEAM
-- 2. Call detect_trending_products(p_hours=>24)
-- 3. Join rights capacity and coverage data in Oracle
...
VALUES ('trend_detection_agent','capacity_action','content_asset',
  :product_id, :json_payload, 0.92, 'proposed');`} />
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { team: 'MEDIA_SIGNAL_TEAM', agent: 'AUDIENCE_SIGNAL_AGENT', tools: ['SIGNAL_SQL_TOOL', 'DETECT_TRENDS_TOOL', 'CREATOR_NETWORK_TOOL', 'LOG_DECISION_TOOL'], color: '#A36472' },
                { team: 'MEDIA_DISTRIBUTION_TEAM', agent: 'COVERAGE_CAPACITY_AGENT', tools: ['CAPACITY_SQL_TOOL', 'CHECK_CAPACITY_TOOL', 'COVERAGE_ROUTE_TOOL', 'LOG_DECISION_TOOL'], color: '#AA643B' },
                { team: 'MEDIA_REVENUE_TEAM', agent: 'CONTENT_REVENUE_AGENT', tools: ['REVENUE_SQL_TOOL', 'LOG_DECISION_TOOL'], color: '#4C825C' },
              ].map(t => (
          ...
          <div>
            ...
            <div className="space-y-1" style={{ fontSize: 9 }}>
              <DiagramBox label="Urgent Audience Signal Detected" sub="social_posts  -  virality_score >= 75" color="#AA643B" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="MEDIA_SIGNAL_TEAM" sub="Ollama reasoning + tool routing" color="#796087" />
              ...
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="MEDIA_DISTRIBUTION_TEAM" sub="Capacity check  -  routing logic" color="#437C94" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
              <DiagramBox label="MEDIA_REVENUE_TEAM" sub="Campaign value  -  promotion windows  -  routing" color="#4C825C" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <JetGlyph iconClass="oj-fwk-icon-users" className="agent-console-page-glyph tone-plum" /> Media and Entertainment Action Console
          </h2>
          <p className="text-sm text-[var(--color-text-dim)] mt-1">
            Seer Media agent teams turn Midnight Harbor launch risks into auditable audience, coverage, retention, and monetization actions using <span className="font-semibold text-[var(--color-text)]">Ollama</span> for reasoning and Oracle for data, SQL execution, PL/SQL tools, and action logging.
          </p>
        </div>
        <div className="flex items-center gap-2">
          {/* AI Profile Selector */}
      ...
      {/* ── Chat Agent ── */}
      <ChatAgent activeProfile={activeProfile} onActionLogged={refreshActionsAfterChat} />
            ...
            return (
              <div key={a.ACTION_ID} className="flex items-start gap-3 p-3 rounded-lg hover:bg-[var(--color-surface-hover)] transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium">{displayIdentifier(a.ACTION_TYPE, ACTION_DISPLAY_LABELS)}</span>
                    <span className="px-1.5 py-0.5 rounded text-[9px] font-medium"
                      ...
                      {displayIdentifier(a.AGENT_NAME, AGENT_DISPLAY_LABELS)}
                    </span>
                    {a.ENTITY_TYPE && (
                      <span className="text-[10px] text-[var(--color-text-dim)]">
                        {displayIdentifier(a.ENTITY_TYPE, ENTITY_DISPLAY_LABELS)}{a.ENTITY_ID ? ` #${a.ENTITY_ID}` : ''}
                      </span>
                    )}
                  </div>
                  {payload && (
                    <p className="text-xs text-[var(--color-text-dim)] mt-0.5 truncate max-w-lg">
                      {actionPreviewText(a, payload)}
                    </p>
                  )}
                </div>
