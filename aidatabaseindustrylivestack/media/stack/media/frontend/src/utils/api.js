...
const DATASET_COMMAND_HEADERS = Object.freeze({
  'X-Media-Command': 'dataset-mutation',
});
...
export async function apiFetch(endpoint, options = {}) {
  ...
  if (_currentDemoUser !== null && _currentDemoUser !== undefined) {
    // Preserve an explicitly requested empty identity. The API must see the
    // empty X-Demo-User header and reject it; omitting the header means the
    // distinct, intentional missing-identity Viewer path.
    headers['X-Demo-User'] = String(_currentDemoUser);
  }
...
function buildApiError(payload, status, correlationId = null) {
  ...
  error.available = payload.available === false ? false : null;
...
export const api = {
  dashboard: {
    ...
    nativeJsonAuditEvidence: () => apiFetch('/dashboard/native-json-audit-evidence'),
  },
  social: {
    posts: (params = {}) => {
  ...
  ml: {
    ...
    modelProvenance: () => apiFetch('/ml/model-provenance'),
    ...
    productClusters: (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      return apiFetch(`/ml/product-clusters?${qs}`);
    },
  ...
  import: {
    ...
    validate: (file) => apiUploadFile('/import/validate', file, { datasetMutation: true }),
    upload: (file) => apiUploadFile('/import/upload', file, { datasetMutation: true }),
