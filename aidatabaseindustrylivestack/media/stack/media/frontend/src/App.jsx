...
const CUSTOMER_STORAGE_KEY = 'mediaLivestack.customerName';
const CUSTOMER_ALIASES = [
  'Seer Media',
  'Seer Media Group',
  'Media and Entertainment Content Intelligence LiveStack',
...
const NAV_ITEMS = [
  ...
  { id: 'dashboard', label: 'Launch Operations Command Center', iconClass: 'oj-fwk-icon oj-fwk-icon-grid', featureTags: ['In-Memory', 'Native JSON'] },
  { id: 'social', label: 'Audience Momentum & Safety Signals', iconClass: 'oj-fwk-icon oj-fwk-icon-sortrelevancehigh', featureTags: ['Vector Search', 'VPD'] },
  { id: 'graph', label: 'Creator & Community Graph', iconClass: 'oj-fwk-icon oj-fwk-icon-node-expand', featureTags: ['Property Graph', 'SQL/PGQ'] },
  { id: 'fulfillment', label: 'Rights, Capacity & Live Event Coverage', iconClass: 'oj-fwk-icon oj-fwk-icon-calendar-clock', featureTags: ['Oracle Spatial', 'VPD'] },
  { id: 'orders', label: 'Campaign & Rights Requests', iconClass: 'oj-fwk-icon oj-fwk-icon-tree-document', featureTags: ['JSON Duality', 'VPD'] },
  { id: 'oml', label: 'Engagement, Revenue & Retention Forecasts', iconClass: 'oj-fwk-icon oj-fwk-icon-view', featureTags: ['OML', 'Vector Search'] },
  { id: 'askdata', label: 'Ask Media and Entertainment Data', iconClass: 'oj-fwk-icon oj-fwk-icon-magnifier', featureTags: ['Natural Language SQL', 'Live Schema'] },
  { id: 'agents', label: 'Media and Entertainment Action Console', iconClass: 'oj-fwk-icon oj-fwk-icon-users', featureTags: ['PL/SQL Tools', 'JSON Audit'] },
];
...
function replaceCustomerTerms(value, profile) {
  ...
  let next = value.replace(/\bseer-media\.example\b/gi, profile.customerDomain);
  for (const alias of CUSTOMER_ALIASES) {
    next = next.replace(new RegExp(`\\b${escapeRegExp(alias)}\\b`, 'g'), profile.customerName);
  }
  next = next.replace(/\bMedia (?:&|and) Entertainment\b/g, profile.customerName);
...
function hasReplaceableCustomerTerms(value) {
  return typeof value === 'string' && (
    /\bseer-media\.example\b/i.test(value) ||
    /\bSeer(?: Media Group| Media| Entertainment| Streaming)?\b/.test(value) ||
    /Media (?:&|and) Entertainment(?: Content Intelligence)? LiveStack/.test(value)
  );
}
    ...
    <div className="customer-name-tool">
      ...
      {isOpen && (
        <form className="customer-name-tool__panel" onSubmit={submit}>
          <label className="customer-name-tool__label" htmlFor="customer-name-input">Replace demo organization</label>
          <input
          ...
          <p className="customer-name-tool__hint">Replaces Seer Media default labels on visible screens.</p>
        </form>
      )}
    </div>
  );
...
export default function App() {
  ...
  const [sceneBoundary, setSceneBoundary] = useState(0);
  ...
  useEffect(() => {
    const invalidateSceneBoundary = (event) => {
      datasetRequestRef.current += 1;
      setActiveDataset(null);
      if (event?.detail?.preserveSceneState !== true) {
        setSceneBoundary((value) => value + 1);
      }
      void refreshActiveDataset();
    };
    window.addEventListener('media:identity-changed', invalidateSceneBoundary);
    window.addEventListener('media:dataset-revision', invalidateSceneBoundary);
    return () => {
      window.removeEventListener('media:identity-changed', invalidateSceneBoundary);
      window.removeEventListener('media:dataset-revision', invalidateSceneBoundary);
    };
  }, [refreshActiveDataset]);
            ...
            <aside className="app-sidebar">
              ...
              <nav className="app-nav" aria-label="Primary">
                {NAV_ITEMS.map(({ id, label, iconClass, featureTags }) => (
                  ...
                  >
                    <span className={`${iconClass} app-nav-icon`} aria-hidden="true" />
                ...
                <div className="app-page-frame">
                  {activePage === 'welcome' ? (
                    ...
                    (() => {
                      ...
                      return <PageComponent key={`${activePage}:${sceneBoundary}`} {...pageProps} />;
                    })()
                  )}
                </div>
              </main>
      ...
      {isDatasetModalOpen && (
        ...
        >
          ...
          <AdminEntry
            ...
            onDatasetChanged={() => {
              void refreshActiveDataset().finally(() => {
                window.dispatchEvent(new CustomEvent('media:dataset-revision', {
                  detail: { changedAt: new Date().toISOString() },
                }));
              });
