...
CREATE TABLE app_dataset_generations (
    ...
    status              VARCHAR2(20) NOT NULL CHECK (status IN
                        ('admitted','applying','ready','active','recovering','failed','rolled_back')),
...
CREATE UNIQUE INDEX uq_app_dataset_generation_work ON app_dataset_generations (
    CASE WHEN status IN ('admitted','applying','ready','recovering') THEN 1 END
);
