...
const DESTRUCTIVE_CONFIRMATIONS = Object.freeze({ '/upload': 'REPLACE_DATASET', '/restore-demo': 'RESTORE_DEMO' });

async function requireDatasetAdmin(req, res, next) {
  const expectedConfirmation = DESTRUCTIVE_CONFIRMATIONS[req.path];
  const headerConfirmation = String(req.get('X-Telco-Dataset-Confirmation') || '').trim();
  const bodyConfirmation = String(req.body?.confirmation || '').trim();
  const confirmation = headerConfirmation || bodyConfirmation;
  ...
  const username = req.demoIdentity?.username;
  if (!username) {
    return res.status(401).json({ ok: false, error: 'A signed demo session is required for dataset administration.' });
  }
  try {
    const identity = await db.resolveDemoIdentity(username);
    if (identity.role !== 'admin') {
      return res.status(403).json({ ok: false, error: 'Dataset administration requires the Oracle-backed admin role.' });
    }
    req.demoIdentity = identity;
    req.demoUser = identity.username;
    return next();
  } catch (_) {
    return res.status(403).json({ ok: false, error: 'The signed demo identity is unavailable.' });
  }
}
...
router.post('/validate', requireDatasetAdmin, uploadSingleFile, async (req, res) => {
  const service = getImportService(req);
...
router.post('/restore-demo/validate', requireDatasetAdmin, async (req, res) => {
  const service = getImportService(req);
