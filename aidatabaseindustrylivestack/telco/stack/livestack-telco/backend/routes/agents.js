...
const STATIC_TEAMS = [
  {
    ...
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed subscriber/network signal analysis over live network experience data.',
  },
  {
    TEAM_NAME: 'FULFILLMENT_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed network access analysis using capacity and routing context.',
  },
  {
    TEAM_NAME: 'COMMERCE_TEAM',
    STATUS: 'ENABLED',
    DESCRIPTION: 'Ollama-backed subscriber operations analysis using service orders and service-value context.',
  },
];

const PUBLIC_TEAM_NAMES = Object.freeze({
  SOCIAL_TREND_TEAM: 'CARE_SIGNAL_TEAM',
  FULFILLMENT_TEAM: 'CARE_ACCESS_TEAM',
  COMMERCE_TEAM: 'CARE_OPERATIONS_TEAM',
});

const INTERNAL_TEAM_NAMES = Object.freeze(
  Object.fromEntries(Object.entries(PUBLIC_TEAM_NAMES).map(([internal, publicName]) => [publicName, internal]))
);
...
function normalizeChatHistory(history) {
  ...
  return history.slice(-12).map((entry) => ({
    ...
    team: PUBLIC_TEAM_NAMES[entry?.team] || entry?.team || null,
    intent: typeof entry?.intent === 'string' ? entry.intent.slice(0, 40) : null,
  })).filter((entry) => entry.text);
}

function isContextualFollowUp(question) {
  return /^(and|also|what about|how about|why|which one|show|give|tell|break|compare|more|can you|could you)|\b(it|that|those|them|the top result|the first|the second|same)\b/i.test(question.trim());
}

function previousRoutingTeam(history) {
  const previous = [...history].reverse().find((entry) => entry.role === 'assistant' && entry.team);
  return previous ? (INTERNAL_TEAM_NAMES[previous.team] || previous.team) : null;
}

function conversationPrompt(question, history) {
  if (!history.length) return question;
  const transcript = history.map((entry) =>
    `${entry.role === 'assistant' ? 'Assistant' : 'User'}${entry.team ? ` [${entry.team}]` : ''}: ${entry.text}`
  ).join('\n');
  return `Continue the conversation. Resolve references in the newest question from prior turns.\n\nConversation:\n${transcript}\nUser: ${question}`;
}

async function buildCollaborationBriefs() {
  const checks = await Promise.allSettled([
    db.execute(
      `SELECT COUNT(*) AS signal_count, ROUND(AVG(virality_score), 1) AS avg_urgency,
              MAX(virality_score) AS peak_urgency
       FROM social_posts WHERE CAST(posted_at AS DATE) >= SYSDATE - 2`
    ),
    db.execute(
      `SELECT COUNT(CASE WHEN quantity_on_hand <= reorder_point THEN 1 END) AS constrained_services,
              NVL(SUM(quantity_on_hand), 0) AS available_capacity
       FROM inventory`
    ),
    db.execute(
      `SELECT COUNT(*) AS service_orders, ROUND(NVL(SUM(order_total), 0), 2) AS service_revenue
       FROM orders WHERE CAST(created_at AS DATE) >= SYSDATE - 30`
    ),
  ]);

  const signal = checks[0].status === 'fulfilled' ? checks[0].value.rows?.[0] || {} : {};
  const access = checks[1].status === 'fulfilled' ? checks[1].value.rows?.[0] || {} : {};
  const operations = checks[2].status === 'fulfilled' ? checks[2].value.rows?.[0] || {} : {};

  return [
    {
      team: 'CARE_SIGNAL_TEAM',
      label: 'Signal Agent',
      tool: 'SIGNAL_CONTEXT_SQL',
      available: checks[0].status === 'fulfilled',
      contribution: checks[0].status === 'fulfilled'
        ? `${Number(signal.SIGNAL_COUNT || 0).toLocaleString()} recent signals; avg urgency ${signal.AVG_URGENCY ?? 'n/a'}, peak ${signal.PEAK_URGENCY ?? 'n/a'}.`
        : 'Signal context was unavailable for this turn.',
    },
    {
      team: 'CARE_ACCESS_TEAM',
      label: 'Network Access Agent',
      tool: 'ACCESS_CONTEXT_SQL',
      available: checks[1].status === 'fulfilled',
      contribution: checks[1].status === 'fulfilled'
        ? `${Number(access.CONSTRAINED_SERVICES || 0).toLocaleString()} constrained service placements; ${Number(access.AVAILABLE_CAPACITY || 0).toLocaleString()} capacity units available.`
        : 'Network access context was unavailable for this turn.',
    },
    {
      team: 'CARE_OPERATIONS_TEAM',
      label: 'Subscriber Operations Agent',
      tool: 'OPERATIONS_CONTEXT_SQL',
      available: checks[2].status === 'fulfilled',
      contribution: checks[2].status === 'fulfilled'
        ? `${Number(operations.SERVICE_ORDERS || 0).toLocaleString()} service orders and $${Number(operations.SERVICE_REVENUE || 0).toLocaleString()} service revenue in 30 days.`
        : 'Subscriber operations context was unavailable for this turn.',
    },
  ];
}
...
function fallbackAgentSummary(teamName, context, question = '', history = []) {
  const contextual = history.length > 0 && isContextualFollowUp(question);
  if (teamName === 'SOCIAL_TREND_TEAM') {
    ...
    if (contextual) {
      ...
      return `${product.PRODUCT_NAME} is the leading current signal result. It is a ${product.CATEGORY} service from ${product.BRAND_NAME}, with ${product.MENTIONS} recent mentions, average urgency ${product.AVG_VIRALITY}, ${Number(product.TOTAL_VIEWS || 0).toLocaleString()} views, and ${product.PEAK_MOMENTUM || 'unclassified'} momentum in the 48-hour window.`;
    }
    return products
      .slice(0, 3)
      .map((product) => {
  ...
  if (teamName === 'FULFILLMENT_TEAM') {
    ...
    if (contextual) {
      const item = alerts[0];
      return `${item.PRODUCT_NAME} at ${item.CENTER_NAME} in ${item.CITY} is the leading access-risk result: ${item.QUANTITY_ON_HAND} capacity units remain against a reorder point of ${item.REORDER_POINT}, classified ${item.STOCK_STATUS}. The access team should validate reservations and prepare alternate routing.`;
    }
    return alerts
      .slice(0, 3)
      .map((item) =>
  ...
  if (contextual && context.category_breakdown?.length) {
    const category = context.category_breakdown[0];
    return `${category.CATEGORY} is the leading service-operations result with ${Number(category.ORDERS || 0).toLocaleString()} orders and $${Number(category.REVENUE || 0).toLocaleString()} revenue in the current 30-day window. Across all categories there are ${Number(totalOrders).toLocaleString()} orders and $${Number(totalRevenue).toLocaleString()} revenue.`;
  }
...
async function askAgentDetailed(teamName, question, { history = [], collaborationBriefs = [] } = {}) {
  const { instructions, context } = await buildAgentContext(teamName);
  context.collaboration_briefs = collaborationBriefs;
  const fallback = fallbackAgentSummary(teamName, context, question, history);
  try {
    const response = await Promise.race([
      summarizeContext({ question: conversationPrompt(question, history), instructions, context }),
      new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 12000)),
    ]);
    return { response, agentUsed: true };
  } catch (_) {
    return { response: fallback, agentUsed: false };
  }
}

async function askAgent(teamName, question, options = {}) {
  const result = await askAgentDetailed(teamName, question, options);
...
router.post('/chat', async (req, res) => {
  ...
  const trendStrong = ['trending', 'demand', 'urgent', 'urgency', 'viral', 'virality', 'mega_viral', 'momentum', 'advocate', 'community', 'instagram', 'hashtag', 'rising'];
  ...
  // Short follow-ups inherit the last lead team instead of falling back to operations.
  if (history.length && isContextualFollowUp(q)) {
    const priorTeam = previousRoutingTeam(history);
    if (priorTeam && PUBLIC_TEAM_NAMES[priorTeam]) {
      team = priorTeam;
      intent = team === 'SOCIAL_TREND_TEAM' ? 'trends' : team === 'FULFILLMENT_TEAM' ? 'fulfillment' : 'commerce';
    }
  }

  const collaborationBriefs = await buildCollaborationBriefs();
  const publicTeam = PUBLIC_TEAM_NAMES[team] || team;
  const engagedAgents = collaborationBriefs.map((brief) => ({
    ...brief,
    status: !brief.available ? 'unavailable' : brief.team === publicTeam ? 'lead' : 'consulted',
  }));
  collaborationBriefs.forEach((brief) => {
    toolsUsed.push({
      tool: brief.tool,
      team: brief.team,
      status: brief.available ? 'success' : 'error',
    });
  });
  ...
  try {
    const agentResult = await Promise.race([
      askAgentDetailed(team, q, { history, collaborationBriefs }),
    ...
    if (agentResponse) {
      toolsUsed.push({
        ...
        team: publicTeam,
        status: agentUsed ? 'success' : 'fallback',
        reason: agentUsed ? undefined : 'runtime timeout; grounded agent fallback used',
      });
    }
  } catch (agentErr) {
    toolsUsed.push({ tool: 'Ollama llama3.2', team: publicTeam, status: 'fallback', reason: agentErr.message });
  }
  ...
  await logAction('chat_agent', 'chat_query', intent, null, {
    ...
    reason: `Chat query routed to ${publicTeam} (intent: ${intent}) with ${engagedAgents.filter((agent) => agent.available).length} specialist context checks`,
  }, 0.90);
