...
const { admitGeneration, markGeneration, activateGeneration, stageGenerationSnapshot, recordFeatureEvidence, listInterruptedGenerations } = require('./datasetGenerationStore');
...
async function acquireOperationLock(kind, message, jobId = null) {
...
async function rebuildProductAttributes(connection) {
  const result = await execSql(connection, `
    INSERT INTO product_attributes (product_id, attributes)
    SELECT p.product_id,
           JSON_OBJECT(
             'serviceId' VALUE p.product_id,
             'serviceName' VALUE p.product_name,
...
function serializeRollbackValue(column, value) {
  if (value == null) return null;
  if (column.type === 'source_id_list') return String(value).split(',').map(normalizeSourceId).filter(Boolean);
  return value instanceof Date ? value.toISOString() : value;
}

async function captureRollbackDataset(connection) {
  const scn = await execSql(connection, 'SELECT TIMESTAMP_TO_SCN(SYSTIMESTAMP) AS snapshot_scn FROM dual');
  const snapshotScn = Number(scn.rows?.[0]?.SNAPSHOT_SCN);
  if (!Number.isFinite(snapshotScn)) throw new ImportError('Oracle SCN snapshot for rollback could not be captured.', 500);
  const tables = {};
  for (const table of TABLES) {
    const columns = table.columns.map((column) => table.name === 'demand_regions' && column.name === 'boundary'
      ? 'SDO_UTIL.TO_WKTGEOMETRY(boundary) AS boundary' : column.name).join(', ');
    const result = await execSql(connection, `SELECT ${columns} FROM ${table.name} AS OF SCN :snapshotScn ORDER BY ${table.pk}`, { snapshotScn });
    const rows = (result.rows || []).map((source, index) => {
      const row = { __lineNumber: index + 2 };
      for (const column of table.columns) row[column.name] = serializeRollbackValue(column, source[column.name.toUpperCase()]);
      row.__sourceId = normalizeSourceId(row[table.pk]);
      return row;
    });
    tables[table.name] = { provided: true, rows, sourceIds: new Set(rows.map((row) => row.__sourceId)) };
  }
  return { format: 'telco-generation-rollback-v1', capturedScn: snapshotScn, tables };
}

function rehydrateRollbackDataset(snapshot) {
  if (!snapshot || snapshot.format !== 'telco-generation-rollback-v1' || !snapshot.tables) throw new ImportError('Generation rollback snapshot is invalid.', 500);
  const tables = {};
  for (const table of TABLES) {
    const saved = snapshot.tables[table.name];
    if (!saved || !Array.isArray(saved.rows)) throw new ImportError(`Rollback snapshot is missing ${table.name}.`, 500);
    const rows = saved.rows.map((source, index) => {
      const row = { __lineNumber: index + 2 };
      for (const column of table.columns) {
        const value = source[column.name];
        row[column.name] = value != null && ['date', 'timestamp'].includes(column.type) ? new Date(value) : value;
        if (column.type === 'source_id_list' && value != null) row[column.name] = Array.isArray(value) ? value : String(value).split(',');
      }
      row.__sourceId = normalizeSourceId(row[table.pk]);
      return row;
    });
...
async function recoverInterruptedGenerations() {
  const interrupted = await listInterruptedGenerations();
  for (const generation of interrupted) {
    const rawSnapshot = generation.ROLLBACK_DATASET_JSON;
    // Admission is durable before the snapshot/apply stage.  An app restart or
    // failure in that narrow interval has not touched dataset rows, so it can
    // be terminalized safely; every later state still requires a snapshot.
    if (String(generation.STATUS).toLowerCase() === 'admitted' && !rawSnapshot) {
      await markGeneration(generation.GENERATION_ID, 'failed', { reason: 'APP_RESTART_BEFORE_APPLY' });
      continue;
    }
    if (!rawSnapshot) throw new ImportError(`Interrupted generation ${generation.GENERATION_ID} has no durable rollback snapshot.`, 503);
    await markGeneration(generation.GENERATION_ID, 'recovering', { reason: 'APP_RESTART_INTERRUPTED' });
    const restored = await executeImportPlan({ dataset: rehydrateRollbackDataset(JSON.parse(rawSnapshot)), dryRun: false, refreshDemoDates: false });
    await markGeneration(generation.GENERATION_ID, 'rolled_back', { reason: 'APP_RESTART_INTERRUPTED', restored: restored.summary });
  }
...
async function executeImportPlan({ dataset, dryRun = false, progress = null, refreshDemoDates = false }) {
  ...
  try {
    ...
    if (progress) progress({ status: 'running', progress: 78, message: 'Rebuilding native JSON service attributes...' });
    const productAttributes = await rebuildProductAttributes(connection);
    ...
    const summary = { ...summarizeCounts(insertedCounts, fallbackCounts, zonesCreated), generated: { ...summarizeCounts(insertedCounts, fallbackCounts, zonesCreated).generated, product_attributes: productAttributes } };
...
async function getActiveDataset() {
  const stored = await getStoredDatasetState() || await inferCurrentDatasetState();
...
async function persistDatasetState(source, warnings, generationId) {
  try {
    const state = buildDatasetState(source);
    await activateGeneration({ generationId, ...state });
...
async function runDatasetValidation({ parsed, fileOnly = false, lockKind, lockMessage }) {
  ...
  const lease = await acquireOperationLock(lockKind, lockMessage);
  try {
    ...
    await endOperation({ leaseToken: lease?.leaseToken });
  }
}
...
async function startDatasetJob({ parsed, kind, lockMessage, queuedMessage, startMessage, completeMessage, datasetSource, initiatingActor = 'admin_jess' }) {
  const job = createJob({
    operation: kind,
  ...
  const lock = await acquireOperationLock(kind, lockMessage, job.jobId);
  let generation;
  try {
    generation = await admitGeneration({ jobId: job.jobId, initiatingActor, source: datasetSource });
  } catch (error) {
    updateJob(job.jobId, {
      status: 'failed', progress: 100,
      message: error.message || 'Dataset generation admission failed.',
      errors: [error.message || 'Dataset generation admission failed.'],
    });
    await endOperation({ leaseToken: lock.leaseToken, jobId: job.jobId });
    throw error;
  }
  ...
  setImmediate(async () => {
    try {
      ...
      let snapshotConnection;
      try {
        snapshotConnection = await db.getConnection();
        const rollbackSnapshot = await captureRollbackDataset(snapshotConnection);
        await stageGenerationSnapshot(generation.generationId, rollbackSnapshot);
      } finally { if (snapshotConnection) try { await snapshotConnection.close(); } catch (_) {} }
      await markGeneration(generation.generationId, 'applying');
      ...
      requiredFeatures.generationId = generation.generationId;
      assertRequiredFeatureEvidence(requiredFeatures);
      await recordFeatureEvidence(generation.generationId, requiredFeatures);
      await markGeneration(generation.generationId, 'ready', { summary: result.summary });
      const activeDataset = await persistDatasetState(datasetSource, warnings, generation.generationId);
      ...
      await markGeneration(generation.generationId, 'failed', { error: err.message || 'Import failed.' });
      updateJob(job.jobId, {
        status: 'failed',
        progress: 100,
...
async function startImport({ req, body = {}, version = IMPORT_VERSION, demoUser = null }) {
  ...
  return await startDatasetJob({
    ...
    initiatingActor: demoUser || 'admin_jess',
  });
}
...
async function startDemoRestore({ version = IMPORT_VERSION, demoUser = null }) {
  const demoDataset = getBundledDemoDataset(version);
  return await startDatasetJob({
    parsed: demoDataset.parsed,
    kind: 'restore_demo',
