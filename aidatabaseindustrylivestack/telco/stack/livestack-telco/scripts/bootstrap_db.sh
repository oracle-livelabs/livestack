...
# VECTOR_EMBEDDING-dependent functions in 04_vector.sql must never compile
# before the ONNX model is registered.  Keep this as a separate schema session
# so a clean database proves the model contract before vector DDL runs.
...
END;
/
SELECT VECTOR_DIMENSION_COUNT(VECTOR_EMBEDDING(ALL_MINILM_L12_V2 USING 'Telco bootstrap model probe' AS DATA))
FROM dual;
EXIT SUCCESS
SQL
