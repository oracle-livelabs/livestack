...
export function UserProvider({ children }) {
    ...
    api.users.list()
      .then(data => {
        ...
        if (admin) {
          ...
          api.session.bootstrap(admin.USERNAME).catch(err => {
            console.warn('Failed to establish signed demo session:', err);
          });
        }
      })
      .catch(err => {
  ...
  const switchUser = useCallback((username) => {
    ...
    if (user) {
      ...
      api.session.bootstrap(user.USERNAME).catch(err => {
        console.warn('Failed to switch signed demo session:', err);
      });
    }
  }, [users]);
