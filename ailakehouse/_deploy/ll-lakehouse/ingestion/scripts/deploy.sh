#!/bin/bash
# ============================================================
# deploy.sh — Deploy PeakGear Sporting Goods Demo on Oracle Linux VM (OCI)
# Run from the scripts/ directory inside the project
# ============================================================

set -e

APP_DIR="/opt/peakgear-sporting-goods-demo"
APP_USER="opc"
NODE_VERSION="20"

echo "=================================================="
echo " PeakGear Sporting Goods Demo — Deployment"
echo "=================================================="
...
# ── 3. Oracle Instant Customer ──────────────────────────────
echo ""
echo "[3/7] Checking Oracle Instant Customer..."

if ! rpm -q oracle-instantcustomer-basic &> /dev/null; then
    echo "  Oracle Instant Customer not found."
    echo "  Install it from: https://www.oracle.com/database/technologies/instant-customer/linux-x86-64-downloads.html"
    echo "  Or use: sudo dnf install oracle-instantcustomer-release-el8"
    echo "  Then: sudo dnf install oracle-instantcustomer-basic oracle-instantcustomer-sqlplus"
    echo ""
    echo "  For wallet-based Oracle AI Database 26ai connections, also set TNS_ADMIN to your wallet directory."
    echo ""
    read -p "  Continue without Instant Customer? (y/n) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then exit 1; fi
else
    echo "  Oracle Instant Customer found"
fi
...
if [ ! -f .env ]; then
    ...
    echo "  Copy .env.example to .env and configure your Oracle AI Database 26ai credentials:"
...
export LD_LIBRARY_PATH=/usr/lib/oracle/23/customer64/lib:$LD_LIBRARY_PATH
export TNS_ADMIN=${TNS_ADMIN:-/opt/oracle/wallet}

# Stop existing instance if running
pm2 delete peakgear-sporting-goods 2>/dev/null || true

# Start with PM2
pm2 start backend/server.js \
    --name peakgear-sporting-goods \
    --env production \
...
echo "   pm2 logs peakgear-sporting-goods — view logs"
echo "   pm2 restart peakgear-sporting-goods — restart app"
echo "   pm2 monit               — real-time monitoring"
echo ""
echo " Next steps:"
echo "   1. Ensure .env is configured with Oracle AI Database 26ai credentials"
