/**
 * Signal Feed API — market and operations signals, source trends, and vector search
 */
    ...
    const result = await db.executeAsUser(`
      SELECT sp.post_id, sp.platform, sp.post_text, sp.posted_at,
             ...
             (SELECT p.category
              ...
              ORDER BY ppm.confidence_score DESC, ppm.mention_id
              FETCH FIRST 1 ROW ONLY) AS product_category,
             COALESCE(
               (SELECT df.region
                FROM post_product_mentions ppm
                JOIN demand_forecasts df ON df.product_id = ppm.product_id
                WHERE ppm.post_id = sp.post_id
                ORDER BY df.predicted_demand DESC, df.forecast_date
...
// GET /api/social/viral — elevated and critical demand signal records
