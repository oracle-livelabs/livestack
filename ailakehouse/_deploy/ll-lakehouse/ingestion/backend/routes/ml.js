 ...
 *   DEMAND_SURGE_MODEL      — Random Forest Classification (sporting goods product surge detection)
 *   CUSTOMER_SEGMENT_MODEL  — K-Means Clustering (customer RFM segmentation)
 *   REVENUE_PREDICT_MODEL   — GLM Regression (transaction revenue prediction)
 *   PRODUCT_CLUSTER_MODEL   — K-Means Clustering (sporting goods product grouping by behavior)
 *
...
function buildDemandProductFeaturesCte(lookbackHoursExpr = null) {
  const socialWhere = lookbackHoursExpr
    ? `WHERE CAST(sp.POSTED_AT AS DATE) >= (
             SELECT MAX(CAST(POSTED_AT AS DATE)) FROM SOCIAL_POSTS
           ) - ${lookbackHoursExpr} / 24`
    : '';
...
async function fallbackDemandForecast({ limit, lookbackHours }) {
  ...
  return {
    ...
    meta: {
      ...
      scoring: 'Weighted market/market signal activity + order demand signal',
...
async function fallbackCustomerSegments({ limit }) {
  ...
  return {
    ...
    meta: {
      model: 'Customer RFM segmentation heuristic (fallback)',
...
async function fallbackRevenueForecast({ lookbackDays, forecastDays }) {
  const histResult = await db.execute(`
    WITH data_anchor AS (
      SELECT MAX(TRUNC(CAST(CREATED_AT AS DATE))) AS ANCHOR_DAY
      FROM ORDERS
    ),
    daily_rev AS (
      SELECT /*+ NO_PARALLEL */
             TRUNC(CAST(o.CREATED_AT AS DATE), 'DD') AS DAY_BUCKET,
             SUM(o.ORDER_TOTAL) AS REVENUE,
             COUNT(o.ORDER_ID) AS ORDER_COUNT,
             AVG(o.ORDER_TOTAL) AS AVG_ORDER_VALUE,
             ROW_NUMBER() OVER (ORDER BY TRUNC(CAST(o.CREATED_AT AS DATE), 'DD')) AS RN
      FROM ORDERS o
      CROSS JOIN data_anchor da
      WHERE da.ANCHOR_DAY IS NOT NULL
        AND CAST(o.CREATED_AT AS DATE) >= da.ANCHOR_DAY - :lookback
      GROUP BY TRUNC(CAST(o.CREATED_AT AS DATE), 'DD')
    ),
    params AS (
      SELECT
  ...
  for (let i = 1; i <= forecastDays; i++) {
    ...
    forecast.push({
      DAY: addDaysToIsoDate(last.DAY, i),
      ACTUAL_REVENUE: null,
      ORDER_COUNT: null,
...
async function fallbackSummary() {
  const [surgeCount, customers, modelCount, regr] = await Promise.all([
    ...
    db.execute(`
      ...
      FROM (
        SELECT SUM(o.ORDER_TOTAL) AS DAY_REV,
               ROW_NUMBER() OVER (ORDER BY TRUNC(CAST(o.CREATED_AT AS DATE))) AS RN
        FROM ORDERS o
        WHERE CAST(o.CREATED_AT AS DATE) >= (
          SELECT MAX(TRUNC(CAST(CREATED_AT AS DATE))) FROM ORDERS
        ) - 30
        GROUP BY TRUNC(CAST(o.CREATED_AT AS DATE))
      )
    `),
  ]);
...
async function fallbackInventoryIntelligence() {
  const result = await db.execute(`
    ...
    capacity_base AS (
      SELECT
        ...
        ROUND(GREATEST(
          NVL(df.PREDICTED_DEMAND, 0),
          CASE
            WHEN NVL(sp.SURGE_PROBABILITY, 0) >= 65 THEN i.REORDER_POINT * 1.55
            WHEN NVL(sp.SURGE_PROBABILITY, 0) >= 45 THEN i.REORDER_POINT * 1.25
            ELSE i.REORDER_POINT * 0.9
          END
          + LEAST(NVL(sp.UNITS_SOLD, 0), 200) * 0.15
          + LEAST(NVL(sp.TOTAL_POSTS, 0), 80) * 0.35
          + NVL(i.QUANTITY_RESERVED, 0) * 0.75
        ), 0) AS PREDICTED_DEMAND,
        NVL(df.SOCIAL_FACTOR, ROUND(1 + NVL(sp.SURGE_PROBABILITY, 0) / 100, 2)) AS SOCIAL_FACTOR,
        NVL(df.CONFIDENCE_LOW, 0) AS CONFIDENCE_LOW,
        NVL(df.CONFIDENCE_HIGH, 0) AS CONFIDENCE_HIGH,
        NVL(df.MODEL_VERSION, 'heuristic-capacity-v1') AS MODEL_VERSION,
        CASE
      ...
      LEFT JOIN latest_forecast df ON p.PRODUCT_ID = df.PRODUCT_ID
    ...
    FROM capacity_base
    ORDER BY OML_SURGE_PROBABILITY DESC, QUANTITY_ON_HAND ASC
  ...
  return {
    ...
    meta: {
      ...
      scoring: 'Weighted market/market signal + inventory JOIN',
      engine: 'Oracle SQL fallback — persisted DBMS_DATA_MINING model not available',
      fallback: true,
    },
  };
...
//
// Scores sporting goods products using the DEMAND_SURGE_MODEL (Random Forest, 50 trees).
    ...
    const histResult = await db.execute(`
      ...
      daily_rev AS (
        SELECT /*+ NO_PARALLEL */
               ...
               AVG(o.order_total)                                AS avg_order_value,
               ROW_NUMBER() OVER (
                 ORDER BY TRUNC(CAST(o.created_at AS DATE), 'DD')
               )                                                 AS rn
        FROM   orders o
        CROSS JOIN data_anchor da
        WHERE  da.anchor_day IS NOT NULL
...
// sporting goods products at risk of capacity shortfall due to signal-driven customer demands.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    const alertsResult = await db.execute(`
      ...
      capacity_scored AS (
        SELECT
          ...
          GREATEST(
            NVL(df.predicted_demand, 0),
            ROUND(
              NVL(sales.units_sold, 0) * (1 + PREDICTION_PROBABILITY(DEMAND_SURGE_MODEL, 'SURGE' USING
                p.category AS category, p.unit_price AS unit_price,
                NVL(eng.total_posts, 0) AS total_posts,
                NVL(eng.avg_sentiment, 0.5) AS avg_sentiment,
                NVL(eng.total_likes, 0) AS total_likes,
              ...
              + NVL(eng.total_posts, 0) * 0.5
              + NVL(i.quantity_reserved, 0) * 0.75
              + CASE WHEN df.predicted_demand IS NULL THEN NVL(i.reorder_point, 0) * 0.65 ELSE 0 END,
            0)
          ) AS predicted_demand,
          NVL(df.social_factor,
            ROUND(1 + PREDICTION_PROBABILITY(DEMAND_SURGE_MODEL, 'SURGE' USING
              p.category AS category, p.unit_price AS unit_price,
              NVL(eng.total_posts, 0) AS total_posts,
              NVL(eng.avg_sentiment, 0.5) AS avg_sentiment,
              NVL(eng.total_likes, 0) AS total_likes,
          ...
          NVL(df.confidence_low, 0) AS confidence_low,
          NVL(df.confidence_high, 0) AS confidence_high,
          NVL(df.model_version, 'latest-oml-score') AS model_version,
          PREDICTION(DEMAND_SURGE_MODEL USING
            p.category AS category, p.unit_price AS unit_price,
            NVL(eng.total_posts, 0) AS total_posts,
            NVL(eng.avg_sentiment, 0.5) AS avg_sentiment,
      ...
      SELECT
        ...
        brand_name,
        center_id, center_name, city, state_province,
        quantity_on_hand, reorder_point, quantity_reserved,
        deficit,
        predicted_demand,
        social_factor,
        confidence_low,
