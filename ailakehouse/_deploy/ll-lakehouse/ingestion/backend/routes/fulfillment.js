/**
 * Fulfillment API — Spatial routing and store / DC site management
 */
...
// GET /api/fulfillment/nearest — find nearest site for a customer+sporting goods product
...
router.get('/customers', async (req, res) => {
  try {
    ...
    console.error('Customer layer error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
// Joins to demand_forecasts by the state code embedded in region_name for 7-day forecast context.
router.get('/demand-regions', async (req, res) => {
  try {
    const result = await db.execute(`
      WITH forecast_bounds AS (
        SELECT MIN(forecast_date) AS min_forecast_date
        FROM demand_forecasts
      ),
      regions AS (
        SELECT r.*,
               CASE
                 WHEN REGEXP_LIKE(r.region_name, '^[A-Z]{2} ')
                 THEN SUBSTR(r.region_name, 1, 2)
                 ELSE r.region_name
               END AS forecast_region
      ...
      SELECT r.region_id,
             ...
             (SELECT ROUND(AVG(df.predicted_demand), 0)
              FROM demand_forecasts df
              CROSS JOIN forecast_bounds fb
              WHERE UPPER(df.region) = UPPER(r.forecast_region)
                AND df.forecast_date BETWEEN fb.min_forecast_date AND fb.min_forecast_date + 7
