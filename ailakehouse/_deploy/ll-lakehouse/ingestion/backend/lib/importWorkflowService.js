...
function buildTemplateReadme() {
  return [
    '# PeakGear Sporting Goods Import Template',
    '',
    ...
    '- Domain labels use the original schema names for compatibility: brands are brands and partners, products are sporting goods products, influencers are signal sources, and social_posts are market or market bulletins.',
...
function buildFallbackShipments(dataset, idMaps) {
  ...
  const carriers = ['SecureCourier', 'StoreLink', 'WireTrack', 'CustomerRoute'];
  const shipStatusMap = {
    confirmed: 'preparing',
    processing: 'packed',
    routed: 'in_transit',
...
async function generateTemplateArchive({ version = IMPORT_VERSION }) {
  ...
  return {
    buffer: zip.toBuffer(),
    fileName: `peakgear-sporting-goods-import-template-${version}.zip`,
