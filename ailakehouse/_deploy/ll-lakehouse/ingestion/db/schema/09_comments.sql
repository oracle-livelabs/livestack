 ...
 *   2. SELECT AI metadata — the PG_COHERE_PROFILE has "comments": true,
...
COMMENT ON TABLE brand_influencer_links IS
  'Many-to-many relationship between brands and partners and signal sources. Tracks signal counts, engagement, and attributed revenue.';

COMMENT ON TABLE demand_regions IS
  'Geographic regions used for sporting goods product demand forecasting. Each region has a boundary polygon, population, income, and signal density.';
...
COMMENT ON TABLE fulfillment_zones IS
  'Delivery zones around store fulfillment sites. Each zone has a boundary polygon and maximum delivery time.';

COMMENT ON TABLE influencer_connections IS
  'Graph edges between retail, creator, and operations signal sources. Used by SQL/PGQ graph queries for network analysis.';

COMMENT ON TABLE signal_embeddings IS
  'Vector embeddings of demand and market signal text. 384-dim vectors from ALL_MINILM_L12_V2 ONNX model for semantic similarity search.';

COMMENT ON TABLE product_attributes IS
  'Extended sporting goods product attributes stored as JSON. Holds category-specific metadata like size range, color, material, care notes, and technical specifications.';

COMMENT ON TABLE product_embeddings IS
  'Vector embeddings of sporting goods product descriptions. 384-dim vectors from ALL_MINILM_L12_V2 ONNX model for semantic product matching.';

COMMENT ON TABLE semantic_matches IS
  'Cached results of vector similarity matching between signals and sporting goods products. Stores similarity scores and match methods.';

COMMENT ON TABLE social_post_payloads IS
  'Raw and enriched JSON payloads for retail demand and market signals. Stores the original feed response and NLP enrichment data.';

COMMENT ON TABLE order_items IS
  'Line items within an order. Each links to a sporting goods product with quantity and price. Revenue = quantity * unit_price.';

COMMENT ON TABLE products IS
  'Products available for customer order. Each sporting goods product belongs to a brand or partner and has a category, price, and tags. This is the ONLY table with sporting goods product categories. Use products.category for sporting goods product category queries. Revenue per category = SUM(order_items.line_total) grouped by products.category via JOIN order_items ON order_items.product_id = products.product_id.';
...
COMMENT ON COLUMN agent_actions.action_type IS
  'Type of action taken: alert, replenishment, allocation_review, returns_audit_review, trend_flag';
COMMENT ON COLUMN agent_actions.entity_type IS
  'Target entity type: sporting goods product, brand or partner, signal_source, order, or signal';
...
COMMENT ON COLUMN brands.brand_slug IS
  'URL-friendly unique brand or partner identifier (e.g. meridiantrust, horizoncapital, voltpay)';
COMMENT ON COLUMN brands.brand_category IS
  'Brand or Partner category (e.g. Retail Sporting Goods, Assortment Planning, Outdoor Performance). This is the brand or partner category, not the sporting goods product category. Use products.category for sporting goods product categories.';
COMMENT ON COLUMN brands.headquarters_city IS
  'City where brand or partner headquarters is located';
...
COMMENT ON COLUMN brands.founded_year IS
  'Year the brand or partner was founded (4-digit year)';
COMMENT ON COLUMN brands.annual_revenue IS
  'Annual brand or partner revenue in US dollars';
...
COMMENT ON COLUMN brand_influencer_links.brand_id IS
  'FK to brands, used as brands and partners in this PeakGear sporting goods demo. JOIN brands ON brands.brand_id = brand_influencer_links.brand_id';
COMMENT ON COLUMN brand_influencer_links.influencer_id IS
  'FK to influencers, used as signal sources in this PeakGear sporting goods demo. JOIN influencers ON influencers.influencer_id = brand_influencer_links.influencer_id';
COMMENT ON COLUMN brand_influencer_links.relationship_type IS
  'Relationship type retained from the original schema; in this demo values represent brand or partner-source signal relationships.';
COMMENT ON COLUMN brand_influencer_links.post_count IS
  'Total number of signals this source has made about this brand or partner';
COMMENT ON COLUMN brand_influencer_links.avg_engagement IS
  'Average engagement rate across signals for this brand or partner-source pair';
COMMENT ON COLUMN brand_influencer_links.revenue_attributed IS
  'Total revenue in US dollars attributed to this source for this brand or partner';
COMMENT ON COLUMN brand_influencer_links.first_mention IS
  'Timestamp of the first signal mentioning this brand or partner by this source';
COMMENT ON COLUMN brand_influencer_links.last_mention IS
  'Timestamp of the most recent signal mentioning this brand or partner by this source';
...
COMMENT ON COLUMN demand_forecasts.product_id IS
  'FK to products, used as sporting goods products in this demo. JOIN products ON products.product_id = demand_forecasts.product_id';
...
COMMENT ON COLUMN demand_regions.social_density IS
  'Demand and market signal density score 0-100 for this region';
...
COMMENT ON COLUMN fulfillment_centers.address_line1 IS
  'Street address of the store fulfillment site';
COMMENT ON COLUMN fulfillment_centers.postal_code IS
  'Postal/zip code of the store fulfillment site';
...
COMMENT ON COLUMN influencers.niche IS
  'Signal niche: creator trends, competitor pricing, POS transactions, returns audit, product content, sporting goods products, or brand quality';
...
COMMENT ON COLUMN inventory.quantity_incoming IS
  'Units currently active transfers or on order from brand or partner';
...
COMMENT ON COLUMN post_product_mentions.product_id IS
  'FK to products, used as sporting goods products. JOIN products ON products.product_id = post_product_mentions.product_id';
...
COMMENT ON COLUMN products.sku IS
  'Catalog code - unique sporting goods product code for inventory tracking';
COMMENT ON COLUMN products.description IS
  'Detailed sporting goods product description text. Used for vector embeddings and full-text search.';
COMMENT ON COLUMN products.subcategory IS
  'Product subcategory within the main category (e.g. Digital Supply under Supply Planning)';
...
COMMENT ON COLUMN products.weight_kg IS
  'Product compatibility metric used by routing cost calculations';
...
COMMENT ON COLUMN products.launch_date IS
  'Date the sporting goods product was first available for order';
...
COMMENT ON COLUMN product_attributes.product_id IS
  'FK to products, used as sporting goods products. JOIN products ON products.product_id = product_attributes.product_id';
COMMENT ON COLUMN product_attributes.attributes IS
  'JSON document with category-specific sporting goods product attributes (size range, color, material, care notes, and specifications)';
...
COMMENT ON COLUMN product_embeddings.product_id IS
  'FK to products, used as sporting goods products. JOIN products ON products.product_id = product_embeddings.product_id';
...
COMMENT ON COLUMN product_embeddings.embedding_text IS
  'Source text that was embedded (sporting goods product name + description + tags)';
...
COMMENT ON COLUMN semantic_matches.post_id IS
  'FK to social_posts. The market or market signal that was matched.';
COMMENT ON COLUMN semantic_matches.product_id IS
  'FK to products. The sporting goods product matched to the signal.';
COMMENT ON COLUMN semantic_matches.similarity_score IS
  'Cosine similarity score 0.0 to 1.0 between signal and sporting goods product embeddings';
...
COMMENT ON COLUMN shipments.carrier IS
  'Secure transfer carrier name (e.g. SecureCourier, StoreLink, WireTrack, CustomerRoute)';
...
COMMENT ON COLUMN shipments.ship_cost IS
  'Service Fee cost in US dollars charged to the customer';
...
COMMENT ON COLUMN social_posts.detected_products IS
  'Comma-separated list of sporting goods product names detected in this signal by NLP';
